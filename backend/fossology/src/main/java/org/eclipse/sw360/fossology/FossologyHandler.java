/*
 * Copyright Siemens AG, 2013-2015, 2019, 2025. Part of the SW360 Portal Project.
 *
...
public class FossologyHandler implements FossologyService.Iface {
    ...
    /**
     * Handle upload step using v2 API with enhanced capabilities
     */
    private void handleUploadStepV2(Iface componentClient, Release release, User user,
        ...
        case NEW:
            ...
            if (existingUploadId > -1) {
                ...
                boolean autoStartScan = fossologyRestConfig.isAutoStartScanEnabled();
                
                log.info("STARTING UPLOAD for file {} with auto-scan: {}", attachmentFilename, autoStartScan);
                CombinedUploadJobResponse response = fossologyRestClient.uploadFileAndScan(
                    attachmentFilename, attachmentStream, uploadDescription, autoStartScan);
                ...
                if (response != null && V2_STATUS_SUCCESS.equals(response.getStatus())) {
                    ...
                    // If scan was automatically started, create scan step
                    if (autoStartScan && response.getJobId() > 0) {
    ...
    ExternalToolProcessStep furthestStep =
        fossologyProcess.getProcessSteps()
                        .get(fossologyProcess.getProcessSteps().size() - 1);
    ...
    case IN_WORK:
        ...
        if (status > 0) {
            ...
            // UTOMATICALLY TRIGGER REPORT GENERATION
    ...
    private int scanStatusCodeV2(Map<String, String> responseMap) {
        ...
        switch (status) {
        case V2_STATUS_COMPLETED:
            return 1;
        case V2_STATUS_QUEUED:
        case V2_STATUS_PROCESSING:
            return 0;
        case V2_STATUS_FAILED:
        default:
            return -1;
        }
    }
    ...
    /**
     * Get information about the current API version being used
     */
    public String getCurrentApiVersion() {
        return fossologyRestClient.getApiVersion();
    }

    /**
     * Check if v2 API features are available
     */
    public boolean isV2ApiAvailable() {
        return fossologyRestClient.isV2ApiAvailable();
    }
}