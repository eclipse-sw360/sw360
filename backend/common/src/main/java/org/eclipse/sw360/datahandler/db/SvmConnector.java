...
public class SvmConnector {
    ...
    public void sendProjectExportForMonitoringLists(String jsonString) throws IOException, SW360Exception {
        if(CommonUtils.isNullEmptyOrWhitespace(SW360Constants.SVM_MONITORING_LIST_API_URL)) {
            return;
        }
        try (CloseableHttpClient httpClient = HttpClients
            ...
            HttpPut httpPut = new HttpPut(SW360Constants.SVM_MONITORING_LIST_API_URL);
    ...
    private SSLConnectionSocketFactory createSslSocketFactoryForSVM() throws IOException {
        try {
            ...
            trustStore.load(new FileInputStream(javaKeystoreFilename), SW360Constants.SVM_JAVA_KEYSTORE_PASSWORD);

            // Loading KeyStore, i.e., PKCS #12 bundle
            KeyStore keyStore = KeyStore.getInstance("PKCS12");
            Optional<InputStream> certStreamOptional = CommonUtils
                    .loadResource(this.getClass(), SW360Constants.SVM_KEY_STORE_FILENAME)
                    .map(ByteArrayInputStream::new);
            if (!certStreamOptional.isPresent()) {
                throw new IOException("Cannot read SVM client certificate");
            }
            keyStore.load(certStreamOptional.get(), SW360Constants.SVM_KEY_STORE_PASSPHRASE);

            SSLContext sslcontext = SSLContexts
                    .custom()
                    .useProtocol("TLSv1.2")
                    .loadTrustMaterial(trustStore)
                    .loadKeyMaterial(keyStore, SW360Constants.SVM_KEY_STORE_PASSPHRASE)
                    .build();
    ...
    public Map<String, Map<String, Object>> fetchComponentMappings() throws SW360Exception, IOException {
        if(CommonUtils.isNullEmptyOrWhitespace(SW360Constants.SVM_COMPONENT_MAPPINGS_API_URL)) {
            return Collections.emptyMap();
        }
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
            HttpGet httpGet = new HttpGet(SW360Constants.SVM_COMPONENT_MAPPINGS_API_URL);
