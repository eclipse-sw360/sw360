...
public class ComponentHandler implements ComponentService.Iface {
	...
	public ComponentHandler() throws IOException {
		this(DatabaseSettings.getConfiguredClient(), DatabaseSettings.getConfiguredHttpClient(),
				DatabaseSettings.COUCH_DB_DATABASE, DatabaseSettings.COUCH_DB_CHANGE_LOGS,
				DatabaseSettings.COUCH_DB_ATTACHMENTS);
	}

	public ComponentHandler(Supplier<CloudantClient> cClient, Supplier<HttpClient> hclient, String dbName,
			String changeLogsDBName, String attachmentDbName) throws IOException {
	...
	public ComponentHandler(ThriftClients thriftClients) throws IOException {
		this(DatabaseSettings.getConfiguredHttpClient(), DatabaseSettings.getConfiguredClient(),
				DatabaseSettings.COUCH_DB_DATABASE, DatabaseSettings.COUCH_DB_CHANGE_LOGS,
				DatabaseSettings.COUCH_DB_ATTACHMENTS, thriftClients);
	}

	public ComponentHandler(Supplier<HttpClient> httpClient, Supplier<CloudantClient> client, String dbName,
			String changeLogsDBName, String attachmentDbName, ThriftClients thriftClients) throws IOException {
	...
	@Override
	public List<Component> refineSearchAccessibleComponents(String text, Map<String, Set<String>> subQueryRestrictions,
			User user) throws TException {
		return componentSearchHandler.searchAccessibleComponents(text, subQueryRestrictions, user);
	}

	@Override
	public List<Component> refineSearchWithAccessibility(String text, Map<String, Set<String>> subQueryRestrictions,
			User user) throws TException {
