...
public class SpdxConnector {
	...
	public static Optional<License> getSpdxLicenseAsSW360License(String licenseId) {
		return getSpdxLicense(licenseId).flatMap(SpdxConnector::getSpdxLicenseAsSW360License);
	}
	...
	public static Optional<License> getSpdxLicenseAsSW360License(SpdxListedLicense spdxListedLicense) {
		try {
			...
			License license = new License().setId(spdxListedLicense.getLicenseId())
					.setShortname(spdxListedLicense.getLicenseId()).setFullname(spdxListedLicense.getName())
					.setText(spdxListedLicense.getLicenseText()).setOSIApproved(isOSIApproved).setFSFLibre(isFSFLibre)
					.setExternalLicenseLink("https://spdx.org/licenses/" + spdxListedLicense.getLicenseId() + ".html")
					.setExternalIds(
							Collections.singletonMap("SPDX-License-Identifier", spdxListedLicense.getLicenseId()));
	...
	/**
	 * returns list of licenses in haystack, whose license text is equivalent to the
	 * one of license
	 * 
	 * @param license
	 * @param haystack
	...
	public static List<License> findMatchingLicenseIDs(String licenseText, List<License> haystack) {
		return haystack.stream().filter(l -> LicenseCompareHelper.isLicenseTextEquivalent(licenseText, l.getText()))
