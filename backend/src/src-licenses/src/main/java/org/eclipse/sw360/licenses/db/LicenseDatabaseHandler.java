...
public class LicenseDatabaseHandler {
	...
	public LicenseDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName) throws MalformedURLException {
		...
		repositories = new DatabaseRepositoryCloudantClient[]{licenseRepository, licenseTypeRepository, obligRepository,
				customPropertiesRepository, obligationElementRepository, obligationNodeRepository};
	...
	public License getLicenseForOrganisationWithOwnModerationRequests(String id, String organisation, User user)
		...
		if (moderationRequestsForDocumentId.isEmpty()) {
			...
			if (moderationRequestOptional.isPresent() && isInProgressOrPending(moderationRequestOptional.get())) {
				...
				license = moderator.updateLicenseFromModerationRequest(license, moderationRequest.getLicenseAdditions(),
						moderationRequest.getLicenseDeletions(), organisation);
	...
	public String addObligations(@NotNull Obligation obligs, User user) throws SW360Exception {
		...
		obligTmp.setDevelopment(false).setDistribution(false).setId(obligs.getId());
	...
	private License updateLicenseFromInputLicense(Optional<License> oldLicense, License inputLicense,
		...
		license.setOSIApproved(Optional.ofNullable(inputLicense.getOSIApproved()).orElse(Quadratic.NA));
		license.setFSFLibre(Optional.ofNullable(inputLicense.getFSFLibre()).orElse(Quadratic.NA));
	...
	public RequestStatus updateLicenseFromAdditionsAndDeletions(License licenseAdditions, License licenseDeletions,
		...
		try {
			...
			license = moderator.updateLicenseFromModerationRequest(license, licenseAdditions, licenseDeletions,
	...
	public List<LicenseType> addLicenseTypes(List<LicenseType> licenseTypes, User user) {
		...
		documentOperationResults = documentOperationResults.stream()
				.filter(res -> res.getError() != null || res.getStatusCode() != HttpStatus.SC_CREATED)
	...
	public List<License> addOrOverwriteLicenses(List<License> licenses, User user, boolean allowOverwriting)
		...
		for (License license : licenses) {
			...
			if (!license.isSetId()) {
				...
				if (allowOverwriting) {
					knownLicenses.stream().filter(kl -> license.getId().equals(kl.getId())).findFirst()
							.map(License::getRevision).ifPresent(license::setRevision);
				} else {
	...
	public RequestSummary deleteAllLicenseInformation() {
		RequestSummary result = new RequestSummary().setRequestStatus(RequestStatus.SUCCESS).setTotalElements(0)
	...
	public RequestSummary importAllSpdxLicenses(User user) {
		...
		return requestSummary.setTotalElements(spdxIds.size()).setRequestStatus(RequestStatus.SUCCESS);
	}
	...
	public RequestSummary importAllOSADLLicenses(User user) {
		...
		try {
			...
			requestSummary.setMessage("{\"licensesSuccess\":" + licensesSuccess.toString() + ",\"licensesMissing\":"
					+ licensesMissing.toString() + "}");
