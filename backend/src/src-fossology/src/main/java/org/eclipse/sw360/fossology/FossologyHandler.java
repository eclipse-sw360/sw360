...
public class FossologyHandler implements FossologyService.Iface {
	...
	private void handleUploadStep(Iface componentClient, Release release, User user,
			ExternalToolProcess fossologyProcess, Attachment sourceAttachment, String uploadDescription)
			throws TException {
		ExternalToolProcessStep furthestStep = fossologyProcess.getProcessSteps()
				.get(fossologyProcess.getProcessSteps().size() - 1);
		switch (furthestStep.getStepStatus()) {
			case NEW :
				...
				InputStream attachmentStream = attachmentConnector.getAttachmentStream(attachmentContent, user,
						release);
			...
			case DONE :
				// start scan
				fossologyProcess
						.addToProcessSteps(createFossologyProcessStep(user, FossologyUtils.FOSSOLOGY_STEP_NAME_SCAN));
				handleScanStep(componentClient, release, user, fossologyProcess);
				break;
			case IN_WORK :
	...
	private void handleScanStep(Iface componentClient, Release release, User user, ExternalToolProcess fossologyProcess)
			throws TException {
		ExternalToolProcessStep furthestStep = fossologyProcess.getProcessSteps()
				.get(fossologyProcess.getProcessSteps().size() - 1);
		...
		switch (furthestStep.getStepStatus()) {
			...
			case DONE :
				// start report
				fossologyProcess
						.addToProcessSteps(createFossologyProcessStep(user, FossologyUtils.FOSSOLOGY_STEP_NAME_REPORT));
				handleReportStep(componentClient, release, user, fossologyProcess);
				break;
			default :
				// do nothing, unknown status
