...
public class ModifiedLevenshteinDistance {
	...
	/**
	 * This is a modified Levenshtein distance in which - allows skipping prefixes
	 * and postfixes of the haystack without any cost - with the restriction that
	 * one is not allowed to skip "everything" - is case insensitive - if one of the
	 * strings is empty the distance Integer.MAX_VALUE is returned - if no char from
	 * the needle is in the haystack the distance Integer.MAX_VALUE is returned
	 *
	 * @param needle
	 * @param haystack
	 ...
	 * @param haystack
	 * @param space
	 *            the chosen representation of the separator
	 * @return the modified Levenshtein distance between the needle and the haystack
	 */
	public static int calculateModifiedLevenshteinDistance(String needle, String haystack, char space) {
		...
		for (int i = 0; i < needleLength; i++)
			oldcost[i] = new LevenshteinCost(i);
		...
		for (int j = 1; j < haystackLength; j++) {
			...
			for (int i = 1; i < needleLength; i++) {
				boolean charsMatch = Character.toLowerCase(needle.charAt(i - 1)) == Character
						.toLowerCase(haystack.charAt(j - 1));
			...
			if (haystack.charAt(j - 1) == space) {
				// skipping postfix of haystack does not cost anything, if it starts with a
				// space
				minimalCostsWhenSkippedSpaceSeperatedPostfix = minimalCostsWhenSkippedSpaceSeperatedPostfix
						.merge(oldcost[needleLength - 1]);
			}
		...
		LevenshteinCost finalCost = oldcost[needleLength - 1].merge(minimalCostsWhenSkippedSpaceSeperatedPostfix);
