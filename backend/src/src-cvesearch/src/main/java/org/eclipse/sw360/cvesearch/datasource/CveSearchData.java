...
public class CveSearchData {
	 ...
	 * sometimes they are plain strings containing the cpe - sometimes they are
	 * objects containing an "id" which is the cpe and an "title" which is the human
	 * readeable name of an release
	 */
	...
	/**
	 * wrapper for date fields in Object, since - the api of version <=2.1 returns a
	 * date as formatted string - the api of version >=2.2 returns a object which
	 * contains the field "$date" which contains a long
	 */
	public static class DateTimeObject {
		...
		public DateTimeObject(long dateAsLong) {
			this.formattedDate = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS").format(new Date(dateAsLong));
		}
	 ...
	 * services that can find these weaknesses in source code and operational
	 * systems as well as better understanding and management of software weaknesses
	...
	/**
	 * CAPEC™ is a comprehensive dictionary and classification taxonomy of known
	 * attacks that can be used by analysts, developers, testers, and educators to
	...
	public Map<String, String> getVulnerable_configuration() {
		...
		nullToEmptySet(vulnerable_configuration).stream().map(VulnerableConfigurationEntry::getAsMapEntry)
	...
	public Map<String, Map<String, String>> getMap_cve_all() {
		...
		BiConsumer<String, Map<String, String>> f = (title, map) -> {
			if (map != null)
				mapOfAll.put(title, map);
		};
