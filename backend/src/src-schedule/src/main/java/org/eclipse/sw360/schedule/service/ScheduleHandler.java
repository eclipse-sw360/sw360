...
public class ScheduleHandler implements ScheduleService.Iface {
	...
	private boolean wrapSupplierException(SupplierThrowingTException body, String serviceName) {
		Supplier<RequestStatus> wrappedBody = () -> {
			try {
				...
				log.error("Was not able to schedule sync for client with name:" + serviceName + " message:"
						+ e.getMessage(), e);
		...
		switch (serviceName) {
			...
			case ThriftClients.DELETE_ATTACHMENT_SERVICE :
				successSync = wrapSupplierException(
						() -> thriftClients.makeAttachmentClient().deleteOldAttachmentFromFileSystem(), serviceName);
				break;
			default :
				log.error("Could not schedule service: " + serviceName
						+ ". Reason: service is not registered in ThriftClients.");
		}
	...
	public RequestStatusWithBoolean isServiceScheduled(String serviceName, User user) {
		...
		return new RequestStatusWithBoolean().setRequestStatus(RequestStatus.SUCCESS).setAnswerPositive(answer);
	}
	...
	public int getFirstRunOffset(String serviceName) {
		return ScheduleConstants.SYNC_FIRST_RUN_OFFSET_SEC.get(serviceName) != null
	...
	public int getInterval(String serviceName) {
		return ScheduleConstants.SYNC_INTERVAL_SEC.get(serviceName) != null
