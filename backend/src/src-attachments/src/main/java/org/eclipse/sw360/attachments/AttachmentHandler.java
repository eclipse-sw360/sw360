...
public class AttachmentHandler implements AttachmentService.Iface {
	...
	public AttachmentHandler() throws MalformedURLException {
		handler = new AttachmentDatabaseHandler(DatabaseSettings.getConfiguredClient(),
				DatabaseSettings.COUCH_DB_DATABASE, DatabaseSettings.COUCH_DB_ATTACHMENTS);
	}

	public AttachmentHandler(Supplier<CloudantClient> httpClient, String dbName, String attachmentDbName)
	...
	public void deleteAttachmentUsagesByUsageDataType(Source usedBy, UsageData usageData) throws TException {
		...
		Set<UsageData._Fields> usageDataTypes = usageData == null
				? Collections.emptySet()
				: ImmutableSet.of(usageData.getSetField());
	...
	public void replaceAttachmentUsages(Source usedBy, List<AttachmentUsage> attachmentUsages) throws TException {
		...
		List<AttachmentUsage> usagesWithNonEmptyType = attachmentUsages.stream().filter(AttachmentUsage::isSetUsageData)
