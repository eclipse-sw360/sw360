...
public class ModerationDatabaseHandler {
	...
	public ModerationDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName, String attachmentDbName)
	...
	public ModerationDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName, String changeLogsDbName,
	...
	public Map<PaginationData, List<ModerationRequest>> getRequestsByModerator(String moderator,
	...
	public ClearingRequest getClearingRequestById(String id, User user) throws SW360Exception {
		...
		if (!(clearingRequest.getClearingTeam().equals(user.getEmail())
			...
			projectDatabaseHandler.getProjectById(clearingRequest.getProjectId(), user); // check if user have READ
																							// access to project.
		}
		return clearingRequest;
	}
	...
	public ClearingRequest getClearingRequestByIdForEdit(String id, User user) throws SW360Exception {
		...
		if (!(clearingRequest.getClearingTeam().equals(user.getEmail())
			...
			throw fail("User " + SW360Utils.printFullname(user) + ", does not have WRITE access to clearing request: "
					+ clearingRequest.getId());
		}
		return clearingRequest;
	}
	...
	public void updateClearingRequestForProjectDeletion(Project project, User user) {
		...
		Comment comment = new Comment()
				.setText(new StringBuilder("Clearing Request is orphaned, as project (name): <b>")
						.append(SW360Utils.printName(project)).append("</b> associated with CR is deleted!")
						.toString());
		comment.setCommentedBy(user.getEmail());
		comment.setAutoGenerated(true);
		comment.setCommentedOn(System.currentTimeMillis());
	...
	public void acceptRequest(ModerationRequest request, String moderationComment, String reviewer) {
		...
		// when an MR requests deletion of a document and is accepted, all outstanding
		// MRs for that document are deleted,
		// which means that at this point we can't be sure that the MR still exists.
		// Therefore, we update it only if it still exists in the DB, but send mail
		// notifications using the data from
		...
		dbHandlerUtil.addChangeLogs(dbRequest, requestBefore, reviewer, Operation.MODERATION_ACCEPT, null,
				Lists.newArrayList(), dbRequest.getDocumentId(), null);
		sendMailNotificationsForAcceptedRequest(request);
	}
	...
	public RequestStatus createRequest(Release release, User user, Boolean isDeleteRequest,
			Function<Release, Set<String>> moderatorsProvider) {
		Release dbrelease;
		try {
	...
	private Set<String> getEccReleaseModerators(Release release) {
		...
		try {
			...
			log.error(
					"Could not get users from database. ECC admins not added as moderators, since department is missing.");
		}
	...
	private void appendCommentRequestingUserToRequest(ModerationRequest request, String comment) {
		/*
		 * For the case that the user is enlarging an existing moderation request, do
		 * not overwrite the old comment, but rather append the new comment to the old
		 * one.
		 */
	...
	private Set<String> getProjectModerators(Project project, String department) {
		...
		CommonUtils.addAll(moderators,
				getUsersAtLeast(UserGroup.CLEARING_ADMIN, project.getBusinessUnit(), true, false));
		return moderators;
	}
	...
	private Set<String> getLicenseModerators(String department) {
		...
		if (moderators.size() == 0) {
			moderators = sw360users.stream()
					.filter(user1 -> PermissionUtils.isUserAtLeast(UserGroup.CLEARING_ADMIN, user1)).map(User::getEmail)
					.collect(Collectors.toSet());
		}
		return moderators;
	}
	...
	private ModerationRequest createStubRequest(User user, boolean isDeleteRequest, String documentId,
		...
		Optional<ModerationRequest> firstModerationRequestOfUser = CommonUtils
				.getFirstModerationRequestOfUser(requestByDocumentId, user.getEmail());
		...
		inactive = users.stream().filter(sw360user -> sw360user.isDeactivated()).map(sw360user -> sw360user.getEmail())
	...
	private void sendMailForNewCommentInCR(ClearingRequest cr, Comment comment, User user) throws SW360Exception {
		...
		String userDetails = new StringBuilder(CommonUtils.nullToEmptyString(user.getUserGroup()))
				.append(MailConstants.DASH).append(SW360Utils.printFullname(user)).toString();
		mailUtil.sendClearingMail(ClearingRequestEmailTemplate.NEW_COMMENT,
				MailConstants.SUBJECT_FOR_CLEARING_REQUEST_COMMENT, recipients, userDetails,
				CommonUtils.nullToEmptyString(cr.getId()), SW360Utils.printName(project),
				CommonUtils.nullToEmptyString(comment.getText()));
	}

	private void sendMailNotificationsForNewRequest(ModerationRequest request, String userEmail) {
		mailUtil.sendMail(request.getModerators(), userEmail, MailConstants.SUBJECT_FOR_NEW_MODERATION_REQUEST,
				MailConstants.TEXT_FOR_NEW_MODERATION_REQUEST, SW360Constants.NOTIFICATION_CLASS_MODERATION_REQUEST,
	...
	private void sendMailNotificationsForUpdatedRequest(ModerationRequest request, String userEmail) {
		mailUtil.sendMail(request.getModerators(), userEmail, MailConstants.SUBJECT_FOR_UPDATE_MODERATION_REQUEST,
				MailConstants.TEXT_FOR_UPDATE_MODERATION_REQUEST, SW360Constants.NOTIFICATION_CLASS_MODERATION_REQUEST,
				ModerationRequest._Fields.MODERATORS.toString());
	}
	...
	private void sendMailToUserForDeclinedRequest(ModerationRequest request) {
		...
		if (isUserRequest) {
			mailUtil.sendMail(request.getRequestingUser(), MailConstants.SUBJECT_FOR_DECLINED_USER_MODERATION_REQUEST,
					MailConstants.TEXT_FOR_DECLINED_USER_MODERATION_REQUEST,
					SW360Constants.NOTIFICATION_CLASS_MODERATION_REQUEST,
					ModerationRequest._Fields.REQUESTING_USER.toString(), false);
		} else {
			mailUtil.sendMail(request.getRequestingUser(), MailConstants.SUBJECT_FOR_DECLINED_MODERATION_REQUEST,
					MailConstants.TEXT_FOR_DECLINED_MODERATION_REQUEST,
					SW360Constants.NOTIFICATION_CLASS_MODERATION_REQUEST,
					ModerationRequest._Fields.REQUESTING_USER.toString(), true,
					ThriftEnumUtils.enumToString(request.getDocumentType()), request.getDocumentName());
		}
	}

	private void sendMailNotificationsForAcceptedRequest(ModerationRequest request) {
		boolean isUserRequest = request.getDocumentType() == DocumentType.USER;
		mailUtil.sendMail(request.getRequestingUser(), MailConstants.SUBJECT_FOR_ACCEPTED_MODERATION_REQUEST,
				MailConstants.TEXT_FOR_ACCEPTED_MODERATION_REQUEST,
				SW360Constants.NOTIFICATION_CLASS_MODERATION_REQUEST,
				ModerationRequest._Fields.REQUESTING_USER.toString(), !isUserRequest,
				ThriftEnumUtils.enumToString(request.getDocumentType()), request.getDocumentName());
	}
}
