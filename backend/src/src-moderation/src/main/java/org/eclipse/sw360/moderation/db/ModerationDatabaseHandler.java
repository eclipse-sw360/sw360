...
public class ModerationDatabaseHandler {
    ...
    public RequestStatus updateClearingRequest(ClearingRequest request, User user, String projectUrl) {
        try {
            ...
            if (CommonUtils.isNotNullEmptyOrWhitespace(currentRequest.getAgreedClearingDate())
                    && CommonUtils.isNotNullEmptyOrWhitespace(request.getAgreedClearingDate())
                    && !currentRequest.getAgreedClearingDate().equals(request.getAgreedClearingDate())) {
                commentText = commentText.append("\nAgreed Clearing Date changed from: <b>")
                        .append(currentRequest.getAgreedClearingDate()).append("</b> to <b>")
                        .append(request.getAgreedClearingDate()).append("</b>");
            }
