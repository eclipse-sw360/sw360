...
public class ModerationRequestRepository extends SummaryAwareRepository<ModerationRequest> {
	...
	private static final String REQUESTING_USERS_VIEW = "function(doc) { " + "  if (doc.type == 'moderation') {"
			+ "    emit(doc.requestingUserDepartment, null);" + "  }" + "}";
	private static final String COUNTBYMODERATIONSTATE = "function(doc) {" + "    if (doc.type == 'moderation') {"
			+ "        var moderationState = doc.moderationState;" + "        for(var i in doc.moderators)"
			+ "        {" + "          if (moderationState === \"INPROGRESS\" || moderationState === \"PENDING\") {"
			+ "            emit([doc.moderators[i], \"OPEN\"], null);" + "          } else {"
	...
	public Map<PaginationData, List<ModerationRequest>> getRequestsByModerator(String moderator,
		...
		Map<PaginationData, List<ModerationRequest>> paginatedModerations = queryViewWithPagination(moderator, pageData,
				open);
	...
	private Map<PaginationData, List<ModerationRequest>> queryViewWithPagination(String moderator,
		...
		switch (sortColumnNo) {
			...
			case 4 :
				...
				qb = ascending
						? qb.sort(Sort.asc("requestingUserDepartment"))
