...
public abstract class ModerationRequestGenerator<U extends TFieldIdEnum, T extends TBase<T, U>> {
	...
	public abstract ModerationRequest setAdditionsAndDeletions(ModerationRequest request, T updateDocument,
			T actualDocument);
	...
	protected void dealWithBaseTypes(U field, FieldMetaData fieldMetaData) {
		...
		} else if (fieldMetaData.valueMetaData.type == TType.STRING || fieldMetaData.valueMetaData.type == TType.ENUM
	...
	private void dealWithStringSets(U field) {
		documentDeletions.setFieldValue(field, getDeletedStrings((Set<String>) actualDocument.getFieldValue(field),
				(Set<String>) updateDocument.getFieldValue(field)));
		documentAdditions.setFieldValue(field, getAddedStrings((Set<String>) actualDocument.getFieldValue(field),
				(Set<String>) updateDocument.getFieldValue(field)));
	}
	...
	protected void dealWithStringtoStringMap(U field) {
		Map<String, String> updateDocumentMap = CommonUtils
				.nullToEmptyMap((Map<String, String>) updateDocument.getFieldValue(field));
		Map<String, String> actualDocumentMap = CommonUtils
				.nullToEmptyMap((Map<String, String>) actualDocument.getFieldValue(field));
		if (updateDocumentMap.equals(actualDocumentMap)) {
			return;
		}
	...
	protected Attachment getAdditionsFromCommonAttachment(Attachment actual, Attachment update) {
		...
		Attachment additions = new Attachment().setAttachmentContentId(actual.getAttachmentContentId())
	...
	protected Attachment getDeletionsFromCommonAttachment(Attachment actual, Attachment update) {
		// new attachments with required fields set
		Attachment deletions = new Attachment().setAttachmentContentId(actual.getAttachmentContentId())
				.setFilename(actual.getFilename());
