...
public class SearchHandler implements SearchService.Iface {
	...
	ImmutableMap<String, String> specialCharToURLEncodedValue = ImmutableMap.of(" ", "%20", "+", "%2B", "@", "%40", "&",
			"%26", "#", "%23", "?", "%3F");
	...
	public SearchHandler(Supplier<HttpClient> hclient, Supplier<CloudantClient> cclient, String dbName)
			throws IOException {
	...
	private void dealWithSpecialCharacters(String text, User user, Set<SearchResult> results,
			AbstractDatabaseSearchHandler dbSearchHandler, final List<String> typeMask) {
		String queryStringQuoted = "\"" + text + "\"";
		results.addAll(dbSearchHandler.searchWithoutWildcard(queryStringQuoted, user, typeMask));
		searchWithDifferentCombinations(queryStringQuoted, user, results, dbSearchHandler, typeMask,
				specialCharToURLEncodedValue);
		...
		searchWithDifferentCombinations(queryStringQuoted, user, results, dbSearchHandler, typeMask,
				urlEncodedValToSPecialChar);

	}

	private void searchWithDifferentCombinations(String queryStringQuoted, User user, Set<SearchResult> results,
			AbstractDatabaseSearchHandler dbSearchHandler, final List<String> typeMask,
