...
public class ThriftExchange {
	...
	public Optional<List<Release>> searchReleaseByNameAndVersion(String name, String version) {
		...
		if (releases != null) {
			return Optional
					.of(releases.stream().filter(r -> r.getVersion().equals(version)).collect(Collectors.toList()));
		} else {
	...
	public Optional<List<License>> searchLicenseByWsName(String wsName) {
		return getFilteredLicenseList(
				license -> license.isSetExternalIds() && CommonUtils
						.nullToEmptyString(license.getExternalIds().get(TranslationConstants.WS_ID)).equals(wsName),
				"wsId=[" + wsName + "]:");
	}
	...
	private Optional<List<License>> getFilteredLicenseList(Predicate<License> filter, String selector) {
		try {
			return Optional.of(thriftClients.makeLicenseClient().getLicenses().stream().filter(filter)
	...
	private boolean hasAccessibleProjectWithWsToken(int wsProjectId, List<Project> accessibleProjects) {
		return accessibleProjects.stream().filter(Project::isSetExternalIds).anyMatch(project -> Integer
				.toString(wsProjectId).equals(project.getExternalIds().get(TranslationConstants.WS_ID)));
	}

	private boolean hasAccessibleProjectWithWsName(String wsProjectName, List<Project> accessibleProjects) {
		return accessibleProjects.stream().anyMatch(project -> wsProjectName.equals(project.getName()));
	}
}
