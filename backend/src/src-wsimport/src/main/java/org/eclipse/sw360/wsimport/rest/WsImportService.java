...
public class WsImportService {
	...
	public WsProject getWsProject(String projectToken, TokenCredentials tokenCredentials) throws JsonSyntaxException {
		...
		try {
			projectVitalString = restClient.getData(GET_PROJECT_VITALS, projectToken, WsTokenType.PROJECT,
					tokenCredentials);
		} catch (IOException | HttpException e) {
			LOGGER.error("Exception with " + GET_PROJECT_VITALS + " request to " + tokenCredentials.getServerUrl()
					+ ", with exception: " + e);
			return null;
		}
		...
		if (projectVitalInformation != null) {
			return new WsProject(projectVitalInformation.getId(), projectVitalInformation.getName(),
					projectVitalInformation.getToken(), projectVitalInformation.getCreationDate());
		} else {
	...
	public WsLibrary[] getProjectLicenses(String projectToken, TokenCredentials tokenCredentials)
			throws JsonSyntaxException {
		String projectLibsString = null;
		try {
			projectLibsString = restClient.getData(GET_PROJECT_LICENSES, projectToken, WsTokenType.PROJECT,
					tokenCredentials);
		} catch (IOException | HttpException e) {
			LOGGER.error("Exception with " + GET_PROJECT_VITALS + " request to " + tokenCredentials.getServerUrl()
					+ ", with exception: " + e);
