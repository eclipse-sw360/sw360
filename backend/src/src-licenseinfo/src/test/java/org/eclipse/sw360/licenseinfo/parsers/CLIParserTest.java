...
public class CLIParserTest {
	...
	public void testIsApplicableTo() throws Exception {
		when(connector.getAttachmentStream(eq(content), any(), any()))
	...
	public void testGetCLIObligations() throws Exception {
		...
		assertThat(oblRes.getObligationsAtProject().get(0).getTopic(),
				equalTo("do not change the nature of the package"));
		assertThat(oblRes.getObligationsAtProject().get(1).getText(),
				equalTo("In any case contact your 3rd Party Software Manager to check the copyleft effect\n"));
		assertThat(oblRes.getObligationsAtProject().get(1).getLicenseIDsSize(), is(4));
		assertThat(oblRes.getObligationsAtProject().get(1).getLicenseIDs(),
				containsInAnyOrder("GPL-1.0+", "GPL-2.0", "GPL-2.0+", "LGPL-2.1+"));
	}
