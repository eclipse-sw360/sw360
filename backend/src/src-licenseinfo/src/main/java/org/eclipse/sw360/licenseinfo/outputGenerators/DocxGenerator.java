...
public class DocxGenerator extends OutputGenerator<byte[]> {
			...
			switch (getOutputVariant()) {
				case DISCLOSURE :
					...
					if (docxTemplateFile.isPresent()) {
						fillDisclosureDocument(xwpfDocument, projectLicenseInfoResults, project, licenseInfoHeaderText,
								false, externalIds);
					} else {
				...
				case REPORT :
					...
					xwpfDocument = new XWPFDocument(new ByteArrayInputStream(docxTemplateFile.get()));
					if (docxTemplateFile.isPresent()) {
						fillReportDocument(xwpfDocument, projectLicenseInfoResults, project, licenseInfoHeaderText,
								true, obligationResults, user, obligationsStatus);
					} else {
						throw new SW360Exception(
	...
	private void fillDisclosureDocument(XWPFDocument document,
			Collection<LicenseInfoParsingResult> projectLicenseInfoResults, Project project,
			String licenseInfoHeaderText, boolean includeObligations, Map<String, String> externalIds)
	...
	private void fillExternalIds(XWPFDocument document, Map<String, String> externalIdMap) {
		if (!externalIdMap.isEmpty()) {
			...
			if (!list.isEmpty()) {
				...
				addFormattedText(tableHeader.getCell(0).addParagraph().createRun(), EXT_ID_TABLE_HEADER_COL1, FONT_SIZE,
						true);
				addFormattedText(tableHeader.getCell(1).addParagraph().createRun(), EXT_ID_TABLE_HEADER_COL2, FONT_SIZE,
						true);
	...
	private void fillReportDocument(XWPFDocument document,
			Collection<LicenseInfoParsingResult> projectLicenseInfoResults, Project project,
			String licenseInfoHeaderText, boolean includeObligations,
			Collection<ObligationParsingResult> obligationResults, User user,
			Map<String, ObligationStatusInfo> obligationsStatus) throws XmlException, TException {
	...
	private void fillSpecialOSSRisksTable(XWPFDocument document, Project project,
		...
		obligationResults.stream().filter(opr -> opr.getStatus() == ObligationInfoRequestStatus.SUCCESS)
				.flatMap(opr -> opr.getObligationsAtProject().stream()).distinct().forEach(o -> {
	...
	private void fillDevelopmentDetailsTable(XWPFDocument document, Project project, User user,
		...
		for (LicenseInfoParsingResult result : projectLicenseInfoResults) {
			...
			String operatingSystems = r.getOperatingSystemsSize() == 0
					? "N/A"
	...
	protected static Set<String> extractMostCommonLicenses(Collection<ObligationParsingResult> obligationResults,
			long threshold) {
		return obligationResults.stream().filter(opr -> opr.getStatus() == ObligationInfoRequestStatus.SUCCESS)
				.flatMap(opr -> opr.getObligationsAtProject().stream()).flatMap(o -> o.getLicenseIDs().stream())
				.collect(Collectors.groupingBy(Function.identity(), Collectors.counting())).entrySet().stream()
				.filter(entry -> entry.getValue().longValue() >= threshold).map(entry -> entry.getKey())
				.map(license -> license.replace("\n", "").replace("\r", "")).collect(Collectors.toSet());
	}

	private void fillProjectComponentOrganisationObligationsTable(XWPFDocument document,
			Map<String, ObligationStatusInfo> obligationsStatusAtProject, List<Obligation> obligations,
			ObligationLevel oblLevel, int tableIndex, String emptyTableText) {
	...
	private void fillComponentObligationsTable(XWPFDocument document,
		...
		obligationResults.stream().filter(opr -> opr.getStatus() == ObligationInfoRequestStatus.SUCCESS)
				.flatMap(opr -> opr.getObligationsAtProject().stream()).filter(
						o -> o.getLicenseIDs().stream()
	...
	private String getFilename(LicenseInfoParsingResult licenseInfoParsingResult) {
		return Optional.ofNullable(licenseInfoParsingResult.getLicenseInfo()).map(LicenseInfo::getFilenames)
				.flatMap(l -> l.stream().findFirst()).orElse(UNKNOWN_FILE_NAME);
	}
