...
public class CombinedCLIParser extends AbstractCLIParser {
	...
	public CombinedCLIParser(AttachmentConnector attachmentConnector,
			AttachmentContentProvider attachmentContentProvider, ComponentDatabaseHandler componentDatabaseHandler) {
	...
	String getCorrelationKey() {
		...
		if (isNullOrEmpty(releaseExternalIdCorrelationKey)) {
			log.warn(
					"Property combined.cli.parser.external.id.correlation.key is not set. Combined CLI parsing will not be able to load names of referenced releases");
		}
	...
	public <T> boolean isApplicableTo(Attachment attachment, User user, T context) throws TException {
		...
		return attachmentContent.getFilename().endsWith(XML_FILE_EXTENSION)
				&& hasCombinedCLIRootElement(attachmentContent, user, context);
	}

	private <T> boolean hasCombinedCLIRootElement(AttachmentContent content, User user, T context) throws TException {
		return hasThisXMLRootElement(content, COMBINED_CLI_ROOT_ELEMENT_NAMESPACE, COMBINED_CLI_ROOT_ELEMENT_NAME, user,
				context);
	}
	...
	private Map<String, Set<LicenseNameWithText>> getLicenseNamesWithTextsByExternalIdsMap(Document doc)
	...
	private Map<String, Set<String>> getCopyrightSetsByExternalIdsMap(Document doc) throws XPathExpressionException {
		...
		return nodeListToStringSetsByExternalId(copyrightNodes, EXTERNAL_ID_ATTRIBUTE_NAME,
				COPYRIGHT_CONTENT_ELEMENT_NAME);
	}

	@NotNull
	private LicenseInfoParsingResult getLicenseInfoParsingResultForExternalId(AttachmentContent attachmentContent,
			Map<String, Release> releasesByExternalId, Map<String, Set<String>> copyrightSetsByExternalId,
			Map<String, Set<LicenseNameWithText>> licenseNamesWithTextsByExternalId, String extId) {
	...
	private Map<String, Release> prepareReleasesByExternalId(String correlationKey) {
		...
		Map<String, Release> releasesByExternalId = idMap.values().stream()
				...
				.collect(Collectors.toMap(r -> r.getExternalIds().get(correlationKey), r -> r, (r1, r2) -> {
					log.warn(String.format("Duplicate externalId in releases %s and %s", SW360Utils.printFullname(r1),
							SW360Utils.printFullname(r2)));
					return r1;
				}));
		return releasesByExternalId;
	}

	private Map<String, Set<String>> nodeListToStringSetsByExternalId(NodeList nodes, String externalIdAttributeName,
	...
	private Map<String, Set<LicenseNameWithText>> nodeListToLicenseNamesWithTextsSetsByExternalId(NodeList nodes,
