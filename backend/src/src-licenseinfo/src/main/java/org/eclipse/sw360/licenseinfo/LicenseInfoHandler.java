...
public class LicenseInfoHandler implements LicenseInfoService.Iface {
	...
	private static final String DEFAULT_LICENSE_INFO_TEXT = SW360Utils.dropCommentedLine(LicenseInfoHandler.class,
			DEFAULT_LICENSE_INFO_HEADER_FILE);
	private static final String DEFAULT_OBLIGATIONS_FILE = "/DefaultObligations.txt";
	private static final String DEFAULT_OBLIGATIONS_TEXT = SW360Utils.dropCommentedLine(LicenseInfoHandler.class,
			DEFAULT_OBLIGATIONS_FILE);
	...
	public LicenseInfoHandler() throws MalformedURLException {
		this(new AttachmentDatabaseHandler(DatabaseSettings.getConfiguredClient(), DatabaseSettings.COUCH_DB_DATABASE,
				DatabaseSettings.COUCH_DB_ATTACHMENTS),
				new ComponentDatabaseHandler(DatabaseSettings.getConfiguredClient(), DatabaseSettings.COUCH_DB_DATABASE,
						DatabaseSettings.COUCH_DB_ATTACHMENTS),
				new ProjectDatabaseHandler(DatabaseSettings.getConfiguredClient(), DatabaseSettings.COUCH_DB_DATABASE,
						DatabaseSettings.COUCH_DB_ATTACHMENTS));
	}

	@VisibleForTesting
	protected LicenseInfoHandler(AttachmentDatabaseHandler attachmentDatabaseHandler,
			ComponentDatabaseHandler componentDatabaseHandler, ProjectDatabaseHandler projectDatabaseHandler)
		...
		this.obligationCache = CacheBuilder.newBuilder().expireAfterWrite(CACHE_TIMEOUT_MINUTES, TimeUnit.MINUTES)
				.maximumSize(CACHE_MAX_ITEMS).build();
		this.licenseInfoCacheForEvaluation = CacheBuilder.newBuilder()
				.expireAfterWrite(CACHE_TIMEOUT_MINUTES, TimeUnit.MINUTES).maximumSize(CACHE_MAX_ITEMS).build();
		this.obligationCacheForEvaluation = CacheBuilder.newBuilder()
				.expireAfterWrite(CACHE_TIMEOUT_MINUTES, TimeUnit.MINUTES).maximumSize(CACHE_MAX_ITEMS).build();
		this.licenseObligationMappingCache = CacheBuilder.newBuilder()
				.expireAfterWrite(CACHE_TIMEOUT_MINUTES, TimeUnit.MINUTES).maximumSize(CACHE_MAX_ITEMS).build();
	...
	public List<LicenseInfoParsingResult> getLicenseInfoForAttachment(Release release, String attachmentContentId,
			boolean includeConcludedLicense, User user) throws TException {
		if (release == null) {
	...
	public LicenseObligationsStatusInfo getProjectObligationStatus(
			Map<String, ObligationStatusInfo> obligationStatusMap, List<LicenseInfoParsingResult> licenseResults,
		...
		Map<String, ObligationStatusInfo> filteredObligationStatusMap = obligationStatusMap.isEmpty()
				? Maps.newHashMap()
				: removeOrphanedObligations(obligationStatusMap, excludedReleaseIdToAcceptedCLI);
		...
		return new LicenseObligationsStatusInfo().setLicenseInfoResults(licenseResults)
				.setObligationStatusMap(filteredObligationStatusMap);
	}

	private Map<String, ObligationStatusInfo> removeOrphanedObligations(
			Map<String, ObligationStatusInfo> obligationStatusMap, Map<String, String> excludedReleaseIdToAcceptedCLI) {
		if (!excludedReleaseIdToAcceptedCLI.isEmpty()) {
			Map<String, ObligationStatusInfo> filteredObligationStatusMap = obligationStatusMap.entrySet().stream()
					.map(entry -> {
	...
	@Override
	public LicenseInfoParsingResult createLicenseToObligationMapping(LicenseInfoParsingResult licenseResult,
			ObligationParsingResult obligationResult) {

		LicenseInfoParsingResult cachedResults = licenseObligationMappingCache
				.getIfPresent(licenseResult.getAttachmentContentId());
		if (cachedResults != null) {
			return cachedResults;
		}

		Map<String, Set<ObligationAtProject>> licenseIdToObligations = obligationResult.getObligationsAtProject()
				.stream()
		...
		licenseInfo.getLicenseNamesWithTexts().forEach(
				license -> license.setObligationsAtProject(licenseIdToObligations.get(license.getLicenseName())));
	...
	private Map<String, ObligationStatusInfo> createLicenseToObligationMappingForReport(Project project,
			Collection<LicenseInfoParsingResult> licenseResults, Collection<ObligationParsingResult> obligationResults,
			Map<Release, Map<String, Boolean>> releaseToSelectedAttachmentIds, User user) throws TException {
		...
		Map<String, ObligationParsingResult> attachmentIdToObligationMap = obligationResults.stream()
				...
				.filter(o -> o.getObligationsAtProjectSize() > 0).collect(Collectors.toList()).stream()
		...
		if (CommonUtils.isNotNullEmptyOrWhitespace(project.getLinkedObligationId())) {
			ObligationList obligation = projectDatabaseHandler.getLinkedObligations(project.getLinkedObligationId(),
					user);
		...
		for (Entry<Release, Map<String, Boolean>> entry : filteredRelToSelAttIds.entrySet()) {
			...
			if (filteredAttachments.size() == 1) {
				...
				if (releaseIdToAcceptedCLI.containsKey(releaseId)
						&& releaseIdToAcceptedCLI.get(releaseId).equals(acceptedAttachmentContentId)) {
				...
				for (String attachmentContentId : entry.getValue().keySet()) {
					...
					if (attachmentContentId.equals(acceptedAttachmentContentId) && null != obligationResult
							&& null != licenseResult) {
		...
		return getProjectObligationStatus(obligationStatusMap, licenseParsingResults, releaseIdToAcceptedCLI)
				.getObligationStatusMap();
	}
	...
	public List<ObligationParsingResult> getObligationsForAttachment(Release release, String attachmentContentId,
		...
		if (release == null) {
			return Collections.singletonList(new ObligationParsingResult()
					.setStatus(ObligationInfoRequestStatus.NO_APPLICABLE_SOURCE).setMessage(MSG_NO_RELEASE_GIVEN));
		}
		...
		try {
			...
			if (applicableParsers.size() == 0) {
				...
				return Collections.singletonList(
						new ObligationParsingResult().setStatus(ObligationInfoRequestStatus.NO_APPLICABLE_SOURCE)
	...
	private LicenseInfoParsingResult assignFileNameToLicenseInfoParsingResult(
			LicenseInfoParsingResult licenseInfoParsingResult, String filename) {
	...
	protected Collection<LicenseInfoParsingResult> getAllReleaseLicenseInfos(
			Map<Release, Map<String, Boolean>> releaseToSelectedAttachmentIds, User user,
			Map<String, Set<LicenseNameWithText>> excludedLicensesPerAttachment) throws TException {
		...
		for (Entry<Release, Map<String, Boolean>> entry : releaseToSelectedAttachmentIds.entrySet()) {
			for (Entry<String, Boolean> attachmentIdUseLicenseInfoFromFileEntry : entry.getValue().entrySet()) {
				...
				if (attachmentContentId != null) {
					Set<LicenseNameWithText> licencesToExclude = excludedLicensesPerAttachment
							.getOrDefault(attachmentContentId, Sets.newHashSet());
					...
					results.addAll(parsedLicenses.stream().map(result -> filterLicenses(result, licencesToExclude))
							.collect(Collectors.toList()));
				}
			}
		}
	...
	private Collection<ObligationParsingResult> getAllReleaseObligations(
			Map<Release, Map<String, Boolean>> releaseToSelectedAttachmentIds, User user) throws TException {
	...
	protected LicenseInfoParsingResult filterLicenses(LicenseInfoParsingResult result,
			Set<LicenseNameWithText> licencesToExclude) {
		...
		if (result.getLicenseInfo() != null) {
			Set<LicenseNameWithText> filteredLicenses = nullToEmptySet(
					result.getLicenseInfo().getLicenseNamesWithTexts()).stream().filter(license -> {
	...
	protected List<LicenseInfoParsingResult> assignReleaseToLicenseInfoParsingResult(
			LicenseInfoParsingResult licenseInfoParsingResult, Release release) {
		return assignReleaseToLicenseInfoParsingResults(Collections.singletonList(licenseInfoParsingResult), release);
	}

	protected List<LicenseInfoParsingResult> assignReleaseToLicenseInfoParsingResults(
			List<LicenseInfoParsingResult> parsingResults, Release release) {
		parsingResults.forEach(r -> {
			// override by given release only if the fields were not set by parser, because
			// parser knows best
	...
	protected List<ObligationParsingResult> assignReleaseToObligationParsingResults(
			List<ObligationParsingResult> parsingResults, Release release) {
		parsingResults.stream().filter(r -> !r.isSetRelease()).forEach(r -> r.setRelease(release));
		return parsingResults;
	}

	private List<LicenseInfoParsingResult> assignComponentToLicenseInfoParsingResults(
			List<LicenseInfoParsingResult> parsingResults, Release release, User user) throws TException {
	...
	protected OutputGenerator<?> getOutputGeneratorByClassnameAndVariant(String generatorClassname,
			OutputFormatVariant generatorVariant) throws TException {
		assertNotNull(generatorClassname);
		assertNotNull(generatorVariant);
		return outputGenerators.stream()
				.filter(outputGenerator -> generatorClassname.equals(outputGenerator.getClass().getSimpleName()))
				.filter(outputGenerator -> generatorVariant.equals(outputGenerator.getOutputVariant())).findFirst()
				.orElseThrow(() -> new TException("Unknown output generator: " + generatorClassname));
	}
