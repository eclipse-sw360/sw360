...
public class CLIParser extends AbstractCLIParser {
	...
	public <T> boolean isApplicableTo(Attachment attachment, User user, T context) throws TException {
		...
		return attachmentContent.getFilename().endsWith(XML_FILE_EXTENSION)
				&& hasCLIRootElement(attachmentContent, user, context);
	}
	...
	public <T> ObligationParsingResult getObligations(Attachment attachment, User user, T context) throws TException {
		...
		try {
			...
			result.setStatus(ObligationInfoRequestStatus.SUCCESS);
				...
				| SW360Exception e) {
			log.error(e);
			result.setStatus(ObligationInfoRequestStatus.FAILURE)
	...
	private <T> List<LicenseInfoParsingResult> getLicenseInfosDelegated(Attachment attachment, User user, T context,
			boolean includeFilesHash) throws TException {
		AttachmentContent attachmentContent = attachmentContentProvider.getAttachmentContent(attachment);
		...
		try {
			...
			result.setStatus(LicenseInfoRequestStatus.SUCCESS);
				...
				| SW360Exception e) {
			log.error(e);
			result.setStatus(LicenseInfoRequestStatus.FAILURE)
					.setMessage("Error while parsing CLI file: " + e.toString());
		} finally {
			closeQuietly(attachmentStream, log);
		}
	...
	private Set<LicenseNameWithText> getLicenseNameWithTexts(Document doc, boolean includeFilesHash)
	...
	private String getSha1Hash(Document doc) throws XPathExpressionException {
		...
		String result = findNamedAttribute(copyrightNodes.item(0), "componentSHA1").map(Node::getNodeValue).orElse("");
		return result;
	}
	...
	private String getComponent(Document doc) throws XPathExpressionException {
		...
		String result = findNamedAttribute(copyrightNodes.item(0), "component").map(Node::getNodeValue).orElse("");
		return result;
	}
	...
	private Set<LicenseNameWithText> nodeListToLicenseNamesWithTextsSet(NodeList nodes, boolean includeFilesHash) {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			licenseNamesWithTexts.add(includeFilesHash
					? getLicenseNameWithTextFromLicenseNodeAndFileHash(nodes.item(i))
