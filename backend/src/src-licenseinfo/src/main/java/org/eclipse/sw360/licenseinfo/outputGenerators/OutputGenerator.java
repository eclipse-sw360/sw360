...
public abstract class OutputGenerator<T> {
	...
	public OutputFormatInfo getOutputFormatInfo() {
		return new OutputFormatInfo().setFileExtension(getOutputType()).setDescription(getOutputDescription())
				.setIsOutputBinary(isOutputBinary()).setGeneratorClassName(this.getClass().getSimpleName())
				.setMimeType(getOutputMimeType()).setVariant(getOutputVariant());
	}
	...
	@NotNull
	protected LicenseInfoParsingResult mergeLicenseInfoParsingResults(LicenseInfoParsingResult r1,
			LicenseInfoParsingResult r2) {
		...
		if (r.isSetLicenseInfo()) {
			r.getLicenseInfo().setLicenseNamesWithTexts(Stream
					.concat(getCollectionStream(r, LicenseInfo::getLicenseNamesWithTexts, Collections::emptySet),
							getCollectionStream(r2, LicenseInfo::getLicenseNamesWithTexts, Collections::emptySet))
					.collect(Collectors.toSet()));

			r.getLicenseInfo()
					.setCopyrights(Stream
							.concat(getCollectionStream(r, LicenseInfo::getCopyrights, Collections::emptySet),
									getCollectionStream(r2, LicenseInfo::getCopyrights, Collections::emptySet))
							.collect(Collectors.toSet()));

			r.getLicenseInfo()
					.setFilenames(Stream
							.concat(getCollectionStream(r, LicenseInfo::getFilenames, Collections::emptyList),
	...
	private <T extends Collection<U>, U> Stream<U> getCollectionStream(LicenseInfoParsingResult r,
			Function<LicenseInfo, T> collectionExtractor, Supplier<T> defaultEmpty) {
		return Optional.of(r).map(LicenseInfoParsingResult::getLicenseInfo).map(collectionExtractor)
	...
	protected SortedMap<String, Set<String>> getSortedAcknowledgements(
		...
		Map<String, Set<String>> acknowledgements = Maps.filterValues(
				Maps.transformValues(sortedLicenseInfos, pr -> Optional
						.ofNullable(pr.getLicenseInfo()).map(LicenseInfo::getLicenseNamesWithTexts)
						.filter(Objects::nonNull).map(s -> s.stream().map(LicenseNameWithText::getAcknowledgements)
	...
	protected static List<LicenseNameWithText> getSortedLicenseNameWithTexts(
		...
		Set<LicenseNameWithText> licenseNamesWithText = projectLicenseInfoResults.stream()
				...
				.map(LicenseInfo::getLicenseNamesWithTexts).filter(Objects::nonNull).flatMap(Set::stream)
				.collect(Collectors.toSet());
	...
	private static <U> SortedMap<String, U> sortStringKeyedMap(Map<String, U> unsorted) {
		...
		if (sorted.size() != unsorted.size()) {
			// there were key collisions and some data was lost -> throw away the sorted map
			// and sort by case sensitive order
			sorted = new TreeMap<>();
			sorted.putAll(unsorted);
		}
		return sorted;
	 ...
	 * sorted by name. Contains only the results with status
	 * {@link LicenseInfoRequestStatus#SUCCESS}</li>
	 * <li>licenseInfoErrorResults: map {@link List}of
	 * {@link LicenseInfoParsingResult} objects, where the key is the name of the
	 * release. Contains only the results with status other than
	 * {@link LicenseInfoRequestStatus#SUCCESS}. These results are not merged,
	 * that's why the map values are lists.</li>
	...
	protected String renderTemplateWithDefaultValues(Collection<LicenseInfoParsingResult> projectLicenseInfoResults,
		...
		Map<String, List<LicenseInfoParsingResult>> badResultsPerRelease = partitionedResults.get(false).stream()
				.collect(Collectors.groupingBy(this::getComponentLongName));
