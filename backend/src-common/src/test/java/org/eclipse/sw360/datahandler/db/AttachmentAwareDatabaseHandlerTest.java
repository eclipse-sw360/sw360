...
public class AttachmentAwareDatabaseHandlerTest {
	...
	public void testGetAllAttachmentsToKeep() throws Exception {
		...
		Attachment attachmentAccepted = new Attachment().setAttachmentContentId("acceptedAtt").setFilename("att1.file")
				.setCheckStatus(CheckStatus.ACCEPTED);
		Attachment attachmentRejected = new Attachment().setAttachmentContentId("rejectedAtt").setFilename("att2.file")
				.setCheckStatus(CheckStatus.REJECTED);
		...
		Set<Attachment> attachmentsToKeep = handler.getAllAttachmentsToKeep(Source.releaseId("dummy"),
		...
		Attachment originalAttachment = new Attachment().setAttachmentContentId("att").setFilename("att.file")
				.setCheckStatus(CheckStatus.ACCEPTED);
		...
		attachmentsToKeep = handler.getAllAttachmentsToKeep(Source.releaseId("dummy"), attachmentsBefore,
				attachmentsAfter);
	...
	public void testGetAllAttachmentsToKeepPreservesUsedAttachments() throws Exception {
		...
		Attachment attachmentUsed = new Attachment().setAttachmentContentId("usedAtt").setFilename("att1.file")
				.setCheckStatus(CheckStatus.NOTCHECKED);
		Attachment attachmentUnused = new Attachment().setAttachmentContentId("unusedAtt").setFilename("att2.file")
				.setCheckStatus(CheckStatus.NOTCHECKED);
		Attachment attachmentUnused2 = new Attachment().setAttachmentContentId("unusedAtt2").setFilename("att3.file")
				.setCheckStatus(CheckStatus.NOTCHECKED);
		Set<Attachment> attachmentsBefore = ImmutableSet.of(attachmentUsed, attachmentUnused, attachmentUnused2);
		Set<Attachment> attachmentsAfter = new HashSet<>();

		when(attachmentDatabaseHandler.getAttachmentUsageCount(
				ImmutableMap.of(Source.releaseId("releaseId"), ImmutableSet.of("usedAtt", "unusedAtt", "unusedAtt2")),
				null)).thenReturn(
						ImmutableMap.of(ImmutableMap.of(Source.releaseId("releaseId"), "usedAtt"), 2,
								ImmutableMap.of(Source.releaseId("releaseId"), "unusedAtt"), 0));

		Set<Attachment> attachmentsToKeep = handler.getAllAttachmentsToKeep(Source.releaseId("releaseId"),
				attachmentsBefore, attachmentsAfter);
	...
	public void testGetAllAttachmentsToKeepCanHandleNull() throws Exception {
		...
		Attachment attachmentAccepted1 = new Attachment().setAttachmentContentId("acceptedAtt1")
				.setFilename("att1.file").setCheckStatus(CheckStatus.ACCEPTED);
		Attachment attachmentRejected1 = new Attachment().setAttachmentContentId("rejectedAtt1")
				.setFilename("att2.file").setCheckStatus(CheckStatus.REJECTED);
		Attachment attachmentRejected2 = new Attachment().setAttachmentContentId("rejectedAtt2")
				.setFilename("att3.file").setCheckStatus(CheckStatus.NOTCHECKED);
		...
		Set<Attachment> attachmentsToKeep = handler.getAllAttachmentsToKeep(Source.releaseId("dummy"), attachments,
				null);
		assertEquals(1, attachmentsToKeep.size());
		...
		// Test what happens if `originalAttachments` are `null` (this means adding of
		// attachments)
		// -> all should be added
