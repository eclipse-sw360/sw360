...
public class ProjectDatabaseHandlerTest {
	...
	public void setUp() throws Exception {
		...
		Project p4 = new Project().setId("P4").setName("Project4").setBusinessUnit("AB CD EF").setCreatedBy("user1")
				...
				.setLinkedProjects(ImmutableMap.<String, ProjectProjectRelationship>builder()
						.put("P5", new ProjectProjectRelationship(ProjectRelationship.CONTAINED)).build());
		projects.add(p4);
		...
		componentHandler = new ComponentDatabaseHandler(DatabaseSettingsTest.getConfiguredClient(), dbName,
				changeLogsDbName, attachmentsDbName);
		attachmentDatabaseHandler = new AttachmentDatabaseHandler(DatabaseSettingsTest.getConfiguredClient(), dbName,
				attachmentsDbName);
		handler = new ProjectDatabaseHandler(DatabaseSettingsTest.getConfiguredClient(), dbName, changeLogsDbName,
				attachmentsDbName, moderator, componentHandler, attachmentDatabaseHandler);
	}
	...
	public void testReleaseIdToEmptyProjects() throws Exception {
		SetMultimap<String, ProjectWithReleaseRelationTuple> releaseIdToProjects = handler
				.releaseIdToProjects(new Project().setId("p4"), user1);
	...
	public void testReleaseIdToProjects() throws Exception {
		...
		p1.setLinkedProjects(ImmutableMap.<String, ProjectProjectRelationship>builder()
				.put("P2", new ProjectProjectRelationship(ProjectRelationship.CONTAINED)).build());
		handler.updateProject(p1, user1);
		Project p2 = handler.getProjectById("P2", user2);

		SetMultimap<String, ProjectWithReleaseRelationTuple> releaseIdToProjects = handler.releaseIdToProjects(p1,
				user1);
