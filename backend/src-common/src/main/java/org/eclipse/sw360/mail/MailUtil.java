...
public class MailUtil extends BackendUtils {
	...
	private static final String NEW_CR_EMAIL_HTML_TEMPLATE = SW360Utils.dropCommentedLine(MailUtil.class,
			NEW_CLEARING_REQUEST_EMAIL_TEMPLATE_FILE);
	private static final String UPDATE_CR_EMAIL_HTML_TEMPLATE = SW360Utils.dropCommentedLine(MailUtil.class,
			UPDATE_CLEARING_REQUEST_EMAIL_TEMPLATE_FILE);
	private static final String UPDATE_PROJECT_WITH_CR_EMAIL_HTML_TEMPLATE = SW360Utils
			.dropCommentedLine(MailUtil.class, UPDATE_PROJECT_WITH_CR_EMAIL_TEMPLATE_FILE);
	private static final String NEW_COMMENT_IN_CR_EMAIL_HTML_TEMPLATE = SW360Utils.dropCommentedLine(MailUtil.class,
			NEW_COMMENT_IN_CR_EMAIL_HTML_TEMPLATE_FILE);
	private static final String CLOSED_OR_REJECTED_CR_EMAIL_HTML_TEMPLATE = SW360Utils.dropCommentedLine(MailUtil.class,
			CLOSED_OR_REJECTED_CR_EMAIL_HTML_TEMPLATE_FILE);
	...
	private static ExecutorService fixedThreadPoolWithQueueSize(int nThreads, int queueSize) {
		// ThreadPoolExecutor.AbortPolicy is used as default which throws
	...
	public void sendClearingMail(ClearingRequestEmailTemplate template, String subjectNameInPropertiesFile,
			Map<String, String> recipients, String... textParameters) {
		MimeMessage messageWithSubjectAndText;
		messageWithSubjectAndText = makeHtmlMessageWithSubjectAndText(template, subjectNameInPropertiesFile,
				textParameters);
		if (!CommonUtils.isNullOrEmptyMap(recipients)) {
			...
			if (isMailWantedBy(requestingUser,
					SW360Utils.notificationPreferenceKey(SW360Constants.NOTIFICATION_CLASS_CLEARING_REQUEST,
				...
				sendMailWithSubjectAndText(recipients.get(ClearingRequest._Fields.CLEARING_TEAM.toString()),
						messageWithSubjectAndText);
			}
		}
	}

	public void sendMail(String recipient, String subjectNameInPropertiesFile, String textNameInPropertiesFile,
			String notificationClass, String roleName, String... textParameters) {
		sendMail(recipient, subjectNameInPropertiesFile, textNameInPropertiesFile, notificationClass, roleName, true,
				textParameters);
	}
	public void sendMail(String recipient, String subjectNameInPropertiesFile, String textNameInPropertiesFile,
			String notificationClass, String roleName, boolean checkWantsNotifications, String... textParameters) {
		sendMail(Sets.newHashSet(recipient), null, subjectNameInPropertiesFile, textNameInPropertiesFile,
				notificationClass, roleName, checkWantsNotifications, textParameters);
	}

	public void sendMail(Set<String> recipients, String excludedRecipient, String subjectNameInPropertiesFile,
			String textNameInPropertiesFile, String notificationClass, String roleName, String... textParameters) {
		sendMail(recipients, excludedRecipient, subjectNameInPropertiesFile, textNameInPropertiesFile,
				notificationClass, roleName, true, textParameters);
	}

	private void sendMail(Set<String> recipients, String excludedRecipient, String subjectNameInPropertiesFile,
			String textNameInPropertiesFile, String notificationClass, String roleName, boolean checkWantsNotifications,
			String... textParameters) {
		MimeMessage messageWithSubjectAndText = makeMessageWithSubjectAndText(subjectNameInPropertiesFile,
				textNameInPropertiesFile, textParameters);
		for (String recipient : nullToEmptySet(recipients)) {
			if (!isNullEmptyOrWhitespace(recipient) && !recipient.equals(excludedRecipient) && (!checkWantsNotifications
					|| isMailWantedBy(recipient, SW360Utils.notificationPreferenceKey(notificationClass, roleName)))) {
	...
	private boolean isMailWantedBy(String userEmail, String notificationPreferenceKey) {
		...
		if (user != null) {
			...
			return user.isWantsMailNotification()
	...
	private MimeMessage makeHtmlMessageWithSubjectAndText(ClearingRequestEmailTemplate template,
			String subjectKeyInPropertiesFile, String... textParameters) {
		...
		try {
			...
			log.error(String.format("Could not format notification email content for key %s ",
					subjectKeyInPropertiesFile), e);
	...
	private MimeMessage makeMessageWithSubjectAndText(String subjectKeyInPropertiesFile, String textKeyInPropertiesFile,
		...
		try {
			...
			log.error(String.format("Could not format notification email content for keys %s and %s",
					subjectKeyInPropertiesFile, textKeyInPropertiesFile), e);
			text.append(mainContentFormat);
		}
	...
	private void writeMessageToLog(MimeMessage message) {
		try {
			log.info(String.format(
					...
					Arrays.toString(message.getRecipients(Message.RecipientType.TO)), message.getSubject(),
					message.getContent()));
