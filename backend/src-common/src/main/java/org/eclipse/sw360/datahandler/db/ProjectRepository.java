...
public class ProjectRepository extends SummaryAwareRepository<Project> {
	...
	private static final String FULL_MY_PROJECTS_VIEW = "function(doc) {\n" + "  if (doc.type == 'project') {\n"
			+ "    var acc = {};\n" + "    if(doc.createdBy)\n" + "      acc[doc.createdBy]=1;\n"
	...
	private static final String MY_ACCESSIBLE_PROJECTS_COUNT = "function(doc) {\n" + "  if (doc.type == 'project') {\n"
	...
	private static final String ACCESSIBLE_PROJECTS_COUNT_FOR_CA_AND_ABOVE = "function(doc) {\n"
			+ "  if (doc.type == 'project') {\n" + "    var acc = {};\n"
			+ "    var meAndModerator = doc.visbility == \"ME_AND_MODERATORS\";\n"
			+ "    if(doc.createdBy && (meAndModerator || doc.visbility == \"PRIVATE\"))\n"
			+ "      acc[doc.createdBy]=1;\n" + "    if(doc.leadArchitect && meAndModerator)\n"
	...
	private static final String BU_PROJECTS_VIEW = "function(doc) {" + "  if (doc.type == 'project') {"
			+ "    emit(doc.businessUnit, doc._id);" + "  }" + "}";

	private static final String BY_NAME_VIEW = "function(doc) {" + "  if (doc.type == 'project') {"
			+ "    emit(doc.name, doc._id);" + "  }" + "}";

	private static final String BY_TAG_VIEW = "function(doc) {" + "  if (doc.type == 'project') {"
			+ "    emit(doc.tag, doc._id);" + "  }" + "}";

	private static final String BY_GROUP_VIEW = "function(doc) {" + "  if (doc.type == 'project') {"
			+ "    emit(doc.businessUnit, doc._id);" + "  }" + "}";

	private static final String BY_TYPE_VIEW = "function(doc) {" + "  if (doc.type == 'project') {"
			+ "    emit(doc.projectType, doc._id);" + "  }" + "}";

	private static final String BY_STATE_VIEW = "function(doc) {" + "    if (doc.type == 'project') {"
			+ "      if(doc.clearingState == 'OPEN')" + "      {" + "        emit(doc.state+0, doc._id);"
			+ "      } else if(doc.clearingState == 'IN_PROGRESS'){" + "        emit(doc.state+1, doc._id);"
			+ "      }else if(doc.clearingState == 'CLOSED'){" + "        emit(doc.state+2, doc._id);" + "      }"
			+ "    }" + "}";

	private static final String BY_RELEASE_ID_VIEW = "function(doc) {" + "  if (doc.type == 'project') {"
			+ "    for(var i in doc.releaseIdToUsage) {" + "      emit(i, doc._id);" + "    }" + "  }" + "}";

	private static final String FULL_BY_RELEASE_ID_VIEW = "function(doc) {" + "  if (doc.type == 'project') {"
			+ "    for(var i in doc.releaseIdToUsage) {" + "      emit(i, doc._id);" + "    }" + "  }" + "}";

	private static final String BY_LINKING_PROJECT_ID_VIEW = "function(doc) {" + "  if (doc.type == 'project') {"
			+ "    for(var i in doc.linkedProjects) {" + "      emit(i, doc._id);" + "    }" + "  }" + "}";

	private static final String BY_EXTERNAL_IDS = "function(doc) {" + "  if (doc.type == 'project') {"
			+ "    for (var externalId in doc.externalIds) {" + "      try {"
			+ "            var values = JSON.parse(doc.externalIds[externalId]);" + "            if(!isNaN(values)) {"
			+ "               emit( [externalId, doc.externalIds[externalId]], doc._id);" + "               continue;"
			+ "            }" + "            for (var idx in values) {"
			+ "              emit( [externalId, values[idx]], doc._id);" + "            }" + "      } catch(error) {"
	...
	public Map<PaginationData, List<Project>> getAccessibleProjectsSummary(User user, PaginationData pageData) {
		...
		try {
			if (queryView != null) {
				...
				projects = projects.stream().skip(pageData.getDisplayStart()).limit(rowsPerPage)
						.collect(Collectors.toList());
			} else {
	...
	public Set<Project> getAccessibleProjects(User user, VendorRepository vendorRepository) {
		/**
		 * This implementation requires multiple DB requests and has its logic
		...
		/**
		 * I have only one day left in the project so I try this, but if there is time
		 * this should be reviewed The ideal solution would be to make a smarter query
		 * with a combined key, but I do not know how easy this is to refactor if say an
		 * enum value gets renamed...
		 **/
