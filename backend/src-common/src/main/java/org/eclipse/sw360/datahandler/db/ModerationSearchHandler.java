...
public class ModerationSearchHandler {
	private static final LuceneSearchView luceneSearchView = new LuceneSearchView("lucene", "moderations",
			"function(doc) {" + "    var ret = new Document();" + "    if(!doc.type) return ret;"
					+ "    if(doc.type != 'moderation') return ret;" + "	 if(!doc.documentId) return ret;"
					+ "    function idx(obj) {" + "        for (var key in obj) {"
					+ "            switch (typeof obj[key]) {" + "                case 'object':"
					+ "                    idx(obj[key]);" + "                    break;"
					+ "                case 'function':" + "                    break;" + "                default:"
					+ "                    ret.add(obj[key]);" + "                    break;" + "            }"
					+ "        }" + "    };" + "    idx(doc);" + "    for(var i in doc.moderators) {  "
					...
					+ "    if(doc.timestamp) {  " + "      var dt = new Date(doc.timestamp); "
					+ "      var formattedDt = dt.getFullYear()+'-'+(dt.getMonth()+1)+'-'+dt.getDate(); "
					+ "      ret.add(formattedDt, {\"field\": \"timestamp\", \"type\": \"date\"} );" + "    }"
					+ "    return ret;" + "}");
	private final LuceneAwareDatabaseConnector connector;

	public ModerationSearchHandler(Supplier<HttpClient> httpClient, Supplier<CloudantClient> cClient, String dbName)
			throws IOException {
	...
	public List<ModerationRequest> search(String text, final Map<String, Set<String>> subQueryRestrictions) {
		return connector.searchViewWithRestrictions(ModerationRequest.class, luceneSearchView, text,
