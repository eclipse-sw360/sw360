...
public class ChangeLogsDatabaseHandler {
	...
	private static final ImmutableSet<String> setOfIgnoredFieldValues = ImmutableSet.<String>builder().add("\"\"")
			.add("[]").add("{}").build();
	private static final ImmutableSet<String> setOfIgnoredFieldNames = ImmutableSet.<String>builder().add("revision")
	...
	public List<ChangeLogs> getChangeLogsByDocumentId(User user, String docId) {
		...
		changeLogsByDocId = changeLogsByDocId.stream().filter(Objects::nonNull)
				.filter(changeLog -> isNotEmptyChangeLog(changeLog)).collect(Collectors.toList());
