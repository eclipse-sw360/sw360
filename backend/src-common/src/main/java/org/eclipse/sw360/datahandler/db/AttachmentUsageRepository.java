...
public class AttachmentUsageRepository extends DatabaseRepositoryCloudantClient<AttachmentUsage> {
	...
	private static final String REFERENCES_RELEASEID = "" + "function(doc) { "
	...
	public List<AttachmentUsage> getUsageForAttachment(String ownerId, String attachmentContentId) {
		...
		UnpaginatedRequestBuilder reqBuilder = viewQuery.newRequest(Key.Type.COMPLEX, Object.class)
				.keys(Key.complex(new String[]{ownerId, attachmentContentId})).includeDocs(true).reduce(false);
	...
	public List<AttachmentUsage> getUsedAttachments(String usedById) {
		...
		UnpaginatedRequestBuilder reqBuilder = viewQuery.newRequest(Key.Type.STRING, Object.class).includeDocs(true)
				.reduce(false).keys(usedById);
		return queryView(reqBuilder);
	}
	...
	public List<AttachmentUsage> getUsageForAttachment(String ownerId, String attachmentContentId, String filter) {
		...
		UnpaginatedRequestBuilder reqBuilder = viewQuery.newRequest(Key.Type.COMPLEX, Object.class).includeDocs(true)
				.reduce(false).keys(Key.complex(new String[]{ownerId, attachmentContentId, filter}));
		return queryView(reqBuilder);
	}
	...
	public List<AttachmentUsage> getUsedAttachments(String usedById, String filter) {
		...
		UnpaginatedRequestBuilder reqBuilder = viewQuery.newRequest(Key.Type.COMPLEX, Object.class).includeDocs(true)
				.reduce(false).keys(Key.complex(new String[]{usedById, filter}));
		return queryView(reqBuilder);
	}
	...
	public Map<Map<String, String>, Integer> getAttachmentUsageCount(Map<String, Set<String>> attachments,
			String filter) {
		...
		UnpaginatedRequestBuilder<com.cloudant.client.api.views.Key.ComplexKey, Object> reqBuilder = viewQuery
				.newRequest(Key.Type.COMPLEX, Object.class).reduce(true).group(true).keys(compexKeys);
	...
	public List<AttachmentUsage> getUsageForAttachments(Map<String, Set<String>> attachments, String filter) {
		...
		MultipleRequestBuilder<com.cloudant.client.api.views.Key.ComplexKey, Object> reqBuilder = viewQuery
				.newMultipleRequest(Key.Type.COMPLEX, Object.class).includeDocs(true).reduce(false).keys(compexKeys);
