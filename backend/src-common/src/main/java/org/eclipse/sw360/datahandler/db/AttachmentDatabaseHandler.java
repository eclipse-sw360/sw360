...
public class AttachmentDatabaseHandler {
	...
	public AttachmentDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName, String attachmentDbName)
	...
	public void deleteUsagesBy(Source usedBy) throws SW360Exception {
		List<AttachmentUsage> existingUsages = attachmentUsageRepository
				.getUsedAttachments(usedBy.getFieldValue().toString());
	...
	public void deleteUsagesBy(Source usedBy, Set<Source> owners) throws SW360Exception {
		...
		List<AttachmentUsage> usagesToDelete = existingUsages.stream()
				.filter(usage -> owners.contains(usage.getOwner())).collect(Collectors.toList());
	...
	public void deleteAttachmentUsagesByUsageDataTypes(Source usedBy, Set<UsageData._Fields> typesToReplace,
			boolean deleteWithEmptyType) throws TException {
		List<AttachmentUsage> existingUsages = attachmentUsageRepository
				.getUsedAttachments(usedBy.getFieldValue().toString());
	...
	protected List<AttachmentUsage> distinctAttachmentUsages(List<AttachmentUsage> attachmentUsages) {
		return attachmentUsages.stream()
				.filter(CommonUtils.distinctByKey(au -> ImmutableList.of(au.getOwner(), au.getUsedBy(),
						au.getAttachmentContentId(), au.isSetUsageData() ? au.getUsageData().getSetField() : "",
	...
	public void deleteAttachmentUsages(List<AttachmentUsage> attachmentUsages) throws SW360Exception {
		List<Response> results = attachmentUsageRepository
				.deleteIds(attachmentUsages.stream().map(AttachmentUsage::getId).collect(Collectors.toList()));
	...
	public List<AttachmentUsage> getAttachmentUsages(Source owner, String attachmentContentId, UsageData filter) {
		if (filter != null && filter.isSet()) {
			return attachmentUsageRepository.getUsageForAttachment(owner.getFieldValue().toString(),
					attachmentContentId, filter.getSetField().toString());
		} else {
			return attachmentUsageRepository.getUsageForAttachment(owner.getFieldValue().toString(),
					attachmentContentId);
		}
	...
	public List<AttachmentUsage> getAttachmentUsages(Source owner, Set<String> attachmentContentIds, UsageData filter) {
		Map<String, Set<String>> attContentIdsByOwnerId = ImmutableMap.of(owner.getFieldValue().toString(),
				attachmentContentIds);

		if (filter != null && filter.isSet()) {
			return attachmentUsageRepository.getUsageForAttachments(attContentIdsByOwnerId,
	...
	public Map<Map<Source, String>, Integer> getAttachmentUsageCount(Map<Source, Set<String>> attachments,
			UsageData filter) {
		Map<String, Source._Fields> idToType = Maps.newHashMap();
		Map<String, Set<String>> queryFor = attachments.entrySet().stream().collect(Collectors.toMap(entry -> {
