...
public class ComponentDatabaseHandler extends AttachmentAwareDatabaseHandler {
	...
	public static final List<EccInformation._Fields> ECC_FIELDS = Arrays.asList(EccInformation._Fields.ECC_STATUS,
			EccInformation._Fields.AL, EccInformation._Fields.ECCN, EccInformation._Fields.MATERIAL_INDEX_NUMBER,
			EccInformation._Fields.ECC_COMMENT);
	...
	public ComponentDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName, String attachmentDbName,
			ComponentModerator moderator, ReleaseModerator releaseModerator, ProjectModerator projectModerator)
	...
	public ComponentDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName, String changeLogsDbName,
			String attachmentDbName, ComponentModerator moderator, ReleaseModerator releaseModerator,
	...
	public ComponentDatabaseHandler(Supplier<CloudantClient> supplier, String dbName, String attachmentDbName)
			throws MalformedURLException {
		this(supplier, dbName, attachmentDbName, new ComponentModerator(), new ReleaseModerator(),
				new ProjectModerator());
	}
	public ComponentDatabaseHandler(Supplier<CloudantClient> supplier, String dbName, String changelogsDbName,
	...
	public ComponentDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName, String changeLogsDbName,
			String attachmentDbName, ThriftClients thriftClients) throws MalformedURLException {
		this(httpClient, dbName, attachmentDbName, new ComponentModerator(thriftClients),
				new ReleaseModerator(thriftClients), new ProjectModerator(thriftClients));
	}
	...
	public List<Release> getReleaseSummary() throws TException {
		...
		releases.forEach(release -> release.setCreatorDepartment(Optional.ofNullable(release.getCreatedBy())
				.map(usersByEmail::get).map(User::getDepartment).orElse(null)));
		return releases;
	}
	...
	public Component getComponent(String id, User user) throws SW360Exception {
		...
		component.setReleases(
				releaseRepository.makeSummaryWithPermissions(SummaryType.SUMMARY, component.releaseIds, user));
	...
	public AddDocumentRequestSummary addComponent(Component component, String user) throws SW360Exception {
		...
		dbHandlerUtil.addChangeLogs(component, null, user, Operation.CREATE, attachmentConnector, Lists.newArrayList(),
				null, null);
		return new AddDocumentRequestSummary().setRequestStatus(AddDocumentRequestStatus.SUCCESS)
	...
	public AddDocumentRequestSummary addRelease(Release release, User user) throws SW360Exception {
		...
		if (isDuplicate(release)) {
			final AddDocumentRequestSummary addDocumentRequestSummary = new AddDocumentRequestSummary()
					.setRequestStatus(AddDocumentRequestStatus.DUPLICATE);
			List<Release> duplicates = releaseRepository.searchByNameAndVersion(release.getName(),
					release.getVersion());
			if (duplicates.size() == 1) {
				duplicates.stream().map(Release::getId).forEach(addDocumentRequestSummary::setId);
			}
			return addDocumentRequestSummary;
		}
		...
		dbHandlerUtil.addChangeLogs(component, oldComponent, user.getEmail(), Operation.UPDATE, attachmentConnector,
				Lists.newArrayList(), release.getId(), Operation.RELEASE_CREATE);
	...
	public RequestStatus updateComponent(Component component, User user) throws SW360Exception {
		...
		if (changeWouldResultInDuplicate(actual, component)) {
			...
			component.setAttachments(
					getAllAttachmentsToKeep(toSource(actual), actual.getAttachments(), component.getAttachments()));
			...
			if (component.getComponentType() != null && !component.getComponentType().equals(actual.getComponentType())
					&& !ComponentType.OSS.equals(component.getComponentType())) {
	...
	private boolean isDependenciesExistInComponent(Component component) {
		...
		if (isValidDependentIds && component.isSetDefaultVendorId()) {
			isValidDependentIds = DatabaseHandlerUtil
					.isAllIdInSetExists(Sets.newHashSet(component.getDefaultVendorId()), vendorRepository);
		}
	...
	private boolean isDependenciesExistsInRelease(Release release) {
		...
		if (release.isSetComponentId()) {
			String componentId = release.getComponentId();
			isValidDependentIds = DatabaseHandlerUtil.isAllIdInSetExists(Sets.newHashSet(componentId),
					componentRepository);
		}
		...
		if (isValidDependentIds && release.isSetReleaseIdToRelationship()) {
			...
			isValidDependentIds = DatabaseHandlerUtil.isAllIdInSetExists(Sets.newHashSet(releaseIds),
					releaseRepository);
		}
	...
	public RequestStatus updateComponentFromAdditionsAndDeletions(Component componentAdditions,
			Component componentDeletions, User user) {

		try {
			Component component = getComponent(componentAdditions.getId(), user);
			component = moderator.updateComponentFromModerationRequest(component, componentAdditions,
					componentDeletions);
	...
	public RequestStatus mergeComponents(String mergeTargetId, String mergeSourceId, Component mergeSelection,
		...
		if (isComponentUnderModeration(mergeTargetId) || isComponentUnderModeration(mergeSourceId)) {
			return RequestStatus.IN_USE;
		}

		try {
			// First merge everything into the new compontent which is mergable in one step
			// (attachments, plain fields)
			...
			// update target first. If updating source fails, no data is lost (but
			...
			log.error("Cannot merge component [" + mergeSource.getId() + "] into [" + mergeTarget.getId()
					+ "]. Releases after merge: " + releaseIds, e);
			return RequestStatus.FAILURE;
		}
	...
	private boolean isComponentUnderModeration(String componentSourceId) throws TException {
		...
		List<ModerationRequest> sourceModerationRequests = moderationClient
				.getModerationRequestByDocumentId(componentSourceId);
	...
	private void mergePlainFields(Component mergeSelection, Component mergeTarget, Component mergeSource) {
		// First handle the creator of the component in a way, that the discarded
		// creator will be on the
		// moderator list afterwards. There is nothing to do, if source and target
		// author are the same
		if (!nullToEmpty(mergeTarget.getCreatedBy()).equals(mergeSource.getCreatedBy())) {
			if (nullToEmpty(mergeSelection.getCreatedBy()).equals(nullToEmpty(mergeTarget.getCreatedBy()))) {
				// creator of the target component should be retained. Add creator of source
				// component to list of moderators.
				...
				// creator of the source component has been selected. Add creator of target
				// component to list of moderators.
		...
		// Handle other fields
		copyFields(mergeSelection, mergeTarget,
				ImmutableSet.<Component._Fields>builder().add(Component._Fields.NAME).add(Component._Fields.CREATED_ON)
						.add(Component._Fields.CREATED_BY).add(Component._Fields.CATEGORIES)
						.add(Component._Fields.COMPONENT_TYPE).add(Component._Fields.DEFAULT_VENDOR_ID)
						.add(Component._Fields.HOMEPAGE).add(Component._Fields.BLOG).add(Component._Fields.WIKI)
						.add(Component._Fields.MAILINGLIST).add(Component._Fields.DESCRIPTION)
						.add(Component._Fields.EXTERNAL_IDS).add(Component._Fields.ADDITIONAL_DATA)
						.add(Component._Fields.COMPONENT_OWNER).add(Component._Fields.OWNER_ACCOUNTING_UNIT)
						.add(Component._Fields.OWNER_GROUP).add(Component._Fields.OWNER_COUNTRY)
						.add(Component._Fields.MODERATORS).add(Component._Fields.SUBSCRIBERS)
						.add(Component._Fields.ROLES).build());
	}
	...
	private void transferReleases(Set<String> releaseIds, Component mergeTarget, Component mergeSource)
	...
	private void updateReleasesAfterMerge(Set<String> targetComponentReleaseIds, Set<String> srcComponentReleaseIds,
		...
		List<Release> releasesToUpdate = releases.stream().filter(r -> {
			...
			dbHandlerUtil.addChangeLogs(r, releaseBefore, userEmail, Operation.UPDATE, attachmentConnector,
					Lists.newArrayList(), mergeTarget.getId(), Operation.MERGE_COMPONENT);
			return r;
	...
	private void updateComponentCompletelyWithoutDeletingAttachment(Component component, User user)
			throws SW360Exception {
		// Prepare component for database
		prepareComponent(component);
		updateModifiedFields(component, user.getEmail());
	...
	public RequestStatus updateRelease(Release release, User user, Iterable<Release._Fields> immutableFields)
			throws SW360Exception {
		removeLeadingTrailingWhitespace(release);
		String name = release.getName();
		String version = release.getVersion();
		...
		if (actual.equals(release)) {
			...
			if ((hasChangesInEccFields && permissions.isActionAllowed(RequestedAction.WRITE_ECC))
				...
				dbHandlerUtil.addChangeLogs(release, actual, user.getEmail(), Operation.UPDATE, attachmentConnector,
						referenceDocLogList, null, null);
				...
				Runnable clearingRequestRunnable = addCrCommentForAttachmentUpdatesInRelease(actual,
						CommonUtils.nullToEmptySet(release.getAttachments()), user);
	...
	private Runnable addCrCommentForAttachmentUpdatesInRelease(Release release, Set<Attachment> updatedAttachments,
			User user) {
		return () -> {
			Set<Attachment> originalAttachments = CommonUtils.nullToEmptySet(release.getAttachments());
			// collect the attachment Ids
			Set<String> originalAttachmentId = originalAttachments.stream().map(Attachment::getAttachmentContentId)
					.collect(Collectors.toSet());
			Set<String> updatedAttachmentId = updatedAttachments.stream().map(Attachment::getAttachmentContentId)
					.collect(Collectors.toSet());

			// check if attachments are updated
			if (!originalAttachmentId.equals(updatedAttachmentId)) {
				// fetch all the projects associated with this release and collect the Clearing
				// request Ids
				...
				if (crIds.size() > 0) {
					...
					StringBuilder commentText = new StringBuilder("Attachment(s) updated for the release: <b>")
							.append(SW360Utils.printFullname(release)).append("</b> (").append(release.getId())
							.append(")");
					if (CommonUtils.isNotEmpty(added)) {
						Set<String> attachmentNames = extractAttachmentNameWithType(updatedAttachments, added);
						commentText.append(System.lineSeparator()).append("Added Attachments: ")
								.append(SW360Utils.spaceJoiner.join(attachmentNames));
					}
					if (CommonUtils.isNotEmpty(removed)) {
						Set<String> attachmentNames = extractAttachmentNameWithType(originalAttachments, removed);
						commentText.append(System.lineSeparator()).append("Removed Attachments: ")
								.append(SW360Utils.spaceJoiner.join(attachmentNames));
					}
					for (String cdId : crIds) {
						Comment comment = new Comment().setText(commentText.toString()).setCommentedBy(user.getEmail())
	...
	private Set<String> extractAttachmentNameWithType(Set<Attachment> attachments, Collection<String> filterCriteria) {
		return attachments.stream().filter(att -> filterCriteria.contains(att.getAttachmentContentId()))
				.map(att -> new StringBuilder(System.lineSeparator()).append("\t").append(att.getFilename())
						.append(DatabaseHandlerUtil.SEPARATOR)
	...
	public boolean hasChangesInEccFields(Release release, Release actual) {
		...
		Function<EccInformation._Fields, Boolean> fieldChanged = f -> {
			...
			return !((changedValue == originalValue) || (changedValue != null && changedValue.equals(originalValue))
					|| ("".equals(changedValue) && originalValue == null)
					|| (changedValue == null && "".equals(originalValue)));
		};
		return ECC_FIELDS.stream().map(fieldChanged).reduce(false, Boolean::logicalOr);
	}

	public boolean hasEmptyEccFields(Release release) {
		EccInformation eccInformation = release.getEccInformation();
		return isNullEmptyOrWhitespace(eccInformation.getAl()) && isNullEmptyOrWhitespace(eccInformation.getEccn())
	...
	public RequestSummary updateReleases(Collection<Release> releases, User user, boolean allowUpdate)
			throws SW360Exception {
		...
		if (allowUpdate || PermissionUtils.isAdmin(user)) {
			...
			if (!documentOperationResults.isEmpty()) {
				...
				final List<Component> componentList = componentRepository
						.get(storedReleases.stream().map(Release::getComponentId).collect(Collectors.toSet()));
	...
	public RequestStatus updateReleaseFromAdditionsAndDeletions(Release releaseAdditions, Release releaseDeletions,
			User user) {
	...
	public void recomputeReleaseDependentFields(Component component, String skipThisReleaseId) {
		...
		for (Release containedRelease : releases) {
			if (containedRelease.getId().equals(skipThisReleaseId))
				continue;
	...
	public RequestStatus mergeReleases(String mergeTargetId, String mergeSourceId, Release mergeSelection,
		...
		if (isReleaseUnderModeration(mergeTargetId) || isReleaseUnderModeration(mergeSourceId)) {
			return RequestStatus.IN_USE;
		}
		try {
			// First merge everything into the new compontent which is mergable in one step
			...
			// it is important to migrate the attachment usages first otherwise they will be
			// delete during project update
	...
	private void mergeReleasePlainFields(Release mergeSelection, Release mergeTarget, Release mergeSource) {
		// First handle the creator of the release in a way, that the discarded creator
		// will be on the
		// moderator list afterwards. There is nothing to do, if source and target
		// author are the same
		if (!nullToEmpty(mergeTarget.getCreatedBy()).equals(mergeSource.getCreatedBy())) {
		...
		copyFields(mergeSelection, mergeTarget, ImmutableSet.<Release._Fields>builder().add(Release._Fields.VENDOR_ID)
				.add(Release._Fields.NAME).add(Release._Fields.VERSION).add(Release._Fields.LANGUAGES)
				.add(Release._Fields.OPERATING_SYSTEMS).add(Release._Fields.CPEID)
				.add(Release._Fields.SOFTWARE_PLATFORMS).add(Release._Fields.RELEASE_DATE)
				.add(Release._Fields.MAIN_LICENSE_IDS).add(Release._Fields.SOURCE_CODE_DOWNLOADURL)
				.add(Release._Fields.BINARY_DOWNLOADURL).add(Release._Fields.MAINLINE_STATE)
				.add(Release._Fields.CREATED_ON).add(Release._Fields.CREATED_BY).add(Release._Fields.CONTRIBUTORS)
				.add(Release._Fields.MODERATORS).add(Release._Fields.SUBSCRIBERS).add(Release._Fields.REPOSITORY)
				.add(Release._Fields.ROLES).add(Release._Fields.EXTERNAL_IDS).add(Release._Fields.ADDITIONAL_DATA)
				.add(Release._Fields.RELEASE_ID_TO_RELATIONSHIP).build());
		...
		copyFields(mergeSelection.getClearingInformation(), mergeTarget.getClearingInformation(),
				ImmutableSet.<ClearingInformation._Fields>builder()
						...
						.add(ClearingInformation._Fields.LICENSE_AGREEMENT).add(ClearingInformation._Fields.SCANNED)
						.add(ClearingInformation._Fields.COMPONENT_CLEARING_REPORT)
						.add(ClearingInformation._Fields.CLEARING_STANDARD)
						.add(ClearingInformation._Fields.EXTERNAL_URL).add(ClearingInformation._Fields.COMMENT)
						.add(ClearingInformation._Fields.REQUEST_ID)
						.add(ClearingInformation._Fields.ADDITIONAL_REQUEST_INFO)
						.add(ClearingInformation._Fields.PROC_START).add(ClearingInformation._Fields.EVALUATED)
						.add(ClearingInformation._Fields.EXTERNAL_SUPPLIER_ID)
						.add(ClearingInformation._Fields.COUNT_OF_SECURITY_VN).build());

		// Handle ECC information
		copyFields(mergeSelection.getEccInformation(), mergeTarget.getEccInformation(),
				ImmutableSet.<EccInformation._Fields>builder().add(EccInformation._Fields.ECC_STATUS)
						.add(EccInformation._Fields.ECC_COMMENT).add(EccInformation._Fields.AL)
						.add(EccInformation._Fields.ECCN).add(EccInformation._Fields.MATERIAL_INDEX_NUMBER)
						.add(EccInformation._Fields.ASSESSOR_CONTACT_PERSON)
						.add(EccInformation._Fields.ASSESSOR_DEPARTMENT).add(EccInformation._Fields.ASSESSMENT_DATE)
						.build());

		// Handle COTS information
		copyFields(mergeSelection.getCotsDetails(), mergeTarget.getCotsDetails(),
				ImmutableSet.<COTSDetails._Fields>builder().add(COTSDetails._Fields.USAGE_RIGHT_AVAILABLE)
						.add(COTSDetails._Fields.COTS_RESPONSIBLE).add(COTSDetails._Fields.CLEARING_DEADLINE)
						.add(COTSDetails._Fields.LICENSE_CLEARING_REPORT_URL).add(COTSDetails._Fields.USED_LICENSE)
						.add(COTSDetails._Fields.CONTAINS_OSS).add(COTSDetails._Fields.OSS_CONTRACT_SIGNED)
						.add(COTSDetails._Fields.OSS_INFORMATION_URL).add(COTSDetails._Fields.SOURCE_CODE_AVAILABLE)
						.build());
	}
	...
	/**
	 * The {{@link #updateRelease(Component, User, Iterable)} does not change the
	 * given release completely according to the user request. As we want to have
	 * exactly the given release as a result, this method is really submitting the
	 * given data to the persistence.
	 */
	private void updateReleaseCompletely(Release release, User user, boolean updateClearingState, boolean cleanup,
			boolean sendmail) throws SW360Exception {
		// Prepare component for database
		prepareRelease(release);
	...
	private void updateReleaseReferencesInProjects(String mergeTargetId, String mergeSourceId, User sessionUser)
			throws TException {
		...
		for (Project project : projects) {
			...
			dbHandlerUtil.addChangeLogs(project, projectBefore, userEmail, Operation.UPDATE, attachmentConnector,
					Lists.newArrayList(), mergeTargetId, Operation.MERGE_RELEASE);
		}
	}

	private void updateReleaseReferencesInAttachmentUsages(String mergeTargetId, String mergeSourceId)
			throws TException {
	...
	private void updateReleaseReferencesInReleases(String mergeTargetId, String mergeSourceId, User sessionUser)
			throws SW360Exception {
	...
	private void updateReleaseReferencesInVulnerabilities(String mergeTargetId, String mergeSourceId, User sessionUser)
			throws TException {
		VulnerabilityService.Iface vulnerabilityService = new ThriftClients().makeVulnerabilityClient();

		List<ReleaseVulnerabilityRelation> relations = vulnerabilityService
				.getReleaseVulnerabilityRelationsByReleaseId(mergeSourceId, sessionUser);
	...
	private void updateReleaseReferencesInProjectRatings(String mergeTargetId, String mergeSourceId, User sessionUser)
			throws TException {
		VulnerabilityService.Iface vulnerabilityService = new ThriftClients().makeVulnerabilityClient();

		List<ProjectVulnerabilityRating> ratings = vulnerabilityService
				.getProjectVulnerabilityRatingsByReleaseId(mergeSourceId, sessionUser);
		for (ProjectVulnerabilityRating rating : ratings) {
			ProjectVulnerabilityRating ratingBefore = rating.deepCopy();
			for (Map<String, List<VulnerabilityCheckStatus>> map : rating.getVulnerabilityIdToReleaseIdToStatus()
					.values()) {
	...
	private void updateParentComponent(Release release, User sessionUser) throws SW360Exception {
		...
		Set<String> releaseIds = nullToEmptyList(component.getReleases()).stream().map(Release::getId)
				.collect(Collectors.toSet());
	...
	public boolean isReleaseActionAllowed(Release release, User user, RequestedAction action) {
		...
		switch (action) {
			case READ :
				...
				isAllowed = isComponentAccessible
	...
	@NotNull
	private List<ReleaseLink> iterateReleaseRelationShips(Map<String, ?> relations, String parentNodeId,
			Deque<String> visitedIds) {
		List<ReleaseLink> out = new ArrayList<>();

		for (Map.Entry<String, ?> entry : relations.entrySet()) {
			String id = entry.getKey();
			Optional<ReleaseLink> releaseLinkOptional = getFilledReleaseLink(id, entry.getValue(), parentNodeId,
					visitedIds);
	...
	private Optional<ReleaseLink> getFilledReleaseLink(String id, Object relation, String parentNodeId,
			Deque<String> visitedIds) {
		...
		if (!visitedIds.contains(id)) {
			...
			if (release != null) {
				...
				log.error("Broken ReleaseLink in release with id: " + parentNodeId + ". Linked release with id " + id
						+ " was not in the release cache");
			}
			visitedIds.pop();
		}
	...
	private void fillValueFieldInReleaseLink(ReleaseLink releaseLink, Object relation) {
		if (relation instanceof ProjectReleaseRelationship) {
			...
			throw new IllegalArgumentException(
					"Only ProjectReleaseRelationship or ReleaseRelationship is allowed as ReleaseLink's relation value");
		}
	}
	...
	private ReleaseLink createReleaseLink(Release release) {
		...
		ReleaseLink releaseLink = new ReleaseLink(release.id, vendorName, release.name, release.version,
		...
		releaseLink.setClearingState(release.getClearingState())
				.setComponentType(Optional.ofNullable(componentRepository.get(release.getComponentId()))
						.map(Component::getComponentType).orElse(null));
	...
	public List<Release> searchAccessibleReleasesByText(ReleaseSearchHandler searchHandler, String searchText,
			User user) {
	...
	/**
	 * Returns full documents straight from repository. Don't want this to get
	 * abused, that's why it's package-private. Used for bulk-computing
	 * ReleaseClearingStateSummaries by ProjectDatabaseHandler. The reason for this
	 * hack is that making summaries (like in getReleases()) takes way too long for
	 * a lot of releases.
	 */
	...
	public Component getComponentForEdit(String id, User user) throws SW360Exception {
		...
		if (moderationRequestsForDocumentId.isEmpty()) {
			...
			if (moderationRequestOptional.isPresent() && isInProgressOrPending(moderationRequestOptional.get())) {
				...
				component = moderator.updateComponentFromModerationRequest(component,
						moderationRequest.getComponentAdditions(), moderationRequest.getComponentDeletions());
	...
	public Release getReleaseForEdit(String id, User user) throws SW360Exception {
		...
		if (moderationRequestsForDocumentId.isEmpty()) {
			...
			if (moderationRequestOptional.isPresent() && isInProgressOrPending(moderationRequestOptional.get())) {
				...
				release = releaseModerator.updateReleaseFromModerationRequest(release,
						moderationRequest.getReleaseAdditions(), moderationRequest.getReleaseDeletions());
				documentState = CommonUtils.getModeratedDocumentState(moderationRequest);
			} else {
				documentState = new DocumentState().setIsOriginalDocument(true)
						.setModerationState(moderationRequestsForDocumentId.get(0).getModerationState());
	...
	public Set<Attachment> getSourceAttachments(String releaseId) throws SW360Exception {
		...
		return nullToEmptySet(release.getAttachments()).stream().filter(Objects::nonNull)
				.filter(input -> input.getAttachmentType() == AttachmentType.SOURCE).collect(Collectors.toSet());
	}
	...
	public RequestStatus splitComponent(Component srcComponent, Component targetComponent, User user)
		...
		try {
			Set<String> srcComponentReleaseIdsAfter = nullToEmptyList(srcComponent.getReleases()).stream()
					.map(Release::getId).collect(Collectors.toSet());
			Set<String> targetComponentReleaseIdsAfter = nullToEmptyList(targetComponent.getReleases()).stream()
					.map(Release::getId).collect(Collectors.toSet());

			Set<String> targetComponentReleaseIdsBefore = nullToEmptyList(targetComponentFromDB.getReleases()).stream()
					.map(Release::getId).collect(Collectors.toSet());
			Set<String> srcComponentReleaseIdsBefore = nullToEmptyList(srcComponentFromDB.getReleases()).stream()
					.map(Release::getId).collect(Collectors.toSet());
			...
			long noOfReleasesNotAllowedToUpdate = getNoOfReleasesNotAllowedToUpdate(srcComponentReleaseIdsMovedFromSrc,
					user);
	...
	private void sendMailNotificationsForNewComponent(Component component, String user) {
		mailUtil.sendMail(component.getComponentOwner(), MailConstants.SUBJECT_FOR_NEW_COMPONENT,
				MailConstants.TEXT_FOR_NEW_COMPONENT, SW360Constants.NOTIFICATION_CLASS_COMPONENT,
				Component._Fields.COMPONENT_OWNER.toString(), component.getName());
		mailUtil.sendMail(component.getModerators(), user, MailConstants.SUBJECT_FOR_NEW_COMPONENT,
				MailConstants.TEXT_FOR_NEW_COMPONENT, SW360Constants.NOTIFICATION_CLASS_COMPONENT,
				Component._Fields.MODERATORS.toString(), component.getName());
		mailUtil.sendMail(component.getSubscribers(), user, MailConstants.SUBJECT_FOR_NEW_COMPONENT,
				MailConstants.TEXT_FOR_NEW_COMPONENT, SW360Constants.NOTIFICATION_CLASS_COMPONENT,
				Component._Fields.SUBSCRIBERS.toString(), component.getName());
		mailUtil.sendMail(SW360Utils.unionValues(component.getRoles()), user, MailConstants.SUBJECT_FOR_NEW_COMPONENT,
				MailConstants.TEXT_FOR_NEW_COMPONENT, SW360Constants.NOTIFICATION_CLASS_COMPONENT,
				Component._Fields.ROLES.toString(), component.getName());
	}

	private void sendMailNotificationsForComponentUpdate(Component component, String user) {
		mailUtil.sendMail(component.getCreatedBy(), MailConstants.SUBJECT_FOR_UPDATE_COMPONENT,
				MailConstants.TEXT_FOR_UPDATE_COMPONENT, SW360Constants.NOTIFICATION_CLASS_COMPONENT,
				Component._Fields.CREATED_BY.toString(), component.getName());
		mailUtil.sendMail(component.getComponentOwner(), MailConstants.SUBJECT_FOR_UPDATE_COMPONENT,
				MailConstants.TEXT_FOR_UPDATE_COMPONENT, SW360Constants.NOTIFICATION_CLASS_COMPONENT,
				Component._Fields.COMPONENT_OWNER.toString(), component.getName());
		mailUtil.sendMail(component.getModerators(), user, MailConstants.SUBJECT_FOR_UPDATE_COMPONENT,
				MailConstants.TEXT_FOR_UPDATE_COMPONENT, SW360Constants.NOTIFICATION_CLASS_COMPONENT,
				Component._Fields.MODERATORS.toString(), component.getName());
		mailUtil.sendMail(component.getSubscribers(), user, MailConstants.SUBJECT_FOR_UPDATE_COMPONENT,
				MailConstants.TEXT_FOR_UPDATE_COMPONENT, SW360Constants.NOTIFICATION_CLASS_COMPONENT,
				Component._Fields.SUBSCRIBERS.toString(), component.getName());
		mailUtil.sendMail(SW360Utils.unionValues(component.getRoles()), user,
				MailConstants.SUBJECT_FOR_UPDATE_COMPONENT, MailConstants.TEXT_FOR_UPDATE_COMPONENT,
				SW360Constants.NOTIFICATION_CLASS_COMPONENT, Component._Fields.ROLES.toString(), component.getName());
	}

	private void sendMailNotificationsForNewRelease(Release release, String user) {
		mailUtil.sendMail(release.getContributors(), user, MailConstants.SUBJECT_FOR_NEW_RELEASE,
				MailConstants.TEXT_FOR_NEW_RELEASE, SW360Constants.NOTIFICATION_CLASS_RELEASE,
				Release._Fields.CONTRIBUTORS.toString(), release.getName(), release.getVersion());
		mailUtil.sendMail(release.getModerators(), user, MailConstants.SUBJECT_FOR_NEW_RELEASE,
				MailConstants.TEXT_FOR_NEW_RELEASE, SW360Constants.NOTIFICATION_CLASS_RELEASE,
				Release._Fields.MODERATORS.toString(), release.getName(), release.getVersion());
		mailUtil.sendMail(release.getSubscribers(), user, MailConstants.SUBJECT_FOR_NEW_RELEASE,
				MailConstants.TEXT_FOR_NEW_RELEASE, SW360Constants.NOTIFICATION_CLASS_RELEASE,
				Release._Fields.SUBSCRIBERS.toString(), release.getName(), release.getVersion());
		mailUtil.sendMail(SW360Utils.unionValues(release.getRoles()), user, MailConstants.SUBJECT_FOR_NEW_RELEASE,
				MailConstants.TEXT_FOR_NEW_RELEASE, SW360Constants.NOTIFICATION_CLASS_RELEASE,
				Release._Fields.SUBSCRIBERS.toString(), release.getName(), release.getVersion());
	}

	private void sendMailNotificationsForReleaseUpdate(Release release, String user) {
		mailUtil.sendMail(release.getCreatedBy(), MailConstants.SUBJECT_FOR_UPDATE_RELEASE,
				MailConstants.TEXT_FOR_UPDATE_RELEASE, SW360Constants.NOTIFICATION_CLASS_RELEASE,
				Release._Fields.CONTRIBUTORS.toString(), release.getName(), release.getVersion());
		mailUtil.sendMail(release.getContributors(), user, MailConstants.SUBJECT_FOR_UPDATE_RELEASE,
				MailConstants.TEXT_FOR_UPDATE_RELEASE, SW360Constants.NOTIFICATION_CLASS_RELEASE,
				Release._Fields.CONTRIBUTORS.toString(), release.getName(), release.getVersion());
		mailUtil.sendMail(release.getModerators(), user, MailConstants.SUBJECT_FOR_UPDATE_RELEASE,
				MailConstants.TEXT_FOR_UPDATE_RELEASE, SW360Constants.NOTIFICATION_CLASS_RELEASE,
				Release._Fields.MODERATORS.toString(), release.getName(), release.getVersion());
		mailUtil.sendMail(release.getSubscribers(), user, MailConstants.SUBJECT_FOR_UPDATE_RELEASE,
				MailConstants.TEXT_FOR_UPDATE_RELEASE, SW360Constants.NOTIFICATION_CLASS_RELEASE,
				Release._Fields.SUBSCRIBERS.toString(), release.getName(), release.getVersion());
		mailUtil.sendMail(SW360Utils.unionValues(release.getRoles()), user, MailConstants.SUBJECT_FOR_UPDATE_RELEASE,
				MailConstants.TEXT_FOR_UPDATE_RELEASE, SW360Constants.NOTIFICATION_CLASS_RELEASE,
				Release._Fields.SUBSCRIBERS.toString(), release.getName(), release.getVersion());
	}
	...
	public ImportBomRequestPreparation prepareImportBom(User user, String attachmentContentId) throws SW360Exception {
		...
		try {
			...
			try (final InputStream inputStream = attachmentStreamConnector
				...
				ImportBomRequestPreparation importBomRequestPreparation = spdxBOMImporter
						.prepareImportSpdxBOMAsRelease(sourceFile);
	...
	private void updateReleaseAfterComponentSplit(Component srcComponentFromDB, Component targetComponentFromDB,
			Set<String> srcComponentReleaseIdsMovedFromSrc, Set<String> targetComponentReleaseIdsBefore, User user)
			throws SW360Exception {
		List<Release> targetComponentReleases = getReleasesForClearingStateSummary(targetComponentReleaseIdsBefore);
		List<Release> srcComponentReleasesMoved = getReleasesForClearingStateSummary(
				srcComponentReleaseIdsMovedFromSrc);
		Set<String> targetComponentReleaseVersions = targetComponentReleases.stream().map(Release::getVersion)
				.collect(Collectors.toSet());
	...
	private void checkSuperAttachmentExists(Release release) {
		if (CommonUtils.isNotEmpty(release.getAttachments())) {
			Set<String> attachmentContentIds = release.getAttachments().stream().map(Attachment::getAttachmentContentId)
					.collect(Collectors.toSet());
