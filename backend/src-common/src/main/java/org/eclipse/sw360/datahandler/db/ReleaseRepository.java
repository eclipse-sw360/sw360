...
public class ReleaseRepository extends SummaryAwareRepository<Release> {
	...
	private static final String SUBSCRIBERS = "function(doc) {" + " if (doc.type == 'release'){"
			+ "    for(var i in doc.subscribers) {" + "      emit(doc.subscribers[i], doc._id);" + "    }" + "  }"
			+ "}";
	private static final String USEDINRELEASERELATION = "function(doc) {" + " if(doc.type == 'release') {"
			+ "   for(var id in doc.releaseIdToRelationship) {" + "     emit(id, doc._id);" + "   }" + " }" + "}";
	private static final String RELEASEBYVENDORID = "function(doc) {" + " if (doc.type == 'release'){"
			+ "     emit(doc.vendorId, doc._id);" + "  }" + "}";
	private static final String RELEASESBYCOMPONENTID = "function(doc) {" + " if (doc.type == 'release'){"
			+ "      emit(doc.componentId, doc._id);" + "  }" + "}";
	private static final String RELEASEIDSBYLICENSEID = "function(doc) {" + "  if (doc.type == 'release'){"
			+ "    for(var i in doc.mainLicenseIds) {" + "      emit(doc.mainLicenseIds[i], doc._id);" + "    }" + "  }"
			+ "}";
	private static final String BYEXTERNALIDS = "function(doc) {" + "  if (doc.type == 'release') {"
	...
	public List<Release> searchByNameAndVersion(String name, String version) {
		List<Release> releasesMatchingName = new ArrayList<Release>(
				getFullDocsById(queryForIdsAsValue("byname", name)));
	...
	public List<Release> getRecentReleases() {
		...
		UnpaginatedRequestBuilder reqBuilder = query.newRequest(Key.Type.STRING, Object.class).limit(5).descending(true)
				.includeDocs(false);
