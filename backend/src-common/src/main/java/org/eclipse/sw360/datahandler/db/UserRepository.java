...
public class UserRepository extends SummaryAwareRepository<User> {
	...
	private static final String BYAPITOKEN = "function(doc) { if (doc.type == 'user') "
			+ "  for (var i in doc.restApiTokens) {" + "    emit(doc.restApiTokens[i].token, doc._id)" + "  }" + "}";
	private static final String BYOIDCCLIENTID = "function(doc) { if (doc.type == 'user') "
			+ "  for (var i in doc.oidcClientInfos) {" + "    emit(i, doc._id)" + "  }" + "}";
	private static final String BYEMAIL = "function(doc) { " + "  if (doc.type == 'user') {"
			+ "    emit(doc.email, doc._id); "
			+ "    if (doc.formerEmailAddresses && Array.isArray(doc.formerEmailAddresses)) {"
			+ "      var arr = doc.formerEmailAddresses;" + "      for (var i = 0; i < arr.length; i++){"
			+ "        emit(arr[i], doc._id);" + "      }" + "    }" + "  }" + "}";

	private static final String USERS_ALL_DEPARTMENT_VIEW = "function(doc) { " + "  if (doc.type == 'user') {"
			+ "    emit(doc.department, null);" + "  }" + "}";

	private static final String USERS_ALL_EMAIL_VIEW = "function(doc) { " + "  if (doc.type == 'user') {"
			+ "    emit(doc.email, null);" + "  }" + "}";
	...
	public User getByExternalId(String externalId) {
		if (externalId == null || "".equals(externalId)) {
			// liferay contains the setup user with externalId=="" and we do not want to
			// match him or any other one with empty externalID
			return null;
		}
	...
	private Map<PaginationData, List<User>> queryViewWithPagination(PaginationData pageData) {
		...
		switch (sortColumnNo) {
			...
			case 6 :
				...
				qb = ascending
						? qb.sort(Sort.asc("secondaryDepartmentsAndRoles"))
