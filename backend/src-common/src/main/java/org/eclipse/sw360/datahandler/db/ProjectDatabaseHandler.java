...
public class ProjectDatabaseHandler extends AttachmentAwareDatabaseHandler {
	...
	public ProjectDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName, String attachmentDbName)
	...
	public ProjectDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName, String changeLogDbName,
	...
	@VisibleForTesting
	public ProjectDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName, String changeLogsDbName,
			String attachmentDbName, ProjectModerator moderator, ComponentDatabaseHandler componentDatabaseHandler,
	...
	@VisibleForTesting
	public ProjectDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName, String attachmentDbName,
			ProjectModerator moderator, ComponentDatabaseHandler componentDatabaseHandler,
			AttachmentDatabaseHandler attachmentDatabaseHandler) throws MalformedURLException {
	...
	public AddDocumentRequestSummary createClearingRequest(ClearingRequest clearingRequest, User user,
			String projectUrl) throws SW360Exception {
		...
		if (!(ProjectClearingState.CLOSED.equals(project.getClearingState())
				|| Visibility.PRIVATE.equals(project.getVisbility()))) {
			...
			if (CommonUtils.isNotNullEmptyOrWhitespace(crId)) {
				...
				return requestSummary.setRequestStatus(AddDocumentRequestStatus.SUCCESS)
						.setId(project.getClearingRequestId());
			} else {
	...
	public RequestStatus sendEmailForClearingRequestUpdate(ClearingRequest clearingRequest, String projectUrl,
	...
	public Project getProjectById(String id, User user) throws SW360Exception {
		...
		if (!makePermission(project, user).isActionAllowed(RequestedAction.READ)) {
			throw fail(403, "User: %s is not allowed to view the requested project: %s", user.getEmail(),
					project.getId());
		}
	...
	public AddDocumentRequestSummary addProject(Project project, User user) throws SW360Exception {
		...
		if (isDuplicate(project)) {
			...
			if (duplicates.size() == 1) {
				duplicates.stream().map(Project::getId).forEach(addDocumentRequestSummary::setId);
			}
		...
		dbHandlerUtil.addChangeLogs(project, null, user.getEmail(), Operation.CREATE, null, Lists.newArrayList(), null,
				null);
		sendMailNotificationsForNewProject(project, user.getEmail());
		return new AddDocumentRequestSummary().setId(project.getId())
	...
	public RequestStatus updateProject(Project project, User user) throws SW360Exception {
		...
		if (changeWouldResultInDuplicate(actual, project)) {
			...
			project.setAttachments(
					getAllAttachmentsToKeep(toSource(actual), actual.getAttachments(), project.getAttachments()));
	...
	private boolean isDependenciesExists(Project project, User user) {
		...
		if (isValidDependentIds && project.isSetLinkedProjects()) {
			...
			isValidDependentIds = DatabaseHandlerUtil.isAllIdInSetExists(projectIds, repository)
					&& verifyLinkedProjectsAreAccessible(projectIds, user);
		}

		if (isValidDependentIds && project.isSetLinkedObligationId()) {
			String obligationId = project.getLinkedObligationId();
			isValidDependentIds = DatabaseHandlerUtil.isAllIdInSetExists(Sets.newHashSet(obligationId),
	...
	private void addCommentToClearingRequest(Project updated, Project current, User user) {
		...
		Collection<Release> releases = CommonUtils
				.nullToEmptyCollection(componentDatabaseHandler.getReleasesForClearingStateSummary(allReleaseIds));
		...
		if (CommonUtils.isNotEmpty(added)) {
			...
			commentText.append(System.lineSeparator()).append("Added Release Ids: ")
					.append(SW360Utils.spaceJoiner.join(releaseNames));
		}
		if (CommonUtils.isNotEmpty(removed)) {
			Set<String> releaseNames = extractReleaseNameWithId(releases, removed);
			commentText.append(System.lineSeparator()).append("Removed Release Ids: ")
					.append(SW360Utils.spaceJoiner.join(releaseNames));
		}
	...
	private Set<String> extractReleaseNameWithId(Collection<Release> releases, Collection<String> filterCriteria) {
		return releases.stream().filter(rel -> filterCriteria.contains(rel.getId()))
				.map(rel -> new StringBuilder(System.lineSeparator()).append("\t").append(SW360Utils.printFullname(rel))
						.append(" (").append(rel.getId()).append(")").toString())
	...
	private ObligationList deleteObligationsOfUnlinkedReleases(Project updated) {
		...
		for (Iterator<Map.Entry<String, ObligationStatusInfo>> it = updatedOsInfoMap.entrySet().iterator(); it
				.hasNext();) {
	...
	private void updateProjectDependentLinkedFields(Project updated, Project actual) throws SW360Exception {
		...
		// update the obligations only if linked obligations were present in current
		// project,
		// and there is change in linked releases in updated project
		if (CommonUtils.isNotNullEmptyOrWhitespace(actual.getLinkedObligationId())
				&& !actualLinkedReleaseIds.equals(updatedLinkedReleaseIds)) {
	...
	public RequestStatus updateProjectFromAdditionsAndDeletions(Project projectAdditions, Project projectDeletions,
			User user) {
	...
	public List<ProjectLink> getLinkedProjects(Project project, boolean deep, User user) {
		...
		fakeRelations.put(project.isSetId() ? project.getId() : DUMMY_NEW_PROJECT_ID,
				new ProjectProjectRelationship(ProjectRelationship.UNKNOWN));
	...
	private Optional<ProjectLink> createProjectLink(String id, ProjectProjectRelationship projectProjectRelationship,
			String parentNodeId, Deque<String> visitedIds, int maxDepth, User user) {
		...
		if (!visitedIds.contains(id) && (maxDepth < 0 || visitedIds.size() < maxDepth)) {
			...
			if (project != null) {
				...
				if (project.isSetReleaseIdToUsage() && (maxDepth < 0 || visitedIds.size() < maxDepth)) { // ProjectLink
																											// on the
																											// last
																											// level
																											// does not
																											...
																											// children
																											// added
					List<ReleaseLink> linkedReleases = componentDatabaseHandler
							.getLinkedReleasesWithAccessibility(project, visitedIds, user);
				...
				projectLink.setNodeId(generateNodeId(id)).setParentNodeId(parentNodeId)
						.setRelation(projectProjectRelationship.getProjectRelationship())
						.setEnableSvm(projectProjectRelationship.isEnableSvm()).setVersion(project.getVersion())
						.setState(project.getState()).setProjectType(project.getProjectType())
						.setClearingState(project.getClearingState()).setTreeLevel(visitedIds.size() - 1);
				...
				log.error("Broken ProjectLink in project with id: " + parentNodeId + ". Linked project with id " + id
						+ " was not found");
			}
			visitedIds.pop();
		}
	...
	private void fillMainlineStates(List<ReleaseLink> linkedReleases,
	...
	public Project getProjectForEdit(String id, User user) throws SW360Exception {
		...
		if (moderationRequestsForDocumentId.isEmpty()) {
			...
			if (moderationRequestOptional.isPresent() && isInProgressOrPending(moderationRequestOptional.get())) {
				...
				project = moderator.updateProjectFromModerationRequest(project, moderationRequest.getProjectAdditions(),
	...
	public List<Project> fillClearingStateSummary(List<Project> projects, User user) {
		Function<Project, Set<String>> extractReleaseIds = project -> CommonUtils
				.nullToEmptyMap(project.getReleaseIdToUsage()).keySet();

		Set<String> allReleaseIds = projects.stream().map(extractReleaseIds).reduce(Sets.newHashSet(), Sets::union);
		if (!allReleaseIds.isEmpty()) {
			Map<String, Release> releasesById = ThriftUtils
					.getIdMap(componentDatabaseHandler.getReleasesForClearingStateSummary(allReleaseIds));
			for (Project project : projects) {
				...
				final ReleaseClearingStateSummary releaseClearingStateSummary = ReleaseClearingStateSummaryComputer
						.computeReleaseClearingStateSummary(releases, project.getClearingTeam());
	...
	public List<ReleaseClearingStatusData> getReleaseClearingStatuses(String projectId, User user)
		...
		List<Release> releasesById = componentDatabaseHandler
				.getDetailedReleasesForExport(releaseIdsToProject.keySet());
		Map<String, Component> componentsById = ThriftUtils.getIdMap(componentDatabaseHandler
				.getComponentsShort(releasesById.stream().map(Release::getComponentId).collect(Collectors.toSet())));
		...
		for (Release release : releasesById) {
			...
			for (ProjectWithReleaseRelationTuple projectWithReleaseRelation : releaseIdsToProject
				...
				mainlineStates
						.add(ThriftEnumUtils.enumToString(projectWithReleaseRelation.getRelation().getMainlineState()));
			...
			releaseClearingStatuses.add(new ReleaseClearingStatusData(release)
					.setProjectNames(joinStrings(projectNames)).setMainlineStates(joinStrings(mainlineStates))
	...
	public List<ReleaseClearingStatusData> getReleaseClearingStatusesWithAccessibility(String projectId, User user)
		...
		SetMultimap<String, ProjectWithReleaseRelationTuple> releaseIdsToProject = releaseIdToProjects(project, user);
		List<Release> releasesById = componentDatabaseHandler
				.getDetailedReleasesWithAccessibilityForExport(releaseIdsToProject.keySet(), user);
		Map<String, Component> componentsById = ThriftUtils.getIdMap(componentDatabaseHandler
				.getComponentsShort(releasesById.stream().map(Release::getComponentId).collect(Collectors.toSet())));
	...
	private void releaseIdToProjects(Project project, User user, Set<String> visitedProjectIds,
			Multimap<String, ProjectWithReleaseRelationTuple> releaseIdToProjects) throws SW360Exception {
	...
	private String appendCompTypeToReleaseVersion(Collection<Release> releases, Set<String> cotsCompIds) {
		...
		for (Release release : releases) {
			if (cotsCompIds.contains(release.getComponentId())) {
				count++;
				release.setVersion(new StringBuilder(CommonUtils.nullToEmptyString(release.getVersion()))
						.append(DatabaseHandlerUtil.SEPARATOR).append(ComponentType.COTS).toString());
			}
		}
	...
	private String getUserDetails(User user) {
		return new StringBuilder(CommonUtils.nullToEmptyString(user.getUserGroup())).append(MailConstants.DASH)
				.append(SW360Utils.printFullname(user)).toString();
	}

	private void sendMailForNewClearing(Project project, String projectUrl, ClearingRequest clearingRequest,
			User user) {
		...
		mailUtil.sendClearingMail(ClearingRequestEmailTemplate.NEW, MailConstants.SUBJECT_FOR_NEW_CLEARING_REQUEST,
				getRecipients(clearingRequest), userDetails, CommonUtils.nullToEmptyString(clearingRequest.getId()),
				CommonUtils.nullToEmptyString(projectUrl), SW360Utils.printName(project),
				String.valueOf(project.getLinkedProjectsSize()), String.valueOf(project.getReleaseIdToUsageSize()),
				String.valueOf(totalCount), String.valueOf(approvedCount), clearingRequest.getRequestedClearingDate(),
				cotsCompCount, commentText.toString());
		if (releases.size() > 0) {
			commentText = new StringBuilder("Linked release(s) with clearing state new:").append(System.lineSeparator())
					.append(commentText);
		}
		commentText.append(System.lineSeparator()).append("Total number of COTS components: ").append(cotsCompCount);
	...
	private void sendMailForUpdatedCR(Project project, String projectUrl, ClearingRequest clearingRequest, User user) {
		...
		String userDetails = getUserDetails(user);
		mailUtil.sendClearingMail(ClearingRequestEmailTemplate.UPDATED,
				MailConstants.SUBJECT_FOR_UPDATED_CLEARING_REQUEST, getRecipients(clearingRequest), userDetails,
				CommonUtils.nullToEmptyString(clearingRequest.getId()), CommonUtils.nullToEmptyString(projectUrl),
				SW360Utils.printName(project), CommonUtils.getEnumStringOrNull(clearingRequest.getClearingState()),
				clearingRequest.getRequestedClearingDate(),
				CommonUtils.nullToEmptyString(clearingRequest.getAgreedClearingDate()),
				extractReleaseNameForClearingEmail(releases));
	}

	private void sendMailForUpdatedProjectWithClearingRequest(Project updated, Collection<Release> releases,
			String cotsCompCount, User user) {
		...
		mailUtil.sendClearingMail(ClearingRequestEmailTemplate.PROJECT_UPDATED,
				MailConstants.SUBJECT_FOR_UPDATED_PROJECT_WITH_CLEARING_REQUEST, getRecipients(clearingRequest),
				userDetails, SW360Utils.printName(updated), updated.getClearingRequestId(),
				String.valueOf(updated.getLinkedProjectsSize()), String.valueOf(updated.getReleaseIdToUsageSize()),
				String.valueOf(totalCount), String.valueOf(approvedCount),
				CommonUtils.getEnumStringOrNull(clearingRequest.getClearingState()),
				clearingRequest.getRequestedClearingDate(),
				CommonUtils.nullToEmptyString(clearingRequest.getAgreedClearingDate()), cotsCompCount,
				extractReleaseNameForClearingEmail(releases));
	}

	private void sendMailForClosedOrRejectedCR(Project project, ClearingRequest clearingRequest, User user,
			boolean isApproved) {
		mailUtil.sendClearingMail(
				isApproved ? ClearingRequestEmailTemplate.CLOSED : ClearingRequestEmailTemplate.REJECTED,
				isApproved
						? MailConstants.SUBJECT_FOR_CLOSED_CLEARING_REQUEST
						: MailConstants.SUBJECT_FOR_REJECTED_CLEARING_REQUEST,
				getRecipients(clearingRequest), project.getClearingRequestId(), SW360Utils.printName(project),
				isApproved ? "closed" : "rejected");
	}

	private void sendMailNotificationsForNewProject(Project project, String user) {
		mailUtil.sendMail(project.getProjectResponsible(), MailConstants.SUBJECT_FOR_NEW_PROJECT,
				MailConstants.TEXT_FOR_NEW_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.PROJECT_RESPONSIBLE.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(project.getProjectOwner(), MailConstants.SUBJECT_FOR_NEW_PROJECT,
				MailConstants.TEXT_FOR_NEW_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.PROJECT_OWNER.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(project.getLeadArchitect(), MailConstants.SUBJECT_FOR_NEW_PROJECT,
				MailConstants.TEXT_FOR_NEW_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.LEAD_ARCHITECT.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(project.getModerators(), user, MailConstants.SUBJECT_FOR_NEW_PROJECT,
				MailConstants.TEXT_FOR_NEW_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.MODERATORS.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(project.getContributors(), user, MailConstants.SUBJECT_FOR_NEW_PROJECT,
				MailConstants.TEXT_FOR_NEW_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.CONTRIBUTORS.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(project.getSecurityResponsibles(), user, MailConstants.SUBJECT_FOR_NEW_PROJECT,
				MailConstants.TEXT_FOR_NEW_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.SECURITY_RESPONSIBLES.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(SW360Utils.unionValues(project.getRoles()), user, MailConstants.SUBJECT_FOR_NEW_PROJECT,
				MailConstants.TEXT_FOR_NEW_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.ROLES.toString(), project.getName(), project.getVersion());
	}

	private void sendMailNotificationsForProjectUpdate(Project project, String user) {
		mailUtil.sendMail(project.getProjectResponsible(), MailConstants.SUBJECT_FOR_UPDATE_PROJECT,
				MailConstants.TEXT_FOR_UPDATE_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.PROJECT_RESPONSIBLE.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(project.getProjectOwner(), MailConstants.SUBJECT_FOR_UPDATE_PROJECT,
				MailConstants.TEXT_FOR_UPDATE_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.PROJECT_OWNER.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(project.getLeadArchitect(), MailConstants.SUBJECT_FOR_UPDATE_PROJECT,
				MailConstants.TEXT_FOR_UPDATE_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.LEAD_ARCHITECT.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(project.getModerators(), user, MailConstants.SUBJECT_FOR_UPDATE_PROJECT,
				MailConstants.TEXT_FOR_UPDATE_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.MODERATORS.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(project.getContributors(), user, MailConstants.SUBJECT_FOR_UPDATE_PROJECT,
				MailConstants.TEXT_FOR_UPDATE_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.CONTRIBUTORS.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(project.getSecurityResponsibles(), user, MailConstants.SUBJECT_FOR_UPDATE_PROJECT,
				MailConstants.TEXT_FOR_UPDATE_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.SECURITY_RESPONSIBLES.toString(), project.getName(), project.getVersion());
		mailUtil.sendMail(SW360Utils.unionValues(project.getRoles()), user, MailConstants.SUBJECT_FOR_UPDATE_PROJECT,
				MailConstants.TEXT_FOR_UPDATE_PROJECT, SW360Constants.NOTIFICATION_CLASS_PROJECT,
				Project._Fields.ROLES.toString(), project.getName(), project.getVersion());
	}
	...
	public RequestSummary importBomFromAttachmentContent(User user, String attachmentContentId) throws SW360Exception {
		...
		try {
			...
			try (final InputStream inputStream = attachmentStreamConnector
				...
				final SpdxBOMImporterSink spdxBOMImporterSink = new SpdxBOMImporterSink(user, this,
	...
	public List<Map<String, String>> getClearingStateInformationForListView(String projectId, User user,
			boolean isInaccessibleLinkMasked) throws SW360Exception {
	...
	private void flattenClearingStatusForLinkedProject(Map<String, ProjectProjectRelationship> linkedProjects,
		...
		linkedProjects.entrySet().stream().forEach(lp -> wrapTException(() -> {
			...
			if (linkedReleases != null && !linkedReleases.isEmpty()) {
				flattenClearingStatusForReleases(linkedReleases, projectOrigin, releaseOrigin, clearingStatusList, user,
						isInaccessibleLinkMasked);
			}
	...
	private void flattenClearingStatusForReleases(Map<String, ProjectReleaseRelationship> linkedReleases,
		...
		linkedReleases.entrySet().stream().forEach(rl -> wrapTException(() -> {
			...
			if (!isInaccessibleLinkMasked
					|| componentDatabaseHandler.isReleaseActionAllowed(rel, user, RequestedAction.READ)) {
				...
				Map<String, String> row = createReleaseCSRow(relation, projectMailLineState, rel, clearingStatusList,
						user, comment);
				if (releaseIdToRelationship != null && !releaseIdToRelationship.isEmpty()) {
					flattenlinkedReleaseOfRelease(releaseIdToRelationship, projectOrigin, releaseOrigin,
	...
	private Map<String, String> createInaccessibleReleaseCSRow(List<Map<String, String>> clearingStatusList)
			throws SW360Exception {
		Map<String, String> row = new HashMap<>();
		row.put("id", "");
		row.put("name", "");
