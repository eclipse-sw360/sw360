...
public class ComponentRepository extends SummaryAwareRepository<Component> {
	...
	private static final String MYCOMPONENTS = "function(doc) {" + "  if (doc.type == 'component') {"
			+ "    emit(doc.createdBy, doc._id);" + "  } " + "}";
	private static final String SUBSCRIBERS = "function(doc) {" + "  if (doc.type == 'component') {"
			+ "    for(var i in doc.subscribers) {" + "      emit(doc.subscribers[i], doc._id);" + "    }" + "  }"
			+ "}";
	private static final String BYNAME = "function(doc) {" + "  if (doc.type == 'component') {"
			+ "    emit(doc.name, doc._id);" + "  } " + "}";
	private static final String BYCOMPONENTTYPE = "function(doc) {" + "  if (doc.type == 'component') {"
			+ "    emit(doc.componentType, doc._id);" + "  } " + "}";
	private static final String FULLBYNAME = "function(doc) {" + "  if (doc.type == 'component') {"
			+ "    emit(doc.name, doc._id);" + "  } " + "}";
	private static final String BYLINKINGRELEASE = "function(doc) {" + "  if (doc.type == 'release') {"
			+ "    for(var i in doc.releaseIdToRelationship) {" + "      emit(i, doc.componentId);" + "    }" + "  }"
			+ "}";
	private static final String BYFOSSOLOGYID = "function(doc) {\n" + "  if (doc.type == 'release') {\n"
			...
			+ "            }\n" + "          }\n" + "        }\n" + "      }\n" + "    }\n" + "  }\n" + "}";
	private static final String BYEXTERNALIDS = "function(doc) {" + "  if (doc.type == 'component') {"
			...
			+ "}";
	private static final String BYDEFAULTVENDORID = "function(doc) {" + "  if (doc.type == 'component') {"
			+ "       emit( doc.defaultVendorId , doc._id);" + "  }" + "}";

	private static final String BYNAMELOWERCASE = "function(doc) {" + "  if (doc.type == 'component') {"
			+ "    emit(doc.name.toLowerCase(), doc._id);" + "  } " + "}";

	private static final String BYMAINLICENSE = "function(doc) {" + "    if (doc.type == 'component') {"
			+ "      if(doc.mainLicenseIds) {" + "            emit(doc.mainLicenseIds.join(), doc._id);"
			+ "      } else {" + "            emit('', doc._id);" + "      }" + "    }" + "}";

	private static final String BYVENDOR = "function(doc) {" + "    if (doc.type == 'component') {"
			+ "      if(doc.vendorNames) {" + "          emit(doc.vendorNames.join(), doc._id);" + "      } else {"
			+ "          emit('', doc._id);" + "      }" + "    }" + "}";

	public ComponentRepository(DatabaseConnectorCloudant db, ReleaseRepository releaseRepository,
	...
	public List<Component> getRecentComponentsSummary(int limit, User user) {
		...
		UnpaginatedRequestBuilder<String, Object> unPagnReques = query.newRequest(Key.Type.STRING, Object.class)
				.includeDocs(true).descending(true);
		if (limit >= 0) {
