...
/**
 * This displays a department field that can be used as extension point for a
 * department search dialog via onclick listeners (see also
...
public class DisplayDepartmentEdit extends NameSpaceAwareTag {
	...
	private void printEmptyDepartment(StringBuilder display) {
		...
		display.append(String.format("<input type=\"hidden\" value=\"\"  id=\"%s\" name=\"%s%s\"/>", id, namespace, id))
				...
				.append(String
						.format("<input type=\"text\" class=\"form-control edit-department clickable\" placeholder=\""
								+ LanguageUtil.get(resourceBundle, "click.to.set.department")
								+ "\" id=\"%sDisplay\" required=\"\"/>", id))
	...
	private void printFullDepartment(StringBuilder display) {
		display.append("<div class=\"form-group\">");
		printLabel(display);
		display.append(String.format("<input type=\"hidden\" value=\"%s\"  id=\"%s\" name=\"%s%s\"/>", departmentId, id,
				namespace, id)).append("<div class=\"form-group has-feedback\">")
	...
	private void printLabel(StringBuilder display) {
		...
		if (StringUtils.isNotEmpty(label)) {
			display.append(String.format("<label class=\"mandatory\" for=\"group\" >%s</label>",
