...
public class DisplayReleaseChanges extends UserAwareTag {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			jspWriter.print(renderString + releaseRelationshipDisplay.toString() + clearingInformationDisplay
					+ eccInformationDisplay + cotsDetailDisplay);
		} catch (Exception e) {
		...
		if (ensureSomethingTodoAndNoNull(Release._Fields.RELEASE_ID_TO_RELATIONSHIP)) {
			...
			if (isClosedModeration) {
				addedReleaseIds = Sets.difference(additions.getReleaseIdToRelationship().keySet(),
						deletions.getReleaseIdToRelationship().keySet());
				removedReleaseIds = Sets.difference(deletions.getReleaseIdToRelationship().keySet(), releaseIdsInDb);
			}
			...
			renderer.renderReleaseLinkListCompare(display, actual.getReleaseIdToRelationship(),
					deletions.getReleaseIdToRelationship(), additions.getReleaseIdToRelationship(), changedReleaseIds,
					request, isClosedModeration);
		}
	}
	...
	private String renderClearingInformation() {
		...
		for (ClearingInformation._Fields field : ClearingInformation._Fields.values()) {
			...
			displaySimpleFieldOrSet(display, actual.getClearingInformation(), additions.getClearingInformation(),
					deletions.getClearingInformation(), field, fieldMetaData, "", isClosedModeration);
		}
	...
	private String renderEccInformation() {
		...
		for (EccInformation._Fields field : EccInformation._Fields.values()) {
			...
			displaySimpleFieldOrSet(display, actual.getEccInformation(), additions.getEccInformation(),
					deletions.getEccInformation(), field, fieldMetaData, "", isClosedModeration);
		}
	...
	private String renderCOTSDetails() {
		...
		for (COTSDetails._Fields field : COTSDetails._Fields.values()) {
			...
			displaySimpleFieldOrSet(display, actual.getCotsDetails(), additions.getCotsDetails(),
					deletions.getCotsDetails(), field, fieldMetaData, "", isClosedModeration);
		}
