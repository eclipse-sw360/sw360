...
@org.osgi.service.component.annotations.Component(immediate = true, properties = {
		...
		"/org/eclipse/sw360/portal/portlets/default.properties"}, property = {
				...
				"javax.portlet.display-name=Moderations", "javax.portlet.info.short-title=Moderations",
				"javax.portlet.info.title=Moderations", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/moderation/view.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class ModerationPortlet extends FossologyAwarePortlet {
	...
	private static final ImmutableList<ModerationRequest._Fields> MODERATION_FILTERED_FIELDS = ImmutableList.of(
			ModerationRequest._Fields.TIMESTAMP, ModerationRequest._Fields.COMPONENT_TYPE,
			ModerationRequest._Fields.DOCUMENT_NAME, ModerationRequest._Fields.REQUESTING_USER,
			ModerationRequest._Fields.REQUESTING_USER_DEPARTMENT, ModerationRequest._Fields.MODERATORS,
	...
	private void serveModerationList(ResourceRequest request, ResourceResponse response, boolean open) {
		...
		long closedModRequestCount = countByModerationState.get("CLOSED") == null
				? 0
				: countByModerationState.get("CLOSED");
		Map<String, Set<String>> filterMap = getModerationFilterMap(request);
		long noOfRecords = filterMap.isEmpty()
				? (open ? openModRequestCount : closedModRequestCount)
	...
	private Map<PaginationData, List<ModerationRequest>> getFilteredModerationList(PortletRequest request,
		...
		try {
			if (filterMap.isEmpty()) {
				...
				moderations = moderations.stream().filter(mr -> mr.getModerators().contains(user.getEmail()))
	...
	private void addCommentToClearingRequest(ResourceRequest request, ResourceResponse response)
			throws PortletException {
		RequestStatus requestStatus = ModerationPortletUtils.addCommentToClearingRequest(request, log);
		ResourceBundle resourceBundle = ResourceBundleUtil.getBundle("content.Language", request.getLocale(),
				getClass());
		serveRequestStatus(request, response, requestStatus,
				LanguageUtil.get(resourceBundle, "error.adding.comment.to.clearing.request"), log);
	}

	private void getProjectDetailsForCR(ResourceRequest request, ResourceResponse response)
		...
		if (ids == null || ids.length == 0) {
			...
			for (Project project : projects) {
				try {
					...
					row.put("bu",
							CommonUtils.isNotNullEmptyOrWhitespace(babl)
									? babl
									: CommonUtils.nullToEmptyString(project.getBusinessUnit()));
	...
	private List<Project> getWithFilledClearingStateSummary(ProjectService.Iface client, List<Project> projects,
			User user) {
		try {
	...
	public void doView(RenderRequest request, RenderResponse response) throws IOException, PortletException {
		...
		} else if (PAGENAME_EDIT_CLEARING_REQUEST.equals(pageName)
				|| PAGENAME_DETAIL_CLEARING_REQUEST.equals(pageName)) {
		...
		if (id != null) {
			try {
				...
				if (ACTION_CANCEL.equals(action)) {
					...
					sessionMessage = LanguageUtil.get(resourceBundle,
							"you.have.cancelled.working.on.the.previous.moderation.request");
				} else if (ACTION_DECLINE.equals(action)) {
					declineModerationRequest(user, moderationRequest, request);

					client.refuseRequest(id, moderationComment, user.getEmail());
					sessionMessage = LanguageUtil.get(resourceBundle,
							"you.have.declined.the.previous.moderation.request");
					...
					sessionMessage = LanguageUtil.get(resourceBundle,
							"you.have.accepted.the.previous.moderation.request");
				} else if (ACTION_POSTPONE.equals(action)) {
					// keep me assigned but do it later... so nothing to be done here, just update
					// the comment message
					moderationRequest.setCommentDecisionModerator(moderationComment);
					client.updateModerationRequest(moderationRequest);
					sessionMessage = LanguageUtil.get(resourceBundle,
							"you.have.postponed.the.previous.moderation.request");
				} else if (ACTION_RENDER_NEXT_AFTER_UNSUBSCRIBE.equals(action)) {
					sessionMessage = LanguageUtil.get(resourceBundle,
							"you.are.removed.from.the.list.of.moderators.for.the.previous.moderation.request");
				} else {
	...
	private void renderClearingRequest(RenderRequest request, RenderResponse response, String pageName)
			throws PortletException {
		...
		try {
			...
			if (CommonUtils.isNotNullEmptyOrWhitespace(clearingRequest.getProjectId())) {
				...
				Project project = projectClient.getProjectById(clearingRequest.getProjectId(),
				...
				request.setAttribute(IS_CLEARING_EXPERT,
						isPrimaryRoleOfUserAtLeastClearingExpert
								|| projectPermission.isUserOfOwnGroupHasRole(clearingExpertRoles,
				...
				List<Project> projects = getWithFilledClearingStateSummary(projectClient, Lists.newArrayList(project),
						user);
			...
			String dateLimit = CommonUtils
					.nullToEmptyString(ModerationPortletUtils.loadPreferredClearingDateLimit(request, user));
	...
	@UsedAsLiferayAction
	public void updateClearingRequest(ActionRequest request, ActionResponse response)
			throws PortletException, IOException {
		...
		if (AddDocumentRequestStatus.SUCCESS.equals(requestSummary.getRequestStatus())) {
			String successMsg = new StringBuilder(LanguageUtil.get(resourceBundle, "clearing.request")).append(" ")
					.append(requestSummary.getId()).append(" ")
					.append(LanguageUtil.get(resourceBundle, "updated.successfully")).toString();
			...
			String errorMsg = LanguageUtil.get(resourceBundle,
					requestSummary.getMessage().replace(' ', '.').toLowerCase());
	...
	private void declineModerationRequest(User user, ModerationRequest moderationRequest, RenderRequest request)
			throws TException {
		switch (moderationRequest.getDocumentType()) {
			case USER :
				UserUtils.deleteLiferayUser(request, moderationRequest.getUser());
				UserService.Iface userClient = thriftClients.makeUserClient();
				userClient.deleteUser(UserCacheHolder.getRefreshedUserFromEmail(moderationRequest.getUser().getEmail()),
						user);
				break;
			default :
				break;
	...
	private void acceptModerationRequest(User user, User requestingUser, ModerationRequest moderationRequest,
			RenderRequest request) throws TException {
		switch (moderationRequest.getDocumentType()) {
			case COMPONENT : {
				...
				if (moderationRequest.isRequestDocumentDelete()) {
					...
					componentClient.updateComponentFromModerationRequest(moderationRequest.getComponentAdditions(),
							moderationRequest.getComponentDeletions(), user);
				}
			}
				break;
			case RELEASE : {
				...
				if (moderationRequest.isRequestDocumentDelete()) {
					...
					if (deleteStatus.equals(RequestStatus.SUCCESS)) {
						SW360Utils.removeReleaseVulnerabilityRelation(moderationRequest.getDocumentId(),
								UserCacheHolder.getUserFromRequest(request));
					}
				} else {
					componentClient.updateReleaseFromModerationRequest(moderationRequest.getReleaseAdditions(),
							moderationRequest.getReleaseDeletions(), user);
				}
			}
				break;
			case PROJECT : {
				...
				if (moderationRequest.isRequestDocumentDelete()) {
					...
					projectClient.updateProjectFromModerationRequest(moderationRequest.getProjectAdditions(),
							moderationRequest.getProjectDeletions(), user);
				}
			}
				break;
			case LICENSE : {
				LicenseService.Iface licenseClient = thriftClients.makeLicenseClient();
				licenseClient.updateLicenseFromModerationRequest(moderationRequest.getLicenseAdditions(),
						moderationRequest.getLicenseDeletions(), user, requestingUser);
			}
				break;
			case USER : {
	...
	private void addModerationBreadcrumb(RenderRequest request, RenderResponse response,
	...
	public void renderStandardView(RenderRequest request, RenderResponse response)
		...
		try {
			...
			Map<Boolean, List<ClearingRequest>> partitionedClearingRequests = clearingRequestsSet.stream()
					.collect(Collectors.groupingBy(ModerationPortletUtils::isClosedClearingRequest));
			...
			try {
				...
				if (actionsAllowed) {
					SessionMessages.add(request, "request_processed",
							LanguageUtil.get(resourceBundle, "you.have.assigned.yourself.to.this.moderation.request"));
	...
	public void renderComponentModeration(RenderRequest request, RenderResponse response,
			ModerationRequest moderationRequest, User user) throws IOException, PortletException, TException {
		...
		if (actual_component == null) {
			renderNextModeration(request, response, user,
					LanguageUtil.get(resourceBundle, "eignored.unretrievable.target"),
					thriftClients.makeModerationClient(), moderationRequest);
			return;
		}
	...
	public void renderReleaseModeration(RenderRequest request, RenderResponse response,
			ModerationRequest moderationRequest, User user) throws IOException, PortletException, TException {
		...
		if (actual_release == null) {
			renderNextModeration(request, response, user,
					LanguageUtil.get(resourceBundle, "ignored.unretrievable.target"),
					thriftClients.makeModerationClient(), moderationRequest);
			return;
		}
	...
	private boolean refuseToDeleteUsedDocument(RenderRequest request, RenderResponse response,
			ModerationRequest moderationRequest, User user, boolean requestDocumentDelete, Boolean is_used)
			throws TException, IOException, PortletException {
		if (requestDocumentDelete && is_used) {
			ModerationService.Iface client = thriftClients.makeModerationClient();
			ResourceBundle resourceBundle = ResourceBundleUtil.getBundle("content.Language", request.getLocale(),
					getClass());
			client.refuseRequest(moderationRequest.getId(),
					LanguageUtil.get(resourceBundle, "you.cannot.delete.a.document.still.used.by.others"),
					user.getEmail());
			renderNextModeration(request, response, user,
					LanguageUtil.get(resourceBundle, "ignored.delete.of.used.target"), client, moderationRequest);
			return true;
		}
		return false;
	}
	...
	private void prepareRelease(RenderRequest request, User user, Release actualRelease) {
		...
		try {
			request.setAttribute(COMPONENT,
					thriftClients.makeComponentClient().getComponentById(actualRelease.getComponentId(), user));
		} catch (TException e) {
	...
	public void renderProjectModeration(RenderRequest request, RenderResponse response,
			ModerationRequest moderationRequest, User user) throws IOException, PortletException, TException {
		final boolean requestDocumentDelete = moderationRequest.isRequestDocumentDelete();
		Boolean is_used = false;
		...
		try {
			...
			request.setAttribute(PortalConstants.DEFAULT_LICENSE_INFO_HEADER_TEXT,
					getDefaultLicenseInfoHeaderText(actual_project.getBusinessUnit()));
	...
	public void renderLicenseModeration(RenderRequest request, RenderResponse response,
			ModerationRequest moderationRequest, User user) throws IOException, PortletException, TException {
		...
		try {
			...
			List<Obligation> obligations = client.getObligations().stream().filter(Objects::nonNull)
	...
	public void renderUserModeration(RenderRequest request, RenderResponse response,
			ModerationRequest moderationRequest, User user) throws IOException, PortletException, TException {
	...
	private Map<String, Set<String>> getModerationFilterMap(PortletRequest request) {
		...
		for (ModerationRequest._Fields filteredField : MODERATION_FILTERED_FIELDS) {
			...
			if (!isNullOrEmpty(parameter) && !((filteredField.equals(ModerationRequest._Fields.COMPONENT_TYPE)
				...
				if (filteredField.equals(ModerationRequest._Fields.TIMESTAMP)
