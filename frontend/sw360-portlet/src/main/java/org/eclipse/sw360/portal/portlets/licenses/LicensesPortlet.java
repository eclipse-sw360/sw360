...
@org.osgi.service.component.annotations.Component(immediate = true, properties = {
		...
		"/org/eclipse/sw360/portal/portlets/default.properties"}, property = {
				...
				"javax.portlet.display-name=Licenses", "javax.portlet.info.short-title=Licenses",
				"javax.portlet.info.title=Licenses", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/licenses/view.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class LicensesPortlet extends Sw360Portlet {
	...
	private List<Obligation> loadLicenseObligation(ResourceRequest request) {
		...
		try {
			...
			obligations = client.getObligations().stream().filter(Objects::nonNull)
		...
		if (id != null) {
			...
			if (isAtLeastClearingAdmin) {
				...
				SessionMessages.add(request, "request_processed",
						LanguageUtil.get(resourceBundle, "you.will.create.a.new.and.unchecked.license"));
			}
	...
	private void prepareStandardView(RenderRequest request) {
		...
		request.setAttribute(IS_USER_AT_LEAST_CLEARING_ADMIN,
				PermissionUtils.isUserAtLeast(UserGroup.CLEARING_ADMIN, user) ? "Yes" : "No");
		try {
			LicenseService.Iface client = thriftClients.makeLicenseClient();
	...
	private void prepareDetailView(RenderRequest request, RenderResponse response)
		...
		if (id != null) {
			try {
				...
				List<Obligation> addedTodos = allTodos.stream().filter(CommonUtils::isTemporaryObligation)
						.collect(Collectors.toList());
				List<Obligation> currentTodos = allTodos.stream().filter(t -> !CommonUtils.isTemporaryObligation(t))
						.collect(Collectors.toList());
	...
	public void update(ActionRequest request, ActionResponse response) throws PortletException, IOException {
		...
		boolean isAttemptToOverwriteExistingByNew = isAttemptToOverwriteExistingByNew(license, user, isNewLicense,
				client);
	...
	private RequestStatus updateLicense(License license, User user, boolean isAttemptToOverwriteExistingByNew,
			LicenseService.Iface client) {
		RequestStatus requestStatus;
		try {
			requestStatus = isAttemptToOverwriteExistingByNew
					? RequestStatus.FAILURE
					: client.updateLicense(license, user, user);
		} catch (TException e) {
	...
	private boolean isAttemptToOverwriteExistingByNew(License license, User user, boolean isNewLicense,
			LicenseService.Iface client) {
		return isNewLicense && checkLicenseExists(license, user, client);
	}

	private License prepareLicenseForUpdate(ActionRequest request, LicenseService.Iface client, String licenseId,
			User user) {
		License license = new License();;
	...
	private License updateLicenseFromRequest(License license, ActionRequest request) {
		...
		Quadratic osiApproved = Quadratic
				.findByValue(Integer.parseInt(request.getParameter(License._Fields.OSIAPPROVED.toString())));
		Quadratic fsfLibre = Quadratic
				.findByValue(Integer.parseInt(request.getParameter(License._Fields.FSFLIBRE.toString())));
		boolean checked = "true".equals(request.getParameter(License._Fields.CHECKED.toString()));
		String licenseTypeString = request
				.getParameter(License._Fields.LICENSE_TYPE.toString() + LicenseType._Fields.LICENSE_TYPE.toString());
		...
		List<String> oblIds = CommonUtils.isNotNullEmptyOrWhitespace(obligationIds)
				? Arrays.asList(obligationIds.split(","))
				: Lists.newArrayList();
		...
		try {
			...
			if (licenseTypeDatabaseId.isPresent()) {
				...
				final LicenseType licenseType = thriftClients.makeLicenseClient()
						.getLicenseTypeById(license.getLicenseTypeDatabaseId());
	...
	public void updateWhiteList(ActionRequest request, ActionResponse response) throws PortletException, IOException {
		...
		// we get a list of obligationDatabaseIds and Booleans and we have to update the
		// whiteList of each oblig if it changed
		String licenseId = request.getParameter(LICENSE_ID);
		String[] whiteList = request.getParameterValues("whiteList");
		if (whiteList == null)
			whiteList = new String[0]; // As empty arrays are not passed as parameters
