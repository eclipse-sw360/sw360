...
public class LinkedReleaseRenderer {
	...
	public <T> void renderReleaseLinkList(StringBuilder display, Map<String, T> releaseRelationshipMap,
			Set<String> releaseIds, String msg, HttpServletRequest request) {
		...
		try {
			...
			for (Release release : componentClient.getReleasesById(releaseIds, user)) {
				candidate.append(String.format("<tr><td>%s</td><td>%s</td></tr>", release.getName(),
						releaseRelationshipMap.get(release.getId())));
			}
		...
		if (!tableContent.isEmpty()) {
			...
			display.append(String.format(
					"<thead><tr><th colspan=\"2\">%s</th></tr><tr><th>"
							+ LanguageUtil.get(resourceBundle, "release.name") + "</th><th>"
							+ LanguageUtil.get(resourceBundle, "release.relationship") + "</th></tr></thead><tbody>",
					msg));
	...
	public <T> void renderReleaseLinkListCompare(StringBuilder display, Map<String, T> oldReleaseRelationshipMap,
			Map<String, T> deleteReleaseRelationshipMap, Map<String, T> updateReleaseRelationshipMap,
			Set<String> releaseIds, HttpServletRequest request, boolean isClosedModeration) {
		if (releaseIds.isEmpty())
			return;
		...
		try {
			...
			for (Release release : componentClient.getReleasesById(changedIds, user)) {
				...
				T updateReleaseRelationship = updateReleaseRelationshipMap.get(releaseId);
				candidate.append(
						String.format("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", release.getName(),
								oldReleaseRelationship, deleteReleaseRelationship, updateReleaseRelationship));
			}
		...
		if (!tableContent.isEmpty()) {
			...
			display.append("<thead><tr><th colspan=\"4\">" + LanguageUtil.get(resourceBundle, "updated.release.links")
					+ "</th></tr><tr><th>" + LanguageUtil.get(resourceBundle, "release.name") + "</th><th>"
					+ LanguageUtil.get(resourceBundle, "current.release.relationship")
					+ "</th><th>Deleted Release relationship</th><th>Suggested release relationship</th></tr></thead><tbody>");
			display.append(tableContent);
			display.append("</tbody></table>");
		}
	}
