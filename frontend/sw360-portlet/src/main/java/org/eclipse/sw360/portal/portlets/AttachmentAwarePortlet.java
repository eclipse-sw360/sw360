...
public abstract class AttachmentAwarePortlet extends Sw360Portlet {
	...
	static {
		...
		for (AttachmentType attachmentType : AttachmentType.values()) {
			ATTACHMENT_TYPE_MAP.put(String.valueOf(attachmentType.getValue()),
					ThriftEnumUtils.enumToString(attachmentType));
		}
	...
	private void setAttachmentsInRequest(PortletRequest request, Set<Attachment> attachments, Source owner) {
		...
		Set<String> attachmentContentIds = atts.stream().map(Attachment::getAttachmentContentId)
				.collect(Collectors.toSet());
		try {
			User user = UserCacheHolder.getUserFromRequest(request);
			Map<String, List<AttachmentUsage>> attachmentUsagesByContentId = client
					.getAttachmentsUsages(owner, attachmentContentIds, null).stream()
					.collect(Collectors.groupingBy(AttachmentUsage::getAttachmentContentId));

			Map<String, List<Project>> usingProjectsByContentId = fetchUsingProjectsForAttachmentUsages(
					attachmentUsagesByContentId, user);
			Map<String, Long> restrictedProjectsCountsByContentId = countRestrictedProjectsByContentId(
					attachmentUsagesByContentId, usingProjectsByContentId);
	...
	@NotNull
	private Map<String, Long> countRestrictedProjectsByContentId(
			Map<String, List<AttachmentUsage>> attachmentUsagesByContentId,
			Map<String, List<Project>> usingProjectsByContentId) {
		return attachmentUsagesByContentId.entrySet().stream()
				.collect(Collectors.toMap(Map.Entry::getKey,
						entry -> distinctProjectIdsFromAttachmentUsages(entry.getValue()).count()
								- usingProjectsByContentId.get(entry.getKey()).size()));
	}

	private Map<String, List<Project>> fetchUsingProjectsForAttachmentUsages(
			Map<String, List<AttachmentUsage>> attachmentUsagesByContentId, User user) throws TException {
		List<String> projectIds = attachmentUsagesByContentId.values().stream().flatMap(Collection::stream)
				.map(AttachmentUsage::getUsedBy).map(Source::getProjectId).distinct().collect(Collectors.toList());
		...
		return Maps.transformValues(attachmentUsagesByContentId,
				attUsages -> distinctProjectIdsFromAttachmentUsages(attUsages).map(usingProjects::get)
	...
	private Stream<String> distinctProjectIdsFromAttachmentUsages(List<AttachmentUsage> usages) {
		return nullToEmptyList(usages).stream().map(AttachmentUsage::getUsedBy).map(Source::getProjectId).distinct();
	}
	...
	protected void setAttachmentsInRequest(PortletRequest request, Component component) {
		setAttachmentsInRequest(request, component.getAttachments(),
				component.getId() == null ? null : Source.componentId(component.getId()));
	}
	...
	protected void setAttachmentsInRequest(PortletRequest request, Release release) {
		setAttachmentsInRequest(request, release.getAttachments(),
				release.getId() == null ? null : Source.releaseId(release.getId()));
	}
	...
	protected void setAttachmentsInRequest(PortletRequest request, Project project) {
		setAttachmentsInRequest(request, project.getAttachments(),
				project.getId() == null ? null : Source.projectId(project.getId()));
	}
	...
	protected void dealWithAttachments(ResourceRequest request, ResourceResponse response, String action)
	...
	private void doGetAttachmentForDisplay(ResourceRequest request, ResourceResponse response)
			throws IOException, PortletException {
	...
	private void serveAttachmentSet(ResourceRequest request, ResourceResponse response)
			throws IOException, PortletException {
		...
		try {
			Set<String> contentIds = attachments.stream().map(Attachment::getAttachmentContentId)
					.collect(Collectors.toSet());
			Map<Map<Source, String>, Integer> usageCountsBySourceAndContentId = thriftClients.makeAttachmentClient()
					.getAttachmentUsageCount(ImmutableMap.of(createSource(documentId, documentType), contentIds), null);
			attachmentUsageCounts = usageCountsBySourceAndContentId.entrySet().stream().collect(
					Collectors.toMap(AttachmentAwarePortlet::getValueFromSingleEntryMapKey, Map.Entry::getValue));
		} catch (TException e) {
	...
	public void deleteUnneededAttachments(String userEmail, String documentId) {
		if (uploadHistoryPerUserEmailAndDocumentId.containsKey(userEmail)) {
			Set<String> uploadedAttachmentIds = nullToEmptySet(
					uploadHistoryPerUserEmailAndDocumentId.get(userEmail).get(documentId));
