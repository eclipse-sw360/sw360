...
public abstract class FossologyAwarePortlet extends LinkedReleasesAndProjectsAwarePortlet {
	...
	protected abstract void dealWithFossologyAction(ResourceRequest request, ResourceResponse response, String action)
			throws IOException, PortletException;

	protected void serveLicenseToSourceFileMapping(ResourceRequest request, ResourceResponse response)
			throws IOException, PortletException {
		...
		final ResourceBundle resourceBundle = ResourceBundleUtil.getBundle("content.Language", request.getLocale(),
				getClass());
		final Predicate<Attachment> isCLI = attachment -> AttachmentType.COMPONENT_LICENSE_INFO_XML
		...
		try {
			...
			List<Attachment> filteredAttachments = CommonUtils.nullToEmptySet(release.getAttachments()).stream()
					.filter(isCLI).collect(Collectors.toList());
			if (filteredAttachments.size() > 1) {
				Predicate<Attachment> isApprovedCLI = attachment -> CheckStatus.ACCEPTED
						.equals(attachment.getCheckStatus());
				filteredAttachments = filteredAttachments.stream().filter(isApprovedCLI).collect(Collectors.toList());
			}
			if (filteredAttachments.size() == 1
					&& filteredAttachments.get(0).getFilename().endsWith(PortalConstants.XML_FILE_EXTENSION)) {
				...
				try {
					...
					if (CommonUtils.isNotEmpty(licenseResults)
							&& LicenseInfoRequestStatus.SUCCESS.equals(licenseResults.get(0).getStatus())) {
						...
						if (CommonUtils.isNotEmpty(licenseNameWithTexts)) {
							...
							for (LicenseNameWithText license : licenseNameWithTexts) {
								...
								jsonObject.put("srcFiles",
										nullToEmptyString(String.join(",", license.getSourceFiles())));
								jsonObject.put("licType",
										SW360Constants.LICENSE_TYPE_GLOBAL.equalsIgnoreCase(license.getType())
							...
							jsonResult.put(SW360Constants.MESSAGE,
									LanguageUtil.get(resourceBundle, "source.file.information.not.found.in.cli"));
						}
					} else {
						jsonResult.put(SW360Constants.STATUS, SW360Constants.FAILURE);
				...
				if (filteredAttachments.size() > 1) {
					jsonResult.put(SW360Constants.MESSAGE,
							LanguageUtil.get(resourceBundle, "multiple.approved.cli.are.found.in.the.release"));
				} else if (filteredAttachments.isEmpty()) {
					jsonResult.put(SW360Constants.MESSAGE,
							LanguageUtil.get(resourceBundle, "cli.attachment.not.found.in.the.release"));
				} else {
					jsonResult.put(SW360Constants.MESSAGE,
							LanguageUtil.get(resourceBundle, "source.file.information.not.found.in.cli"));
				}
	...
	protected void putReleasesAndProjectIntoRequest(PortletRequest request, String projectId, User user)
		...
		List<ReleaseClearingStatusData> releaseClearingStatuses = client
