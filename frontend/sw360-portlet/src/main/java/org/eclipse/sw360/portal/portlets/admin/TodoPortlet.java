...
@org.osgi.service.component.annotations.Component(immediate = true, properties = {
		...
		"/org/eclipse/sw360/portal/portlets/admin.properties"}, property = {"javax.portlet.name=" + TODOS_PORTLET_NAME,

				"javax.portlet.display-name=Obligations", "javax.portlet.info.short-title=Obligations",
				"javax.portlet.info.title=Obligations", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/admin/obligations/view.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class TodoPortlet extends Sw360Portlet {
	...
	private void serveObligationElementSearchResults(ResourceRequest request, ResourceResponse response,
			String searchText) throws IOException, PortletException {
		...
		include("/html/admin/obligations/ajax/searchObligationElementsAjax.jsp", request, response,
	...
	private String buildJsonObligationText(com.liferay.portal.kernel.json.JSONObject jsonObject, int level) {
		...
		try {
			...
			if (!obligationNode.getNodeType().equals("ROOT")) {
				if (obligationNode.getNodeType().equals("Obligation")) {
					ObligationElement obligationElement = client
							.getObligationElementById(obligationNode.getOblElementId());
		...
		if (jsonObject.getJSONArray("children").length() != 0) {
			for (int i = 0; i < jsonObject.getJSONArray("children").length(); i++) {
				com.liferay.portal.kernel.json.JSONObject contactObject = jsonObject.getJSONArray("children")
						.getJSONObject(i);
