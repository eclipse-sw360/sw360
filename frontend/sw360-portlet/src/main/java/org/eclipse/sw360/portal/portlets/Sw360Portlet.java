...
abstract public class Sw360Portlet extends MVCPortlet {
	...
	public void activate(Map<String, Object> properties) {
		...
		log.info("Portlet [" + (portletName != null ? portletName : this.getClass().getSimpleName())
				+ "] has been ENABLED.");
	}
	...
	protected void addBreadcrumbEntry(PortletRequest request, String name, PortletURL url) {
		PortalUtil.addPortletBreadcrumbEntry(
				PortalUtil.getOriginalServletRequest(PortalUtil.getHttpServletRequest(request)), name, url.toString());
	}
	...
	protected void renderRequestPreparation(PortletRequest request, MimeResponse response,
			ImportBomRequestPreparation requestPreparation) {
		JSONObject jsonObject = JSONFactoryUtil.createJSONObject();
	...
	protected void renderRemoveModerationRequestStatus(PortletRequest request, MimeResponse response,
			RemoveModeratorRequestStatus status) {
		JSONObject jsonObject = JSONFactoryUtil.createJSONObject();
	...
	protected void serveRequestStatus(PortletRequest request, ResourceResponse response, RequestStatus requestStatus,
			String message, Logger log) {
		if (requestStatus != RequestStatus.FAILURE) {
			request.setAttribute(PortalConstants.REQUEST_STATUS, requestStatus.toString());
			// We want failures to call the error method of the AJAX call so we do not send
			// it
	...
	public void dealWithUserAction(ResourceRequest request, ResourceResponse response, String action)
		...
		if (PortalConstants.USER_SEARCH.equals(action)) {
			...
			try {
				...
				request.setAttribute(PortalConstants.USER_SEARCH_GOT_TRUNCATED,
						isListTruncated(users, truncatedUserList));
	...
	public void setSessionMessage(PortletRequest request, RequestStatus requestStatus, String type, String verb,
		...
		switch (requestStatus) {
			case SUCCESS :
				statusMessage = new StringBuilder(type).append(" ").append(name).append(" ").append(verb)
						.append("d successfully!").toString();
	...
	protected void serveReleaseSearch(ResourceRequest request, ResourceResponse response, String searchText)
			throws IOException, PortletException {
		...
		try {
			...
			if (isNullOrEmpty(searchText)) {
				...
				if (vendorIds != null && vendorIds.size() > 0) {
					searchResult.addAll(
							componentClient.getAccessibleReleasesFromVendorIds(Sets.newHashSet(vendorIds), user));
				}
				return searchResult;
			}
		} catch (TException e) {
