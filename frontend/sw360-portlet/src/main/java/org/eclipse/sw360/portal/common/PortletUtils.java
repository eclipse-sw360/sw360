...
public class PortletUtils {
	...
	public static <U extends TFieldIdEnum, T extends TBase<T, U>> void setFieldValue(PortletRequest request, T instance,
			U field, FieldMetaData fieldMetaData, String prefix) {
	 ...
	 */
	public static Set<Attachment> updateAttachmentsFromRequest(PortletRequest request,
			Set<Attachment> documentAttachments) {
		Set<Attachment> attachments = Sets.newHashSet();

		User user = UserCacheHolder.getUserFromRequest(request);
		String[] ids = request.getParameterValues(
				Release._Fields.ATTACHMENTS.toString() + Attachment._Fields.ATTACHMENT_CONTENT_ID.toString());
		String[] fileNames = request
				.getParameterValues(Release._Fields.ATTACHMENTS.toString() + Attachment._Fields.FILENAME.toString());
		String[] types = request.getParameterValues(
				Release._Fields.ATTACHMENTS.toString() + Attachment._Fields.ATTACHMENT_TYPE.toString());
		String[] createdComments = request.getParameterValues(
				Release._Fields.ATTACHMENTS.toString() + Attachment._Fields.CREATED_COMMENT.toString());
		String[] checkStatuses = request.getParameterValues(
				Release._Fields.ATTACHMENTS.toString() + Attachment._Fields.CHECK_STATUS.toString());
		String[] checkedComments = request.getParameterValues(
				Release._Fields.ATTACHMENTS.toString() + Attachment._Fields.CHECKED_COMMENT.toString());
		...
		if (ids == null || ids.length == 0) {
			...
			LOGGER.error(
					"Invalid request content. One of the attachment parameters is null. No attachments will be saved or deleted.");
			return documentAttachments;
		} else if (!CommonUtils.allHaveSameLength(ids, fileNames, types, createdComments, checkStatuses,
				checkedComments)) {
			LOGGER.error(
					"Not all of the attachment parameter arrays have the same length! No attachments will be saved or deleted.");
			return documentAttachments;
		}
	...
	public static Map<String, Set<String>> getCustomMapFromRequest(PortletRequest request, String mapKey,
			String mapValue) {
		...
		List<String> keyAndValueParameterIds = Collections.list(parameterNames).stream()
				.filter(p -> p.startsWith(mapKey)).map(s -> s.replace(mapKey, "")).collect(Collectors.toList());
	...
	public static Map<String, String> getMapWithJoinedValueFromRequest(PortletRequest request, String key,
			String value) {
		Map<String, Set<String>> customMap = getCustomMapFromRequest(request, key, value);
		return customMap.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, e -> {
			try {
				if (isNotEmpty(e.getValue()) && e.getValue().size() == 1) {
					return e.getValue().stream().findFirst().orElse("");
				}
	...
	public static Map<String, String> getExternalIdMapFromRequest(PortletRequest request) {
		return getMapWithJoinedValueFromRequest(request, PortalConstants.EXTERNAL_ID_KEY,
				PortalConstants.EXTERNAL_ID_VALUE);
	}

	public static Map<String, Set<UserGroup>> getSecondaryDepartmentAndRolesMapFromRequest(PortletRequest request,
	...
	public static Map<String, String> getExternalUrlMapFromRequest(PortletRequest request) {
		return getMapWithJoinedValueFromRequest(request, PortalConstants.EXTERNAL_URL_KEY,
				PortalConstants.EXTERNAL_URL_VALUE);
	}
	...
	public static void getBaBlSelection(RenderRequest request, User user) {
		Map<String, CustomField> customFieldMap = CustomFieldHelper.getCustomFields(request, user,
				CustomFieldPageIdentifier.PROJECT);
	...
	public static void setCustomFieldsDisplay(RenderRequest request, User user, Component component) {
		Map<String, CustomField> customFieldMap = CustomFieldHelper.getCustomFields(request, user,
				CustomFieldPageIdentifier.COMPONENT);
	...
	public static void setCustomFieldsDisplay(RenderRequest request, User user, Release release) {
		Map<String, CustomField> customFieldMap = CustomFieldHelper.getCustomFields(request, user,
				CustomFieldPageIdentifier.RELEASE);
		setCustomFieldsDisplay(customFieldMap, release.getAdditionalData());
	}

	private static void setCustomFieldsDisplay(Map<String, CustomField> customFieldMap,
	...
	private static void setCustomFieldsEdit(RenderRequest request, Map<String, CustomField> customFieldMap,
			Map<String, String> additionalData) {
