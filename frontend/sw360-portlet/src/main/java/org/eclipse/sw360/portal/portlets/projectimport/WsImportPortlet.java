...
@org.osgi.service.component.annotations.Component(immediate = true, properties = {
		...
		"/org/eclipse/sw360/portal/portlets/default.properties"}, property = {
				...
				"javax.portlet.display-name=WhiteSource Import", "javax.portlet.info.short-title=WhiteSource Import",
				"javax.portlet.info.title=WhiteSource Import", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/projects/wsimport.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class WsImportPortlet extends Sw360Portlet {
	...
	private TokenCredentials getTokenCredentialsFromSession(PortletSession session) {
		...
		return new TokenCredentials().setToken(token).setServerUrl(server).setUserKey(userKey);
	}
		...
		try {
			...
			if (!isImportSuccessful(importStatus)) {
				if (importStatus.getRequestStatus().equals(RequestStatus.FAILURE)) {
					...
					LOGGER.error("Importing has not succeeded for the following IDs: "
	...
	private void importWsProjects(User user, List<String> selectedIds, JSONObject responseData,
			TokenCredentials tokenCredentials) throws PortletException, IOException {
