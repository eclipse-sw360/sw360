...
@Component(immediate = true, properties = {"/org/eclipse/sw360/portal/portlets/base.properties",
		"/org/eclipse/sw360/portal/portlets/admin.properties"}, property = {
				...
				"javax.portlet.display-name=User Administration", "javax.portlet.info.short-title=User",
				"javax.portlet.info.title=User Administration", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/admin/user/view.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class UserPortlet extends Sw360Portlet {
	...
	private boolean updateUserObjectFromRequest(ActionRequest request, ActionResponse response, UserCSV userCSV,
		...
		Map<String, ClientMetadata> userAccessFromRequest = PortletUtils.getOidcClientMapFromRequest(request);
		...
		if (PortalConstants.SSO_LOGIN_ENABLED) {
			...
			if (isLiferayUserNew) {
				...
				newlyCreatedUser.setSecondaryDepartmentsAndRoles(secondaryDepartmentAndRolesMapFromRequest)
						.setOidcClientInfos(userAccessFromRequest).setFullname(liferayCreatedOrUpdated.getFullName())
	...
	private JSONArray getUserData(List<org.eclipse.sw360.datahandler.thrift.users.User> userList,
		...
		int upperLimit = paginationParameters.getDisplayLength() == -1
				? userList.size()
	...
	private Map<PaginationData, List<org.eclipse.sw360.datahandler.thrift.users.User>> getFilteredUsersList(
			PortletRequest request, PaginationData pageData, UserService.Iface client,
	...
	public void backUpUsers(ResourceRequest request, ResourceResponse response)
		...
		for (User liferayUser : liferayUsers) {
			...
			boolean wantsMailNotification = sw360user.isSetWantsMailNotification()
					? sw360user.wantsMailNotification
					: true;
	...
	private void createOrganizations(PortletRequest request, List<UserCSV> users)
		...
		/*
		 * Find the departments of the users, create the head departments and then
	...
	private void createOrganizations(PortletRequest request, Iterable<String> departments)
		...
		for (String headDepartment : headDepartments) {
			...
			if (organizationId == 0) { // The organization does not yet exist
				Organization organization = createOrganization(serviceContext, headDepartment, country.getCountryId(),
		...
		for (String department : departments) {
			...
			if (organizationId == 0) { // The organization does not yet exist
				createOrganization(serviceContext, department, country.getCountryId(),
						organizationIds.get(extractHeadDept(department)).intValue());
			}
		}
	}

	private Organization createOrganization(ServiceContext serviceContext, String headDepartment, long countryId,
			int parentId) throws PortalException, SystemException {
		return OrganizationServiceUtil.addOrganization(parentId, headDepartment,
				OrganizationConstants.TYPE_ORGANIZATION, RegionConstants.DEFAULT_REGION_ID, countryId,
	...
	private User dealWithUser(PortletRequest request, UserCSV userRec) {
		...
		try {
			...
			if (user != null) {
				UserUtils.synchronizeUserWithDatabase(userRec, thriftClients, userRec::getEmail, userRec::getGid,
