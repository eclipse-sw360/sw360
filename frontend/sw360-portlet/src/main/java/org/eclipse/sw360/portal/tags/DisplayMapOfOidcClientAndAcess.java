...
public class DisplayMapOfOidcClientAndAcess extends SimpleTagSupport {
	...
	private static String getclientIdAndMetadataString(String clientId, ClientMetadata clientMetadata) {
		StringBuilder sb = new StringBuilder();
		sb.append("<li><span>").append(StringEscapeUtils.escapeXml(clientMetadata.getName()))
				.append("</span><span> -> </span><span> ").append(StringEscapeUtils.escapeXml(clientId))
				.append("</span><span> -> </span><span> ")
				.append(ThriftEnumUtils.enumToString(clientMetadata.getAccess())).append("</span></li>");
		return sb.toString();
	}
}
