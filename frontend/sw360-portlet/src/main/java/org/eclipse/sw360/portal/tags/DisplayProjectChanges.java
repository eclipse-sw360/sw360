...
public class DisplayProjectChanges extends UserAwareTag {
	...
	private void renderLinkedProjects(StringBuilder display, User user) {
		...
		if (ensureSomethingTodoAndNoNullLinkedProjects()) {
			...
			renderProjectLinkList(display, deletions.getLinkedProjects(), removedProjectIds,
					LanguageUtil.get(resourceBundle, "removed.project.links"), user);
			renderProjectLinkList(display, additions.getLinkedProjects(), addedProjectIds,
					LanguageUtil.get(resourceBundle, "added.project.links"), user);
	...
	private void renderProjectLinkList(StringBuilder display,
			Map<String, ProjectProjectRelationship> projectRelationshipMap, Set<String> projectIds, String msg,
			User user) {
		...
		try {
			...
			for (ProjectLink projectLink : client.getLinkedProjects(filteredMap, user)) {
				candidate.append(String.format("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", projectLink.getName(),
						projectLink.getRelation(), projectLink.isEnableSvm()));
			}
		...
		if (!tableContent.isEmpty()) {
			...
			display.append(String.format("<thead><tr><th colspan=\"3\">%s</th></tr>" + "<tr><th>"
					+ LanguageUtil.get(resourceBundle, "project.name") + "</th><th>"
					+ LanguageUtil.get(resourceBundle, "project.relationship") + "</th>" + "<th>"
					+ LanguageUtil.get(resourceBundle, "enable.svm") + "</th>" + "</tr></thead><tbody>", msg));
	...
	private void renderProjectLinkListCompare(StringBuilder display,
			...
			Map<String, ProjectProjectRelationship> updateProjectRelationshipMap, Set<String> projectIds, User user) {
		if (projectIds.isEmpty())
			return;
		...
		try {
			...
			for (String projectId : projectIds) {
				...
				ProjectRelationship oldProjectRelationship = oldProjectRelationshipMap.get(projectId)
						.getProjectRelationship();
				ProjectRelationship deleteProjectRelationship = deleteProjectRelationshipMap.get(projectId)
						.getProjectRelationship();
			...
			// ! This code doubling is done to reduce the database queries. I.e. one big
			// query instead of multiple small ones
			for (ProjectLink projectLink : client.getLinkedProjects(changeMap, user)) {
				...
				candidate.append(String.format(
						"<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>",
						projectLink.getName(), oldProjectRelationship, deleteProjectRelationship,
						updateProjectRelationship, oldEnableSvm, deleteEnableSvm, updateEnableSvm));
			}
		...
		if (!tableContent.isEmpty()) {
			...
			display.append("<thead><tr><th colspan=\"7\">" + LanguageUtil.get(resourceBundle, "updated.project.links")
					+ "</th></tr>" + "<tr><th>" + LanguageUtil.get(resourceBundle, "project.name") + "</th>" + "<th>"
					+ LanguageUtil.get(resourceBundle, "current.project.relationship") + "</th>" + "<th>"
					+ LanguageUtil.get(resourceBundle, "deleted.project.relationship") + "</th>" + "<th>"
					+ LanguageUtil.get(resourceBundle, "suggested.project.relationship") + "</th>" + "<th>"
					+ LanguageUtil.get(resourceBundle, "current.linked.project.enable.svm") + "</th>" + "<th>"
					+ LanguageUtil.get(resourceBundle, "deleted.linked.project.enable.svm") + "</th>" + "<th>"
					+ LanguageUtil.get(resourceBundle, "suggested.linked.project.enable.svm") + "</th></tr>"
					+ "</thead><tbody>");
			display.append(tableContent);
			display.append("</tbody></table>");
		}
	...
	private void renderReleaseIdToUsage(StringBuilder display, User user) {
		...
		if (ensureSomethingTodoAndNoNullReleaseIdUsage()) {
			...
			Set<String> changedReleaseIds = Sets.intersection(additions.getReleaseIdToUsage().keySet(),
					deletions.getReleaseIdToUsage().keySet());
			changedReleaseIds = Sets.intersection(changedReleaseIds, actual.getReleaseIdToUsage().keySet());// remove
																											// projects
																											// already
																											// deleted
																											// in
																											// database
			Set<String> removedReleaseIds = Sets.difference(deletions.getReleaseIdToUsage().keySet(),
					changedReleaseIds);
			...
			ResourceBundle resourceBundle = ResourceBundleUtil.getBundle("content.Language", request.getLocale(),
					getClass());
			renderer.renderReleaseLinkList(display, deletions.getReleaseIdToUsage(), removedReleaseIds,
					LanguageUtil.get(resourceBundle, "removed.release.links"), request);
			renderer.renderReleaseLinkList(display, additions.getReleaseIdToUsage(), addedReleaseIds,
					LanguageUtil.get(resourceBundle, "added.release.links"), request);
			renderer.renderReleaseLinkListCompare(display, actual.getReleaseIdToUsage(),
					deletions.getReleaseIdToUsage(), additions.getReleaseIdToUsage(), changedReleaseIds, request,
					isClosedModeration);
		}
	}
	...
	private Project prepareLicenseInfoHeaderTextInProject(Project project) {
		...
		String defaultTextAsHtmlForDisplay = "<span title=\"" + defaultLicenseInfoHeaderText + "\">"
				+ PortalConstants.DEFAULT_LICENSE_INFO_HEADER_TEXT_FOR_DISPALY + "</span>";

		if (!modifiedProject.isSetLicenseInfoHeaderText()) {
			// if the project contains the default license info header text, we wrap it into
			// an html span-element such that the default text is given as a hover text.
			// this is only done for displaying it in a three-way merge in a moderation
			// request.
			modifiedProject.setLicenseInfoHeaderText(defaultTextAsHtmlForDisplay);
		} else {
			// for a custom text escape html properly
			modifiedProject.setLicenseInfoHeaderText(
					StringEscapeUtils.escapeHtml(modifiedProject.getLicenseInfoHeaderText()).replace("\n", "<br>"));
		}
	...
	private Project prepareObligationsTextInProject(Project project) {
		...
		String defaultTextAsHtmlForDisplay = "<span title=\"" + defaultObligationsText + "\">"
				+ PortalConstants.DEFAULT_OBLIGATIONS_TEXT_FOR_DISPALY + "</span>";

		if (!modifiedProject.isSetObligationsText()) {
			// if the project contains the default clearing summary text, we wrap it into an
			// html span-element such that the default text is given as a hover text.
			// this is only done for displaying it in a three-way merge in a moderation
			// request.
			modifiedProject.setObligationsText(defaultTextAsHtmlForDisplay);
		} else {
			// for a custom text escape html properly
			modifiedProject.setObligationsText(
					StringEscapeUtils.escapeHtml(modifiedProject.getObligationsText()).replace("\n", "<br>"));
		}
