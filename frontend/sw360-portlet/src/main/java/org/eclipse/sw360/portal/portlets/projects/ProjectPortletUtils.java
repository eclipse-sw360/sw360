...
public class ProjectPortletUtils {
		...
		for (Project._Fields field : Project._Fields.values()) {
			switch (field) {
				...
				case LICENSE_INFO_HEADER_TEXT :
					// if `LICENSE_INFO_HEADER_TEXT` is not in the request then we want this to be
					// unset in the `project`
				...
				case OBLIGATIONS_TEXT :
					// if `OBLIGATIONS_TEXT` is not in the request then we want this to be unset in
					// the `project`
	...
	private static void updateLinkedReleasesFromRequest(PortletRequest request,
			Map<String, ProjectReleaseRelationship> releaseUsage) {
		releaseUsage.clear();
		String[] ids = request
				.getParameterValues(Project._Fields.RELEASE_ID_TO_USAGE.toString() + ReleaseLink._Fields.ID.toString());
		String[] relations = request.getParameterValues(Project._Fields.RELEASE_ID_TO_USAGE.toString()
				+ ProjectReleaseRelationship._Fields.RELEASE_RELATION.toString());
		String[] mainlStates = request.getParameterValues(Project._Fields.RELEASE_ID_TO_USAGE.toString()
				+ ProjectReleaseRelationship._Fields.MAINLINE_STATE.toString());
		String[] comments = request.getParameterValues(
				Project._Fields.RELEASE_ID_TO_USAGE.toString() + ProjectReleaseRelationship._Fields.COMMENT.toString());
		if (ids != null && relations != null && mainlStates != null && ids.length == relations.length
				&& ids.length == mainlStates.length) {
	...
	private static void updateLinkedProjectsFromRequest(PortletRequest request,
			Map<String, ProjectProjectRelationship> linkedProjects) {
		linkedProjects.clear();
		String[] ids = request
				.getParameterValues(Project._Fields.LINKED_PROJECTS.toString() + ProjectLink._Fields.ID.toString());
		String[] relations = request.getParameterValues(
				Project._Fields.LINKED_PROJECTS.toString() + ProjectLink._Fields.RELATION.toString());
	...
	public static ProjectVulnerabilityRating updateProjectVulnerabilityRatingFromRequest(
		...
		if (vulnerabilityIds.length != releaseIds.length) {
			String message = "Length of vulnerabilities (" + vulnerabilityIds.length
	...
	private static VulnerabilityCheckStatus newVulnerabilityCheckStatusFromRequest(ResourceRequest request) {
		VulnerabilityRatingForProject vulnerabilityRatingForProject = VulnerabilityRatingForProject
				.findByValue(Integer.parseInt(request.getParameter(PortalConstants.VULNERABILITY_RATING_VALUE)));

		return new VulnerabilityCheckStatus().setCheckedBy(UserCacheHolder.getUserFromRequest(request).getEmail())
	 ...
	 *
	 * @param attachmentContentIdsWithPath
	 ...
	 */
	public static Map<String, Set<LicenseNameWithText>> getExcludedLicensesPerAttachmentIdFromRequest(
			Set<String> attachmentContentIdsWithPath, ResourceRequest request) {
		...
		for (String attachmentContentIdWithPath : attachmentContentIdsWithPath) {
			...
			Map<String, LicenseNameWithText> licenseStore = (Map<String, LicenseNameWithText>) request
					.getPortletSession()
					.getAttribute(ProjectPortlet.LICENSE_STORE_KEY_PREFIX + attachmentContentIdWithPathWithoutRel);
			if (licenseStore == null) {
				throw new IllegalStateException("No license store found for attachment content id with path "
						+ attachmentContentIdWithPathWithoutRel);
			}

			Set<Integer> includedIds = Arrays.stream(checkboxes).map(s -> Integer.valueOf(s))
	...
	public static Map<Source, Set<String>> extractContainedAttachments(Collection<ProjectLink> projectLinks) {
		...
		for (ProjectLink projectLink : projectLinks) {
			for (ReleaseLink releaseLink : projectLink.linkedReleases) {
				Set<String> attachmentIds = attachments.getOrDefault(Source.releaseId(releaseLink.getId()),
						Sets.newHashSet());
				attachmentIds.addAll(releaseLink.getAttachments().stream().map(a -> a.getAttachmentContentId())
	...
	private static List<AttachmentUsage> makeAttachmentUsagesFromRequestParameters(ResourceRequest request,
		...
		Set<String> selectedUsages = new HashSet<>(
				arrayToList(request.getParameterValues(PROJECT_SELECTED_ATTACHMENT_USAGES)));
		Set<String> changedUsages = new HashSet<>(
				arrayToList(request.getParameterValues(PROJECT_SELECTED_ATTACHMENT_USAGES_SHADOWS)));
		...
		return usagesSubset.stream().map(s -> parseAttachmentUsageFromString(projectId, s, includeConludedLicenses))
				.filter(Objects::nonNull).collect(Collectors.toList());
	}

	private static AttachmentUsage parseAttachmentUsageFromString(String projectId, String s,
			List<String> includeConludedLicense) {
		...
		AttachmentUsage usage = new AttachmentUsage(Source.releaseId(releaseId), attachmentContentId,
	...
	/**
	 * Here, "equivalent" means an AttachmentUsage should replace another one in the
	 * DB, not that they are equal. I.e, they have the same attachmentContentId,
	 * owner, usedBy, and same UsageData type.
	 */
	@NotNull
	...
	static AttachmentUsage mergeAttachmentUsages(AttachmentUsage u1, AttachmentUsage u2) {
		...
		switch (u1.getUsageData().getSetField()) {
			case LICENSE_INFO :
				mergedUsage.getUsageData().getLicenseInfo()
						.setExcludedLicenseIds(Sets.union(
								...
								Optional.of(u2).map(AttachmentUsage::getUsageData).map(UsageData::getLicenseInfo)
										.map(LicenseInfoUsage::getExcludedLicenseIds).orElse(Collections.emptySet())));
				break;
			case SOURCE_PACKAGE :
			case MANUALLY_SET :
