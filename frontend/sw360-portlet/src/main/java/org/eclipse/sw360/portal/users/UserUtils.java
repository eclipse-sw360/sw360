...
public class UserUtils {
	...
	public static <T> org.eclipse.sw360.datahandler.thrift.users.User synchronizeUserWithDatabase(T source,
			ThriftClients thriftClients, Supplier<String> emailSupplier, Supplier<String> extIdSupplier,
			BiConsumer<org.eclipse.sw360.datahandler.thrift.users.User, T> synchronizer) {
	...
	@NotNull
	public static Set<String> prepareFormerEmailAddresses(org.eclipse.sw360.datahandler.thrift.users.User thriftUser,
			String email) {
	...
	public static String displayUser(String email, org.eclipse.sw360.datahandler.thrift.users.User user) {
		String userString;
		if (user != null) {
			userString = "<a href=\"mailto:" + user.getEmail() + "\">" + user.getGivenname() + " " + user.getLastname()
					+ "</a>";
		} else {
	...
	public static List<Organization> getOrganizations(RenderRequest request) {
		...
		try {
			// This only gives top-level organizations, not the whole tree. TODO: check
			// whether it's necessary to load all organizations
			organizations = OrganizationLocalServiceUtil.getOrganizations(companyId,
	...
	public static void activateLiferayUser(PortletRequest request,
	...
	public static User findLiferayUser(PortletRequest request, org.eclipse.sw360.datahandler.thrift.users.User user)
			throws PortalException, SystemException {
		return findLiferayUser(new PortletRequestAdapter(request), user.getEmail(), user.getExternalid());
	}

	public static User findLiferayUser(RequestAdapter requestAdapter, String email, String externalId)
	...
	public void synchronizeUserWithDatabase(User user) {
		...
		org.eclipse.sw360.datahandler.thrift.users.User refreshed = UserCacheHolder
				.getRefreshedUserFromEmail(userEmailAddress);
		if (!equivalent(refreshed, user)) {
			synchronizeUserWithDatabase(user, thriftClients, user::getEmailAddress, user::getScreenName,
					UserUtils::fillThriftUserFromLiferayUser);
	...
	public static void fillThriftUserFromUserCSV(final org.eclipse.sw360.datahandler.thrift.users.User thriftUser,
			final UserCSV userCsv) {
	...
	public static void fillThriftUserFromLiferayUser(final org.eclipse.sw360.datahandler.thrift.users.User thriftUser,
			final User user) {
	...
	public static void fillThriftUserFromThriftUser(final org.eclipse.sw360.datahandler.thrift.users.User thriftUser,
			final org.eclipse.sw360.datahandler.thrift.users.User user) {
