...
public class ErrorMessages {
	...
	// this map is used in errorKeyToMessage.jspf to generate key-value pairs for
	// the liferay-ui error tag
	public static final ImmutableList<String> allErrorMessages = ImmutableList.<String>builder().add(PROJECT_NOT_ADDED)
			.add(PROJECT_DUPLICATE).add(PROJECT_NAMING_ERROR).add(CLOSED_UPDATE_NOT_ALLOWED).add(COMPONENT_NOT_ADDED)
			.add(COMPONENT_DUPLICATE).add(COMPONENT_NAMING_ERROR).add(RELEASE_NOT_ADDED).add(RELEASE_DUPLICATE)
			.add(RELEASE_NAME_VERSION_ERROR).add(DUPLICATE_ATTACHMENT).add(LICENSE_USED_BY_RELEASE)
			.add(DOCUMENT_USED_BY_PROJECT_OR_RELEASE).add(DOCUMENT_NOT_PROCESSED_SUCCESSFULLY)
			.add(DEFAULT_ERROR_MESSAGE).add(FIRST_NAME_CANNOT_BE_EMPTY).add(LAST_NAME_CANNOT_BE_EMPTY)
			.add(EMAIL_NOT_VALID).add(DEPARTMENT_CANNOT_BE_EMPTY).add(EXTERNAL_ID_CANNOT_BE_EMPTY)
			.add(PASSWORD_CANNOT_BE_EMPTY).add(PASSWORDS_DONT_MATCH).add(COULD_NOT_CREATE_USER_MODERATION_REQUEST)
			.add(EMAIL_ALREADY_EXISTS).add(EXTERNAL_ID_ALREADY_EXISTS).add(DOCUMENT_NOT_AVAILABLE)
			.add(ERROR_GETTING_PROJECT).add(ERROR_GETTING_COMPONENT).add(ERROR_COMPONENT_NOT_ACCESSIBLE)
			.add(ERROR_RELEASE_OR_COMPONENT_NOT_ACCESSIBLE).add(ERROR_GETTING_RELEASE).add(ERROR_GETTING_LICENSE)
			.add(LICENSE_SHORTNAME_TAKEN).add(UPDATE_FAILED_SANITY_CHECK).add(REST_API_TOKEN_ERROR)
			.add(REST_API_TOKEN_NAME_DUPLICATE).add(REST_API_EXPIRE_DATE_NOT_VALID)
			.add(ERROR_PROJECT_OR_DEPENDENCIES_NOT_FOUND).add(ERROR_PROJECT_OR_LINKEDPROJECT_NOT_ACCESSIBLE)
			.add(ERROR_GETTING_CLEARING_REQUEST).add(LICENSE_TYPE_DUPLICATE).add(LICENSE_TYPE_ACCESS_DENIED)
			.add(ERROR_USER_CREATE).add(ERROR_USER_UPDATE).add(ERROR_USER_ACTIVATE_DEACTIVATE).add(OBLIGATION_NOT_ADDED)
			.add(OBLIGATION_NOT_UPDATED).add(ERROR_VULNERABILITY_USED_BY_RELEASE).add(ERROR_VULNERABILITY_DELETE)
			.add(ERROR_ADD_VULNERABILITY).add(ERROR_UPDATE_VULNERABILITY).add(VENDOR_DUPLICATE).add(ERROR_VENDOR)
			.build();
