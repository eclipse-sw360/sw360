...
/**
 * Helper class to map organization from identity provider to sw360 internal org
 * names and create organizations in Liferay if necessary
 *
 * Adapted from
 * https://github.com/sw360/ldapOrganizationMappingExt/blob/master/ldapAdapterEXT-ext-impl/src/main/java/com/bosch/osmi/sw360/ldapAdapter/CustomPortalLDAPImporterImpl.java
 */
public class OrganizationHelper {
	...
	public void reassignUserToOrganizationIfNecessary(User user, Organization organization)
		...
		if (organization != null && userIsNotInOrganization(user, organization.getOrganizationId())) {
			...
			log.info("OrganizationHelper adds user " + user.getEmailAddress() + " to the organization "
	...
	public Organization addOrGetOrganization(String organizationName, long companyId)
			throws PortalException, SystemException {
	...
	private Organization addOrganization(String organizationName, long countryId, User user) throws SystemException {
		...
		try {
			OrganizationLocalService organizationLocalService = (OrganizationLocalService) PortalBeanLocatorUtil
					.locate(OrganizationLocalService.class.getName());
			organization = organizationLocalService.addOrganization(user.getUserId(),
					OrganizationConstants.DEFAULT_PARENT_ORGANIZATION_ID, organizationName, TYPE_REGULAR_ORGANIZATION,
					RegionConstants.DEFAULT_REGION_ID, countryId, ListTypeConstants.ORGANIZATION_STATUS_DEFAULT,
					"Automatically created during LDAP import", false, null);

			OrganizationMembershipPolicyUtil.verifyPolicy(organization);

			log.info("added organization with name: " + organization.getName() + " and id:"
					+ organization.getOrganizationId());
		} catch (PortalException e) {
			log.error(
					"A creator or parent organization with the primary key could not be found or the organization's information was invalid",
					e);
		}
		return organization;
	}
	...
	public String mapOrganizationName(String name) {
		...
		return sortedOrganizationMappings.stream().filter(matcher).findFirst().map(Map.Entry::getValue).orElse(name);
	}
	...
	private static void loadOrganizationHelperSettings() {
		...
		List<Object> mappingSourceKeys = sw360Properties.keySet().stream().filter(
				p -> ((String) p).startsWith(MAPPING_KEYS_PREFIX) && !((String) p).endsWith(MAPPING_VALUES_SUFFIX))
