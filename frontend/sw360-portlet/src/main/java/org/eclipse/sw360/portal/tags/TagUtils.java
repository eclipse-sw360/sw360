...
public class TagUtils {
	...
	public static <U extends TFieldIdEnum, T extends TBase<T, U>> void displaySimpleFieldOrSet(StringBuilder display,
			T oldInstance, T additions, T deletions, U field, FieldMetaData fieldMetaData, String prefix,
		...
		if ((oldFieldValue != null && !oldFieldValue.equals(updateFieldValue)) || oldFieldValue == null
			...
			if (fieldMetaData.valueMetaData.type == TType.SET) {
				displaySet(display, (Set<String>) oldFieldValue, (Set<String>) updateFieldValue,
						(Set<String>) deletedFieldValue, field, prefix);
			} else if (fieldMetaData.valueMetaData.type == TType.MAP && CommonUtils.isMapFieldMapOfStringSets(field,
					oldInstance, additions, deletions, LogManager.getLogger(TagUtils.class))) {
				displayCustomMap(display, (Map<String, Set<String>>) oldFieldValue,
						(Map<String, Set<String>>) updateFieldValue, (Map<String, Set<String>>) deletedFieldValue,
						field, prefix);
			} else {
	...
	private static <U extends TFieldIdEnum> void displayCustomMap(StringBuilder display,
			Map<String, Set<String>> oldFieldValue, Map<String, Set<String>> updateFieldValue,
			Map<String, Set<String>> deletedFieldValue, U field, String prefix) {
		Set<String> keySetOfChanges = unifiedKeyset(updateFieldValue, deletedFieldValue);

		for (String key : keySetOfChanges) {
			displaySet(display, getNullToEmptyValue(oldFieldValue, key), getNullToEmptyValue(updateFieldValue, key),
					getNullToEmptyValue(deletedFieldValue, key), field, prefix, key);
		}
	...
	private static <U extends TFieldIdEnum, T extends TBase<T, U>> void displaySimpleField(StringBuilder display,
			Object oldFieldValue, Object updateFieldValue, Object deletedFieldValue, U field,
			FieldMetaData fieldMetaData, String prefix) {
		String oldDisplay = null;
	...
	public static void addDownloadLink(PageContext pageContext, JspWriter jspWriter, String name, String id,
			String contextType, String contextId) throws IOException, JspException {
	...
	public static void addDownloadLink(PageContext pageContext, JspWriter jspWriter, String name,
		...
		UrlWriter urlWriter = resourceUrl(pageContext)
				...
				.withParam(PortalConstants.CONTEXT_TYPE, contextType).withParam(PortalConstants.CONTEXT_ID, contextId);
		for (String id : ids) {
			urlWriter.withParam(PortalConstants.ATTACHMENT_ID, id);
		}
		urlWriter.writeUrlToJspWriter();
		jspWriter.write(format("'><img src='%s/images/download_enabled.jpg' alt='Download %s' title='Download %s'/>",
