...
@org.osgi.service.component.annotations.Component(immediate = true, properties = {
		...
		"/org/eclipse/sw360/portal/portlets/admin.properties"}, property = {
				...
				"javax.portlet.display-name=Schedule Administration", "javax.portlet.info.short-title=Schedule",
				"javax.portlet.info.title=Schedule Administration", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/admin/scheduleAdmin/view.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class ScheduleAdminPortlet extends Sw360Portlet {
	...
	private void prepareStandardView(RenderRequest request, RenderResponse response) {
		try {
			...
			int offsetInSecondsForDeleteAttachment = scheduleClient
			...
			int intervalInSecondsForDeleteAttachment = scheduleClient
					.getInterval(ThriftClients.DELETE_ATTACHMENT_SERVICE);
			request.setAttribute(PortalConstants.DELETE_ATTACHMENT_INTERVAL,
					CommonUtils.formatTime(intervalInSecondsForDeleteAttachment));
	...
	private boolean isCveSearchScheduled(ScheduleService.Iface scheduleClient, User user) throws TException {
		RequestStatusWithBoolean requestStatus = scheduleClient.isServiceScheduled(ThriftClients.CVESEARCH_SERVICE,
				user);
	...
	public void scheduleCveSearch(ActionRequest request, ActionResponse response) throws PortletException, IOException {
		try {
			User user = UserCacheHolder.getUserFromRequest(request);
			RequestSummary requestSummary = new ThriftClients().makeScheduleClient()
					.scheduleService(ThriftClients.CVESEARCH_SERVICE);
	...
	@UsedAsLiferayAction
	public void unscheduleCveSearch(ActionRequest request, ActionResponse response)
			throws PortletException, IOException {
		try {
			User user = UserCacheHolder.getUserFromRequest(request);
			RequestStatus requestStatus = new ThriftClients().makeScheduleClient()
					.unscheduleService(ThriftClients.CVESEARCH_SERVICE, user);
	...
	@UsedAsLiferayAction
	public void unscheduleAllServices(ActionRequest request, ActionResponse response)
			throws PortletException, IOException {
		try {
			User user = UserCacheHolder.getUserFromRequest(request);
	...
	private boolean isDeleteAttachmentScheduled(ScheduleService.Iface scheduleClient, User user) throws TException {
		RequestStatusWithBoolean requestStatus = scheduleClient
				.isServiceScheduled(ThriftClients.DELETE_ATTACHMENT_SERVICE, user);
		if (RequestStatus.SUCCESS.equals(requestStatus.getRequestStatus())) {
			return requestStatus.isAnswerPositive();
		} else {
	...
	@UsedAsLiferayAction
	public void scheduleDeleteAttachment(ActionRequest request, ActionResponse response)
			throws PortletException, IOException {
		try {
			RequestSummary requestSummary = new ThriftClients().makeScheduleClient()
	...
	@UsedAsLiferayAction
	public void unscheduleDeleteAttachment(ActionRequest request, ActionResponse response)
			throws PortletException, IOException {
		try {
			User user = UserCacheHolder.getUserFromRequest(request);
			RequestStatus requestStatus = new ThriftClients().makeScheduleClient()
					.unscheduleService(ThriftClients.DELETE_ATTACHMENT_SERVICE, user);
			setSessionMessage(request, requestStatus, "Task", "unschedule");
		} catch (TException e) {
	...
	@UsedAsLiferayAction
	public void triggerDeleteAttachment(ActionRequest request, ActionResponse response)
			throws PortletException, IOException {
		try {
			RequestStatus requestStatus = new ThriftClients().makeAttachmentClient()
					.deleteOldAttachmentFromFileSystem();
