...
@org.osgi.service.component.annotations.Component(immediate = true, properties = {
		...
		"/org/eclipse/sw360/portal/portlets/user.properties"}, property = {
				...
				"javax.portlet.display-name=Preferences", "javax.portlet.info.short-title=Preferences",
				"javax.portlet.info.title=Preferences", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/preferences/view.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class UserPreferencesPortlet extends Sw360Portlet {
	...
	private void prepareStandardView(RenderRequest request) {
		final User user = UserCacheHolder
				.getRefreshedUserFromEmail(UserCacheHolder.getUserFromRequest(request).getEmail());
	...
	public void createToken(ActionRequest request, ActionResponse response) {
		...
		if (!isValidExpireDays(restApiToken)) {
			log.error("Token expiration days [" + restApiToken.getNumberOfDaysValid() + "] is not valid for user "
					+ user.getEmail());
	...
	public void savePreferences(ActionRequest request, ActionResponse response) {
		...
		try {
			...
			PortletUtils.setFieldValue(request, user, User._Fields.WANTS_MAIL_NOTIFICATION,
					User.metaDataMap.get(User._Fields.WANTS_MAIL_NOTIFICATION), "");

			if (user.isWantsMailNotification()) { // if !wantsMailNotification, all the other checkboxes are disabled
													// and therefore not submitted
	...
	private boolean isValidExpireDays(RestApiToken restApiToken) {
		String configExpireDays = restApiToken.getAuthorities().contains("WRITE")
				? API_TOKEN_MAX_VALIDITY_WRITE_IN_DAYS
				: API_TOKEN_MAX_VALIDITY_READ_IN_DAYS;
	...
	private Set<String> getApiAuthorities(PortletRequest request) {
		...
		if (requestParams.contains(AUTHORITIES_WRITE)) {
			// User needs at least the role which is defined in sw360.properties (default
			// admin)
