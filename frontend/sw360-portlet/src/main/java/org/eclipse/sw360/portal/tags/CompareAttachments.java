...
public class CompareAttachments extends ContextAwareTag {
	...
	public static final List<Attachment._Fields> RELEVANT_FIELDS = FluentIterable
			.from(copyOf(Attachment._Fields.values())).filter(CompareAttachments::isFieldRelevant).toList();
		...
		try {
			...
			if (!isChanged) {
				renderString = "<div class=\"alert alert-info\">"
						+ LanguageUtil.get(resourceBundle, "no.changes.in.attachments") + "</div>";
			}
	...
	private boolean renderAttachments(JspWriter jspWriter, Set<Attachment> currentAttachments,
			Set<Attachment> addedAttachments, Set<Attachment> deletedAttachments, String contextType, String contextId)
		...
		if (!isClosedModeration) {
			deletedAttachmentIds = Sets.intersection(deletedAttachmentIds, currentAttachmentIds);// remove what was
																									// deleted already
																									// in the database
		}

		boolean isDeleted = renderAttachmentList(jspWriter,
				isClosedModeration ? deletedAttachmentsById : currentAttachmentsById, deletedAttachmentIds, "Deleted",
				contextType, contextId, isClosedModeration);
		boolean isAdded = renderAttachmentList(jspWriter, addedAttachmentsById, addedAttachmentIds, "Added",
				contextType, contextId, false);
		boolean isChanged = renderAttachmentComparison(jspWriter, currentAttachmentsById, deletedAttachmentsById,
				addedAttachmentsById, commonAttachmentIds);
		return isDeleted || isAdded || isChanged;
	}

	private boolean renderAttachmentList(JspWriter jspWriter, Map<String, Attachment> allAttachments,
			Set<String> attachmentIds, String msg, String contextType, String contextId, boolean isDeletedAttachment)
			throws JspException, IOException {
		if (attachmentIds.isEmpty())
			return false;
		jspWriter.write(
				String.format("<table class=\"%s\" id=\"%s%s\" style=\"white-space: pre-wrap;word-break: break-all;\">",
						tableClasses, idPrefix, msg));
	...
	private void renderAttachmentRowHeader(JspWriter jspWriter, String msg) throws IOException {
		jspWriter.write(String.format("<thead><tr><th colspan=\"%d\"> %s Attachments: </th></tr><tr>",
				RELEVANT_FIELDS.size(), msg));
	...
	private void renderAttachmentRow(JspWriter jspWriter, Attachment attachment, String contextType, String contextId,
			boolean isDeletedAttachment) throws JspException, IOException {
		jspWriter.write("<tr>");
		for (Attachment._Fields field : RELEVANT_FIELDS) {
			...
			if (field.equals(Attachment._Fields.FILENAME)) {
				jspWriter.append(
						String.format("<td>%s", getDisplayString(fieldMetaData.valueMetaData.type, fieldValue)));
				...
				jspWriter.append(
						String.format("<td>%s</td>", getDisplayString(fieldMetaData.valueMetaData.type, fieldValue)));
			}
		}
		jspWriter.append("</tr>");
	}

	private boolean renderAttachmentComparison(JspWriter jspWriter, Map<String, Attachment> currentAttachmentsById,
			Map<String, Attachment> deletedAttachmentsById, Map<String, Attachment> addedAttachmentsById,
			Set<String> commonAttachmentIds) throws IOException {
		if (commonAttachmentIds.isEmpty())
			return false;

		StringBuilder candidate = new StringBuilder();
		for (String commonAttachmentId : commonAttachmentIds) {
			renderCompareAttachment(candidate, currentAttachmentsById.get(commonAttachmentId),
					deletedAttachmentsById.get(commonAttachmentId), addedAttachmentsById.get(commonAttachmentId));
		}
	...
	private void renderCompareAttachment(StringBuilder display, Attachment old, Attachment deleted, Attachment added) {
		...
		if (old.equals(added))
			return;
		display.append(String.format("<table class=\"%s\" id=\"%schanges%s\" >", tableClasses, idPrefix,
				old.getAttachmentContentId()));
		display.append(
				String.format("<thead><tr><th colspan=\"4\"> Changes for Attachment %s </th></tr>", old.getFilename()));
		...
		for (Attachment._Fields field : RELEVANT_FIELDS) {
			...
			FieldMetaData fieldMetaData = Attachment.metaDataMap.get(field);
			displaySimpleFieldOrSet(display, old, added, deleted, field, fieldMetaData,
					Release._Fields.ATTACHMENTS.getFieldName(), isClosedModeration);
		}
