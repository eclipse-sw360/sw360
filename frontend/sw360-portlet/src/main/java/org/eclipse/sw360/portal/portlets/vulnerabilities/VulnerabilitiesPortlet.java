...
@org.osgi.service.component.annotations.Component(immediate = true, properties = {
		...
		"/org/eclipse/sw360/portal/portlets/default.properties"}, property = {
				...
				"javax.portlet.display-name=Vulnerabilities", "javax.portlet.info.short-title=Vulnerabilities",
				"javax.portlet.info.title=Vulnerabilities", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/vulnerabilities/view.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class VulnerabilitiesPortlet extends Sw360Portlet {
	...
	public static final Set<Vulnerability._Fields> FILTERED_FIELDS = ImmutableSet.of(Vulnerability._Fields.EXTERNAL_ID,
			Vulnerability._Fields.VULNERABLE_CONFIGURATION);

	private static final int DEFAULT_VIEW_SIZE = 200;

	// Helper methods
	private void addVulnerabilityBreadcrumb(RenderRequest request, RenderResponse response,
	...
	private void removeVulnerability(ResourceRequest request, ResourceResponse response) {
		...
		if (requestStatus == RequestStatus.SUCCESS) {
			serveRequestStatus(request, response, requestStatus,
					"Vulnerability " + request.getParameter(VULNERABILITY_ID) + " has been deleted", log);
		} else if (requestStatus == RequestStatus.IN_USE) {
			serveRequestStatus(request, response, requestStatus, ErrorMessages.ERROR_VULNERABILITY_USED_BY_RELEASE,
					log);
		} else {
	...
	private void getFilteredVulnerabilityList(PortletRequest request) throws IOException {
		...
		try {
			...
			int limit = CustomFieldHelper.loadAndStoreStickyViewSize(request, user,
					CUSTOM_FIELD_VULNERABILITIES_VIEW_SIZE);
			...
			if (!isNullOrEmpty(externalId) || !isNullOrEmpty(vulnerableConfig)) {
				vulnerabilities = vulnerabilityClient.getVulnerabilitiesByExternalIdOrConfiguration(externalId,
						vulnerableConfig, user);
	...
	private void prepareDetailView(RenderRequest request, RenderResponse response)
		...
		if (externalId != null) {
			try {
				...
				VulnerabilityWithReleaseRelations vulnerabilityWithReleaseRelations = client
						.getVulnerabilityWithReleaseRelationsByExternalId(externalId, user);
	...
	private List<Release> getReleasesFromRelations(User user,
		...
		if (vulnerabilityWithReleaseRelations != null) {
			...
			Set<String> ids = relations.stream().map(ReleaseVulnerabilityRelation::getReleaseId)
	...
	private void prepareEditView(RenderRequest request, RenderResponse response) {
		...
		List<String> vulnerabilityImpacts = Stream.of(VulnerabilityImpact.values()).map(VulnerabilityImpact::name)
				.collect(Collectors.toList());
		List<String> vulnerabilityAccessAuthentications = Stream.of(VulnerabilityAccessAuthentication.values())
				.map(VulnerabilityAccessAuthentication::name).collect(Collectors.toList());
		List<String> vulnerabilityAccessComplexities = Stream.of(VulnerabilityAccessComplexity.values())
				.map(VulnerabilityAccessComplexity::name).collect(Collectors.toList());
		List<String> vulnerabilityAccessVectors = Stream.of(VulnerabilityAccessVector.values())
				.map(VulnerabilityAccessVector::name).collect(Collectors.toList());
	...
	@UsedAsLiferayAction
	public void removeVulnerability(ActionRequest request, ActionResponse response)
			throws IOException, PortletException {
		final RequestStatus requestStatus = deleteVulnerability(request, log);
		if (requestStatus == RequestStatus.SUCCESS) {
	...
	public RequestStatus getVulnerabilityByExternalId(ResourceRequest request, ResourceResponse response) {
		...
		VulnerabilityService.Iface vulnerabilityClient = thriftClients.makeVulnerabilityClient();
		try {
			Vulnerability vulnerabilityByExternalId = vulnerabilityClient
					.getVulnerabilityByExternalId(vulnerabilityExternalId, user);
	...
	public static void updateVulnerabilityFromRequest(PortletRequest request, Vulnerability vulnerability)
		...
		String[] externalComponentIdArray = request
				.getParameterValues(String.valueOf(Vulnerability._Fields.ASSIGNED_EXT_COMPONENT_IDS));
	...
	private static void setFieldValue(PortletRequest request, Vulnerability vulnerability,
			Vulnerability._Fields field) {
