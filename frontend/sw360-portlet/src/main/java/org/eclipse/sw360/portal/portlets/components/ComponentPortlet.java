...
@org.osgi.service.component.annotations.Component(immediate = true, properties = {
		...
		"/org/eclipse/sw360/portal/portlets/default.properties"}, property = {
				...
				"javax.portlet.display-name=Components", "javax.portlet.info.short-title=Components",
				"javax.portlet.info.title=Components", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/components/view.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class ComponentPortlet extends FossologyAwarePortlet {
	...
	private static final ImmutableList<Component._Fields> componentFilteredFields = ImmutableList.of(
			...
			Component._Fields.SOFTWARE_PLATFORMS, Component._Fields.OPERATING_SYSTEMS, Component._Fields.VENDOR_NAMES,
			Component._Fields.COMPONENT_TYPE, Component._Fields.MAIN_LICENSE_IDS, Component._Fields.CREATED_BY,
			Component._Fields.CREATED_ON, Component._Fields.BUSINESS_UNIT);
	...
	private void importBom(ResourceRequest request, ResourceResponse response) {
		...
		try {
			final RequestSummary requestSummary = componentClient.importBomFromAttachmentContent(user,
					attachmentContentId);
	...
	private void prepareImportBom(ResourceRequest request, ResourceResponse response) {
		...
		String attachmentContentId = request.getParameter(ATTACHMENT_CONTENT_ID);
		try {
			final ImportBomRequestPreparation importBomRequestPreparation = componentClient.prepareImportBom(user,
					attachmentContentId);
	...
	private void serveCheckComponentName(ResourceRequest request, ResourceResponse response) throws IOException {
		...
		if (cName != null && !cName.isEmpty()) {
			...
			Set<String> splitExtendedCName = splitCName.stream().map(LuceneAwareDatabaseConnector::prepareWildcardQuery)
			...
			try {
				...
				similarComponents.addAll(cClient.refineSearch(null, filterMap).stream()
						.limit(MAX_RESULT_LIMIT_CHECK_COMPONENT_NAME).collect(Collectors.toList()));
				...
				resultComponents = similarComponents.stream().distinct()
						.sorted(Comparator.comparing(Component::getName)).collect(Collectors.toList());
			} catch (TException e) {
	...
	private void respondSimilarComponentsResponseJson(ResourceRequest request, ResourceResponse response,
		...
		jsonGenerator.writeStringField("title", "Duplicate Component Check");
	...
	private void loadSpdxLicenseInfo(ResourceRequest request, ResourceResponse response) {
		...
		try {
			...
			attachmentType = release.getAttachments().stream()
					.filter(att -> attachmentContentId.equals(att.getAttachmentContentId()))
			...
			if (attachmentName.endsWith(PortalConstants.RDF_FILE_EXTENSION)) {
				if (AttachmentType.INITIAL_SCAN_REPORT.equals(attachmentType)) {
					...
					licenseToSrcFilesMap = licenseWithTexts.stream()
		...
		try {
			...
			if (concludedLicenseIds.size() > 0) {
				...
				jsonGenerator.writeArrayFieldStart(LICENSE_IDS);
				mainLicenseNames.forEach(licenseId -> wrapException(() -> {
					jsonGenerator.writeString(licenseId);
				}));
				jsonGenerator.writeEndArray();
			}
			...
			for (Map.Entry<String, Set<String>> entry : licenseToSrcFilesMap.entrySet()) {
				...
				entry.getValue().forEach(srcFile -> wrapException(() -> {
					jsonGenerator.writeString(srcFile);
				}));
				jsonGenerator.writeEndArray();
			}
			jsonGenerator.writeEndObject();
	...
	private void loadAssessmentSummaryInfo(ResourceRequest request, ResourceResponse response) {
		...
		try {
			...
			if (CommonUtils.isNullOrEmptyMap(assessmentSummaryMap)) {
				...
				jsonGenerator.writeStringField("msg",
						LanguageUtil.get(resourceBundle, "assessment.summary.information.is.not.present.in.CLI.file"));
			} else {
	...
	private void prepareReleaseDuplicate(RenderRequest request, RenderResponse response) throws PortletException {
		...
		try {
			...
			Release release = PortletUtils.cloneRelease(emailFromRequest,
					client.getAccessibleReleaseById(releaseId, user));
	...
	private void prepareComponentForMergeOrSplit(RenderRequest request, RenderResponse response, String pageName)
	...
	@UsedAsLiferayAction
	public void releaseMergeWizardStep(ActionRequest request, ActionResponse response)
			throws IOException, PortletException {
		int stepId = Integer.parseInt(request.getParameter("stepId"));
		try {
			HttpServletResponse httpServletResponse = PortalUtil.getHttpServletResponse(response);
	...
	private void generateReleaseMergeWizardStep0Response(ActionRequest request, JsonGenerator jsonGenerator)
			throws IOException, TException {
		User sessionUser = UserCacheHolder.getUserFromRequest(request);
	...
	private void generateReleaseMergeWizardStep1Response(ActionRequest request, JsonGenerator jsonGenerator)
			throws IOException, TException {
		User sessionUser = UserCacheHolder.getUserFromRequest(request);
		...
		// find matching source code attachment pair, otherwise we will not allow the
		// merge
		...
		for (Attachment attachment : nullToEmptySet(releaseTarget.getAttachments())) {
			if (attachment.getAttachmentType().equals(AttachmentType.SOURCE)
					|| attachment.getAttachmentType().equals(AttachmentType.SOURCE_SELF)) {
		...
		if (foundSourceAttachments && !matchingPair) {
			jsonGenerator.writeStartObject();
			jsonGenerator.writeStringField("error",
					"Both releases must have at least one pair of same source attachments or no source attachments at all. Otherwise a merge is not possible.");
			jsonGenerator.writeEndObject();
			return;
		}
		...
		addToMap(displayInformation, "repositorytype",
				releaseTarget.getRepository() != null ? releaseTarget.getRepository().getRepositorytype() : null);
		addToMap(displayInformation, "repositorytype",
				releaseSource.getRepository() != null ? releaseSource.getRepository().getRepositorytype() : null);
		...
		jsonGenerator.writeRaw("\"usageInformation\":"
				+ OBJECT_MAPPER.writeValueAsString(getUsageInformationForReleaseMerge(releaseSourceId, sessionUser)));
	...
	private Map<String, Integer> getUsageInformationForReleaseMerge(String releaseSourceId, User sessionUser)
			throws TException {
		...
		List<ReleaseVulnerabilityRelation> releaseVulnerabilities = vulnerabilityClient
				.getReleaseVulnerabilityRelationsByReleaseId(releaseSourceId, sessionUser);
		usageInformation.put("releaseVulnerabilities", releaseVulnerabilities.size());
		List<ProjectVulnerabilityRating> projectRatings = vulnerabilityClient
				.getProjectVulnerabilityRatingsByReleaseId(releaseSourceId, sessionUser);
	...
	private void generateReleaseMergeWizardStep2Response(ActionRequest request, JsonGenerator jsonGenerator)
			throws IOException, TException {
		Release releaseSelection = OBJECT_MAPPER.readValue(request.getParameter(RELEASE_SELECTION), Release.class);
		String releaseSourceId = request.getParameter(RELEASE_SOURCE_ID);
		// FIXME: maybe validate the component
		...
		// adding common title
		jsonGenerator
				.writeRaw("\"" + RELEASE_SELECTION + "\":" + JSON_THRIFT_SERIALIZER.toString(releaseSelection) + ",");
	...
	private void generateReleaseMergeWizardStep3Response(ActionRequest request, JsonGenerator jsonGenerator)
		...
		// perform the real merge, update merge target and delete merge source
		RequestStatus status = cClient.mergeReleases(releaseSelection.getId(), releaseSourceId, releaseSelection,
				sessionUser);
		...
		if (status == RequestStatus.IN_USE) {
			jsonGenerator.writeStringField("error",
					"Cannot merge when one of the releases has an active moderation request.");
		} else if (status == RequestStatus.ACCESS_DENIED) {
			jsonGenerator.writeStringField("error", "You do not have sufficient permissions.");
		} else if (status == RequestStatus.FAILURE) {
			jsonGenerator.writeStringField("error", "An unknown error occurred during merge.");
		...
		if (!isNullOrEmpty(id)) {
			try {
				...
				if (IS_COMPONENT_VISIBILITY_RESTRICTION_ENABLED) {
					...
					request.setAttribute(IS_USER_ALLOWED_TO_MERGE,
							PermissionUtils.isUserAtLeast(USER_ROLE_ALLOWED_TO_MERGE_OR_SPLIT_COMPONENT, user));
				}
				request.setAttribute(COMPONENT_VISIBILITY_RESTRICTION, IS_COMPONENT_VISIBILITY_RESTRICTION_ENABLED);
				...
				Set<UserGroup> allSecRoles = !CommonUtils.isNullOrEmptyMap(user.getSecondaryDepartmentsAndRoles())
						? user.getSecondaryDepartmentsAndRoles().entrySet().stream()
								.flatMap(entry -> entry.getValue().stream()).collect(Collectors.toSet())
						: new HashSet<UserGroup>();
				boolean isVulEditable = PermissionUtils.isUserAtLeast(UserGroup.SECURITY_ADMIN, user) || PermissionUtils
						.isUserAtLeastDesiredRoleInSecondaryGroup(UserGroup.SECURITY_ADMIN, allSecRoles);
	...
	private void prepareReleaseDetailView(RenderRequest request, RenderResponse response) throws PortletException {
		...
		try {
			...
			for (int i = 0; i < component.releases.size(); i++) {
				Collections.sort(component.releases,
						(release1, release2) -> release1.getVersion().compareTo(release2.getVersion()));
			}
	...
	private String createFossologyJobViewLink(ExternalToolProcessStep processStep,
		...
		if (!CommonUtils.isNullEmptyOrWhitespace(fossologyHostName) && !CommonUtils.isNullEmptyOrWhitespace(uploadId)
	...
	private void setSpdxAttachmentsInRequest(RenderRequest request, Release release) {
		...
		Set<AttachmentType> attTypes = attachments.stream().map(Attachment::getAttachmentType)
				.collect(Collectors.toUnmodifiableSet());
		Set<Attachment> spdxAttachments = Sets.newHashSet();
		if (attTypes.contains(AttachmentType.COMPONENT_LICENSE_INFO_COMBINED)
				|| attTypes.contains(AttachmentType.COMPONENT_LICENSE_INFO_XML)) {
	...
	private String formatedMessageForVul(List<VerificationStateInfo> infoHistory) {
		return CommonVulnerabilityPortletUtils.formatedMessageForVul(infoHistory, e -> e.getVerificationState().name(),
				e -> e.getCheckedOn(), e -> e.getCheckedBy(), e -> e.getComment(), e -> "");
	}

	private void putVulnerabilitiesInRequestRelease(RenderRequest request, String releaseId, User user,
			boolean isVulEditable) throws TException {
	...
	private void setUsingDocs(RenderRequest request, String releaseId, User user, ComponentService.Iface client)
		...
		if (releaseId != null) {
			...
			request.setAttribute(ALL_USING_PROJECTS_COUNT, allUsingProjectsCount);
			final Set<Component> usingComponentsForRelease = client
					.getUsingComponentsWithAccessibilityForRelease(releaseId, user);
	...
	private Map<String, Set<String>> getComponentFilterMap(PortletRequest request) {
		...
		for (Component._Fields filteredField : componentFilteredFields) {
			...
			if (!isNullOrEmpty(parameter) && !(filteredField.equals(Component._Fields.COMPONENT_TYPE)
				...
				if (filteredField.equals(Component._Fields.CREATED_ON)
						&& isNotNullEmptyOrWhitespace(request.getParameter(PortalConstants.DATE_RANGE))) {
					Date date = new Date();
	...
	private void prepareRequestForEditAfterDuplicateOrNamingError(ActionRequest request, Component component)
			throws TException {
		request.setAttribute(COMPONENT, component);
		setAttachmentsInRequest(request, component);
		request.setAttribute(USING_PROJECTS, Collections.emptySet());
	...
	public void applyFilters(ActionRequest request, ActionResponse response) throws PortletException, IOException {
		...
		response.setRenderParameter(PortalConstants.DATE_RANGE,
				nullToEmpty(request.getParameter(PortalConstants.DATE_RANGE)));
		response.setRenderParameter(PortalConstants.END_DATE,
				nullToEmpty(request.getParameter(PortalConstants.END_DATE)));
	}
	...
	private void updateVulnerabilityVerification(ResourceRequest request, ResourceResponse response)
		...
		try {
			...
			for (int i = 0; i < vulnerabilityIds.length; i++) {
				...
				ReleaseVulnerabilityRelation dbRelation = vulClient.getRelationByIds(releaseId, dbVulnerability.getId(),
						user);
	...
	private void serveComponentList(ResourceRequest request, ResourceResponse response) throws PortletException {
		...
		Map<String, Set<String>> filterMap = getComponentFilterMap(request);
		JSONArray jsonComponents = getComponentData(pageDataComponentList.values().iterator().next(),
				paginationParameters, filterMap);
	...
	private Map<PaginationData, List<Component>> getFilteredComponentList(PortletRequest request,
			...
			if (isAccessibleComponent) {
				...
				if (comp.isSetDefaultVendorId()) {
					...
					try {
						...
						log.error("Could not get vendor for id [" + comp.getDefaultVendorId()
								+ "] in component with id [" + comp.getId() + "] because of: ", e);
					}
