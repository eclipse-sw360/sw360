...
@org.osgi.service.component.annotations.Component(immediate = true, properties = {
		...
		"/org/eclipse/sw360/portal/portlets/default.properties"}, property = {
				...
				"javax.portlet.display-name=BDP Import", "javax.portlet.info.short-title=BDP Import",
				"javax.portlet.info.title=BDP Import", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/projects/import.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class BdpImportPortlet extends Sw360Portlet {
	...
	private boolean isImportSuccessful(ImportStatus importStatus) {
		return (importStatus.isSetRequestStatus() && importStatus.getRequestStatus().equals(RequestStatus.SUCCESS)
				&& importStatus.getFailedIds().isEmpty());
	}
		...
		try {
			...
			if (!isImportSuccessful(importStatus)) {
				if (importStatus.getRequestStatus().equals(RequestStatus.FAILURE)) {
					...
					log.error("Importing has not succeeded for the following IDs: "
							+ importStatus.getFailedIds().toString());
				}
			}
		} catch (TException e) {
	...
	public JSONObject handleRequestedAjaxAction(String requestedAction, ResourceRequest request,
			ResourceResponse response, LoginState loginState) throws IOException, PortletException {
	...
	private void importBdpProjects(User user, List<String> selectedIds, JSONObject responseData,
			RemoteCredentials remoteCredentials) throws PortletException, IOException {
	...
	void setNewImportSource(RemoteCredentials newCredentials, PortletSession session, JSONObject responseData,
		...
		if (serverUrl.isEmpty()) {
			loginState.logout();
			responseData.put(ProjectImportConstants.RESPONSE__STATUS, ProjectImportConstants.RESPONSE__DB_URL_NOT_SET);
		} else {
			putRemoteCredentialsIntoSession(session, newCredentials);
			responseData.put(ProjectImportConstants.RESPONSE__STATUS, ProjectImportConstants.RESPONSE__DB_CHANGED);
			responseData.put(ProjectImportConstants.RESPONSE__DB_URL, serverUrl);
			loginState.login(serverUrl);
		}
	}

	private void updateImportables(JSONObject responseData, LoginState loginState, RemoteCredentials remoteCredentials,
			String projectName) throws JsonProcessingException {
