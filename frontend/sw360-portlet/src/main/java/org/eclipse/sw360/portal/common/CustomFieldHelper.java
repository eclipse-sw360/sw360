...
public class CustomFieldHelper {
	...
	public static int loadAndStoreStickyViewSize(PortletRequest request, User user, String fieldName) {
		...
		if (isNullEmptyOrWhitespace(view_size)) {
			limit = CustomFieldHelper.loadField(Integer.class, request, user, fieldName).orElse(DEFAULT_VIEW_SIZE);
		} else {
	...
	public static <T extends Serializable> Optional<T> loadField(Class<T> type, PortletRequest request, User user,
			String field) {
		try {
			ExpandoBridge exp = getUserExpandoBridge(request, user);
	...
	public static Map<String, CustomField> getCustomFields(PortletRequest request, User user,
			CustomFieldPageIdentifier identifier) {
		try {
	...
	private static ExpandoBridge getUserExpandoBridge(PortletRequest request, User user)
		...
		ensureUserCustomFieldExists(liferayUser, CUSTOM_FIELD_VULNERABILITIES_VIEW_SIZE,
				ExpandoColumnConstants.INTEGER);
		ensureUserCustomFieldExists(liferayUser, CUSTOM_FIELD_PREFERRED_CLEARING_DATE_LIMIT,
				ExpandoColumnConstants.INTEGER);
		return liferayUser.getExpandoBridge();
	}

	private static void ensureUserCustomFieldExists(com.liferay.portal.kernel.model.User liferayUser,
			String customFieldName, int customFieldType) throws PortalException, SystemException {
		ExpandoBridge exp = liferayUser.getExpandoBridge();
		if (!exp.hasAttribute(customFieldName)) {
			...
			ExpandoColumn column = ExpandoColumnLocalServiceUtil.getColumn(companyId, exp.getClassName(),
			...
			for (String roleName : roleNames) {
				...
				if (role != null && column != null) {
					ResourcePermissionLocalServiceUtil.setResourcePermissions(companyId, ExpandoColumn.class.getName(),
							ResourceConstants.SCOPE_INDIVIDUAL, String.valueOf(column.getColumnId()), role.getRoleId(),
