 ...
 */
@Component(immediate = true, service = AutoLogin.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class SSOAutoLogin extends LoggingComponent implements AutoLogin {
	...
	protected void activate() {
		...
		log.info(String.format(
				"Expecting the following header values for auto login email: '%s', external ID: '%s', given name: '%s', surname: '%s', group: %s",
				AUTH_EMAIL_HEADER, AUTH_EXTID_HEADER, AUTH_GIVEN_NAME_HEADER, AUTH_SURNAME_HEADER,
	...
	public String[] login(HttpServletRequest request, HttpServletResponse response) throws AutoLoginException {
		...
		log.info(String.format(
				"Attempting auto login for email: '%s', external ID: '%s', given name: '%s', surname: '%s', group: %s",
		...
		try {
			...
			return new String[]{String.valueOf(user.getUserId()), user.getPassword(), // Encrypted Liferay password
	...
	private User updateLiferayUserEmailIfNecessary(String email, User user) throws PortalException, SystemException {
		if (!email.equals(user.getEmailAddress())) {
			user = UserLocalServiceUtil.updateEmailAddress(user.getUserId(), user.getPasswordUnencrypted(), email,
					email);
		}
		return user;
	}

	private User findOrCreateLiferayUser(HttpServletRequest request, String email, String extId, String givenName,
			String surname, long companyId, String organizationName) throws SystemException, PortalException {
		User user;
		try {
	...
	@NotNull
	public User createLiferayUser(HttpServletRequest request, String emailId, String extid, String givenName,
			String surname, long companyId, String organizationName) throws SystemException, PortalException {
		User user;
		String password = UUID.randomUUID().toString();

		user = UserPortletUtils.addLiferayUser(request, givenName, surname, emailId, organizationName,
				RoleConstants.USER, false, extid, password, false, true);
		if (user == null) {
			throw new AutoLoginException(
					"Couldn't create user for '" + emailId + "' and company id: '" + companyId + "'");
		}
