...
public class ModerationPortletUtils {
	...
	public static Integer loadPreferredClearingDateLimit(PortletRequest request, User user) {
		Integer limit = CustomFieldHelper
				.loadField(Integer.class, request, user, CUSTOM_FIELD_PREFERRED_CLEARING_DATE_LIMIT).orElse(0);
		...
		if (null != id) {
			...
			try {
				...
				if (CommonUtils.isNullEmptyOrWhitespace(clearingTeam)) {
					log.warn(
							"Invalid clearingTeam email: " + clearingTeam + " is entered, by user: " + user.getEmail());
					return requestSummary.setMessage("Invalid clearingTeam email");
				}
				clearingRequest.setClearingTeam(clearingTeam);
				String preferredClearingDate = request
						.getParameter(ClearingRequest._Fields.REQUESTED_CLEARING_DATE.toString());
				if (CommonUtils.isNotNullEmptyOrWhitespace(preferredClearingDate)
						&& !preferredClearingDate.equals(clearingRequest.getRequestedClearingDate())
				...
				if (CommonUtils.isNotNullEmptyOrWhitespace(isClearingExpertEdit)
						&& Boolean.parseBoolean(isClearingExpertEdit)) {
					...
					if (CommonUtils.isNotNullEmptyOrWhitespace(agreedDate)
							&& !agreedDate.equals(clearingRequest.getAgreedClearingDate())
							&& !SW360Utils.isValidDate(agreedDate, DateTimeFormatter.ISO_LOCAL_DATE, null)) {
						log.warn("Invalid agreed clearing date: " + agreedDate + " is entered, by user: "
								+ user.getEmail());
				...
				RequestStatus status = client.updateClearingRequest(clearingRequest, user,
						CommonUtils.nullToEmptyString(projectUrl));
	...
	private static AddDocumentRequestSummary reOpenClearingRequest(String id, PortletRequest request, User user) {
		...
		try {
			...
			String priority = (criticalCount > 1)
					? ""
					: request.getParameter(ClearingRequest._Fields.PRIORITY.toString());
			Integer dateLimit = ModerationPortletUtils.loadPreferredClearingDateLimit(request, user);
			dateLimit = (CommonUtils.isNotNullEmptyOrWhitespace(priority) && criticalCount < 2)
					? 0
					: (dateLimit < 1) ? 7 : dateLimit;
			if (!SW360Utils.isValidDate(preferredDate, DateTimeFormatter.ISO_LOCAL_DATE, Long.valueOf(dateLimit))) {
				log.warn("Invalid requested clearing date: " + preferredDate + " is entered, by user: "
						+ user.getEmail());
	...
	public static boolean isOpenModerationRequest(ModerationRequest mr) {
		return mr.getModerationState() == ModerationState.PENDING
				|| mr.getModerationState() == ModerationState.INPROGRESS;
	}

	public static boolean isClosedClearingRequest(ClearingRequest cr) {
		return cr.getClearingState() == ClearingRequestState.CLOSED
				|| cr.getClearingState() == ClearingRequestState.REJECTED;
	}
}
