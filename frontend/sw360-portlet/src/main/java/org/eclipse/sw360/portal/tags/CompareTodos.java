...
/**
 * Display the obligations that are added or added to/removed from department
 * whitelist
 *
...
public class CompareTodos extends NameSpaceAwareTag {
	...
	private void renderTodos(StringBuilder display, List<Obligation> current, List<Obligation> update,
			List<Obligation> delete) {
		List<Obligation> newWhitelistedTodos = update.stream().filter(CommonUtils::isTemporaryObligation)
		...
		renderTodoList(display, newWhitelistedTodosById, newWhitelistedTodoIds,
				"Add to database and to whitelist of " + department);

		List<Obligation> newBlacklistedTodos = update.stream().filter(CommonUtils::isTemporaryObligation)
				.filter(oblig -> !oblig.isSetWhitelist() || !oblig.getWhitelist().contains(department))
				.collect(Collectors.toList());
		...
		renderTodoList(display, newBlacklistedTodosById, newBlacklistedTodoIds,
				"Add to database and <em>not</em> to whitelist of " + department);

		Map<String, Obligation> currentTodosById = getTodosById(current);
		Set<String> currentTodoIds = currentTodosById.keySet();
		List<Obligation> whitelistedTodos = update.stream().filter(oblig -> !CommonUtils.isTemporaryObligation(oblig))
				.filter(oblig -> oblig.isSetWhitelist() && oblig.getWhitelist().contains(department))
		...
		List<Obligation> blacklistedTodos = delete.stream()
				.filter(oblig -> oblig.isSetWhitelist() && oblig.getWhitelist().contains(department))
				.filter(oblig -> currentTodoIds.contains(oblig.getId()))
				.filter(oblig -> currentTodosById.get(oblig.getId()).isSetWhitelist()
						&& currentTodosById.get(oblig.getId()).getWhitelist().contains(department))
	...
	private void renderTodoList(StringBuilder display, Map<String, Obligation> allTodos, Set<String> obligIds,
			String msg) {
		if (obligIds.isEmpty())
			return;
