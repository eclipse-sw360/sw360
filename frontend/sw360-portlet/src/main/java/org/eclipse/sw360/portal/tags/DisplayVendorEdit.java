...
public class DisplayVendorEdit extends NameSpaceAwareTag {
	...
	private void printEmptyVendor(StringBuilder display) {
		...
		printLabel(display);
		display.append(String.format("<input type=\"hidden\" readonly=\"\" value=\"\"  id=\"%s\" name=\"%s%s\"/>", id,
				namespace, id)).append("<div class=\"form-group has-feedback\">")
				.append(String.format(
						"<input type=\"text\" readonly=\"\" class=\"form-control edit-vendor clickable\" placeholder=\""
								+ LanguageUtil.get(resourceBundle, "click.to.set.vendor")
								+ "\" id=\"%sDisplay\" data-release-id=\"%s\"/>",
						id, releaseId))
	...
	private void printFullVendor(StringBuilder display, Vendor vendor) {
		display.append("<div class=\"form-group\">");
		printLabel(display);
		display.append(String.format("<input type=\"hidden\" readonly=\"\" value=\"%s\"  id=\"%s\" name=\"%s%s\"/>",
				vendor.getId(), id, namespace, id)).append("<div class=\"form-group has-feedback\">")
				.append(String.format(
