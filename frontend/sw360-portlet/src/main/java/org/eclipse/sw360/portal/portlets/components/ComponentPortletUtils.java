...
public abstract class ComponentPortletUtils {
		...
		for (Release._Fields field : Release._Fields.values()) {
			switch (field) {
				...
				case RELEASE_ID_TO_RELATIONSHIP :
					...
					break;
				case CLEARING_STATE :
					// skip setting CLEARING_STATE. it is supposed to be set only programmatically,
	...
	private static List<Component._Fields> extractFieldsForComponentUpdate(List<String> requestParams,
			Component component) {
		return component.getId() == null
				? Arrays.asList(Component._Fields.values())
				: Arrays.stream(Component._Fields.values()).filter(f -> requestParams.contains(f.name()))
	...
	private static void updateLinkedReleaseFromRequest(PortletRequest request,
			Map<String, ReleaseRelationship> linkedReleases) {
		linkedReleases.clear();
		String[] ids = request.getParameterValues(
				Release._Fields.RELEASE_ID_TO_RELATIONSHIP.toString() + ReleaseLink._Fields.ID.toString());
		String[] relations = request.getParameterValues(Release._Fields.RELEASE_ID_TO_RELATIONSHIP.toString()
				+ ReleaseLink._Fields.RELEASE_RELATIONSHIP.toString());
	...
	private static void setFieldValue(PortletRequest request, Repository repository, Repository._Fields field) {
		PortletUtils.setFieldValue(request, repository, field, Repository.metaDataMap.get(field),
				Release._Fields.REPOSITORY.toString());
	}

	private static void setFieldValue(PortletRequest request, ClearingInformation clearingInformation,
			ClearingInformation._Fields field) {
		PortletUtils.setFieldValue(request, clearingInformation, field, ClearingInformation.metaDataMap.get(field),
				Release._Fields.CLEARING_INFORMATION.toString());
	}

	private static void setFieldValue(PortletRequest request, EccInformation eccInformation,
			EccInformation._Fields field) {
		PortletUtils.setFieldValue(request, eccInformation, field, EccInformation.metaDataMap.get(field),
				Release._Fields.ECC_INFORMATION.toString());
	}

	private static void setFieldValue(PortletRequest request, COTSDetails cotsDetails, COTSDetails._Fields field) {
		PortletUtils.setFieldValue(request, cotsDetails, field, COTSDetails.metaDataMap.get(field),
				Release._Fields.COTS_DETAILS.toString());
	}
	...
	public static RequestStatus deleteRelease(PortletRequest request, Logger log) {
		...
		if (releaseId != null) {
			try {
				...
				RequestStatus deleteStatus = client.deleteRelease(releaseId,
						UserCacheHolder.getUserFromRequest(request));
				if (deleteStatus.equals(RequestStatus.SUCCESS)) {
					SW360Utils.removeReleaseVulnerabilityRelation(releaseId,
							UserCacheHolder.getUserFromRequest(request));
				}
				return deleteStatus;
	...
	public static ReleaseVulnerabilityRelation updateReleaseVulnerabilityRelationFromRequest(
			ReleaseVulnerabilityRelation dbRelation, ResourceRequest request) {
		...
		VerificationState verificationState = VerificationState
				.findByValue(Integer.parseInt(request.getParameter(PortalConstants.VULNERABILITY_VERIFICATION_VALUE)));
