...
@org.osgi.service.component.annotations.Component(immediate = true, properties = {
		...
		"/org/eclipse/sw360/portal/portlets/default.properties",}, property = {
				...
				"javax.portlet.display-name=Projects", "javax.portlet.info.short-title=Projects",
				"javax.portlet.info.title=Projects", "javax.portlet.resource-bundle=content.Language",
				"javax.portlet.init-param.view-template=/html/projects/view.jsp",}, service = Portlet.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
public class ProjectPortlet extends FossologyAwarePortlet {
	...
	private static final ImmutableList<Project._Fields> projectFilteredFields = ImmutableList.of(
			...
			Project._Fields.PROJECT_RESPONSIBLE, Project._Fields.NAME, Project._Fields.STATE,
			Project._Fields.CLEARING_STATE, Project._Fields.TAG, Project._Fields.ADDITIONAL_DATA);
	...
	private void serveViewDepartment(ResourceRequest request, ResourceResponse response)
			throws IOException, PortletException {
		...
		try {
			...
			if (CommonUtils.isNullEmptyOrWhitespace(topic)) {
				...
				throw new IllegalArgumentException(
						"Invalid obligation topic for project obligation id: " + obligationId);
			}
	...
	private void importBom(ResourceRequest request, ResourceResponse response) {
		...
		try {
			final RequestSummary requestSummary = projectClient.importBomFromAttachmentContent(user,
					attachmentContentId);
	...
	private void createClearingRequest(ResourceRequest request, ResourceResponse response) throws PortletException {
		...
		try {
			...
			dateLimit = (ClearingRequestPriority.CRITICAL.equals(clearingRequest.getPriority()) && criticalCount < 2)
					? 0
			...
			if (!SW360Utils.isValidDate(clearingRequest.getRequestedClearingDate(), DateTimeFormatter.ISO_LOCAL_DATE,
				...
				requestSummary = new AddDocumentRequestSummary().setRequestStatus(AddDocumentRequestStatus.FAILURE)
						.setMessage("Invalid requested clearing date");
			} else {
		...
		try {
			...
			if (!AddDocumentRequestStatus.SUCCESS.equals(requestSummary.getRequestStatus())) {
				ResourceBundle resourceBundle = ResourceBundleUtil.getBundle("content.Language", request.getLocale(),
						getClass());
				jsonGenerator.writeStringField(SW360Constants.MESSAGE,
						LanguageUtil.get(resourceBundle, requestSummary.getMessage().replace(' ', '.').toLowerCase()));
			} else {
	...
	private void showClearingRequest(ResourceRequest request, ResourceResponse response)
		...
		try {
			...
			log.error(
					"Error fetching clearing request " + clearingId + " for project: " + clearingRequest.getProjectId(),
					e);
			response.setProperty(ResourceResponse.HTTP_STATUS_CODE, "500");
		}
	}
	...
	private void saveAttachmentUsages(ResourceRequest request, ResourceResponse response) throws IOException {
		...
		try {
			...
			if (PermissionUtils.makePermission(project, user).isActionAllowed(RequestedAction.WRITE)) {
				...
				List<AttachmentUsage> allUsagesByProjectAfterCleanUp = attachmentClient
						.getUsedAttachments(Source.projectId(projectId), null);
				List<AttachmentUsage> usagesToCreate = selectedUsagesFromRequest.stream()
						.filter(usage -> allUsagesByProjectAfterCleanUp.stream().noneMatch(isUsageEquivalent(usage)))
						.collect(Collectors.toList());
	...
	private void downloadLicenseInfo(ResourceRequest request, ResourceResponse response) throws IOException {
		...
		boolean isOnlyApprovedAttachmentSelected = Boolean
				.parseBoolean(request.getParameter(PortalConstants.ONLY_APPROVED));
		String fileName = "";
		if (CommonUtils.isNotNullEmptyOrWhitespace(CLEARING_REPORT_TEMPLATE_TO_FILENAMEMAPPING)
				&& CommonUtils.isNotNullEmptyOrWhitespace(selectedTemplate)) {
			Map<String, String> tmplateToFileName = Arrays
					.stream(PortalConstants.CLEARING_REPORT_TEMPLATE_TO_FILENAMEMAPPING.split(","))
		...
		if (YES.equals(isEmptyFile)) {
			try {
				...
				log.error("Error getting empty licenseInfo file for project with id " + projectId + " and generator "
						+ outputGenerator, e);
				response.setProperty(ResourceResponse.HTTP_STATUS_CODE,
						Integer.toString(HttpServletResponse.SC_INTERNAL_SERVER_ERROR));
			}
		...
		boolean isLinkedProjectPresent = Boolean
				.parseBoolean(request.getParameter(PortalConstants.IS_LINKED_PROJECT_PRESENT));
		List<String> selectedReleaseRelationships = getSelectedReleaseRationships(request);
		String[] selectedAttachmentIdsWithPathArray = request
				.getParameterValues(PortalConstants.LICENSE_INFO_RELEASE_TO_ATTACHMENT);
		...
		final Set<String> listOfSelectedRelationshipsInString = listOfSelectedRelationships.stream()
				.map(ReleaseRelationship::name).collect(Collectors.toSet());

		String selectedProjectRelation = request.getParameter(PortalConstants.SELECTED_PROJECT_RELATIONS);
		List<String> selectedProjectRelationStrAsList = selectedProjectRelation == null
				? Lists.newArrayList()
		...
		Set<String> selectedAttachmentIdsWithPath = filterAttachmentSelectionOnProjectRelation(
				listOfSelectedProjectRelationships, selectedAttachmentIdsWithPathArray, user, project);

		Set<String> filteredSelectedAttachmentIdsWithPath = filterSelectedAttachmentIdsWithPath(
				selectedAttachmentIdsWithPath, listOfSelectedRelationshipsInString);
		...
		try {
			...
			saveSelectedReleaseAndProjectRelations(projectId, listOfSelectedRelationships,
					listOfSelectedProjectRelationships, isLinkedProjectPresent);
	...
	private void downloadEmptyLicenseInfo(ResourceRequest request, ResourceResponse response, Project project,
			User user, String outputGenerator, String fileName) throws TException, IOException {
		final LicenseInfoService.Iface licenseInfoClient = thriftClients.makeLicenseInfoClient();
		LicenseInfoFile licenseInfoFile = licenseInfoClient.getLicenseInfoFile(project, user, outputGenerator,
	...
	private void sendLicenseInfoResponse(ResourceRequest request, ResourceResponse response, Project project,
		...
		String documentVariant = licenseInfoFile.getOutputFormatInfo().getVariant() == OutputFormatVariant.DISCLOSURE
				? "LicenseInfo"
				: "ProjectClearingReport";
		...
		try {
			Function<String, UsageData> usageDataGenerator = attachmentContentId -> {
				...
				String projectPath = String.join(":",
						Arrays.copyOf(splittedAttachmentContentId, splittedAttachmentContentId.length - 4));
			...
			// there's no need to abort the user's desired action just because the ancillary
			// action of storing selection failed
	...
	private void exportExcel(ResourceRequest request, ResourceResponse response) {
		...
		try {
			...
			PortletResponseUtil.sendFile(request, response, filename, exporter.makeExcelExport(listOfProjects),
					CONTENT_TYPE_OPENXML_SPREADSHEET);
		} catch (IOException | TException e) {
			log.error("An error occurred while generating the Excel export", e);
			response.setProperty(ResourceResponse.HTTP_STATUS_CODE,
	...
	private void exportReleasesSpreadsheet(ResourceRequest request, ResourceResponse response) {
		...
		try {
			...
			if (project != null) {
				List<ReleaseClearingStatusData> releaseStringMap = client
						.getReleaseClearingStatusesWithAccessibility(id, user);
	...
	private void serveNewTableRowLinkedProjects(ResourceRequest request, ResourceResponse response, String[] linkedIds)
			throws IOException, PortletException {
		final User user = UserCacheHolder.getUserFromRequest(request);
	...
	private void serveNewTableRowLinkedRelease(ResourceRequest request, ResourceResponse response, String[] linkedIds)
			throws IOException, PortletException {
		final User user = UserCacheHolder.getUserFromRequest(request);
		request.setAttribute(IS_USER_AT_LEAST_CLEARING_ADMIN,
	...
	private void serveReleasesFromLinkedProjects(ResourceRequest request, ResourceResponse response, String projectId)
			throws IOException, PortletException {
		...
		try {
			...
			Map<String, ProjectProjectRelationship> linkedProjects = CommonUtils
	...
	private void serveAttachmentFileLicenses(ResourceRequest request, ResourceResponse response) throws IOException {
		...
		try {
			...
			licenseInfos
					.forEach(licenseInfoResult -> addLicenseInfoResultToJsonSerializableLicensesList(licenseInfoResult,
							licenses, licenseStore::put));
				...
				List<Map<String, String>> licensesAsObject = licenseNamesWithTexts.stream()
						...
						.map(licenseNameWithText -> {
							...
							data.put(LICENSE_NAME_WITH_TEXT_NAME,
									Strings.isNullOrEmpty(licenseNameWithText.getLicenseName())
											? EMPTY
	...
	private void prepareStandardView(RenderRequest request) throws IOException {
		...
		try {
			...
			String dateLimit = CommonUtils.nullToEmptyString(ModerationPortletUtils
					.loadPreferredClearingDateLimit(request, UserCacheHolder.getUserFromRequest(request)));
	...
	private Map<PaginationData, List<Project>> getFilteredProjectList(PortletRequest request, PaginationData pageData)
			throws IOException {
		final User user = UserCacheHolder.getUserFromRequest(request);
	...
	private Map<PaginationData, List<Project>> findProjectsByFiltersOrId(Map<String, Set<String>> filterMap, String id,
			User user, PaginationData pageData) {
		ProjectService.Iface projectClient = thriftClients.makeProjectClient();
		Map<PaginationData, List<Project>> ctToProjects = Maps.newHashMap();
		List<Project> projectList;
		try {
			if (!isNullOrEmpty(id)) { // the presence of the id signals to load linked projects hierarchy instead of
										// using filters
		...
		if (id != null) {
			try {
				...
				Collection<ProjectLink> links = allSubProjectLinks.stream().collect(
						Collectors.toMap(ProjectLink::getId, Function.identity(), (oldValue, newValue) -> oldValue))
						.values();
				...
				request.setAttribute(IS_USER_ADMIN,
						PermissionUtils.isUserAtLeast(UserGroup.SW360_ADMIN, user) ? YES : NO);
				...
				Set<String> groupsWithCrDisabled = Stream
						.of(PortalConstants.DISABLE_CLEARING_REQUEST_FOR_PROJECT_WITH_GROUPS.toLowerCase().split(","))
						.collect(Collectors.toSet());
				if (CommonUtils.isNotEmpty(groupsWithCrDisabled)
						&& groupsWithCrDisabled.contains(project.getBusinessUnit().toLowerCase())
	...
	private Map<String, ObligationStatusInfo> loadLicenseObligation(ResourceRequest request) {
		...
		licensesFromAttachmentUsage.entrySet().stream().forEach(entry -> wrapTException(() -> {
			...
			lic.getObligations().stream().filter(Objects::nonNull).forEach(obl -> {
				String keyofObl = CommonUtils.isNotNullEmptyOrWhitespace(obl.getTitle())
						? obl.getTitle()
						: obl.getText();
		...
		attachmentIdToReleaseMap.entrySet().stream().filter(entry -> entry.getKey() != null && entry.getValue() != null)
				.forEach(entry -> wrapTException(() -> {
					...
					Set<String> licenseIds = licenseInfoForAttachment.stream().filter(Objects::nonNull)
							...
							.map(licenseNamesWithTexts -> CommonUtils
									.isNotNullEmptyOrWhitespace(licenseNamesWithTexts.getLicenseSpdxId())
											? licenseNamesWithTexts.getLicenseSpdxId()
	...
	private void prepareLicenseInfo(RenderRequest request, RenderResponse response)
		...
		if (id != null) {
			try {
				...
				request.setAttribute(PortalConstants.PROJECT_RELEASE_TO_RELATION,
						fetchProjectReleaseToRelation(mappedProjectLinks));
				request.setAttribute(PortalConstants.PROJECT_USED_RELEASE_RELATIONS, fetchUsedReleaseRelationships(id));
				request.setAttribute(PortalConstants.LINKED_PROJECT_RELATION,
						fetchLinkedProjectRelations(mappedProjectLinks, id));
	...
	public static void addCustomErrorMessageForMissingAttchmnt(String errorMessage, RenderRequest request,
	 ...
	 *
	 * @param request
	 *            the request to store the paths map into
	 * @param mappedProjectLinks
	 *            the list of projectlinks which describe the project tree whose
	 *            paths map should be generated
	 */
	...
	private String formatedMessageForVul(List<VulnerabilityCheckStatus> statusHistory) {
		return CommonVulnerabilityPortletUtils.formatedMessageForVul(statusHistory,
				...
				e -> e.getComment(), e -> e.getProjectAction());
	}

	private boolean addToVulnerabilityRatings(Map<String, Map<String, String>> vulnerabilityIdToReleaseIdToAction,
	...
	private void putVulnerabilitiesMetadatasInRequest(PortletRequest request, List<VulnerabilityDTO> vuls,
		...
		for (VulnerabilityDTO vul : vuls) {
			...
			boolean wasAddedVulChecked = addToVulnerabilityRatings(vulnerabilityActions, vulnerabilityRatings,
					vulnerabilityTooltips, vulnerabilityIdToStatusHistory, vul);
		...
		try {
			...
			if (id != null) {
				...
				if (RequestStatus.SUCCESS.equals(requestStatus)
						&& CommonUtils.isNotNullEmptyOrWhitespace(request.getParameter(OBLIGATION_DATA))) {
					updateLinkedObligations(request, project, user, client);
				}
				if (RequestStatus.DUPLICATE.equals(requestStatus)
						|| RequestStatus.DUPLICATE_ATTACHMENT.equals(requestStatus)
	...
	private RequestStatus updateLinkedObligations(ActionRequest request, Project project, User user,
			ProjectService.Iface client) {
		try {
			final JsonNode rootNode = OBJECT_MAPPER.readTree(request.getParameter(OBLIGATION_DATA));
			final boolean isDeleteAllOrphanObligations = Boolean
					.valueOf(request.getParameter(DELETE_ALL_ORPHAN_OBLIGATIONS));
			...
			Map<String, ObligationStatusInfo> obligationStatusInfo = isObligationPresent
					&& obligation.getLinkedObligationStatusSize() > 0
							? obligation.getLinkedObligationStatus()
							: Maps.newHashMap();
			...
			return isObligationPresent
					? client.updateLinkedObligations(obligation, user)
	...
	private void copyAttachmentUsagesForClonedProject(ActionRequest request, String sourceProjectId,
			String newProjectId) throws TException, PortletException {
		try {
			AttachmentService.Iface attachmentClient = thriftClients.makeAttachmentClient();
	...
	private String getProjectDefaultLicenseInfoHeaderText(String group) {
		...
		try {
			String fileName = "";
			if (CommonUtils
					.isNotNullEmptyOrWhitespace(PortalConstants.LICENSE_INFO_HEADER_TEXT_FILE_NAME_BY_PROJECT_GROUP)) {
				Map<String, String> groupToFileName = Arrays
						.stream(PortalConstants.LICENSE_INFO_HEADER_TEXT_FILE_NAME_BY_PROJECT_GROUP.split(","))
						.collect(Collectors.toMap(k -> k.split(":")[0].trim().toUpperCase(), v -> v.split(":")[1],
	...
	private ObligationList loadLinkedObligations(PortletRequest request) {
		...
		try {
			...
			obligation.setLinkedObligationStatus(setLicenseInfoWithObligations(request, obligationStatusMap,
					releaseIdToAcceptedCLI, releases, user));
		} catch (TException e) {
	...
	private Map<String, ObligationStatusInfo> setLicenseInfoWithObligations(PortletRequest request,
		...
		try {
			...
			request.setAttribute(PROJECT_OBLIGATIONS_INFO_BY_RELEASE,
					filterAndSortLicenseInfo(licenseObligation.getLicenseInfoResults()));
		} catch (TException e) {
	...
	private int getObligationsFromReadmeOSSCount(Map<String, ObligationStatusInfo> obligationStatusMap) {
		if (CommonUtils.isNotEmpty(obligationStatusMap.keySet())) {
			return Math.toIntExact(obligationStatusMap.values().stream()
					.filter(obligation -> obligation.getObligationLevel() == null).count());
		}
		return 0;
	}
	...
	private void addLicenseToLinkedReleases(ResourceRequest request, ResourceResponse response)
			throws IOException, PortletException {
		ProjectService.Iface client = thriftClients.makeProjectClient();
		ComponentService.Iface componentClient = thriftClients.makeComponentClient();
		LicenseInfoService.Iface licenseInfoClient = thriftClients.makeLicenseInfoClient();
		...
		try {
			...
			Predicate<LicenseInfoParsingResult> filterConcludedLicense = result -> (null != result.getLicenseInfo()
					&& null != result.getLicenseInfo().getConcludedLicenseIds());

			for (String releaseId : releaseIds) {
				Release release = componentClient.getReleaseById(releaseId, user);
				List<Attachment> filteredAttachments = SW360Utils.getApprovedClxAttachmentForRelease(release);
				Set<String> mainLicenses = release.getMainLicenseIds() == null
						? new HashSet<String>()
						: release.getMainLicenseIds();
				Set<String> otherLicenses = release.getOtherLicenseIds() == null
						? new HashSet<String>()
				...
				if (filteredAttachments.size() == 1) {
					...
					oneCLI.add(release);
					List<LicenseInfoParsingResult> licenseInfoResult = licenseInfoClient
							.getLicenseInfoForAttachment(release, attachment.getAttachmentContentId(), true, user);
					List<LicenseNameWithText> licenseWithTexts = licenseInfoResult.stream().filter(filterLicenseResult)
							...
							.filter(license -> !license.getLicenseName()
									...
									&& !license.getLicenseName().equalsIgnoreCase(SW360Constants.NO_ASSERTION)) // exclude
																												// unknown,
																												// n/a
																												// and
																												// noassertion
					...
					if (attachmentName.endsWith(PortalConstants.RDF_FILE_EXTENSION)) {
						mainLicenses.addAll(licenseInfoResult.stream().filter(filterConcludedLicense)
								.flatMap(
										singleResult -> singleResult.getLicenseInfo().getConcludedLicenseIds().stream())
								.collect(Collectors.toSet()));
						otherLicenses.addAll(licenseWithTexts.stream().map(LicenseNameWithText::getLicenseName)
								.collect(Collectors.toSet()));
						otherLicenses.removeAll(mainLicenses);
					} else if (attachmentName.endsWith(PortalConstants.XML_FILE_EXTENSION)) {
						mainLicenses.addAll(licenseWithTexts.stream().filter(filterLicense)
								.map(LicenseNameWithText::getLicenseName).collect(Collectors.toSet()));
						otherLicenses.addAll(licenseWithTexts.stream().filter(filterLicense.negate())
								.map(LicenseNameWithText::getLicenseName).collect(Collectors.toSet()));
					}
				} else {
			...
			log.error(String.format("Error while adding license info to linked releases for project: %s ", projectId),
					e);
		}
		try {
	...
	private void loadSpdxLicenseInfo(ResourceRequest request, ResourceResponse response)
		...
		final LicenseInfoService.Iface licenseInfoClient = thriftClients.makeLicenseInfoClient();
		final JSONObject jsonResult = createJSONObject();
		final ResourceBundle resourceBundle = ResourceBundleUtil.getBundle("content.Language", request.getLocale(),
				getClass());
		final Predicate<Attachment> isISR = attachment -> AttachmentType.INITIAL_SCAN_REPORT
				.equals(attachment.getAttachmentType());
		...
		try {
			...
			if (attachments.size() == 1) {
				...
				List<LicenseInfoParsingResult> licenseInfoResult = licenseInfoClient
						.getLicenseInfoForAttachment(release, attachmentContentId, true, user);
				List<LicenseNameWithText> licenseWithTexts = licenseInfoResult.stream().filter(filterLicenseResult)
						.flatMap(result -> result.getLicenseInfo().getLicenseNamesWithTexts().stream())
						.filter(license -> !license.getLicenseName()
								.equalsIgnoreCase(SW360Constants.LICENSE_NAME_UNKNOWN)
				...
				if (attachmentName.endsWith(PortalConstants.RDF_FILE_EXTENSION)) {
					totalFileCount = licenseInfoResult.stream()
							.flatMap(result -> result.getLicenseInfo().getLicenseNamesWithTexts().stream())
							.map(LicenseNameWithText::getSourceFiles).filter(Objects::nonNull).flatMap(Set::stream)
							.collect(Collectors.toSet()).size();
					concludedLicenseIds = licenseInfoResult.stream().filter(filterConcludedLicense)
				...
				try {
					...
					if (concludedLicenseIds.size() > 0) {
						jsonGenerator.writeStringField(LICENSE_PREFIX,
								LanguageUtil.get(resourceBundle, "concluded.license.ids"));
						jsonGenerator.writeArrayFieldStart(LICENSE_IDS);
						concludedLicenseIds.forEach(licenseId -> wrapException(() -> {
							jsonGenerator.writeString(licenseId);
						}));
						jsonGenerator.writeEndArray();
					}
					jsonGenerator.writeStringField("otherLicense",
							LanguageUtil.get(resourceBundle, "other.license.ids"));
					jsonGenerator.writeArrayFieldStart("otherLicenseIds");
					otherLicenseNames.forEach(licenseId -> wrapException(() -> {
						jsonGenerator.writeString(licenseId);
					}));
					jsonGenerator.writeEndArray();
					if (AttachmentType.INITIAL_SCAN_REPORT.equals(attachmentType)) {
						jsonGenerator.writeStringField(LICENSE_PREFIX,
								LanguageUtil.get(resourceBundle, "possible.main.license.ids"));
					...
					log.error("Cannot write JSON response for attachment id " + attachmentContentId + " in release "
							+ releaseId + ".", e);
					response.setProperty(ResourceResponse.HTTP_STATUS_CODE, "500");
				}
			} else {
				...
				if (attachments.size() > 1) {
					jsonGenerator.writeStringField(SW360Constants.MESSAGE,
							LanguageUtil.get(resourceBundle, "multiple.isr.are.found.in.the.release"));
				} else if (attachments.isEmpty()) {
					jsonGenerator.writeStringField(SW360Constants.MESSAGE,
							LanguageUtil.get(resourceBundle, "isr.attachment.not.found.in.the.release"));
				} else {
					jsonGenerator.writeStringField(SW360Constants.MESSAGE,
							LanguageUtil.get(resourceBundle, "license.information.not.found.in.isr"));
				}
				jsonGenerator.writeEndObject();
				jsonGenerator.close();
			}
	...
	private void serveProjectList(ResourceRequest request, ResourceResponse response)
		...
		JSONObject jsonResult = createJSONObject();
		Map<String, Set<String>> filterMap = loadFilterMapFromRequest(request);
		int count = (int) (filterMap.isEmpty()
				? getAccessibleProjectsCount(request)
	...
	public JSONArray getProjectData(List<Project> projectList, PaginationParameters projectParameters,
		...
		for (int i = start; i < count; i++) {
			...
			if (isNotEmpty && groupsWithCrDisabled.contains(project.getBusinessUnit().toLowerCase())
					&& Objects.isNull(project.getClearingRequestId())) {
	...
	private void setDefaultRequestAttributes(RenderRequest request, String group) {
		request.setAttribute(DOCUMENT_TYPE, SW360Constants.TYPE_PROJECT);
		request.setAttribute(DEFAULT_LICENSE_INFO_HEADER_TEXT,
				getProjectDefaultLicenseInfoHeaderText(CommonUtils.nullToEmptyString(group)));
			...
			for (String linkedId : projectIds) {
				...
				try {
					...
					if (project.isSetLinkedProjects()
							&& project.getLinkedProjects().keySet().contains(sourceProjectId)) {
					...
					project.putToLinkedProjects(sourceProjectId,
							new ProjectProjectRelationship(ProjectRelationship.CONTAINED));
					String cyclicLinkedProjectPath = client.getCyclicLinkedProjectPath(project, user);
					if (!isNullEmptyOrWhitespace(cyclicLinkedProjectPath)) {
					...
					if (RequestStatus.SUCCESS.equals(status)) {
						...
						jsonObject = buildResponse(srcProject, project,
								srcProject.getName() + " can not be linked to " + project.getName(), false, true);
					}
				} catch (SW360Exception sw360Exp) {
					if (sw360Exp.getErrorCode() == 403) {
						jsonObject = buildResponse(srcProject, project,
								"Project or its Linked Projects are restricted and / or not accessible.", false, true);
		...
		try {
			...
			if (RequestStatus.SENT_TO_MODERATOR.equals(status)) {
				...
				jsonObject.put("message",
						"Moderation request sending failed to update the Project " + project.getName());
			}
			writeJSON(request, response, jsonObject);
