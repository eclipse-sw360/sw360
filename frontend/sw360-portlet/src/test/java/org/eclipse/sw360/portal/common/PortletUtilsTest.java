...
public class PortletUtilsTest {
	...
	public void testUpdateAttachmentsUncheckRemovesCheckedFields() {
		...
		// run test
		Set<Attachment> updatedAttachments = PortletUtils.updateAttachmentsFromRequest(request,
				Sets.newHashSet(attachment));
	...
	protected static void setAttachmentIds(PortletRequest request, String... ids) {
		Mockito.when(request.getParameterValues(
	...
	protected static User createUser(String id, String email, String department) {
		return new User().setId(id).setEmail(email).setDepartment(department).setExternalid("").setLastname("")
				.setGivenname("");
	}
}
