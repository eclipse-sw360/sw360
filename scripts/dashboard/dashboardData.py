...
output_folder = f"/{base_folder}/frontend/sw360-portlet/src/main/resources/META-INF/resources/html/dashboard/data"
...
def formatForTimeSeries(result, doc):
    upd_result = []
...
def queryExecutionComponentByType(db):
        ...
        #Existing View
        result = fetchResults(db, design_doc, "bycomponenttype")
...
def queryExecutionCountAll(db):
        ...
        design_doc_arr = {
            "Components": "_design/Component",
            "Projects": "_design/Project",
            "Releases": "_design/Release"
        }

        for key, design_doc in design_doc_arr.items():
            item_count = len(fetchResults(db, design_doc, "all"))
            lengthArr.append({"key": key, "value": item_count})

        data = [{"type": entry["key"], "value": entry["value"]} for entry in lengthArr]
        field_names = ["type", "value"]
...
def queryCompProjRelTimeSeriesExecution(db):
    ...
    design_doc_arr = {
            "Component": "_design/Component",
            "Project": "_design/Project",
            "Release": "_design/Release"
        }
    combined_data = {}
    for key, design_doc in design_doc_arr.items():
            result = fetchResults(db, design_doc, "byCreatedOn")
            data = formatForTimeSeries(result, key)
            for dataObj in data:
                year = dataObj["Year"]
                if year not in combined_data:
                    combined_data[year] = {"Year": year}
                combined_data[year].update(dataObj)
...
def getCLIList(db,dbattach):
    ...
    design_doc = "_design/Project"
    view = "byReleaseAndBusinessUnit"
    map_function = {
    "map": "function(doc) {  if (doc.type == 'project') {  {   for(var i in doc.releaseIdToUsage) {      emit(i, {businessUnit:doc.tag, group:doc.businessUnit, id:doc._id});  }  }  }}"
    }
    ...
    result = fetchResults(db, design_doc, "byReleaseAndBusinessUnit")
    ...
    for item in list(result):
        if "value" in item and "businessUnit" in item["value"]:
            ...
            unique_key = (key, group)  # Combining key and business unit for uniqueness
...
def queryExecutionReleasesECCClearedStatuss(db):
...
def queryExecutionReleasesECCClearedStatus(db):
        ...
        view = "byECCStatus"
        map_function = {
        'map': "function(doc) {  if (doc.type == 'release') {  emit(doc.componentId, doc.eccInformation.eccStatus) }}"
        }

        #Creating temporary view byECCStatus for release
        saveNewView(db, design_doc, view, map_function)
        
        result = list(fetchResults(db, design_doc, "byECCStatus"))
        ...
        merged_documents = [
            {"id": doc["key"], "status": doc["value"], "type": comp_lookup[doc["key"]]["componentType"] if doc["key"] in comp_lookup else None}
            for doc in result
        ]
...
def queryExecutionMostUsedComp(db):
    ...
    design_doc = "_design/Project"
    #Existing view
    result = list(fetchResults(db, design_doc, "byreleaseid"))
    id_list = [row.key for row in result]
...
def queryExecutionMostUsedLicenses(db):
    ...
    map_function = {
        'map': "function(doc) {    if (doc.type == 'component') {      if(doc.mainLicenseIds) { for(var i in doc.mainLicenseIds){           emit(doc.mainLicenseIds[i], doc._id);      }} else {            emit('EMPTY', doc._id);      }   } }"
        }
    
    #Creating temporary view bymainLicenseIdArr for Component
    saveNewView(db, design_doc, view, map_function)
    
    result = fetchResults(db, design_doc, "bymainLicenseIdArr")
...
def run():
    ...
    getCLIList(db, dbAttach)
    queryExecutionReleasesECCClearedStatus(db)
    queryExecutionAttachmentUsageAll(dbAttach)
