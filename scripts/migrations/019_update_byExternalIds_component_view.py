...
new_value = "function(doc) {  if (doc.type == 'component') {    for (var externalId in doc.externalIds) {      try {            var values = JSON.parse(doc.externalIds[externalId]);            for (var idx in values) {              emit( [externalId, values[idx]], doc._id);            }      } catch(error) {          emit( [externalId, doc.externalIds[externalId]], doc._id);      }    }  }}"
