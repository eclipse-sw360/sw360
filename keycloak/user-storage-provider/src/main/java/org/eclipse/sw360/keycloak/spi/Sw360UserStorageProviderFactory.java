...
public class Sw360UserStorageProviderFactory implements UserStorageProviderFactory<Sw360UserStorageProvider>, ImportSynchronization {
    ...
    private static final String DEFAULT_USERNAME = "username.unknown";
    ...
    public void init(Config.Scope config) {
        ...
        if (thriftUrl != null && !thriftUrl.isEmpty()) {
            ...
            logger.warn("No 'thriftServerUrl' found in config, using default: '{}'", Sw360UserService.thriftServerUrl);
        }
    }
   ...
   private Set<String> getExistingUserEmails(KeycloakSessionFactory sessionFactory, String realmId) {
    ...
    private void populateUserAttributes(UserModel user, RealmModel realm, User externalUser, UserGroup externalUserUserGroup) {
        ...
        user.setUsername(
                Optional.ofNullable(externalUser.getFullname())
                        .filter(StringUtils::isNotBlank)
                        .orElseGet(() -> {
                            logger.warn("Full name is null or empty for user: {}, assigning default username.", externalUser.getEmail());
