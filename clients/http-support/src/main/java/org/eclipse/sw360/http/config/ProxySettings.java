...
public final class ProxySettings {
	/**
	 * Constant representing an undefined proxy host. If this value is set for the
	 * host, this is interpreted as the default proxy selector.
	 */
	...
	/**
	 * Creates a new instance of {@code ProxySettings} with the proxy parameters
	 * specified.
	 *
	 * @param proxyHost
	 *            the proxy host
	...
	/**
	 * Returns an instance of {@code ProxySettings} that indicates that no proxy is
	 * to be used.
	 *
	...
	/**
	 * Returns an instance of {@code ProxySettings} that indicates that the default
	 * {@code ProxySelector} (obtained via the {@code ProxySelector.getDefault()}
	 * method) should be queried for each request. In this mode, the usage of
	 * proxies can be enabled or disabled on a per URL basis.
	 *
	 ...
	 * configuration settings. In the configuration, it can be stated explicitly
	 * whether a proxy is to be used or not. So it is possible that valid settings
	 * for the proxy host and port are provided, but the resulting settings should
	 * nevertheless refer to an undefined proxy. If proxy usage is explicitly
	 * enabled, but no valid proxy host or port are configured, the resulting
	 * settings point to the default proxy selector.
	 *
	 * @param useProxy
	 *            flag whether a proxy should be used
	 * @param host
	 *            the proxy host (may be undefined)
	 * @param port
	 *            the proxy port (may be undefined)
	 * @return the new {@code ProxySettings} instance
	 */
	public static ProxySettings fromConfig(boolean useProxy, String host, int port) {
		...
		return host != null && !UNDEFINED_HOST.equals(host) && port > 0 ? useProxy(host, port) : defaultProxySelector();
	}
	 ...
	 *         should be used; <strong>false</strong> for a direct Internet
	 *         connection
	 */
	...
	/**
	 * Returns a flag whether the default proxy selector should be used. In this
	 * mode, from the {@code ProxySelector.getDefault()} method the default proxy
	 * selector is obtained. This selector is then called for each request to obtain
	 * a proxy on a per URL basis.
	 *
	 * @return <strong>true</strong> if the default proxy selector should be used;
	...
	/**
	 * Returns a flag whether a proxy should be completely disabled. In this mode, a
	 * direct Internet connection is enforced.
	 *
	...
	public boolean equals(Object o) {
		...
		return getProxyPort() == settings.getProxyPort() && Objects.equals(getProxyHost(), settings.getProxyHost());
	}
	...
	public String toString() {
		return "ProxySettings{" + ", proxyHost='" + proxyHost + '\'' + ", proxyPort=" + proxyPort + '}';
	}
