 ...
 * </p>
 * <p>
 * This exception class stores the response status code, so that it is available
 * for further investigation. A cause is not available because the request
 * completed normally. In order to identify the request that failed, a tag can
 * be provided that is added to the exception message. This is useful if a
 * future is constructed from the results of multiple requests.
 * </p>
 */
public class FailedRequestException extends IOException {
	...
	/**
	 * Creates a new instance of {@code FailedRequestException} and initializes it
	 * with a tag and the status code that caused the exception.
	 *
	 * @param tag
	 *            a tag to identify the failed request
	 * @param statusCode
	...
	/**
	 * Creates a new instance of {@code FailedRequestException} and initializes it
	 * with a tag, the error status code, and the message sent from the server as
	...
	/**
	 * Generates a message for this exception based on the parameters passed in.
	 *
	 * @param tag
	 *            a tag to identify the failed request
	 * @param statusCode
	...
	private static String generateMessage(String tag, int statusCode, String serverMessage) {
		...
		if (serverMessage != null && !serverMessage.isEmpty()) {
			buf.append(" Message from the server:").append(System.lineSeparator()).append('"').append(serverMessage)
					.append('"');
		}
		return buf.toString();
	}
