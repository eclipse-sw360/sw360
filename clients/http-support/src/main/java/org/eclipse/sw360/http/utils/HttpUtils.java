...
public final class HttpUtils {
	 ...
	 * {@link #checkResponse(ResponseProcessor, Predicate)} method to check whether
	 * a response was successful. This predicate returns <strong>true</strong> if
	 * and only if the passed in {@code Response} reports itself as successful.
	 *
	...
	/**
	 * Blocks until the given future is completed, returns its result, and handles
	 * occurring exceptions. {@code HttpClient} has an asynchronous API, but this
	 * method offers an easy way to transform this to a blocking programming model
	 * by just waiting for the result to become available. To simplify exception
	 * handling for the caller, the various checked exceptions thrown by
	 * {@code Future.get()} are wrapped into {@code IOException} exceptions.
	 *
	 * @param future
	 ...
	 * @throws IOException
	 *             if the future failed or waiting was interrupted
	 */
	 ...
	 * consistent. They are sometimes wrapped in a {@code CompletionException} and
	 * sometimes not. This method can be used to obtain the actual cause of a
	 * failure by removing all enclosing {@code CompletionException} instances.
	 *
	 * @param ex
	 *            the exception to be unwrapped
	 * @return the unwrapped exception or <strong>null</strong> if none can be found
	 */
	 ...
	 * successful response. This is the case if the status code is in the range of
	 * [200, 300).
	 *
	 * @param status
	 ...
	 * successful based on a given predicate and allows tagging the request. The
	 * resulting processor invokes the given predicate on the response received from
	 * the server. If the predicate yields <strong>false</strong>, a
	 * {@link FailedRequestException} is thrown that is initialized with the tag and
	 * the response status code. Otherwise, the original {@code ResponseProcessor}
	 * is invoked, which can now safely generate its result.
	 *
	 * @param processor
	 *            the {@code ResponseProcessor} to wrap
	 * @param successPredicate
	 *            a predicate to determine whether the response is successful
	 * @param tag
	 *            a tag to identify the request
	 * @param <T>
	 *            the result type of the {@code ResponseProcessor}
	...
	public static <T> ResponseProcessor<T> checkResponse(ResponseProcessor<T> processor,
			Predicate<Response> successPredicate, String tag) {
		return response -> {
	...
	/**
	 * Returns a {@code ResponseProcessor} that checks whether a request was
	 * successful based on a given predicate. This variant does not add a tag to the
	 * request.
	 *
	 * @param processor
	 *            the {@code ResponseProcessor} to wrap
	 ...
	 * determine whether a request was successful and allows tagging the request.
	 * This method is equivalent to the overloaded {@code checkResponse()} method
	 * using {@link #SUCCESS_STATUS} as predicate.
	 *
	 * @param processor
	 *            the {@code ResponseProcessor} to wrap
	 * @param tag
	...
	/**
	 * Returns a {@code ResponseProcessor} that checks the HTTP status code to
	 * determine whether a request was successful. This method is equivalent to the
	 * overloaded {@code checkResponse()} method using {@link #SUCCESS_STATUS} as
	 * predicate. This variant does not add a tag to the request.
	 *
	 * @param processor
	 *            the {@code ResponseProcessor} to wrap
	 * @param <T>
	...
	/**
	 * Returns a predicate that checks a response for a specific status code. The
	 * response is considered successful if and only if the status code matches
	...
	/**
	 * Creates a {@code FailedRequestException} based on the passed in response and
	 * request tag. The properties of the exception are initialized accordingly. The
	 * response entity is read as well and stored in the exception.
	 *
	 * @param response
	 ...
	 * specified to map the JSON payload of a response to an object of the given
	 * result class. The resulting processor directly accesses the payload of the
	 * response; it can be combined with one of the {@code checkResponse()} methods
	 * to make sure that the response is successful before it is processed.
	 *
	 * @param mapper
	 *            the JSON mapper
	 * @param resultClass
	...
	/**
	 * Returns a {@code ResponseProcessor} that uses the {@code ObjectMapper}
	 * specified to map the JSON payload of a response to an object of the type
	 * defined by the given reference. This is analogous to the overloaded method,
	 * but allows for more flexibility to specify the result type.
	 *
	 * @param mapper
	 *            the JSON mapper
	 * @param typeReference
	 *            the reference defining the target type
	 * @param <T>
	 *            the type of the resulting object
	 * @return the {@code ResponseProcessor} doing a JSON de-serialization
	 */
	...
	/**
	 * Returns a very simple {@code Consumer} for a {@code RequestBuilder} that just
	 * configures the builder with the given URI. This causes an HTTP GET request to
	 * this URI without further properties.
	 *
	 * @param uri
	...
	/**
	 * Returns a special {@code ResponseProcessor} that does not do any processing,
	 * but just returns the value <strong>null</strong>. Such a processor can be
	 * useful for requests that do not return a response body, e.g. POST requests to
	 * create or manipulate entities. When combined with a processor produced by one
	 * of the {@code checkResponse()} methods it is possible to have some error
	 * handling, but skip the evaluation of the response body.
	 *
	 ...
	 * @return the encoded string (<strong>null</strong> if the input string was
	 *         <strong>null</strong>)
	 */
	 ...
	 *            the URL
	 * @param params
	 *            a map with the query parameters to append
	 * @return the resulting URL string with query parameters
	 * @throws NullPointerException
	 *             if the URL or the map with parameters is <strong>null</strong>
	...
	/**
	 * Adds the given query parameters to a URL. Each parameter value is encoded.
	 * The parameters are appended to the URL string using the correct separator
	 * characters. If the {@code filterUndefined} flag is set to
	 * <strong>true</strong>, parameters having a null or empty string value are
	 * filtered out.
	 *
	 * @param url
	 *            the URL
	 ...
	 *            flag whether undefined parameters should be ignored
	 * @return the resulting URL string with query parameters
	 * @throws NullPointerException
	 *             if the URL or the map with parameters is <strong>null</strong>
	 */
	...
	/**
	 * Adds a single query parameter to a URL. This is a convenience function for
	 * the case that there is only a single query parameter needed. Note that this
	 * method does not filter out undefined parameters.
	 *
	 * @param url
	 *            the URL
	 * @param key
	 ...
	 * @throws NullPointerException
	 *             if the URL is <strong>null</strong>
	 */
	...
	/**
	 * Wraps the given exception into an {@code IOException} if necessary. If the
	 * exception is already an {@code IOException}, it is returned directly.
	 *
	 * @param e
	...
	/**
	 * Checks whether the value of a parameter is defined. This is used to filter
	 * out undefined query parameters.
	 *
	 * @param value
	 *            the value to check
