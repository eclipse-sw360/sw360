...
public class HttpClientFactoryImpl implements HttpClientFactory {
	...
	private static OkHttpClient createClient(HttpClientConfig config) {
		...
		if (!config.proxySettings().isDefaultProxySelectorUse()) {
			Proxy proxy = config.proxySettings().isNoProxy() ? Proxy.NO_PROXY : createProxy(config.proxySettings());
			builder.proxy(proxy);
		}
	...
	/**
	 * Using the Property CLIENT_ACCESS_UNVERIFIED_PROPERTY, the connection to the
	 * client can be done without verification of the ssl certificate
	 *
