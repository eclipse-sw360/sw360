 ...
 * checks that guarantee that the interfaces are consistent. So this is verified
 * manually by this test. The test compares the asynchronous and synchronous
 * interfaces of the different client adapters. The methods in the asynchronous
 * adapters returning a future are iterated over, and it is checked that there
 * is a corresponding method in the synchronous interface with the same name,
 * method parameters, and return type.
 */
public class SyncClientAdaptersTest {
	/**
	 * Verifies that a synchronous client adapter interface is consistent with its
	 * asynchronous counterpart. All asynchronous methods are checked whether they
	 * have a correct synchronous counterpart.
	 *
	 * @param asyncIfc
	 *            the class of the asynchronous interface
	 * @param syncIfc
	 *            the class for the synchronous interface
	 */
	 ...
	 * corresponds to the given asynchronous method. The method must have the same
	 * name and parameter types, but instead of a future, it must return the generic
	 * type of the future (which can itself be a generic type). By checking the
	 * actual values of type variables in generic types, it can be verified that the
	 * return types actually are compliant with each other.
	 *
	 * @param syncIfc
	 *            the class for the synchronous interface
	 * @param method
	...
	private static void checkMethod(Class<?> syncIfc, Method method) {
		...
		if (syncMethod == null) {
			throw new AssertionError(
					"Could not find match for method " + method + " in synchronous interface " + syncIfc);
		}
		ParameterizedType type = (ParameterizedType) method.getGenericReturnType();
		Type actualTypeArgument = type.getActualTypeArguments()[0];
		if (!checkMethodReturnTypes(actualTypeArgument, syncMethod.getGenericReturnType())) {
			throw new AssertionError(
					"Incompatible return type of method " + syncMethod + ". Expected " + actualTypeArgument);
		}
	}

	/**
	 * Compares the return types of a corresponding pair of an asynchronous and
	 * synchronous method. Here some corner cases have to be taken into account; for
	 * instance, the case that the return type is generic.
	 *
	 * @param asyncReturnType
	 *            the return type of the asynchronous method (which has already been
	 *            extracted from the future)
	 * @param syncReturnType
	 *            the return type of the synchronous method
	 ...
	 *
	 * @param clazz
	 *            the class that should declare the method
	 * @param method
	 *            the method to be looked up
	 * @return the method that was found or <strong>null</strong> if there was no
	 *         match
	 */
	 ...
	 * wrapper class. Otherwise, the class is returned unchanged. This is necessary
	 * to match methods with primitive return types or void correctly.
	 *
	 * @param type
