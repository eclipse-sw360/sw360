...
public class SW360AttachmentClientIT extends AbstractMockServerTest {
	...
	/**
	 * Returns an {@code AttachmentProcessor} that does not expect to be invoked.
	 *
	...
	public void testUploadAttachment() throws URISyntaxException, IOException {
		if (!RUN_REST_INTEGRATION_TEST) {
			...
			wireMockRule.stubFor(post(urlPathEqualTo(urlPath + "/attachments"))
					...
					.withMultipartRequestBody(aMultipart().withName("file").withBody(binaryEqualTo(attachmentContent)))
					.willReturn(aJsonResponse(HttpConstants.STATUS_OK).withBody(toJson(release))));

			SW360Release modifiedRelease = waitFor(
	...
	public void testUploadAttachmentError() throws URISyntaxException, IOException {
		...
		SW360Release release = new SW360Release();
		release.getLinks().setSelf(new Self(selfUrl));
		wireMockRule.stubFor(post(anyUrl()).willReturn(aJsonResponse(HttpConstants.STATUS_ERR_BAD_REQUEST)));
	...
	public void testProcessAttachment() throws IOException {
		...
		SW360AttachmentAwareClient.AttachmentProcessor<SW360Project> processor = stream -> objectMapper
				.readValue(stream, SW360Project.class);
		wireMockRule.stubFor(get(urlPathEqualTo(itemRef + "/attachments/" + attachmentID))
				...
				.willReturn(aJsonResponse(HttpConstants.STATUS_OK).withBodyFile(testFile)));

		SW360Project actResult = waitFor(
				attachmentClient.processAttachment("https://host.to.be.ignored" + itemRef, attachmentID, processor));
	...
	public void testProcessAttachmentNotFound() {
		...
		FailedRequestException exception = expectFailedRequest(
				attachmentClient.processAttachment(wireMockRule.baseUrl(), attachmentId, processor),
	...
	public void testProcessAttachmentNoContent() throws IOException, URISyntaxException {
		...
		byte[] data = waitFor(attachmentClient.processAttachment(wireMockRule.baseUrl(), attachmentId, processor));
	...
	public void testDeleteAttachments() throws IOException, URISyntaxException {
		if (!RUN_REST_INTEGRATION_TEST) {
			...
			wireMockRule.stubFor(delete(urlPathEqualTo(urlPath + "/attachments/" + attachId1 + "," + attachId2))
					.willReturn(aJsonResponse(HttpConstants.STATUS_OK).withBody(toJson(releaseUpdated))));

			SW360Release result = waitFor(
					attachmentClient.deleteAttachments(release, Arrays.asList(attachId1, attachId2)));
	...
	public void testDeleteAttachmentsError() throws IOException {
		SW360Release release = readTestJsonFile(resolveTestFileURL("release.json"), SW360Release.class);
		wireMockRule.stubFor(delete(anyUrl()).willReturn(aJsonResponse(HttpConstants.STATUS_ERR_NOT_FOUND)));

		FailedRequestException exception = expectFailedRequest(
				attachmentClient.deleteAttachments(release, Collections.singleton("foo")),
				HttpConstants.STATUS_ERR_NOT_FOUND);
