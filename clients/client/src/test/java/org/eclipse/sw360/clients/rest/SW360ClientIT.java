...
public class SW360ClientIT extends AbstractMockServerTest {
	...
	/**
	 * Prepares the mock token provider to answer requests for an access token with
	 * the standard token.
	 */
	 ...
	 * tokens. The first token is considered expired, so that another token needs to
	 * be fetched.
	 *
	 * @param token
	...
	private void givenExpiredAccessToken(String token) {
		...
		prepareAccessTokens(tokenProvider, CompletableFuture.completedFuture(new AccessToken(token)),
				CompletableFuture.completedFuture(ACCESS_TOKEN));
	}

	/**
	 * Prepares the mock token provider to report a failure when asked for an access
	 * token.
	 *
	...
	/**
	 * Invokes a standard request on the test client and returns the future with the
	 * result.
	 *
	...
	/**
	 * Invokes a standard request on the test client and returns the result. (The
	 * method blocks until the result arrives.)
	 *
	 ...
	 *
	 * @param projectList
	 *            the project list returned by a request
	 */
	...
	/**
	 * Checks whether the given list of projects contains the expected test data.
	 *
	 * @param projects
	 *            the list with projects to be checked
	 */
	private static void checkTestProjects(List<SW360Project> projects) {
		String[] expectedProjects = {"Project_Foo", "Project_Bar", "Project_other", "Project_test"};
	...
	/**
	 * Checks that the result future obtained from the test client failed with an
	 * exception of the given type and returns this exception.
	 *
	 * @param result
	 *            the result future
	 * @param exClass
	...
	/**
	 * Checks whether the future obtained from the test client failed because of a
	 * {@link FailedRequestException} with the expected status code.
	 *
	 * @param result
	 *            the result future
	 * @param status
	...
	/**
	 * Checks that the token provider was invoked to invalidate the given access
	 * token.
	 *
	 * @param token
