...
public class AbstractMockServerTest {
	...
	/**
	 * Convenience method to generate an initialized response builder for a JSON
	 ...
	 *
	 * @param builder
	 *            the builder to be decorated
	 * @param expToken
	 *            the expected access token
	...
	protected static MappingBuilder authorized(MappingBuilder builder, String expToken) {
		return builder.withHeader(HttpConstants.HEADER_AUTHORIZATION, equalTo(HttpConstants.AUTH_BEARER + expToken));
	}
	...
	/**
	 * Returns a mock token provider. The object returned is actually a spy, which
	 * allows configuring the tokens to be retrieved while having other
	...
	/**
	 * Prepares the given mock token provider to return specific result futures for
	 * subsequent invocations. The mock provider is expected to have been created
	 * using {@link #createMockTokenProvider()}.
	 *
	 * @param provider
	 *            the mock token provider
	 * @param tokenFuture
	 *            the first result to be returned
	 * @param moreTokenFutures
	 *            an arbitrary number of further results
	 */
	@SafeVarargs
	protected static void prepareAccessTokens(AccessTokenProvider provider, CompletableFuture<AccessToken> tokenFuture,
	...
	/**
	 * Returns a URL to the test file with the given name. The file is looked up in
	 * the files directory of the Wiremock server.
	 *
	 * @param name
	 *            the name of the desired test file
	...
	/**
	 * Returns a URL to the test file with the given name. The file is looked up in
	 * the /files/integrationtest/ directory of the Wiremock server.
	 *
	 * @param name
	 *            the name of the desired test file
	 * @return the URL to the test file specified
	...
	/**
	 * Parses a JSON file specified by the given URL and returns its de-serialized
	 * content.
	 *
	 * @param url
	...
	protected static <T extends SW360HalResource<?, ?>> void assertHasLinks(Collection<T> data) {
		boolean hasLinks = data.stream().allMatch(comp -> comp.getLinks().getSelf() != null);
		assertThat(hasLinks).isTrue();
	}

	/**
	 * Expects that the given future has failed and returns the causing exception.
	 *
	 * @param future
	 *            the future to check
	...
	/**
	 * Expects that the given future has failed with an exception of the given type
	 * and returns this exception.
	 *
	 * @param future
	 *            the future to check
	 * @param exClass
	 ...
	 * @return the exception that caused the future to fail
	 */
	protected static <E extends Throwable> E extractException(CompletableFuture<?> future, Class<? extends E> exClass) {
	 ...
	 * {@code FailedRequestException} exception with the status code specified. The
	 * exception is returned for further inspection.
	 *
	 * @param future
	 *            the future to check
	 * @param statusCode
	...
	/**
	 * Creates a configuration for the client library with test properties. This
	 * configuration also contains a fully configured HTTP client and an object
	 * mapper.
	 *
	...
	/**
	 * A specialized {@code AccessTokenProvider} implementation used within tests.
	 * This class increases the visibility of some methods, so that they can be
	 * manipulated by spies.
	 */
