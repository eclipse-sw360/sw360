...
public class SW360LicenseClientAdapterAsyncImplTest {
	...
	public void testGetLicenses() {
		...
		when(licenseClient.getLicenses()).thenReturn(CompletableFuture.completedFuture(licenses));
	...
	public void testGetLicenseByName() {
		...
		Optional<SW360License> sw360LicenseByLicense = block(licenseClientAdapter.getLicenseByName(LICENSE_NAME));
	...
	public void testGetLicenseByNameUnresolved() {
		FailedRequestException exception = new FailedRequestException("get_license",
				HttpConstants.STATUS_ERR_NOT_FOUND);
		when(licenseClient.getLicenseByName(LICENSE_NAME)).thenReturn(FutureUtils.failedFuture(exception));
	...
	public void testEnrichSparseLicense() {
		SW360License license = prepareLicenseClientGetLicenseByName();
		SW360SparseLicense sparseLicense = new SW360SparseLicense().setShortName(LICENSE_NAME);
	...
	public void testCreateLicense() {
		SW360License licenseTemplate = new SW360License().setFullName("testLicense");
		SW360License licenseCreated = new SW360License().setShortName("createdLicense").setFullName("testLicense");
	...
	public void testDeleteLicense() {
		...
		when(licenseClient.deleteLicense(Mockito.any())).thenReturn(CompletableFuture.completedFuture(200));
	...
	private SW360License prepareLicenseClientGetLicenseByName() {
		SW360License license = new SW360License().setShortName(LICENSE_NAME);
		when(licenseClient.getLicenseByName(LICENSE_NAME)).thenReturn(CompletableFuture.completedFuture(license));
