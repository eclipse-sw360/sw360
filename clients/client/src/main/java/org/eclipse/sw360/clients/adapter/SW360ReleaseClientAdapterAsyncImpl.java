...
class SW360ReleaseClientAdapterAsyncImpl implements SW360ReleaseClientAdapterAsync {
	...
	public CompletableFuture<SW360Release> createRelease(SW360Release release) {
		...
		}, "Cannot create release for " + release.getName()).thenCompose(this::assignReleaseToComponent)
				.thenCompose(getReleaseClient()::createRelease);
	}

	/**
	 * Obtains the component a new release belongs to and establishes the link. It
	 * is also checked whether the release is not already present for this
	 * component.
	 *
	 * @param release
	...
	private CompletableFuture<SW360Release> assignReleaseToComponent(SW360Release release) {
		...
		return fetchComponentToAssign(componentFromRelease).thenApply(cta -> {
	...
	/**
	 * Obtains the component to assign to a release. The component is looked up by
	 * the release name. If it does not exist yet, it is newly created.
	 *
	 * @param component
	 *            the component created based on the release
	...
	private CompletableFuture<SW360Component> fetchComponentToAssign(SW360Component component) {
		return getComponentAdapter().getComponentByName(component.getName())
				.thenCompose(optExistingComponent -> optExistingComponent.map(CompletableFuture::completedFuture)
	...
	private static boolean containsRelease(SW360Component component, SW360Release release) {
		return component.getEmbedded().getReleases().stream().map(SW360SparseRelease::getVersion)
				.anyMatch(release.getVersion()::equals);
	}

	@Override
	public CompletableFuture<AttachmentUploadResult<SW360Release>> uploadAttachments(
	...
	public CompletableFuture<Optional<SW360Release>> getReleaseById(String releaseId) {
		return FutureUtils.optionalFuture(getReleaseClient().getRelease(releaseId));
	}
	...
	public CompletableFuture<Optional<SW360SparseRelease>> getSparseReleaseByExternalIds(Map<String, ?> externalIds) {
		return getReleaseClient().getReleasesByExternalIds(externalIds).thenApply(releases -> {
			if (releases.isEmpty()) {
	...
	public CompletableFuture<Optional<SW360SparseRelease>> getSparseReleaseByNameAndVersion(String componentName,
			String version) {
		return getComponentAdapter().getComponentByName(componentName).thenApply(optComponent -> optComponent
				.map(SW360Component::getEmbedded).map(SW360ComponentEmbedded::getReleases)
				.flatMap(releases -> releases.stream().filter(rel -> version.equals(rel.getVersion())).findFirst()));

	}

	@Override
	public CompletableFuture<Optional<SW360Release>> getReleaseByVersion(SW360Component component,
			String releaseVersion) {
		if (component != null && component.getEmbedded() != null && component.getEmbedded().getReleases() != null) {

			List<SW360SparseRelease> releases = component.getEmbedded().getReleases();
			Optional<String> releaseId = releases.stream()
					.filter(release -> release.getVersion().equals(releaseVersion)).findFirst()
	...
	@Override
	public <T> CompletableFuture<T> processAttachment(SW360Release release, String attachmentId,
	...
	public CompletableFuture<SW360Release> deleteAttachments(SW360Release release, Collection<String> attachmentIds) {
		return attachmentIds.isEmpty()
				? CompletableFuture.completedFuture(release)
	...
	public CompletableFuture<SW360Release> updateRelease(SW360Release release) {
		return FutureUtils.wrapInFuture(() -> {
			...
			return SW360ReleaseAdapterUtils.validateRelease(release);
		}, "Cannot update release for " + release.getName()).thenCompose(getReleaseClient()::patchRelease);
	}
