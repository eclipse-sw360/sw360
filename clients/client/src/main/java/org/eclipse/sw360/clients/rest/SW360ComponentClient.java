...
public class SW360ComponentClient extends SW360Client {
	...
	/**
	 * Creates a new instance of {@code SW360ComponentClient} and initializes it
	 ...
	 *
	 * @param componentId
	 *            the ID of the component in question
	 * @return a future with details about this component
	 */
	public CompletableFuture<SW360Component> getComponent(String componentId) {
		return executeJsonRequest(HttpUtils.get(resourceUrl(COMPONENTS_ENDPOINT, componentId)), SW360Component.class,
				TAG_GET_COMPONENT);
	}

	/**
	 * Executes a search query based on the parameters provided and returns a future
	 * with a result object. The result contains the entities matched by the search
	 * criteria and paging-related metadata if available. (If the search parameters
	 * do not use paging, the paging-related objects in the result are
	 * <strong>null</strong>.)
	 *
	 * @param searchParams
	 *            the object with search parameters
	...
	public CompletableFuture<PagingResult<SW360SparseComponent>> search(ComponentSearchParams searchParams) {
		...
		return executeJsonRequestWithDefault(HttpUtils.get(url), SW360ComponentList.class, TAG_GET_COMPONENTS,
				SW360ComponentList::new).thenApply(SW360ComponentClient::createPagingComponentResult);
	}
	 ...
	 *
	 * @param sw360Component
	 *            a data object for the component to be created
	...
	public CompletableFuture<SW360Component> createComponent(SW360Component sw360Component) {
		return executeJsonRequest(builder -> builder.method(RequestBuilder.Method.POST)
				.uri(resourceUrl(COMPONENTS_ENDPOINT)).body(body -> body.json(sw360Component)), SW360Component.class,
				TAG_CREATE_COMPONENT);
	}

	/**
	 * Updates a release based on the data object passed in and returns a future
	 * with the result.
	 *
	 * @param component
	 *            a data object for the component to be updated
	...
	public CompletableFuture<SW360Component> patchComponent(SW360Component component) {
		return executeJsonRequest(
				builder -> builder.method(RequestBuilder.Method.PATCH)
						.uri(resourceUrl(COMPONENTS_ENDPOINT, component.getId())).body(body -> body.json(component)),
				SW360Component.class, TAG_UPDATE_COMPONENT);
	}

	/**
	 * Triggers a DELETE operation for the components identified by the given IDs.
	 *
	 * @param idsToDelete
	 *            a collection with the IDs of the components to delete
	 * @return a future with the {@code MultiStatusResponse} returned by the server
	 */
	...
	/**
	 * Generates a map with query parameters for a search based on the parameters
	 * object provided.
	 *
	 * @param searchParams
	...
	/**
	 * Generates a string representation for a query parameter with multiple values.
	 *
	 * @param values
	 *            the list with parameter values
