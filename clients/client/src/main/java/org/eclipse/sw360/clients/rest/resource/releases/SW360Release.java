...
public final class SW360Release extends SW360HalResource<SW360ReleaseLinkObjects, SW360ReleaseEmbedded> {
	...
	public String getComponentId() {
		return Optional.ofNullable(getLinks()).map(SW360ReleaseLinkObjects::getSelfComponent)
	...
	public Set<String> getMainLicenseIds() {
		return Optional
				.ofNullable(getEmbedded().getLicenses()).map(lics -> lics.stream().filter(Objects::nonNull)
						.map(SW360SparseLicense::getShortName).collect(Collectors.toSet()))
	...
	public SW360Release setMainLicenseIds(Set<String> mainLicenseIds) {
		if (!mainLicenseIds.isEmpty()) {
			List<SW360SparseLicense> licenses = mainLicenseIds.stream()
					.map(licenseId -> new SW360SparseLicense().setShortName(licenseId)).collect(Collectors.toList());
	...
	public Map<String, String> getCoordinates() {
		return externalIds.entrySet().stream().filter(e -> e.getValue().startsWith("pkg:"))
				.filter(e -> isValidPkgUrl(e.getValue()))
				.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
	}

	public SW360Release setCoordinates(Map<String, String> coordinates) {
		coordinates.entrySet().stream().filter(entry -> isValidPkgUrl(entry.getValue()))
	...
	public Set<String> getHashes() {
		return externalIds.entrySet().stream().filter(e -> e.getKey().startsWith(HASHES_PREFIX))
				.map(Map.Entry::getValue).collect(Collectors.toSet());
	}

	private void dropAllHashes() {
		externalIds.keySet().stream().filter(s -> s.startsWith(HASHES_PREFIX)).forEach(externalIds::remove);
	}
	...
	public SW360Release mergeWith(SW360Release releaseWithPrecedence) {
		...
		final Set<SW360SparseAttachment> releaseWithPrecedenceAttachments = releaseWithPrecedence.getEmbedded()
				.getAttachments();
		if (!releaseWithPrecedenceAttachments.isEmpty()) {
			if (getEmbedded().getAttachments().isEmpty()) {
				...
				getEmbedded().setAttachments(
						mergeAttachments(getEmbedded().getAttachments(), releaseWithPrecedenceAttachments));
			}
		}
	...
	private Set<SW360SparseAttachment> mergeAttachments(Set<SW360SparseAttachment> attachments,
	...
	public boolean equals(Object o) {
		...
		return Objects.equals(name, release.name) && Objects.equals(version, release.version)
				&& Objects.equals(cpeId, release.cpeId) && Objects.equals(downloadurl, release.downloadurl)
