...
public final class MultiStatusResponse {
	...
	/**
	 * Creates a new instance of {@code MultiStatusResponse} with the given map of
	 * single response codes. The map contains the IDs of resources as keys and the
	 * corresponding status codes as values.
	 *
	 * @param responses
	 *            the map with status codes
	 */
	...
	/**
	 * Creates a new instance of {@code MultiStatusResponse} that is initialized
	 * from a JSON document. This method can be used to parse a response received
	 * from SW360.
	 *
	 * @param mapper
	 *            the object mapper to parse the JSON stream
	 * @param stream
	...
	/**
	 * Returns an unmodifiable map with the single responses stored in this object.
	 * The keys of the map are the IDs of the resources that were manipulated. The
	 * values are the status codes for the single operations.
	 *
	...
	/**
	 * Returns a flag whether all the responses managed by this object have been
	 * successful.
	 *
	...
	public boolean isAllSuccess() {
		return responses.values().stream().allMatch(HttpUtils::isSuccessStatus);
	}

	/**
	 * Returns a flag whether this object contains information about the resource ID
	 * specified.
	 *
	 * @param resId
	 *            the resource ID in question
	 * @return a flag whether a status about this resource is stored in this object
	 */
	...
	/**
	 * Returns the status code for the resource with the given ID. The ID must be
	 * present, otherwise a {@code NoSuchElementException} exception is thrown.
	 *
	 * @param resId
	...
	public int getStatus(String resId) {
		...
		if (status == null) {
			throw new NoSuchElementException(
					"Unknown resource ID: " + resId + "; not contained in this multi-status response.");
		}
		return status;
	}
	...
	/**
	 * Extracts the resource ID from the given object map. Throws an exception if
	 ...
	 *             if the resource ID cannot be resolved
	 */
	private static String extractResourceId(Map<String, Object> map) {
		...
		if (!(resourceId instanceof String)) {
			throw new IllegalArgumentException(
					"Unexpected JSON response. Expected resource ID, but found " + resourceId);
		}
	 ...
	 * @throws IllegalArgumentException
	 *             if the status code could not be extracted
	 */
	private static Integer extractStatus(Map<String, Object> map) {
		Object status = map.get(PROP_STATUS);
		if (!(status instanceof Integer)) {
			throw new IllegalArgumentException("Unexpected JSON response. Expected status code, but found " + status);
		}
