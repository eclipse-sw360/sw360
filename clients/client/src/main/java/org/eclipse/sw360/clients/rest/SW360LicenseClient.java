...
public class SW360LicenseClient extends SW360Client {
	...
	public CompletableFuture<List<SW360SparseLicense>> getLicenses() {
		return executeJsonRequestWithDefault(HttpUtils.get(resourceUrl(LICENSES_ENDPOINT)), SW360LicenseList.class,
				TAG_GET_LICENSES, SW360LicenseList::new).thenApply(SW360ResourceUtils::getSw360SparseLicenses);
	}

	/**
	 * Returns a future with detail information of a license selected by its name.
	 * If the name provided cannot be resolved, the future fails with a
	...
	/**
	 * Creates a new license based on the properties of the data object passed in.
	 *
	 * @param license
	 *            the data object for the license to be created
	 * @return a future with the newly created license
	 */
	public CompletableFuture<SW360License> createLicense(SW360License license) {
		return executeJsonRequest(builder -> builder.uri(resourceUrl(LICENSES_ENDPOINT))
				.method(RequestBuilder.Method.POST).body(bodyBuilder -> bodyBuilder.json(license)), SW360License.class,
				TAG_CREATE_LICENSE);
	}

	/**
	 * Triggers a DELETE operation for the License identified by the given IDs.
	 *
	 * @param licenseId
