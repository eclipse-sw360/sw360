 ...
 * adapter object and implements the corresponding synchronous interface. Method
 * invocations are handled by delegating to the asynchronous adapter and waiting
 * for the resulting {@code CompletableFuture} to complete. This is possible as
 * the synchronous and asynchronous adapter interface exactly correspond to each
 * other and only differ in the return types of their methods.
 * </p>
 */
final class SyncClientAdapterHandler implements InvocationHandler {
	 ...
	 *
	 * @param asyncIfcClass
	 *            the asynchronous class of the delegate
	 * @param delegate
	 *            the delegate object
	 */
	 ...
	 *
	 * @param syncIfcClass
	 *            the (synchronous) interface class to be implemented
	 * @param asyncIfcClass
	 *            the (asynchronous) interface class of the delegate
	 * @param delegateAdapter
	 *            the (asynchronous) adapter to delegate to
	 * @param <S>
	 *            the type of the synchronous interface to be implemented
	 * @param <A>
	 *            the type of the asynchronous delegate adapter
	...
	/**
	 * {@inheritDoc} This implementation invokes the method to be handled on the
	 * delegate object. If this results in a {@code CompletableFuture}, it waits for
	 * the future to complete. Otherwise, the result is returned directly. (That
	 * way, some special methods - e.g. the ones returning the underlying SW360
	 * client - can be handled correctly as well.)
	 */
	@Override
	...
	/**
	 * Handles a method that is declared by the Object class. Such methods typically
	 * also need to be delegated to the delegate object; however, they cannot be
	 * found in the asynchronous interface class. The equals() is special though. We
	 * consider two proxy objects equal if they refer to the same delegate.
	 *
	 * @param method
	 *            the method to be handled
	 * @return the result of the invocation
	 * @throws IllegalAccessException
	 *             if a method is not accessible
	 * @throws InvocationTargetException
	 *             if a method invocation fails
	 */
