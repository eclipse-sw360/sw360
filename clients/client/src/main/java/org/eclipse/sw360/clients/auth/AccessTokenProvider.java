 ...
 * request execution. The cache for the token is then invalidated. This causes a
 * new token to be requested the next time a client asks for one.
 * </p>
 * <p>
 ...
 * access token concurrently, the class ensures that only a single token request
...
public class AccessTokenProvider {
	...
	/**
	 * Stores the future with the current access token. Once a token request has
	 * been initiated, all requesting clients can be passed this object; they are
	 * then notified as soon as the future is completed (or immediately if the
	 * future is already completed).
	 */
	private CompletableFuture<AccessToken> tokenFuture;

	/**
	 * Stores the current access token once it becomes available. This is needed by
	 * the {@code invalidate()} method, to make sure that only the correct token
	 * gets invalidated.
	 */
	private AccessToken accessToken;

	/**
	 * Creates a new instance of {@code AccessTokenProvider} that uses the provided
	 * {@code SW360AuthenticationClient} to obtain access tokens.
	 *
	 * @param authClient
	 *            the underlying authentication client
	 */
	...
	/**
	 * Obtains an access token first and then invokes a function to consume it and
	 * produce a future result. This can be used for instance to send a request as
	 * soon as a token becomes available.
	 *
	 * @param func
	 *            the function producing the future result with the access token
	 * @param <T>
	...
	public <T> CompletableFuture<T> doWithToken(Function<? super AccessToken, ? extends CompletableFuture<T>> func) {
		return obtainAccessToken().thenCompose(
				token -> func.apply(token).whenComplete((result, exception) -> onRequestComplete(token, exception)));
	}

	/**
	 * Returns a future with an {@code AccessToken} to be used to authenticate
	 * against the SW360 server. If necessary, the underlying authentication client
	 * is asked to fetch a new token. A successful token result is cached and passed
	 * directly to clients asking later. In case of an error, the result is not
	 * cached, so that a subsequent token request causes another invocation of the
	 * {@code SW360AuthenticationClient}.
	 *
	...
	protected synchronized CompletableFuture<AccessToken> obtainAccessToken() {
		...
		if (tokenFuture == null) {
			...
			tokenFuture = result = getAuthClient().getOAuth2AccessToken().thenApply(AccessToken::new);
	...
	/**
	 * Invalidates the token cached internally if it equals the token specified.
	 * This method is called automatically if a request using a token fails with a
	 * 401 Unauthorized error. The cache is then cleared, so the next time a client
	 * asks for a token, a new one is requested from the server. The token parameter
	 * is required to deal with a race condition: If there are multiple concurrent
	 * requests that all fail with a 401 error, this method will be called multiple
	 * times. In this constellation, it has to be prevented that a token that has
	 * just been renewed is invalidated immediately again; this is achieved by
	 * comparing the cached token against the specified one.
	 *
	 * @param token
	 *            the token that should be invalidated
	 */
	...
	/**
	 * An action that is called for each request after its completion. It checks
	 * whether the request failed with an exception indicating that the access token
	 * has expired. If so, the token cache is invalidated.
	 *
	 * @param token
	 *            the current access token
	 * @param exception
	 *            the exception from the request future
	 */
	...
	/**
	 * Clears the cached access token unconditionally. Note that this method must be
