...
public class SW360AuthenticationClient {
	...
	/**
	 * Creates a new instance of {@code SW360AuthenticationClient} and initializes
	 * it with the current configuration.
	 *
	 * @param clientConfig
	 *            the SW360 client configuration
	 */
	...
	/**
	 * Requests a new access token from the authorization server that is defined by
	 * the SW360 client configuration. An asynchronous request is sent, and a future
	...
	public CompletableFuture<String> getOAuth2AccessToken() {
		...
		return getClientConfig().getHttpClient().execute(this::initTokenRequest,
				HttpUtils.checkResponse(this::extractToken, TAG));
	}

	/**
	 * Constructs the request for a new access token using the passed in request
	 * builder.
	 *
	 * @param builder
	 *            the request builder
	...
	private void initTokenRequest(RequestBuilder builder) {
		builder.uri(getClientConfig().getAuthURL()).method(RequestBuilder.Method.POST)
				.header(HEADER_AUTHORIZATION,
						generateBasicAuthHeader(getClientConfig().getClientId(), getClientConfig().getClientPassword()))
	...
	/**
	 * Tries to extract the access token from the response received from the OAuth
	 * server. The server is expected to send a JSON response with an object
	 * containing the token in a well-known property. If the token cannot be
	 * extracted for whatever reason, an exception is thrown. Note that when this
	 * method is called the response status has already been checked to be
	 * successful.
	 *
	 * @param response
	 *            the response
	 ...
	 * @throws IOException
	 *             if JSON processing fails or the response has an unexpected format
	 */
	private String extractToken(Response response) throws IOException {
		...
		if (!(token instanceof String)) {
			throw new IOException("Could not extract access token from server response. " + "The attribute '"
					+ JSON_TOKEN_KEY + "' is not present or has an unexpected value.");
		}
	...
	/**
	 * Generates the Authorization header for the token request. The authentication
	 * is done via Basic Auth using the client ID and password.
	 *
	 * @param user
