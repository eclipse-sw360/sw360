 ...
 * size. A single request yields a specific page. So the whole result set can be
 * processed in multiple chunks.
 * </p>
 * <p>
 * Results for paging requests in SW360 contain some additional objects. Next to
 * the actual result, there are special links supporting the navigation through
 * the result set. There is some paging-related metadata as well, such as the
 * number of total pages available. This class exposes this additional
 * information.
 * </p>
 * <p>
 ...
 * must be prepared that get methods can also return <strong>null</strong>. The
 * result list must be non <strong>null</strong> though.
 * </p>
 *
 * @param <T>
 *            the type of the result objects contained in this result
 */
public final class PagingResult<T> {
	...
	/**
	 * Creates a new instance of {@code PagingResult} and initializes it with the
	 * data to be hold.
	 *
	 * @param result
	 *            the query result (must not be <strong>null</strong>)
	 * @param paging
	 *            the paging metadata
	 * @param pagingLinkObjects
	 *            the paging links
	 * @throws NullPointerException
	 *             if the result list is <strong>null</strong>
	 */
	...
	/**
	 * Returns the {@code Paging} object with metadata about the pages available in
	 * the search result. This can be <strong>null</strong> if no paging information
	 * has been provided at construction.
	 *
	...
	/**
	 * Returns the object with links to navigate through the result pages. This can
	 * be <strong>null</strong> if no paging information has been provided at
	 * construction.
	 *
	...
	public boolean equals(Object o) {
		...
		return getResult().equals(result1.getResult()) && Objects.equals(getPaging(), result1.getPaging())
