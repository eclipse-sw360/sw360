...
public class SW360ReleaseClient extends SW360AttachmentAwareClient<SW360Release> {
	...
	/**
	 * Returns a future with detail information about the release with the given ID.
	 * If the release cannot be found, the future fails with a
	...
	public CompletableFuture<SW360Release> getRelease(String releaseId) {
		return executeJsonRequest(HttpUtils.get(resourceUrl(RELEASES_ENDPOINT_APPENDIX, releaseId)), SW360Release.class,
				TAG_GET_RELEASE);
	}

	/**
	 * Returns a future with a list of releases that match the external IDs passed
	 * to this method.
	 *
	 * @param externalIds
	 *            a map with the IDs to be matched and their values
	...
	private String getExternalIdUrl(Map<String, ?> externalIds) {
		return HttpUtils.addQueryParameters(resourceUrl(RELEASES_ENDPOINT_APPENDIX, PATH_SEARCH_EXT_IDS), externalIds);
	}

	/**
	 * Creates a new release resource based on the data object passed in and returns
	 * a future with the result.
	 *
	 * @param sw360Release
	 *            the data object defining the release to be created
	...
	public CompletableFuture<SW360Release> createRelease(SW360Release sw360Release) {
		return executeJsonRequest(builder -> builder.method(RequestBuilder.Method.POST)
				.uri(resourceUrl(RELEASES_ENDPOINT_APPENDIX)).body(body -> body.json(sw360Release)), SW360Release.class,
				TAG_CREATE_RELEASE);
	}

	/**
	 * Modifies an existing release based on a data object and returns a future with
	 * the result.
	 *
	 * @param sw360Release
	 *            the data object defining the release to be modified and the new
	 *            properties
	 * @return a future with the resulting entity
	 */
	public CompletableFuture<SW360Release> patchRelease(SW360Release sw360Release) {
		return executeJsonRequest(builder -> builder.method(RequestBuilder.Method.PATCH)
				...
				.body(body -> body.json(sw360Release)), SW360Release.class, TAG_UPDATE_RELEASE);
	}

	/**
	 * Triggers a DELETE operation for the releases identified by the given IDs.
	 *
	 * @param idsToDelete
	 *            a collection with the IDs of the releases to delete
