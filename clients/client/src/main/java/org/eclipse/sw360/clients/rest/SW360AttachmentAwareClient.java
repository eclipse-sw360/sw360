 ...
 *
 * @param <T>
 *            the type of the resource handled by this class
 */
public abstract class SW360AttachmentAwareClient<T extends SW360HalResource<?, ?>> extends SW360Client {
	...
	/**
	 * Returns the type of the resource class for which attachments are managed by
	 * this client. This is needed to do the correct JSON deserialization.
	 *
	...
	/**
	 * Uploads an attachment file, creates an entity for it, and assigns it to the
	 * entity represented by the passed in object. The path provided must point to
	 * an existing file. A future with the modified entity (the one the attachment
	 * has been added to) is returned.
	 *
	 * @param itemToModify
	 *            a data object defining the resource to attach the file
	 * @param fileToAttach
	...
	public CompletableFuture<T> uploadAndAttachAttachment(T itemToModify, Path fileToAttach,
		...
		return executeJsonRequest(
				builder -> builder.method(RequestBuilder.Method.POST)
						...
						.multiPart("attachment", part -> part.json(sw360Attachment))
						.multiPart("file", part -> part.file(fileToAttach, HttpConstants.CONTENT_OCTET_STREAM)),
				getHandledClassType(), TAG_UPLOAD_ATTACHMENT);
	}

	/**
	 * Fetches the content of an attachment and passes it to the
	 * {@code AttachmentProcessor} provided. A future with the result produced by
	 * the processor is returned. If the attachment cannot be resolved, the
	 * resulting future fails with a
	 * {@link org.eclipse.sw360.http.utils.FailedRequestException} with status code
	 * 404; it contains an {@code IOException} if there was a problem with the
	 * processor or if the server could not be contacted.
	 *
	 * @param itemHref
	 *            the base resource URL to access the attachment entity
	 * @param attachmentId
	 *            the ID of the attachment to be processed
	 * @param processor
	 *            the object to process the attachment's content
	 * @param <S>
	...
	public <S> CompletableFuture<S> processAttachment(String itemHref, String attachmentId,
		...
		return executeRequest(
				builder -> builder.uri(resolveAgainstBase(url).toString()).header(HttpConstants.HEADER_ACCEPT,
						"application/*"),
				response -> processor.processAttachmentStream(response.bodyStream()), TAG_DOWNLOAD_ATTACHMENT);
	}

	/**
	 * Deletes the specified attachments from the entity provided. A future with the
	 * updated entity is returned. If successful, callers should inspect the
	 * attachments of the entity to find out whether actually all could be deleted.
	 * (SW360 does not allow deleting attachments already in use; if only some of
	 * the attachments could be deleted, the request is successful, but the
	 * resulting entity will still contain the problematic ones.)
	 *
	 * @param entity
	 *            the entity to be updated
	 * @param attachmentIds
	 *            a list with the IDs of the attachments to be deleted
	...
	public CompletableFuture<T> deleteAttachments(T entity, Collection<String> attachmentIds) {
		String url = entity.getSelfLink().getHref() + ATTACHMENTS_ENDPOINT + "/" + String.join(",", attachmentIds);
		return executeJsonRequest(
				builder -> builder.uri(resolveAgainstBase(url).toString()).method(RequestBuilder.Method.DELETE),
				getHandledClassType(), TAG_DELETE_ATTACHMENT);
	}

	/**
	 * <p>
	 * An interface for accessing and processing attachments from SW360 resources.
	 * </p>
	 * <p>
	 * An object implementing this interface needs to be provided to the
	 * {@link #processAttachment(String, String, AttachmentProcessor)} method. It is
	 * invoked with the input stream for the attachment's content. This stream can
	 * be consumed to produce an arbitrary result. An obvious use case is copying
	 * the stream to a local file, which corresponds to an attachment download
	 * operation. But by providing different processor implementations, attachments
	 * can be handled in flexible ways.
	 * </p>
	 *
	 * @param <R>
	 *            the result type of this processor
	 */
	@FunctionalInterface
	public interface AttachmentProcessor<R> {
		/**
		 * Processes a stream with the content of an attachment and produces a result
		 * based on this operation. Implementations do not have to close the stream.
		 *
		 * @param stream
		 *            the stream with the content of the attachment
