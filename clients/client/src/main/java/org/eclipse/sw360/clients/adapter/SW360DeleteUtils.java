...
class SW360DeleteUtils {
	...
	/**
	 * Deletes the entities specified using the given {@code EntityDeleter}. This
	 * method checks whether the collection with IDs to delete is not empty; in this
	 * case, the delete operation is skipped.
	 *
	 * @param deleter
	 *            the object handling delete requests
	 * @param idsToDelete
	 *            a collection with the resource IDs to delete
	...
	/**
	 * Deletes a single entity using the given {@code EntityDeleter} and evaluates
	 * the {@link MultiStatusResponse} returned by the server. This method yields a
	 * failed future if the response from the server indicates that the entity could
	 * not be deleted or if an unexpected response is received.
	 *
	 * @param deleter
	 *            the object handling delete requests
	 * @param resourceId
	 *            the ID of the resource entity to be deleted
	 * @param resourceType
	 *            a string for the resource type, to be used when generating error
	 *            messages
	...
	/**
	 * Checks whether the given multi-status response reports a successful delete
	 * operation on the entity specified. If this is not the case, an exception is
	 * thrown, which will then cause the result future to fail.
	 *
	 * @param status
	 *            the status response to be checked
	 * @param resourceId
	 *            the ID of the resource entity to be deleted
	 * @param resourceType
	 *            a string for the resource type
	 */
	private static void checkResponse(MultiStatusResponse status, String resourceId, String resourceType) {
		...
		if (!HttpUtils.isSuccessStatus(status.getStatus(resourceId))) {
			FailedRequestException requestException = new FailedRequestException(
					generateDeleteTag(resourceId, resourceType), status.getStatus(resourceId));
			throw new SW360ClientException("Delete operation failed", requestException);
		}
	}

	/**
	 * Generates the tag for an exception indicating a failed request to delete the
	 * resource specified.
	 *
	 * @param resourceId
	...
	interface EntityDeleter {
		 ...
		 *
		 * @param idsToDelete
		 *            a collection with the IDs to delete
		 * @return a future with the result of the multi-delete operation
		 */
