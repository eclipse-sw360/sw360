 ...
 * </p>
 * <p>
 * The SW360 components endpoint supports a number of parameters to restrict the
 * components returned by a search. This class makes the search criteria
...
public final class ComponentSearchParams {
	 ...
	 * unrestricted search on all components. This should be used with care, as the
	 * search might yield a huge result set.
	 */
	...
	/**
	 * Creates a new instance of {@code ComponentSearchParams} that is initialized
	 * from the passed in builder object.
	 *
	 * @param builder
	 *            the builder
	 */
	...
	/**
	 * Returns the component name to search for. A <strong>null</strong> value or
	 * empty string means that this search criterion is disabled.
	 *
	...
	/**
	 * Returns the component type to search for or <strong>null</strong> if this
	 * search criterion is disabled.
	 *
	...
	/**
	 * Returns the parameter for the page index to be requested from the server. A
	 * <strong>null</strong> or empty value means that paging is not used for the
	 * search.
	 *
	 ...
	 * components to be contained on a result page. A <strong>null</strong> or empty
	 * value means that paging is disabled or the default page size should be used.
	 *
	 ...
	 * search result. Each clause consists of a field name and an optional direction
	...
	/**
	 * Returns a (unmodifiable) list with the names of fields to be returned in the
	 * search result. This allows to restrict the search result to a set of fields
	 * relevant for the current use case.
	 *
	...
	public boolean equals(Object o) {
		...
		return Objects.equals(getName(), params.getName())
				...
				&& getOrderClauses().equals(params.getOrderClauses()) && getFields().equals(params.getFields());
	}

	@Override
	public int hashCode() {
		return Objects.hash(getName(), getComponentType(), getPageIndex(), getPageSize(), getOrderClauses(),
				getFields());
	}
	...
	public static class Builder {
		...
		/**
		 * Creates a new instance of {@code Builder} that is initialized with some
		 * default values. All search criteria are disabled per default.
		 */
		private Builder() {
		...
		/**
		 * Sets the search criterion for the component name. The search result will
		 * contain all the components whose name includes this string.
		 *
		 * @param name
		...
		/**
		 * Sets the search criterion for the component type. This limits the search
		 * result to components with this type.
		 *
		 * @param componentType
		 *            the component type to search for
		 * @return this builder
		...
		/**
		 * Sets the index of the page to be requested from the server. Setting a value
		 * &gt;= 0 enables paging of the result set. Using this mechanism, large result
		 * sets can be processed in multiple chunks.
		 *
		 * @param pageIndex
		 *            the index of the page to be requested
		 * @return this builder
		 */
		...
		/**
		 * Sets the number of entries contained on a result page. This property is
		 * evaluated only if paging is enabled. It then allows controlling the size of
		 * the chunks that are queried.
		 *
		 * @param pageSize
		 *            the number of elements of a result page
		 * @return this builder
		 */
		...
		/**
		 * Specifies a field for an ascending order. The {@code order()} methods can be
		 * called multiple times to define the sort order of the search result.
		 *
		 * @param field
		...
		/**
		 * Specifies a field for a descending order. The {@code order()} methods can be
		 * called multiple times to define the sort order of the search result.
		 *
		 * @param field
		 *            the name of the field to sort
		...
		/**
		 * Adds the fields provided to the list of fields to query from the server.
		 * Using this method, it is possible to restrict the data that is queried. The
		 * embedded components in the result will contain only the fields listed here.
		 * (If this method is not called, all fields available are retrieved.) Unknown
		 * fields are ignored. Note that not all fields of component entities are
		 * supported by this mechanism.
		 *
		 * @param fields
		 *            a set of fields to retrieve from the server
		 * @return this builder
		 */
		...
		/**
		 * Returns a new {@code ComponentSearchParams} instance that is initialized from
		 * the properties set for this builder.
		 *
