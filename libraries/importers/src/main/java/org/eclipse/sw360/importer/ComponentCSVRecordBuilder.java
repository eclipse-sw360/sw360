...
public class ComponentCSVRecordBuilder extends CustomizedCSVRecordBuilder<ComponentCSVRecord> {
	...
	public ComponentCSVRecord build() {
		return new ComponentCSVRecord(componentName, componentDescription, componentCreatedOn, componentType,
				componentCreatedBy, componentSubscribers, categories, softwarePlatforms, componentHomePage,
				componentMailingList, componentWiki, componentBlog, componentWikipedia, componentOpenHub, releaseName,
				releaseVersion, releaseDate, CPEId, releaseCreatedOn, releaseCreatedBy, releaseRepositoryURL,
				releaseRepositoryType, releaseMainlineState, releaseClearingState, releaseContributors,
				releaseModerators, releaseSubscribers, releaseLanguages, releaseOperatingSystems,
				releaseMainLicenseNames, releaseSourceCodeDownloadURL, releaseBinaryDownloadURL, vendorName,
				vendorShortname, vendorUrl, cIExternalSupplierID, cIAdditionalInfo, cIEvaluated, cIProcStart,
				cIRequestId, cIScanned, cIClearingStandard, cIComment, cIExternalUrl, cIBinariesOriginalFromCommunity,
				cIBinariesSelfMade, cIComponentLicenseInformation, cISourceCodeDelivery,
				cISourceCodeOriginalFromCommunity, cISourceCodeToolMade, cISourceCodeSelfMade, cIScreenshotOfWebSite,
				cIFinalizedLicenseScanReport, cILicenseScanReportResult, cILegalEvaluation, cILicenseAgreement,
				cIComponentClearingReport, cICountOfSecurityVn, eccStatus, eccAL, eccECCN, eccMaterialIndexNumber,
				eccComment, eccAssessorContactPerson, eccAssessorDepartment, eccAssessmentDate);
	}
	...
	public ComponentCSVRecordBuilder fill(ClearingInformation cI) {
		...
		if (cI.isSetBinariesOriginalFromCommunity())
			setcIBinariesOriginalFromCommunity(cI.binariesOriginalFromCommunity);
		if (cI.isSetBinariesSelfMade())
			setcIBinariesSelfMade(cI.binariesSelfMade);
		if (cI.isSetComponentLicenseInformation())
			setcIComponentLicenseInformation(cI.componentLicenseInformation);
		if (cI.isSetSourceCodeDelivery())
			setcISourceCodeDelivery(cI.sourceCodeDelivery);
		if (cI.isSetSourceCodeOriginalFromCommunity())
			setcISourceCodeOriginalFromCommunity(cI.sourceCodeOriginalFromCommunity);
		if (cI.isSetSourceCodeToolMade())
			setcISourceCodeToolMade(cI.sourceCodeToolMade);
		if (cI.isSetSourceCodeSelfMade())
			setcISourceCodeSelfMade(cI.sourceCodeSelfMade);
		if (cI.isSetScreenshotOfWebSite())
			setcIScreenshotOfWebSite(cI.screenshotOfWebSite);
		if (cI.isSetFinalizedLicenseScanReport())
			setcIFinalizedLicenseScanReport(cI.finalizedLicenseScanReport);
		if (cI.isSetLicenseScanReportResult())
			setcILicenseScanReportResult(cI.licenseScanReportResult);
		if (cI.isSetLegalEvaluation())
			setcILegalEvaluation(cI.legalEvaluation);
		if (cI.isSetLicenseAgreement())
			setcILicenseAgreement(cI.licenseAgreement);
		if (cI.isSetComponentClearingReport())
			setcIComponentClearingReport(cI.componentClearingReport);

		if (cI.isSetCountOfSecurityVn())
			setcICountOfSecurityVn(cI.countOfSecurityVn);
