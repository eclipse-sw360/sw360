...
public class LicsImporter {
	...
	public LicsImporter(LicenseService.Iface licenseClient, boolean overwriteIfExternalIdMatches,
			boolean overwriteIfIdMatchesEvenWithoutExternalIdMatch) {
	...
	private Map<Integer, LicenseType> parseLicsLicenseTypeMap(User user, Map<String, InputStream> inputMap)
			throws TException {
		log.debug("Parsing license types ...");
		return getIdentifierToTypeMapAndWriteMissingToDatabase(licenseClient, inputMap.get(LICENSETYPE_FILE),
				LicenseType.class, Integer.class, user);
	}
	...
	private Map<Integer, Obligation> enhanceLicsTodosWithCustomProperties(User user, Map<String, InputStream> inputMap,
			Map<Integer, Obligation> obligMap) throws TException {
		if (inputMap.containsKey(CUSTOM_PROPERTIES_FILE)) {
			log.debug("Parsing custom properties ...");
			Map<Integer, ConvertRecord.PropertyWithValue> customPropertiesMap = getCustomPropertiesWithValuesByIdAndWriteMissingToDatabase(
			...
			Map<Integer, Set<Integer>> obligPropertiesMap = ConvertRecord
					.convertRelationalTableWithIntegerKeys(obligPropertiesRecord);

			return updateTodoMapWithCustomPropertiesAndWriteToDatabase(licenseClient, obligMap, customPropertiesMap,
					obligPropertiesMap, user);
		}
		return obligMap;
	}
	...
	private Map<String, Map<String, Set<String>>> genExternalIdToIdLookupMap(List<License> licenses) {
		...
		licenses.stream().filter(license -> license.getExternalIds() != null)
	...
	private Optional<String> lookupIdInExtarnalIdToIdLookupMap(Map<String, Map<String, Set<String>>> map,
			String externalIdName, String externalId) {
		if (map.containsKey(externalIdName)) {
			if (map.get(externalIdName).containsKey(externalId)) {
				...
				if (matchingIds.size() <= 1) {
					...
					log.warn("The external id=[" + externalIdName + ":" + externalId
							+ "] was not uniq, it is found at licenses with the ids: "
	...
	private Optional<String> lookupExternalIdsInExtarnalIdToIdLookupMap(Map<String, Map<String, Set<String>>> map,
			Map<String, String> externalIds) {
		final Set<String> collectedIds = externalIds.entrySet().stream()
				.map(e -> lookupIdInExtarnalIdToIdLookupMap(map, e.getKey(), e.getValue())).filter(Optional::isPresent)
	...
	private List<License> reworkAndFilterLicenses(List<License> licensesToAdd, List<License> knownLicenses) {
		...
		for (License licenseToAdd : licensesToAdd) {
			Optional<String> idFromExternalIdLookup = lookupExternalIdsInExtarnalIdToIdLookupMap(
					externalIdToIdLookupMap, licenseToAdd.getExternalIds());
			if (idFromExternalIdLookup.isPresent() && overwriteIfExternalIdMatches) {
				if (!idFromExternalIdLookup.get().equals(licenseToAdd.getId())) {
					log.warn("The id=[" + licenseToAdd.getId() + "] did not match [" + idFromExternalIdLookup.get()
							+ "], which was determined via external IDs. It gets overwritten.");
