...
public class ComponentImportUtilsTest extends ComponentAndAttachmentAwareDBTest {
	...
	public void testImportOnEmptyDb() throws Exception {
		...
		final AttachmentContent overwriter = new AttachmentContent().setId(OVERRIDING_ID).setOnlyRemote(true)
				.setRemoteUrl(REMOTE_URL).setType(TYPE_ATTACHMENT);
		final AttachmentContent addition = new AttachmentContent().setId(ADDITIONAL_ID).setOnlyRemote(true)
				.setRemoteUrl(REMOTE_URL).setType(TYPE_ATTACHMENT);
		...
		FluentIterable<ComponentAttachmentCSVRecord> compAttachmentCSVRecords = getCompAttachmentCSVRecordsFromTestFile(
				attachmentsFilename);

		ComponentImportUtils.writeAttachmentsToDatabase(compAttachmentCSVRecords, user, componentClient,
				attachmentClient);
		...
		final AttachmentContent attachmentContent = attachmentClient
				.getAttachmentContent(getCreatedAttachmentContentId());
	...
	public void testImportTwiceWithOnlyAPart() throws Exception {
		...
		ComponentImportUtils.writeToDatabase(compCSVRecords.limit(1), componentClient, vendorClient, attachmentClient,
				user);
	...
	private void assertExpectedComponentsInDb() throws TException {
		...
		assertThat(getReleaseIds(component.getReleases()),
