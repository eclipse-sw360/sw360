...
public class ReleaseExporter extends ExcelExporter<Release, ReleaseHelper> {
	...
	public static final List<Release._Fields> RELEASE_IGNORED_FIELDS = ImmutableList.<Release._Fields>builder()
			.add(REVISION).add(DOCUMENT_STATE).add(PERMISSIONS).add(VENDOR_ID).build();

	public static final List<Release._Fields> RELEASE_REQUIRED_FIELDS = ImmutableList.<Release._Fields>builder()
			.add(COMPONENT_ID).add(NAME).add(VERSION).add(MAIN_LICENSE_IDS).add(OTHER_LICENSE_IDS).add(CLEARING_STATE)
			.add(ECC_INFORMATION).add(VENDOR).add(EXTERNAL_IDS).build();

	public static final List<EccInformation._Fields> ECC_IGNORE_FIELDS = ImmutableList.<EccInformation._Fields>builder()
			.add(MATERIAL_INDEX_NUMBER).add(ASSESSOR_CONTACT_PERSON).add(ECC_COMMENT).build();

	public static final List<Release._Fields> RELEASE_RENDERED_FIELDS_PROJECTS = Release.metaDataMap.keySet().stream()
			.filter(k -> RELEASE_REQUIRED_FIELDS.contains(k)).collect(Collectors.toList());

	public static final List<Vendor._Fields> VENDOR_IGNORED_FIELDS = ImmutableList.<Vendor._Fields>builder()
			.add(Vendor._Fields.PERMISSIONS).add(Vendor._Fields.REVISION).add(Vendor._Fields.ID)
			.add(Vendor._Fields.TYPE).build();

	public static final List<Release._Fields> RELEASE_RENDERED_FIELDS = Release.metaDataMap.keySet().stream()
			.filter(k -> !RELEASE_IGNORED_FIELDS.contains(k)).collect(Collectors.toList());
	...
	private static void addToHeaders(List<String> headers, Release._Fields field) {
		switch (field) {
			...
			case CLEARING_INFORMATION :
				...
				break;
			case ECC_INFORMATION :
				EccInformation.metaDataMap.keySet().forEach(f -> headers.add("ECC information: " + f.getFieldName()));
				break;
			default :
				headers.add(displayNameFor(field.getFieldName(), nameToDisplayName));
		}
	...
	private void preloadLinkedReleasesFor(List<Release> releases) throws SW360Exception {
		Set<String> linkedReleaseIds = releases.stream().map(Release::getReleaseIdToRelationship)
				.filter(Objects::nonNull).map(Map::keySet).flatMap(Set::stream).collect(Collectors.toSet());
