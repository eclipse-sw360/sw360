...
public class TypeMappings {
	...
	public static <T, U> ImmutableList<T> getElementsWithIdentifiersNotInSet(Function<T, U> getIdentifier,
			Set<U> theSet, List<T> candidates) {
		return FluentIterable.from(candidates)
				.filter(CommonUtils.afterFunction(getIdentifier).is(containedWithAddIn(theSet))).toList();
	}
	...
	public static <T, U> Function<T, U> getIdentifier(Class<T> clazz,
			@SuppressWarnings("unused") Class<U> uClass /* used to infer type */) throws SW360Exception {
	...
	@SuppressWarnings("unchecked")
	public static <T> List<T> addAlltoDB(LicenseService.Iface licenseClient, Class<T> clazz, List<T> candidates,
	...
	@NotNull
	public static <T, U> Map<U, T> getIdentifierToTypeMapAndWriteMissingToDatabase(LicenseService.Iface licenseClient,
			InputStream in, Class<T> clazz, Class<U> uClass, User user) throws TException {
		Map<U, T> typeMap;
		...
		final ImmutableList<T> filteredList = getElementsWithIdentifiersNotInMap(getIdentifier(clazz, uClass), typeMap,
				recordsToAdd);
		List<T> added = null;
	...
	@NotNull
	public static Map<Integer, Obligation> getTodoMapAndWriteMissingToDatabase(LicenseService.Iface licenseClient,
		...
		final ImmutableList<Obligation> filteredTodos = getElementsWithIdentifiersNotInMap(getTodoIdentifier(),
				obligMap, obligationsToAdd);
		...
		if (filteredTodos.size() > 0) {
			...
			if (addedTodos != null) {
				final ImmutableMap<Integer, Obligation> addedTodoMap = Maps.uniqueIndex(addedTodos,
						getTodoIdentifier());
	...
	@NotNull
	public static Map<Integer, Obligation> updateTodoMapWithCustomPropertiesAndWriteToDatabase(
			LicenseService.Iface licenseClient, Map<Integer, Obligation> obligMap,
			Map<Integer, ConvertRecord.PropertyWithValue> customPropertiesMap,
			Map<Integer, Set<Integer>> obligPropertiesMap, User user) throws TException {
		...
		if (obligMap.values().size() > 0) {
			final List<Obligation> addedTodos = licenseClient
					.addListOfObligations(obligMap.values().stream().collect(Collectors.toList()), user);
			if (addedTodos != null) {
				final ImmutableMap<Integer, Obligation> addedTodoMap = Maps.uniqueIndex(addedTodos,
						getTodoIdentifier());
				obligMap.putAll(addedTodoMap);
	...
	public static Map<Integer, ConvertRecord.PropertyWithValue> getCustomPropertiesWithValuesByIdAndWriteMissingToDatabase(
			LicenseService.Iface licenseClient, InputStream inputStream, User user) throws TException {
		List<CSVRecord> records = ImportCSV.readAsCSVRecords(inputStream);
		Optional<CustomProperties> dbCustomProperties = CommonUtils
				.wrapThriftOptionalReplacement(licenseClient.getCustomProperties(SW360Constants.TYPE_OBLIGATIONS));
		...
		customProperties.setPropertyToValues(
				CommonUtils.mergeMapIntoMap(propertyToValuesToAdd, customProperties.getPropertyToValues()));
		licenseClient.updateCustomProperties(customProperties, user);
		Map<Integer, ConvertRecord.PropertyWithValue> propertiesWithValuesById = ConvertRecord
				.convertCustomPropertiesById(records);
