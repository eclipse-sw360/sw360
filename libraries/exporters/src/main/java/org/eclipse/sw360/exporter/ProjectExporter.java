...
public class ProjectExporter extends ExcelExporter<Project, ProjectHelper> {
	...
	private static final List<Project._Fields> PROJECT_REQUIRED_FIELDS = ImmutableList.<Project._Fields>builder()
			.add(NAME).add(VERSION).add(BUSINESS_UNIT).add(PROJECT_TYPE).add(TAG).add(CLEARING_STATE)
			.add(PROJECT_RESPONSIBLE).add(PROJECT_OWNER).add(SECURITY_RESPONSIBLES).add(ENABLE_VULNERABILITIES_DISPLAY)
			.add(CREATED_ON).build();

	public static final List<Project._Fields> PROJECT_RENDERED_FIELDS = Project.metaDataMap.keySet().stream()
			.filter(k -> PROJECT_REQUIRED_FIELDS.contains(k)).collect(Collectors.toList());

	public static List<String> HEADERS = PROJECT_RENDERED_FIELDS.stream().map(Project._Fields::getFieldName)
			.map(n -> SW360Utils.displayNameFor(n, nameToDisplayName)).collect(Collectors.toList());

	public static List<String> HEADERS_EXTENDED_BY_RELEASES = ExporterHelper.addSubheadersWithPrefixesAsNeeded(HEADERS,
			ReleaseExporter.RELEASE_HEADERS_PROJECT_EXPORT, "release: ");
	...
	private void preloadRelatedDataFor(List<Project> projects, boolean withLinkedOfLinked, User user)
			throws SW360Exception {
		Function<Function<Project, Map<String, ?>>, Set<String>> extractIds = mapExtractor -> projects.stream()
				.map(mapExtractor).filter(Objects::nonNull).map(Map::keySet).flatMap(Set::stream)
	...
	private void preloadLinkedReleases(Set<String> linkedReleaseIds, boolean withLinkedOfLinked) throws SW360Exception {
		...
		if (withLinkedOfLinked) {
			Set<String> linkedOfLinkedReleaseIds = releasesById.values().stream()
					.map(Release::getReleaseIdToRelationship).filter(Objects::nonNull).map(Map::keySet)
					.flatMap(Set::stream).collect(Collectors.toSet());

			Map<String, Release> joinedMap = new HashMap<>();
			Map<String, Release> linkedOfLinkedReleasesById = ThriftUtils
					.getIdMap(helper.getReleases(linkedOfLinkedReleaseIds));
