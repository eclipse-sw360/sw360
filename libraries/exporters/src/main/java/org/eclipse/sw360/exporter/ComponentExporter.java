...
public class ComponentExporter extends ExcelExporter<Component, ComponentHelper> {
	...
	private static final List<Component._Fields> COMPONENT_IGNORED_FIELDS = ImmutableList.<Component._Fields>builder()
			.add(ID).add(REVISION).add(DOCUMENT_STATE).add(PERMISSIONS).add(RELEASES).build();

	public static final List<Component._Fields> COMPONENT_RENDERED_FIELDS = Component.metaDataMap.keySet().stream()
			.filter(k -> !COMPONENT_IGNORED_FIELDS.contains(k)).collect(Collectors.toList());

	public static List<String> HEADERS = COMPONENT_RENDERED_FIELDS.stream().map(Component._Fields::getFieldName)
	...
	public static List<String> HEADERS_EXTENDED_BY_RELEASES = ExporterHelper.addSubheadersWithPrefixesAsNeeded(HEADERS,
			ReleaseExporter.HEADERS, "release: ");
	...
	public ComponentExporter(ComponentService.Iface componentClient, User user, boolean extendedByReleases)
	...
	private void preloadLinkedReleasesFor(List<Component> components, boolean extendedByReleases)
			throws SW360Exception {
		Set<String> linkedReleaseIds = components.stream().map(Component::getReleaseIds).filter(Objects::nonNull)
