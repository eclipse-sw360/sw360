...
public class ConvertRecord {
	...
	public static void fillTodoCustomPropertyInfo(List<Obligation> obligations,
			List<PropertyWithValueAndId> customProperties, SetMultimap<String, Integer> obligCustomPropertyMap) {
		...
		return new Serializer<Obligation>() {
			...
			public Function<Obligation, List<String>> transformer() {
				return oblig -> {
					...
					out.add(Optional.ofNullable(oblig.getExternalIds()).map(gson::toJson).map(Object::toString)
							.orElse("{}"));
			...
			public List<String> headers() {
				return ImmutableList.of("Title", "Text", "Development", "Distribution", "External IDs",
						"Obligation Type", "Obligation Level");
			}
		};
	}
	...
	public static List<License> fillLicenses(List<CSVRecord> records, Map<Integer, LicenseType> licenseTypeMap,
			Map<Integer, Obligation> obligMap, Map<String, Set<Integer>> licenseTodo) {
		...
		for (CSVRecord record : records) {
			...
			if (record.size() > 8) {
				Optional.ofNullable(record.get(8))
						.map(json -> gson.fromJson(json, new TypeToken<Map<String, String>>() {
		...
		return new Serializer<License>() {
			...
			public Function<License, List<String>> transformer() {
				return license -> {
					...
					out.add(license.isSetLicenseType()
							? ((Integer) license.getLicenseType().getLicenseTypeId()).toString()
					...
					out.add(Optional.ofNullable(license.getExternalIds()).filter(json -> !"NULL".equals(json))
							.map(gson::toJson).map(Object::toString).orElse("{}"));
					return out;
				};
			}
			...
			@Override
			public List<String> headers() {
				return ImmutableList.of("Identifier", "Fullname", "Type", "OSI Approved", "FSF Free/Libre",
						"reviewdate", "Text", "External Link", "External IDs");
			}
		};
	}
	...
	public static <T> List<List<String>> serialize(List<T> in, List<String> headers,
