...
public class ReleaseHelper implements ExporterHelper<Release> {
	 ...
	 * {{@link #setPreloadedLinkedReleases(List, boolean)} so that we can also
	 * preload the necessary components. If you miss that step, we will have to load
	 * each component separately on demand which might take some additional time.
	 *
	 * @param cClient
	...
	public ReleaseHelper(ComponentService.Iface cClient, User user,
		...
		if (this.releaseClearingStatuses != null) {
			...
			this.releaseClearingStatusDataByRelease = releaseClearingStatuses.stream()
					.collect(Collectors.toMap(ReleaseClearingStatusData::getRelease, rcs -> rcs));
		}
	}
	...
	private void addFieldValueToRow(List<String> row, Release._Fields field, Release release,
			boolean isForProjectExport) throws SW360Exception {
		switch (field) {
			case COMPONENT_ID :
	...
	private void addInaccessibleFieldValueToRow(List<String> row, Release._Fields field, Release release,
			boolean isForProjectExport) throws SW360Exception {
		switch (field) {
			case NAME :
	...
	private void addEccInformationToRow(EccInformation eccInformation, List<String> row, boolean isForProjectExport)
			throws SW360Exception {
		try {
			EccInformation.metaDataMap.keySet().stream()
					.filter(e -> (!isForProjectExport || !ReleaseExporter.ECC_IGNORE_FIELDS.contains(e)))
					.forEach(f -> wrapSW360Exception(() -> {
	...
	private void addReleaseIdToRelationShipToRow(Map<String, ReleaseRelationship> releaseIdToRelationship,
			List<String> row) throws SW360Exception {
		if (releaseIdToRelationship != null) {
			row.add(fieldValueAsString(putAccessibleReleaseNamesInMap(releaseIdToRelationship,
					getReleases(releaseIdToRelationship.keySet()), user, ReleaseRelationship.UNKNOWN)));
		} else {
			row.add("");
		}
	}
