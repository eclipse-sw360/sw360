...
public class CommonUtils {
	...
	private static List<String> MULTIPLE_FILE_EXTENSIONS = Arrays.asList(".tar.gz", ".tar.bz2", ".tar.xz", ".tar.lz",
			".tar.lzma");
	...
	public static final CSVFormat sw360CsvFormat = CSVFormat.RFC4180.withQuote('\'').withEscape('\\')
	...
	/**
	 * Returns a sorted list containing the elements of the given collection. The
	 * list is sorted alphabetically, ignoring case.
	 */
	public static List<String> getSortedList(Collection<String> collection) {
		return collection != null
				? CASE_INSENSITIVE_ORDERING.immutableSortedCopy(collection)
				: ImmutableList.<String>of();
	}

	/**
	 * Returns a sorted map containing the elements of the given unsorted map. The
	 * map is sorted follow the order.
	 */
	...
	/**
	 * Return true if and item is contained in a collection, false otherwise. Null
	 * objects make the function returns false
	 */
	...
	/**
	 * Gets the intersection of two sets, or the not null set. Returns: * the
	 * intersection of both sets if they are both not null * the set that is not
	 * null, if only one is not null
	 *
	 * @throws NullPointerException
	 *             if both sets are null
	 */
	...
	public static Optional<Attachment> getAttachmentOptional(final String attachmentId, Set<Attachment> attachments) {
		return attachments.stream().filter(attachment -> attachmentId.equals(attachment.getAttachmentContentId()))
				.findFirst();
	}
	...
	public static Optional<ModerationRequest> getFirstModerationRequestOfUser(
		...
		return moderationRequestsForDocumentId.stream()
				.filter(moderationRequest -> moderationRequest.getRequestingUser().equals(email)).findFirst();
	}
	...
	public static boolean isStillRelevant(ModerationRequest request) {
		return request.getModerationState().equals(ModerationState.PENDING)
				|| request.getModerationState().equals(ModerationState.INPROGRESS);
	}
	...
	public static void getMessageForRequestSummary(RequestSummary releaseRequestSummary, String typeInfo,
		...
		if (releaseRequestSummary.isSetTotalAffectedElements() && releaseRequestSummary.isSetTotalElements()) {
			stringBuilder.append("Affected ").append(typeInfo).append(" elements: ")
					.append(releaseRequestSummary.getTotalAffectedElements()).append(" of ")
					.append(releaseRequestSummary.getTotalElements()).append(". ");
		}
	}
	...
	@NotNull
	public static RequestSummary addRequestSummaries(RequestSummary left, String typeInfoLeft, RequestSummary right,
			String typeInfoRight) {
	...
	public static ImmutableList<String> getAttachmentURLsFromAttachmentContents(
			List<AttachmentContent> attachmentContents) {
	...
	public static RequestSummary getRequestSummary(List<String> ids, List<Response> documentOperationResults) {
		...
		requestSummary.requestStatus = documentOperationResults.isEmpty()
				? RequestStatus.SUCCESS
				: RequestStatus.FAILURE;
	...
	public static <U extends TFieldIdEnum, T extends TBase<T, U>> boolean isMapFieldMapOfStringSets(U field, T document,
			T documentAdditions, T documentDeletions, Logger logger) {
		List<Map<String, Object>> maps = Arrays.asList((Map<String, Object>) document.getFieldValue(field),
				(Map<String, Object>) documentAdditions.getFieldValue(field),
				(Map<String, Object>) documentDeletions.getFieldValue(field));
		List<Map<String, ?>> nonEmptyMaps = maps.stream().filter(m -> m != null && !m.isEmpty())
				.collect(Collectors.toList());
		if (nonEmptyMaps.isEmpty()) {
			logger.info(
					"Field was empty in document, documentAdditions and documentDeletions: " + field.getFieldName());
			return false;
		}

		Object value = nonEmptyMaps.stream().findAny().get().entrySet().stream()
				.map(e -> ((Map.Entry<String, Object>) e).getValue()).findAny().get();
		...
		Map<String, Set<Object>> mapWithNonEmptySet = (Map<String, Set<Object>>) nonEmptyMapsContainingNonEmptySet
				.stream().findAny().get();
		Object element = getNonEmptySetFromMapOfSets(mapWithNonEmptySet).get().stream().findAny().get();
	...
	private static Optional<Set<Object>> getNonEmptySetFromMapOfSets(Map<String, Set<Object>> map) {
		return map.entrySet().stream().map(e -> ((Map.Entry<String, Set<Object>>) e).getValue())
