...
public class SW360Constants {
	...
	/**
	 * Hashmap containing the name field for each type. Used by the search service
	 * to fill the search results
	 */
	public static final Map<String, String> MAP_FULLTEXT_SEARCH_NAME = ImmutableMap.<String, String>builder()
			.put(TYPE_LICENSE, "fullname").put(TYPE_OBLIGATIONS, "text").put(TYPE_OBLIGATION, "name")
			.put(TYPE_USER, "email").put(TYPE_VENDOR, "fullname").put(TYPE_COMPONENT, "name")
			.put(TYPE_RELEASE, "name version").put(TYPE_PROJECT, "name version").build();

	public static final Collection<AttachmentType> LICENSE_INFO_ATTACHMENT_TYPES = Arrays
			.asList(AttachmentType.COMPONENT_LICENSE_INFO_XML, AttachmentType.COMPONENT_LICENSE_INFO_COMBINED);
	public static final Collection<AttachmentType> SOURCE_CODE_ATTACHMENT_TYPES = Arrays.asList(AttachmentType.SOURCE,
	...
	public static final Map<String, List<Map.Entry<String, String>>> NOTIFIABLE_ROLES_BY_OBJECT_TYPE = ImmutableMap.<String, List<Map.Entry<String, String>>>builder()
			.put(NOTIFICATION_CLASS_PROJECT,
					ImmutableList.<Map.Entry<String, String>>builder()
							...
							.build())
			.put(NOTIFICATION_CLASS_COMPONENT,
					ImmutableList.<Map.Entry<String, String>>builder()
			...
			.put(NOTIFICATION_CLASS_RELEASE, ImmutableList.<Map.Entry<String, String>>builder()
					.add(pair(Release._Fields.CREATED_BY, "Creator"), pair(Release._Fields.CONTRIBUTORS, "Contributor"),
					...
					.build())
			.put(NOTIFICATION_CLASS_MODERATION_REQUEST,
					ImmutableList.<Map.Entry<String, String>>builder()
			...
			.put(NOTIFICATION_CLASS_CLEARING_REQUEST, ImmutableList.<Map.Entry<String, String>>builder()
					.add(pair(ClearingRequest._Fields.REQUESTING_USER, "Requesting User")).build())
			.build();
	public static final List<String> NOTIFICATION_EVENTS_KEYS = NOTIFIABLE_ROLES_BY_OBJECT_TYPE.entrySet().stream()
			.map(notificationClassEntry -> notificationClassEntry.getValue().stream().map(roleEntry -> SW360Utils
					.notificationPreferenceKey(notificationClassEntry.getKey(), roleEntry.getKey())))
			.flatMap(Function.identity()).collect(Collectors.toList());

	static final Map<String, Boolean> DEFAULT_NOTIFICATION_PREFERENCES = NOTIFICATION_EVENTS_KEYS.stream()
