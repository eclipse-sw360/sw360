...
public class SW360Utils {
	...
	public static final Comparator<ReleaseLink> RELEASE_LINK_COMPARATOR = Comparator
			.comparing(rl -> getReleaseFullname(rl.getVendor(), rl.getName(), rl.getVersion()).toLowerCase());
	...
	/**
	 * Tries to parse a given date in format "yyyy-MM-dd HH:mm:ss" to a Date,
	...
	public static Map<String, String> getReleaseIdtoAcceptedCLIMappings(
			Map<String, ObligationStatusInfo> obligationStatusMap) {
		if (null == obligationStatusMap) {
			return Maps.newHashMap();
		}
		return obligationStatusMap.values().stream()
				.flatMap(e -> (e.getReleaseIdToAcceptedCLI() != null
						? e.getReleaseIdToAcceptedCLI()
	...
	public static Collection<ProjectLink> getLinkedProjects(Project project, boolean deep, ThriftClients thriftClients,
	...
	public static Collection<ProjectLink> getLinkedProjects(String id, boolean deep, ThriftClients thriftClients,
			Logger log, User user) {
		if (id != null) {
			try {
				ProjectService.Iface client = thriftClients.makeProjectClient();
	...
	public static Collection<ProjectLink> getLinkedProjectsAsFlatList(String id, boolean deep,
			ThriftClients thriftClients, Logger log, User user) {
	...
	public static void filteredListOfLinkedProject(Collection<ProjectLink> linkedProjects,
		...
		if (CommonUtils.isNotEmpty(linkedProjects)) {
			Collection<ProjectLink> linkedProjectsFiltered = linkedProjects.stream()
					.filter(projectLink -> selectedProjectRelationAsList.contains(projectLink.getRelation()))
	...
	public static List<ReleaseLink> getLinkedReleasesWithAccessibility(Project project, ThriftClients thriftClients,
			Logger log, User user) {
		if (project != null && project.getReleaseIdToUsage() != null) {
			try {
				ComponentService.Iface componentClient = thriftClients.makeComponentClient();
	...
	public static List<ReleaseLink> getLinkedReleaseRelations(Release release, ThriftClients thriftClients,
			Logger log) {
	...
	public static List<ReleaseLink> getLinkedReleaseRelationsWithAccessibility(Release release,
			ThriftClients thriftClients, Logger log, User user) {
		if (release != null && release.getReleaseIdToRelationship() != null) {
			try {
				ComponentService.Iface componentClient = thriftClients.makeComponentClient();
				return componentClient.getLinkedReleaseRelationsWithAccessibility(release.getReleaseIdToRelationship(),
						user);
			} catch (TException e) {
				log.error("Could not get linked releases", e);
			}
	...
	public static String fieldValueAsString(Object fieldValue) throws SW360Exception {
		...
		if (fieldValue instanceof Map) {
			...
			// cannot use CommonUtils.nullToEmptyString here, because it calls toString() on
			// non-null objects,
			// which destroys the chance for ObjectMapper to serialize values according to
			// its configuration
	...
	private static class TEnumSerializer extends JsonSerializer<TEnum> {
		@Override
		public void serialize(TEnum value, JsonGenerator jgen, SerializerProvider provider)
	...
	private static class ProjectReleaseRelationshipSerializer extends JsonSerializer<ProjectReleaseRelationship> {
		@Override
		public void serialize(ProjectReleaseRelationship value, JsonGenerator jgen, SerializerProvider provider)
				throws IOException, JsonProcessingException {
	...
	public static <T> Map<String, T> putReleaseNamesInMap(Map<String, T> map, List<Release> releases) {
		...
		releases.stream().forEach(r -> releaseNamesMap.put(printName(r), map.get(r.getId())));
		return releaseNamesMap;
	}

	public static <T> Map<String, T> putAccessibleReleaseNamesInMap(Map<String, T> map, List<Release> releases,
			User user, T inaccessibleValue) {
		if (map == null || releases == null) {
			return Collections.emptyMap();
		}
		Map<String, T> releaseNamesMap = new HashMap<>();
		...
		if (!CommonUtils.isNullOrEmptyCollection(releases)) {
			for (Release release : releases) {
				if (release.isSetPermissions() && release.getPermissions().get(RequestedAction.READ)) {
					...
					releaseNamesMap.put(String.format("%s%d", INACCESSIBLE_RELEASE, inaccessbileReleaseCount),
							inaccessibleValue);
				}
			}
		}
	...
	public static <T> Map<String, T> putProjectNamesInMap(Map<String, T> map, List<Project> projects) {
		...
		projects.stream().forEach(p -> projectNamesMap.put(printName(p), map.get(p.getId())));
	...
	public static <T> Set<T> unionValues(Map<?, Set<T>> map) {
		return nullToEmptyMap(map).values().stream().filter(Objects::nonNull).reduce(Sets::union)
	...
	public static int getTotalReleaseCount(ReleaseClearingStateSummary clearingSummary) {
		return clearingSummary.getNewRelease() + clearingSummary.getReportAvailable()
				+ clearingSummary.getUnderClearing() + clearingSummary.getSentToClearingTool()
				+ clearingSummary.getApproved();
	}
	...
	public static Map<String, ObligationStatusInfo> getProjectComponentOrganisationLicenseObligationToDisplay(
		...
		Map<String, ObligationStatusInfo> mapOfObligations = obligations.stream().filter(Objects::nonNull)
				...
				.collect(Collectors.toMap(o -> CommonUtils.nullToEmptyString(o.getTitle()).replaceAll("\r\n", " "),
						o -> {
	...
	public static void copyLinkedObligationsForClonedProject(Project newProject, Project sourceProject,
			ProjectService.Iface client, User user) {
		try {
			...
			if (!linkedObligations.isEmpty()) {
				client.addLinkedObligations(new ObligationList().setProjectId(newProject.getId())
						.setLinkedObligationStatus(linkedObligations), user);
			}
		} catch (TException e) {
	...
	public static void removeReleaseVulnerabilityRelation(String releaseId, User user) {
		...
		try {
			List<ReleaseVulnerabilityRelation> releaseVulnerabilityRelations = vulnerabilityService
					.getReleaseVulnerabilityRelationsByReleaseId(releaseId, user);
