...
public class DatabaseConnector extends StdCouchDbConnector {
	 ...
	 * @param dbName
	 *            name of the database on the CouchDB server
	 */
	 ...
	 *
	 * @param httpClient
	 *            Supplier<HttpClient> with authentication of CouchDB server
	 * @param dbName
	 *            name of the database on the CouchDB server
	 */
	 ...
	 *            name of the database on the CouchDB server
	 * @param mapperFactory
	 *            Specific mapper factory to use for serialization
	 */
	public DatabaseConnector(HttpClient httpClient, String dbName, MapperFactory mapperFactory)
			throws MalformedURLException {
		this(dbName, new DatabaseInstance(httpClient), mapperFactory);
	}

	private DatabaseConnector(String dbName, DatabaseInstance instance, MapperFactory mapperFactory)
			throws MalformedURLException {
	...
	public Optional<Status> restrictAccessToAdmins() {
		...
		Security security = Optional.ofNullable(getSecurity()).orElse(new Security());

		SecurityGroup adminGroup = Optional.ofNullable(security.getAdmins()).orElse(new SecurityGroup());
		SecurityGroup memberGroup = Optional.ofNullable(security.getMembers()).orElse(new SecurityGroup());
	...
	/**
	 * Creates the Object as a document in the database. If the id is not set it
	...
	/**
	 * Get a list of documents from their IDs. All documents should be of the same
	 * type.
	 */
	public <T> List<T> get(Class<T> type, Collection<String> ids, boolean ignoreNotFound) {
		...
		ViewQuery q = new ViewQuery().allDocs().includeDocs(true).keys(idSet);
	...
	/**
	 * Delete the document with the given id. Returns true if the delete was
	 * successful.
	 */
