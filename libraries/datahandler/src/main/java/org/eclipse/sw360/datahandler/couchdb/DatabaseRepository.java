...
public class DatabaseRepository<T> extends CouchDbRepositorySupport<T> {
	...
	private static List<String> getIdList(ViewResult rows) {
		return rows.getRows().stream().map(ViewResult.Row::getId).collect(Collectors.toList());
	}
	...
	public Set<String> queryForIdsAsComplexValues(String queryName, Map<String, Set<String>> keys) {
		Set<ComplexKey> complexKeys = keys.entrySet().stream().map(DatabaseRepository::createComplexKeys)
	 ...
	 * <p/>
	 * If the object has no revision set, it will be created, otherwise it will be
	 * updated.
	 * <p/>
	 * Some documents may successfully be saved and some may not. The response will
	 * tell the application which documents were saved or not. In the case of a
	 * power failure, when the database restarts some may have been saved and some
	 * not.
	 *
	 * @param list
	 *            , all objects will have their id and revision set.
	 * @return The list will only contain entries for documents that has any kind of
	 *         error code returned from CouchDB. i.e. the list will be empty if
	 ...
	 *            , the objects that will be deleted
	 * @return The list will only contain entries for documents that has any kind of
	 *         error code returned from CouchDB. i.e. the list will be empty if
	 *         everything was completed successfully.
	 */
	...
	/**
	 * Returns the total count of documents in the repository (given by the all
	 * view)
	 */
