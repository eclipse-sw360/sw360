...
public abstract class Moderator<U extends TFieldIdEnum, T extends TBase<T, U>> {
	...
	protected T updateBasicField(U field, FieldMetaData fieldMetaData, T document, T documentAdditions,
			T documentDeletions) {
			...
			case TType.MAP :
				if (isMapFieldMapOfStringSets(field, document, documentAdditions, documentDeletions, log)) {
					document.setFieldValue(field,
							updateCustomMap((Map<String, Set<String>>) document.getFieldValue(field),
	...
	protected <S> T updateEnumMap(U field, Class<? extends TEnum> S, T document, T documentAdditions,
			T documentDeletions) {
	...
	protected T updateStringMap(U field, T document, T documentAdditions, T documentDeletions) {
		...
		if (documentAdditions.isSet(field)) {
			for (Map.Entry<String, ProjectReleaseRelationship> entry : ((Map<String, ProjectReleaseRelationship>) documentAdditions
					.getFieldValue(field)).entrySet()) {
				if (!document.isSet(field)) {
					document.setFieldValue(field, new HashMap<>());
				}
				Map<String, ProjectReleaseRelationship> documentMap = (Map<String, ProjectReleaseRelationship>) document
						.getFieldValue(field);
				if (documentMap.containsKey(entry.getKey())) {
					documentMap.replace(entry.getKey(), entry.getValue());
				} else {
		...
		if (documentDeletions.isSet(field) && document.isSet(field)) {
			for (Map.Entry<String, ProjectReleaseRelationship> entry : ((Map<String, ProjectReleaseRelationship>) documentDeletions
					.getFieldValue(field)).entrySet()) {
				if (!documentAdditions.isSet(field)
						|| !((Map<String, ProjectReleaseRelationship>) documentAdditions.getFieldValue(field))
	...
	protected Map<String, Set<String>> updateCustomMap(Map<String, Set<String>> map, Map<String, Set<String>> addMap,
			Map<String, Set<String>> deleteMap) {
