...
public class LuceneAwareDatabaseConnector extends LuceneAwareCouchDbConnector {
	...
	private static final List<String> LUCENE_SPECIAL_CHARACTERS = Arrays
			.asList("[\\\\\\+\\-\\!\\~\\*\\?\\\"\\^\\:\\(\\)\\{\\}\\[\\]]", "\\&\\&", "\\|\\|", "/");
	...
	public LuceneAwareDatabaseConnector(Supplier<HttpClient> httpClient, Supplier<CloudantClient> cClient,
	 ...
	 */
	public LuceneAwareDatabaseConnector(DatabaseConnector connector, Supplier<CloudantClient> cClient)
			throws IOException {
	...
	public boolean addView(LuceneSearchView function) {
		...
		return uploader.updateSearchFunctionIfNecessary(this, function.searchView, function.searchFunction,
				function.searchBody);
	}
	...
	private LuceneResult callLuceneDirectly(LuceneSearchView function, String queryString, boolean includeDocs)
		...
		queryURI.append(function.searchView.startsWith(DesignDocument.ID_PREFIX)
				? function.searchView
	 ...
	 */
	public <T> List<T> searchViewWithRestrictions(Class<T> type, LuceneSearchView luceneSearchView, String text,
	...
	public static String prepareWildcardQuery(String query) {
		...
		if (query.startsWith("\"") && query.endsWith("\"")) {
			...
			String wildCardQuery = Arrays.stream(sanitizeQueryInput(query).split(" "))
					.map(q -> leadingWildcardChar + q + "*").collect(Collectors.joining(" "));
