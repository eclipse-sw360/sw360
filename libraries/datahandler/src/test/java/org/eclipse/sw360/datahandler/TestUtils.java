...
public class TestUtils {
	...
	private static final List<String> dbNames = ImmutableList.of(DatabaseSettingsTest.COUCH_DB_DATABASE,
			DatabaseSettingsTest.COUCH_DB_ATTACHMENTS, DatabaseSettingsTest.COUCH_DB_USERS);
	...
	public static <T extends TBase<T, F>, F extends TFieldIdEnum> Comparator<T> orderByField(final F field) {
		return orderByField(field, usingToString());
	}

	public static <T extends TBase<T, F>, F extends TFieldIdEnum> Ordering<T> orderByField(final F field,
			final Comparator<Object> fieldComparator) {
	...
	@SafeVarargs
	public static <T extends TBase<T, F>, F extends TFieldIdEnum> Matcher<? super T> equalTo(final T expected,
			final F... fields) {
		if (fields.length == 0) {
