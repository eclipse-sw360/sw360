...
NOTE:
REST API tokens with both READ and WRITE authorities can be successfully generated when SW360 is correctly installed and configured as per the official setup instructions.

During testing on a local SW360 instance using the default configuration and setup steps described in this guide, REST API tokens were generated with both READ and WRITE authorities without requiring any additional configuration changes.

If a generated token only contains READ authority, please ensure that:

* The SW360 backend and frontend are correctly installed and running
* The user is properly authenticated
* The installation steps, including database and authorization setup, were completed successfully

This confirms that WRITE authority token generation works as expected in a properly configured SW360 environment.
