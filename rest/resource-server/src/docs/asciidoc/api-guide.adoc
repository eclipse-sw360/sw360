...
IMPORTANT:
WRITE authority tokens can only be generated by users belonging to the user group configured in `rest.write.access.usergroup` (default: SW360_ADMIN).

If a user does not belong to this group, the generated REST API token will only have READ authority even if WRITE permission is selected in the UI.

This behavior is enforced by backend validation in Sw360UserService.
