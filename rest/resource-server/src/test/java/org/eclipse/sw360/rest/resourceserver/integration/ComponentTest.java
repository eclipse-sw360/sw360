...
public class ComponentTest extends TestIntegrationBase {
	...
	public void should_get_all_components() throws IOException {
		...
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/components", HttpMethod.GET, new HttpEntity<>(null, headers),
				String.class);
	...
	public void should_get_all_components_wrong_page() throws IOException, TException {
		...
		HttpHeaders headers = getHeaders(port);
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/components?page=5&page_entries=10", HttpMethod.GET,
	...
	public void should_get_all_components_with_field() throws IOException {
		...
		HttpHeaders headers = getHeaders(port);
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/components?fields=" + extraField, HttpMethod.GET,
				new HttpEntity<>(null, headers), String.class);
		assertEquals(HttpStatus.OK, response.getStatusCode());
	...
	public void should_update_component_valid() throws IOException, TException {
		...
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/components/" + componentId, HttpMethod.PATCH,
	...
	public void should_update_component_invalid() throws IOException, TException {
		...
		body.put("name", updatedComponentName);
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/components/someRandomId123", HttpMethod.PATCH,
				new HttpEntity<>(body, headers), String.class);
	...
	public void should_delete_component_valid() throws IOException, TException {
		...
		HttpHeaders headers = getHeaders(port);
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/components/" + componentId, HttpMethod.DELETE,
				new HttpEntity<>(null, headers), String.class);
	...
	public void should_delete_component_invalid() throws IOException, TException {
		...
		HttpHeaders headers = getHeaders(port);
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/components/" + invalidComponentId, HttpMethod.DELETE,
				new HttpEntity<>(null, headers), String.class);
	...
	public void should_delete_attachments_successfully() throws TException, IOException {
		...
		given(componentServiceMock.updateComponent(any(), eq(TestHelper.getTestUser()))).will(invocationOnMock -> {
		...
		given(attachmentServiceMock.filterAttachmentsToRemove(Source.componentId(componentId),
				component.getAttachments(), attachmentIds)).willReturn(Collections.singleton(attachments.get(1)));

		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/components/" + componentId + "/attachments/" + strIds,
	...
	public void should_delete_attachments_with_failure_handling() throws TException, IOException {
		...
		given(componentServiceMock.getComponentForUserById(componentId, TestHelper.getTestUser()))
				.willReturn(component);
		given(attachmentServiceMock.filterAttachmentsToRemove(Source.releaseId(componentId), component.getAttachments(),
				Collections.singletonList(attachmentId))).willReturn(Collections.emptySet());

		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/components/" + componentId + "/attachments/" + attachmentId,
				HttpMethod.DELETE, new HttpEntity<>(null, getHeaders(port)), String.class);
		assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, response.getStatusCode());
		then(componentServiceMock).should(never()).updateComponent(any(), any());
	}
