...
public class LicenseSpecTest extends TestRestDocsSpecBase {
	...
	public void should_document_get_licenses() throws Exception {
		...
		mockMvc.perform(
				get("/api/licenses").header("Authorization", "Bearer " + accessToken).accept(MediaTypes.HAL_JSON))
				.andExpect(status().isOk())
				.andDo(this.documentationHandler.document(
						links(linkWithRel("curies").description("Curies are used for online documentation")),
						responseFields(
								subsectionWithPath("_embedded.sw360:licenses")
										.description("An array of <<resources-licenses, Licenses resources>>"),
	...
	public void should_document_get_license() throws Exception {
		...
		mockMvc.perform(get("/api/licenses/" + license.getId())
				...
				.andExpect(status().isOk())
				.andDo(this.documentationHandler.document(
						links(linkWithRel("self").description("The <<resources-licenses,Licenses resource>>")),
						responseFields(fieldWithPath("fullName").description("The full name of the license"),
								fieldWithPath("shortName").description("The short name of the license, optional"),
								subsectionWithPath("externalIds").description(
										"When releases are imported from other tools, the external ids can be stored here"),
								...
								fieldWithPath("checked").description(
										"The information, whether the license is already checked, optional and defaults to true"),
								subsectionWithPath("OSIApproved")
										.description("The OSI aprroved information, possible values are: "
												+ Arrays.asList(Quadratic.values())),
								fieldWithPath("FSFLibre").description("The FSF libre information, possible values are: "
										+ Arrays.asList(Quadratic.values())),
								subsectionWithPath("_embedded.sw360:obligations")
										.description("An array of <<resources-obligations, Obligations obligations>>"),
								subsectionWithPath("_links")
										.description("<<resources-index-links,Links>> to other resources"))));
	}
	...
	public void should_document_delete_license() throws Exception {
		String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
		mockMvc.perform(delete("/api/licenses/" + license.getId()).header("Authorization", "Bearer " + accessToken)
	...
	public void should_document_link_obligation() throws Exception {
		...
		String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
		mockMvc.perform(post("/api/licenses/" + license.getId() + "/obligations").contentType(MediaTypes.HAL_JSON)
	...
	public void should_document_unlink_obligation() throws Exception {
		...
		String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
		this.mockMvc.perform(patch("/api/licenses/" + license.getId() + "/obligations").contentType(MediaTypes.HAL_JSON)
				.content(this.objectMapper.writeValueAsString(obligationIds))
