...
public class UserSpecTest extends TestRestDocsSpecBase {
	...
	public void should_document_get_users() throws Exception {
		...
		mockMvc.perform(get("/api/users").header("Authorization", "Bearer " + accessToken).accept(MediaTypes.HAL_JSON))
				...
				.andDo(this.documentationHandler.document(
						...
						responseFields(
								...
								subsectionWithPath("_embedded.sw360:users")
										.description("An array of <<resources-users, User resources>>"),
	...
	public void should_document_get_user_by_id() throws Exception {
		...
		mockMvc.perform(get("/api/users/byid/" + user.getId())
				...
				.andExpect(status().isOk())
				.andDo(this.documentationHandler.document(
						links(linkWithRel("self").description("The <<resources-users,User resource>>")),
						responseFields(fieldWithPath("email").description("The user's email"),
								fieldWithPath("userGroup").description(
										"The user group, possible values are: " + Arrays.asList(UserGroup.values())),
								...
								subsectionWithPath("secondaryDepartmentsAndRoles")
										.description("The user's secondary departments and roles"),
