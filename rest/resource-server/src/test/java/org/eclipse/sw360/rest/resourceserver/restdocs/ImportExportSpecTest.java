...
public class ImportExportSpecTest extends TestRestDocsSpecBase {
    ...
    public void should_document_get_download_attachment_template() throws Exception {
        String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
        mockMvc.perform(get("/api/importExport/downloadAttachmentSample")
                .header("Authorization", "Bearer " + accessToken).accept(MediaTypes.HAL_JSON))
                .andExpect(status().isOk());
    }
    ...
    public void should_document_get_download_attachment_information() throws Exception {
        String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
        mockMvc.perform(get("/api/importExport/downloadAttachmentInfo").header("Authorization", "Bearer " + accessToken)
    ...
    public void should_document_get_download_release_sample() throws Exception {
        String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
        mockMvc.perform(get("/api/importExport/downloadReleaseSample").header("Authorization", "Bearer " + accessToken)
                .accept(MediaTypes.HAL_JSON)).andExpect(status().isOk());
    }
    ...
    public void should_document_get_download_release_link_info() throws Exception {
        String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
        mockMvc.perform(get("/api/importExport/downloadReleaseLink").header("Authorization", "Bearer " + accessToken)
                .accept(MediaTypes.HAL_JSON)).andExpect(status().isOk());
    }
    ...
    public void should_document_get_download_component_details() throws Exception {
        String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
        mockMvc.perform(get("/api/importExport/downloadComponent").header("Authorization", "Bearer " + accessToken)
                .accept(MediaTypes.HAL_JSON)).andExpect(status().isOk());
    }
}
