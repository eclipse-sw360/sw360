...
public class ReleaseTest extends TestIntegrationBase {
	...
	public void before() throws TException {
		...
		given(this.releaseServiceMock.getReleaseForUserById(eq(TestHelper.getDummyReleaseListForTest().get(0).getId()),
				eq(user))).willReturn(TestHelper.getDummyReleaseListForTest().get(0));
		given(this.releaseServiceMock.getReleasesForUser(any())).willReturn(TestHelper.getDummyReleaseListForTest());
		given(this.releaseServiceMock.getReleaseForUserById(eq(TestHelper.getDummyReleaseListForTest().get(1).getId()),
				eq(user))).willReturn(TestHelper.getDummyReleaseListForTest().get(1));
	...
	public void should_update_release_valid() throws IOException, TException {
		...
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/releases/" + TestHelper.release1Id, HttpMethod.PATCH,
	...
	public void should_update_release_invalid() throws IOException, TException {
		...
		body.put("name", updatedReleaseName);
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/releases/unknownId123", HttpMethod.PATCH,
				new HttpEntity<>(body, headers), String.class);
	...
	public void should_get_all_releases_with_fields() throws IOException {
		...
		HttpHeaders headers = getHeaders(port);
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/releases?fields=" + extraField, HttpMethod.GET,
	...
	public void should_delete_releases() throws IOException, TException {
		...
		HttpHeaders headers = getHeaders(port);
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/releases/" + TestHelper.release1Id + "," + unknownReleaseId,
	...
	public void should_delete_release() throws IOException, TException {
		...
		HttpHeaders headers = getHeaders(port);
		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/releases/" + unknownReleaseId, HttpMethod.DELETE,
				new HttpEntity<>(null, headers), String.class);
		List<MultiStatus> multiStatusList = new ArrayList<>();
		multiStatusList.add(new MultiStatus(unknownReleaseId, HttpStatus.INTERNAL_SERVER_ERROR));
		TestHelper.handleBatchDeleteResourcesResponse(response, multiStatusList);
	...
	public void should_delete_attachments_successfully() throws TException, IOException {
		...
		given(releaseServiceMock.updateRelease(any(), eq(TestHelper.getTestUser()))).will(invocationOnMock -> {
		...
		given(attachmentServiceMock.filterAttachmentsToRemove(Source.releaseId(release.getId()),
				release.getAttachments(), attachmentIds)).willReturn(Collections.singleton(attachments.get(1)));

		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/releases/" + TestHelper.release1Id + "/attachments/" + strIds,
				HttpMethod.DELETE, new HttpEntity<>(null, getHeaders(port)), String.class);
		assertEquals(HttpStatus.OK, response.getStatusCode());
		...
		Set<String> attachmentFileNames = StreamSupport.stream(jsonAttachments.spliterator(), false)
				.map(node -> node.get("filename").textValue()).collect(Collectors.toSet());
	...
	public void should_delete_attachments_with_failure_handling() throws TException, IOException {
		...
		given(releaseServiceMock.getReleaseForUserById(release.id, TestHelper.getTestUser())).willReturn(release);
		given(attachmentServiceMock.filterAttachmentsToRemove(Source.releaseId(release.getId()),
				release.getAttachments(), Collections.singletonList(attachmentId))).willReturn(Collections.emptySet());

		ResponseEntity<String> response = new TestRestTemplate().exchange(
				"http://localhost:" + port + "/api/releases/" + TestHelper.release1Id + "/attachments/" + attachmentId,
				HttpMethod.DELETE, new HttpEntity<>(null, getHeaders(port)), String.class);
		assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, response.getStatusCode());
		then(releaseServiceMock).should(never()).updateRelease(any(), any());
	}
}