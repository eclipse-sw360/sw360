...
public class ReleaseTest extends TestIntegrationBase {
        ...
        public void before() throws TException, IOException {
                ...
                Attachment att2 = att1.deepCopy().setAttachmentType(AttachmentType.BINARY)
                                .setCreatedComment("Created Comment")
                                .setCheckStatus(CheckStatus.ACCEPTED).setCheckedComment("Checked Comment")
                                .setCheckedOn("2021-04-27")
                ...
                given(this.attachmentServiceMock.getAttachmentContent(any()))
                                .willReturn(new AttachmentContent().setId("1231231254")
                                                .setFilename("spring-core-4.3.4.RELEASE.jar").setContentType("binary"));
                ...
                given(this.sw360VendorService.getVendorById(any()))
                ...
                given(this.attachmentServiceMock.getAttachmentResourcesFromList(any(), any(), any()))
                                .willReturn(CollectionModel.of(attachmentResources));
                ...
                given(this.releaseServiceMock.refineSearch(any(), any(), any())).willReturn(
                                Collections.singletonMap(
                                                new PaginationData().setRowsPerPage(releaseList.size())
                                                                .setDisplayStart(0)
                                                                .setTotalRowCount(releaseList.size()),
                                                releaseList));
                ...
                given(this.releaseServiceMock.getProjectsByRelease(eq(release.getId()), any()))
                                .willReturn(new HashSet<>());
                given(this.releaseServiceMock.deleteRelease(eq(release.getId()), any()))
                ...
                given(this.releaseServiceMock.searchReleaseByNamePaginated(any(), any())).willReturn(
                                Collections.singletonMap(
                                                new PaginationData()
                                                                .setRowsPerPage(TestHelper
                                                                                .getDummyReleaseListForTest().size())
                                                                .setDisplayStart(0)
                                                                .setTotalRowCount(TestHelper
                                                                                .getDummyReleaseListForTest().size()),
                ...
                given(this.releaseServiceMock.setComponentDependentFieldsInRelease(any(Release.class), any()))
                ...
                given(this.releaseServiceMock.setComponentDependentFieldsInRelease(any(List.class), any()))
                                .willAnswer(invocation -> {
                ...
                given(this.releaseServiceMock.convertToEmbeddedWithExternalIds(eq(release))).willReturn(
                                new Release("Angular", "2.3.0", "17653524")
                                                .setId(release.getId())
                                                .setExternalIds(Collections.singletonMap("mainline-id-component",
                                                                "1432")));
                given(this.releaseServiceMock.countProjectsByReleaseId(eq(release.getId()))).willReturn(2);
                given(this.releaseServiceMock.getReleaseForUserById(eq(TestHelper.release1Id), any()))
                                .willReturn(getDummyReleaseListForTest().get(0));
                given(this.releaseServiceMock.getReleaseForUserById(eq(TestHelper.releaseId2), any()))
                                .willReturn(getDummyReleaseListForTest().get(1));
                given(this.releaseServiceMock.updateRelease(any(), any())).willReturn(RequestStatus.SUCCESS);
                given(this.releaseServiceMock.deleteRelease(eq(TestHelper.release1Id), any()))
                                .willReturn(RequestStatus.SUCCESS);
                given(this.releaseServiceMock.getProjectsByRelease(eq(TestHelper.release1Id), any()))
                                .willReturn(new HashSet<>());
                given(this.releaseServiceMock.getUsingComponentsForRelease(eq(TestHelper.release1Id), any()))
                                .willReturn(new HashSet<>());
                ...
                given(this.attachmentServiceMock.getAttachmentContent(any())).willReturn(new AttachmentContent()
                                .setId("1231231254").setFilename("test.jar").setContentType("binary"));
        }
        ...
        public void should_update_release_valid() throws IOException {
                ...
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases/" + TestHelper.release1Id,
                                HttpMethod.PATCH,
        ...
        public void should_update_release_invalid() throws IOException, TException {
                ...
                body.put("name", updatedReleaseName);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases/unknownId123",
                                HttpMethod.PATCH,
                                new HttpEntity<>(body, headers),
                                String.class);
        ...
        public void should_get_all_releases_with_fields() throws IOException {
                ...
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases?fields=" + extraField,
                                HttpMethod.GET,
        ...
        public void should_delete_release() throws IOException, TException {
                ...
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases/" + unknownReleaseId,
                                HttpMethod.DELETE,
                                new HttpEntity<>(null, headers),
                                String.class);
                List<MultiStatus> multiStatusList = new ArrayList<>();
        ...
        public void should_get_empty_collection_for_invalid_sha() throws IOException {
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases?sha1=" + attachmentShaInvalid,
                                HttpMethod.GET,
        ...
        public void should_get_collection_for_duplicated_shas() throws IOException, TException {
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases?sha1="
                                                + TestHelper.attachmentShaUsedMultipleTimes
        ...
        public void should_delete_attachments_successfully() throws TException, IOException {
                ...
                given(releaseServiceMock.updateRelease(any(), any()))
                                .will(invocationOnMock -> {
                                        refUpdatedRelease
                                                        .set(new Release((Release) invocationOnMock.getArguments()[0]));
        ...
        public void should_delete_attachments_with_failure_handling() throws TException, IOException {
                ...
                given(releaseServiceMock.getReleaseForUserById(release.id, TestHelper.getTestUser()))
                                .willReturn(release);
                given(attachmentServiceMock.filterAttachmentsToRemove(Source.releaseId(release.getId()),
        ...
        public void should_get_all_releases() throws IOException {
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases?page=0&page_entries=5&sort=name,desc",
                                HttpMethod.GET,
                                new HttpEntity<>(null, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
        ...
        public void should_get_all_releases_with_all_details() throws IOException {
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases?allDetails=true",
                                HttpMethod.GET,
                                new HttpEntity<>(null, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
        ...
        public void should_get_releases_by_lucene_search() throws IOException {
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange("http://localhost:" + port
                                + "/api/releases?name=Test&luceneSearch=true&page=0&page_entries=5&sort=name,desc",
                                HttpMethod.GET,
                                new HttpEntity<>(null, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
        ...
        public void should_get_recent_releases() throws IOException {
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases/recentReleases",
                                HttpMethod.GET,
                                new HttpEntity<>(null, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
        ...
        public void should_get_releases_by_name() throws IOException, TException {
                ...
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases?name=Test",
                                HttpMethod.GET,
                                new HttpEntity<>(null, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
        ...
        public void should_get_usedbyresource_for_release() throws IOException, TException {
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases/usedBy/" + TestHelper.release1Id,
                                HttpMethod.GET,
                                new HttpEntity<>(null, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
        ...
        public void should_get_releases_by_externalIds() throws IOException {
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases?externalId=mainline-id-component:1432",
                                HttpMethod.GET,
                                new HttpEntity<>(null, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
        ...
        public void should_get_release_attachment_info() throws IOException, TException {
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases/" + TestHelper.release1Id + "/attachments",
                                HttpMethod.GET,
                                new HttpEntity<>(null, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
                TestHelper.checkResponse(response.getBody(), "attachments", 1); // Changed from 0 to 1 to match mocked
                                                                                // data
        }
        ...
        public void should_link_releases_to_release() throws IOException, TException {
                ...
                HttpHeaders headers = getHeaders(port);
                headers.setContentType(MediaType.APPLICATION_JSON);
                Map<String, ReleaseRelationship> body = Collections.singletonMap("90876",
                                ReleaseRelationship.DYNAMICALLY_LINKED);

                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases/" + TestHelper.release1Id + "/releases",
                                HttpMethod.POST,
                                new HttpEntity<>(body, headers),
                                String.class);
        ...
        public void should_get_release_subscription() throws IOException, TException {
                ...
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases/mySubscriptions",
                                HttpMethod.GET,
                                new HttpEntity<>(null, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
        ...
        public void should_write_spdx_licenses_info_into_release() throws IOException, TException {
                ...
                ResponseEntity<String> response = new TestRestTemplate().exchange(
                                "http://localhost:" + port + "/api/releases/" + TestHelper.release1Id + "/spdxLicenses",
                                HttpMethod.POST,
                                new HttpEntity<>(body, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
        ...
        public void should_check_cyclic_hierarchy_of_a_release_with_other_releases() throws IOException, TException {
                given(this.releaseServiceMock.checkForCyclicLinkedReleases(any(), any(), any()))
                                .willReturn("No cyclic links found");
        ...
        public void should_trigger_fossology_process() throws IOException, TException {
                ...
                HttpHeaders headers = getHeaders(port);
                ResponseEntity<String> response = new TestRestTemplate().exchange("http://localhost:" + port
                                + "/api/releases/" + TestHelper.release1Id
                                + "/triggerFossologyProcess?markFossologyProcessOutdated=true&uploadDescription=Test upload",
                                HttpMethod.GET,
                                new HttpEntity<>(null, headers),
                                String.class);
                assertEquals(HttpStatus.OK, response.getStatusCode());
        ...
        @Test
        public void should_update_release_without_validating_unchanged_licenses() throws IOException, TException {
                // Setup: Release with a license that doesn't exist in the database
                Release existingRelease = new Release("Test", "1.0", "comp123");
                existingRelease.setId(TestHelper.release1Id);
                Set<String> existingLicenses = new HashSet<>();
                existingLicenses.add("NonExistentLicense-1.0"); // This license doesn't exist
                existingRelease.setMainLicenseIds(existingLicenses);

                given(this.releaseServiceMock.getReleaseForUserById(eq(TestHelper.release1Id), any()))
                                .willReturn(existingRelease);
                given(this.releaseServiceMock.updateRelease(any(), any()))
                                .willReturn(RequestStatus.SUCCESS);

                // Test: Update only the description (not licenses)
                HttpHeaders headers = getHeaders(port);
                headers.setContentType(MediaType.APPLICATION_JSON);
                Map<String, Object> body = new HashMap<>();
                body.put("description", "Updated description"); // NOT updating licenses
                ...
                // Assert: Should succeed without license validation error
                assertEquals(HttpStatus.OK, response.getStatusCode());
        }

        @Test
        public void should_fail_when_updating_with_invalid_license() throws IOException, TException {
                // Setup
                given(this.releaseServiceMock.getReleaseForUserById(eq(TestHelper.release1Id), any()))
                                .willReturn(getDummyReleaseListForTest().get(0));
                given(this.licenseServiceMock.getLicenseById(eq("InvalidLicense")))
                                .willThrow(new ResourceNotFoundException("License not found"));

                // Test: Explicitly update licenses with invalid ID
                HttpHeaders headers = getHeaders(port);
                headers.setContentType(MediaType.APPLICATION_JSON);
                Map<String, Object> body = new HashMap<>();
                body.put("mainLicenseIds", new String[] { "InvalidLicense" }); // UPDATING licenses
                ...
                assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
                assertTrue(response.getBody().contains("does not exist in SW360 database"));
        }
}
