...
public class ApiSpecTest extends TestRestDocsSpecBase {
	...
	public void should_document_headers() throws Exception {
		...
		this.mockMvc
				.perform(RestDocumentationRequestBuilders.get("/api").header("Authorization", "Bearer " + accessToken)
	...
	public void should_document_error_bad_request() throws Exception {
		...
		this.mockMvc
				.perform(post("/api/projects")
						.contentType(MediaTypes.HAL_JSON).content("{").header("Authorization", "Bearer " + accessToken))
				...
				.andDo(this.documentationHandler.document(responseFields(
						...
						fieldWithPath("message").description(
								"The error message, e.g. JSON parse error: Unexpected end-of-input: expected close marker for Object"))));
	}
	...
	public void should_document_error_unauthorized() throws Exception {
		this.mockMvc
				.perform(RestDocumentationRequestBuilders.get("/api").header("Authorization", "Bearer " + "123456789"))
				.andExpect(status().isUnauthorized())
				.andDo(this.documentationHandler.document(
						responseFields(fieldWithPath("error").description("The error code, e.g. invalid_token"),
	...
	public void should_document_error_not_found() throws Exception {
		String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
		this.mockMvc.perform(post("/api/endpoint_not_found").contentType(MediaTypes.HAL_JSON).content("{}")
	...
	public void should_document_error_method_not_allowed() throws Exception {
		...
		this.mockMvc
				.perform(RestDocumentationRequestBuilders
						.delete("/api").header("Authorization", "Bearer " + accessToken))
				.andExpect(status().isMethodNotAllowed())
				.andDo(this.documentationHandler.document(
						responseFields(fieldWithPath("timestamp").description("The timestamp when the error occurred"),
								...
								fieldWithPath("message").description(
										"The error message, e.g. Request method 'DELETE' not supported"))));
	}

	@Test
	public void should_document_error_unsupported_media_type() throws Exception {
		String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
		this.mockMvc.perform(post("/api/projects").contentType(MediaType.APPLICATION_XML)
				.content("<?xml version=\"1.0\" encoding=\"UTF-8\"?>").header("Authorization", "Bearer " + accessToken))
				...
				.andDo(this.documentationHandler.document(responseFields(
						...
						fieldWithPath("message").description(
								"The error message, e.g. Content type 'application/text;charset=UTF-8' not supported"))));
	}

	@Test
	public void should_document_error_internal_error() throws Exception {
		given(this.projectServiceMock.getProjectForUserById(anyString(), any()))
				.willThrow(new RuntimeException(new TException("Internal error processing getProjectById")));
		String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
		this.mockMvc
				.perform(RestDocumentationRequestBuilders
						.get("/api/projects/12321").header("Authorization", "Bearer " + accessToken))
				...
				.andDo(this.documentationHandler.document(
						responseFields(fieldWithPath("timestamp").description("The timestamp when the error occurred"),
								...
								fieldWithPath("message").description(
										"The error message, e.g. Internal error processing getProjectById"))));
	}

	@Test
	public void should_document_index() throws Exception {
		String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
		this.mockMvc.perform(get("/api").header("Authorization", "Bearer " + accessToken).accept(MediaTypes.HAL_JSON))
				.andExpect(status().isOk())
				.andDo(this.documentationHandler.document(links(
						...
						linkWithRel("sw360:attachments")
								.description("The <<resources-attachments,Attachments resource>>"),
						...
						linkWithRel("sw360:licenseinfo")
								.description("The <<resources-licenseinfo,Licenseinfo resource>>"),
						linkWithRel("sw360:vulnerabilities")
								.description("The <<resources-vulnerabilities,Vulnerabilities resource>>"),
						linkWithRel("sw360:vulnerabilities")
								.description("The <<resources-vulnerabilities,Vulnerabilities resource>>"),
						...
						linkWithRel("sw360:clearingRequests")
								.description("The <<resources-clearingRequest,ClearingRequest resource>>"),
						linkWithRel("sw360:obligations")
								.description("The <<resources-obligations,Obligation resource>>"),
