...
public class ComponentSpecTest extends TestRestDocsSpecBase {
    ...
    public void before() throws TException, IOException {
        ...
        angularComponent.setSoftwarePlatforms(new HashSet<>(List.of("Linux")));
        angularComponent.setMainLicenseIds(new HashSet<>(List.of("123")));
        angularComponent.setSubscribers(new HashSet<>(List.of("Mari")));
        ...
        angularTargetComponent.setSoftwarePlatforms(new HashSet<>(List.of("Linux")));
        angularTargetComponent.setMainLicenseIds(new HashSet<>(List.of("123")));
        angularTargetComponent.setSubscribers(new HashSet<>(List.of("Mari")));
        ...
        springComponent.setSoftwarePlatforms(new HashSet<>(List.of("Windows")));
        springComponent.setMainLicenseIds(new HashSet<>(List.of("222")));
        springComponent.setSubscribers(new HashSet<>(List.of("Natan")));
        ...
        String releaseName = release1.getName() + " " + release1.getVersion();
    ...
    public void should_document_get_components_by_lucene_search() throws Exception {
        mockMvc.perform(get("/api/components").header("Authorization", TestHelper.generateAuthHeader(testUserId, testUserPassword))
                .queryParam("name", angularComponent.getName()).queryParam("luceneSearch", "true").queryParam("sort", "name,desc")
                .queryParam("page", "0").queryParam("page_entries", "5").accept(MediaTypes.HAL_JSON)).andExpect(status().isOk())
                .andDo(this.documentationHandler.document(
                        queryParameters(parameterWithName("name").description("name of components"),
        ...
        mockMvc.perform(get("/api/components")
                ...
                .andDo(this.documentationHandler.document(
                        ...
                        responseFields(
                                ...
                                subsectionWithPath("_embedded.sw360:components.[]_embedded.sw360:attachments.[]md5").description("The attachment md5 value").optional().type(String.class),
                                subsectionWithPath("_embedded.sw360:components.[]_embedded.sw360:attachments.[]sha256").description("The attachment sha256 value").optional().type(String.class),
                                subsectionWithPath("_embedded.sw360:components.[]_embedded.sw360:attachments.[]setMd5").description("Boolean flag indicating if MD5 hash is set").optional().type(Boolean.class),
                                subsectionWithPath("_embedded.sw360:components.[]_embedded.sw360:attachments.[]setSha256").description("Boolean flag indicating if SHA256 hash is set").optional().type(Boolean.class),
    ...
    public void should_document_get_components_with_fields() throws Exception {
        mockMvc.perform(get("/api/components")
                .header("Authorization", TestHelper.generateAuthHeader(testUserId, testUserPassword))
                .queryParam("fields", "ownerGroup,ownerCountry")
                .queryParam("page", "0")
                .queryParam("page_entries", "5")
                .queryParam("sort", "name,desc")
                .accept(MediaTypes.HAL_JSON))
