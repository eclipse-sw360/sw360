...
public class ObligationSpecTest extends TestRestDocsSpecBase {
	...
	public void before() throws TException {
		...
		given(this.obligationServiceMock.deleteObligation(eq(obligation.getId()), any()))
				.willReturn(RequestStatus.SUCCESS);

		when(this.obligationServiceMock.createObligation(any(), any()))
				.then(invocation -> new Obligation("This is the text of my Test Obligation").setId("1234567890")
						.setTitle("Test Obligation").setObligationLevel(ObligationLevel.LICENSE_OBLIGATION)
	...
	public void should_document_get_obligations() throws Exception {
		...
		mockMvc.perform(
				get("/api/obligations").header("Authorization", "Bearer " + accessToken).accept(MediaTypes.HAL_JSON))
				.andExpect(status().isOk())
				.andDo(this.documentationHandler.document(
						links(linkWithRel("curies").description("Curies are used for online documentation")),
						responseFields(
								subsectionWithPath("_embedded.sw360:obligations[]title")
										.description("The title of the obligation"),
								subsectionWithPath("_embedded.sw360:obligations")
										.description("An array of <<resources-obligations, Obligations resources>>"),
	...
	public void should_document_get_obligation() throws Exception {
		...
		mockMvc.perform(get("/api/obligations/" + obligation.getId())
				...
				.andExpect(status().isOk())
				.andDo(this.documentationHandler.document(
						links(linkWithRel("self").description("The <<resources-obligations, Obligations resource>>")),
						responseFields(fieldWithPath("title").description("The title of the obligation"),
								fieldWithPath("text").description("The text of the obligation"),
								fieldWithPath("obligationLevel").description(
										"The level of the obligation: [ORGANISATION_OBLIGATION, PROJECT_OBLIGATION, COMPONENT_OBLIGATION, LICENSE_OBLIGATION]"),
								fieldWithPath("obligationType").description(
										"The type of the obligation: [PERMISSION, RISK, EXCEPTION, RESTRICTION, OBLIGATION]"),
								subsectionWithPath("_links")
										.description("<<resources-index-links,Links>> to other resources"))));
	}
	...
	public void should_document_create_obligation() throws Exception {
		...
		String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
		this.mockMvc
				.perform(post("/api/obligations").contentType(MediaTypes.HAL_JSON)
				...
				.andDo(this.documentationHandler.document(requestFields(
						...
						fieldWithPath("text").description("The text of the obligation"),
						fieldWithPath("obligationLevel").description(
								"The level of the obligation: [COMPONENT_OBLIGATION, ORGANISATION_OBLIGATION, PROJECT_OBLIGATION, LICENSE_OBLIGATION]"),
						fieldWithPath("obligationType").description(
								"The type of the obligation: [RESTRICTION, OBLIGATION, PERMISSION, EXCEPTION, RISK]")),
						responseFields(fieldWithPath("title").description("The title of the obligation"),
								fieldWithPath("text").description("The text of the obligation"),
								fieldWithPath("obligationLevel").description(
										"The level of the obligation: [COMPONENT_OBLIGATION, ORGANISATION_OBLIGATION, PROJECT_OBLIGATION, LICENSE_OBLIGATION]"),
