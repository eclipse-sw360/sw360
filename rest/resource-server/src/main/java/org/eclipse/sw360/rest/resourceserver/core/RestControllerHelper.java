...
public class RestControllerHelper<T> {
	...
	private PagedModel.PageMetadata createPageMetadata(PaginationResult paginationResult) {
		...
		return new PagedModel.PageMetadata(paginationOptions.getPageSize(), paginationOptions.getPageNumber(),
				paginationResult.getTotalCount(), paginationResult.getTotalPageCount());
	}
	...
	public void addEmbeddedReleases(HalResource halResource, Set<String> releases,
			Sw360ReleaseService sw360ReleaseService, User user) throws TException {
	...
	public void addEmbeddedReleases(HalResource halResource, List<Release> releases) {
	...
	public void addEmbeddedAttachments(HalResource halResource, Set<Attachment> attachments) {
	...
	private void isLicenseValid(Set<String> licenses) {
		...
		if (!licenseIncorrect.isEmpty()) {
			throw new HttpMessageNotReadableException(
					"License with ids " + licenseIncorrect + " does not exist in SW360 database.");
		}
	}
	...
	public Release convertToEmbeddedReleaseWithDet(Release release) {
		List<String> fields = List.of("id", "name", "version", "cpeid", "createdBy", "createdOn", "componentId",
				"componentType", "additionalData", "clearingState", "mainLicenseIds", "binaryDownloadurl",
				"sourceCodeDownloadurl", "releaseDate", "externalIds", "languages", "operatingSystems",
				"softwarePlatforms", "vendor", "mainlineState");
	...
	public <T> ResponseEntity searchByExternalIds(MultiValueMap<String, String> externalIdsMultiMap,
			AwareOfRestServices<T> service, User user) throws TException {
