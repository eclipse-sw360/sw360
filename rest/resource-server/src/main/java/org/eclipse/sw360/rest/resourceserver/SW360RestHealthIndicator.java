...
public class SW360RestHealthIndicator implements HealthIndicator {
	...
	public Health health() {
		...
		if (!restState.isUp()) {
			Health.Builder builderWithDetails = Health.down().withDetail(rest_state_detail, restState);
			for (Exception exception : exceptions) {
				builderWithDetails = builderWithDetails.withException(exception);
			}
			return builderWithDetails.build();
		}
		return Health.up().withDetail(rest_state_detail, restState).build();
	}
	...
	private boolean isThriftReachable(List<Exception> exception) {
		...
		try {
			...
			if (health.getStatus().equals(Status.UP)) {
				...
				exception.add(
						new Exception(health.getStatus().toString(), new Throwable(health.getDetails().toString())));
				return false;
			}
		} catch (TException e) {
			exception.add(e);
