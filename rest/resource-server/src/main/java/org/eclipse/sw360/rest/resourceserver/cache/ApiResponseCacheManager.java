...
public class ApiResponseCacheManager {
     ...
     * Returns stats even if cache is disabled (for admin visibility of orphan files).
     */
    public List<CacheStatistics> getEndpointVariantStatistics(CachedEndpoint endpoint) {
        ensureInitialized();

        CacheConfiguration config = configs.get(endpoint);
        // Use endpoint config if available, otherwise create a minimal config for scanning
        if (config == null) {
            config = CacheConfiguration.builder()
                    .endpoint(endpoint)
                    .enabled(false)
                    .cacheDirectory(Paths.get(cacheDirectory))
                    .build();
     ...
     * Scans disk for cache files to ensure all variants are invalidated,
     * including those not currently loaded in memory.
     * Also deletes files even if cache is currently disabled (files may exist from previous config).
     */
    public void invalidateAll() {
        ensureInitialized();

        // First, invalidate any in-memory cache instances
        caches.values().forEach(ResponseCache::invalidate);
        int inMemoryCount = caches.size();

        // Also scan disk for cache files that may not be in memory
        int diskFilesDeleted = 0;
        Path cacheDir = Paths.get(cacheDirectory);
        for (CachedEndpoint endpoint : CachedEndpoint.values()) {
            diskFilesDeleted += invalidateCacheFilesOnDisk(endpoint, cacheDir);
        }

        log.info("All API response caches invalidated ({} in-memory variants, {} files on disk)",
                inMemoryCount, diskFilesDeleted);
    }
    ...
    public void invalidate(CachedEndpoint endpoint) {
        ...
        // Also scan disk for cache files that may not be in memory
        // Use config if available, otherwise use default directory
        CacheConfiguration config = configs.get(endpoint);
        Path cacheDir = (config != null) ? config.getCacheDirectory() : Paths.get(cacheDirectory);
        int diskFilesDeleted = invalidateCacheFilesOnDisk(endpoint, cacheDir);

        log.info("Cache invalidated for {} ({} in-memory variants, {} files on disk)",
                endpoint, entriesToInvalidate.size(), diskFilesDeleted);
    }

    /**
     * Scan disk and delete all cache files for a specific endpoint.
     * This ensures cache files are deleted even if they weren't loaded in memory.
     *
     * @return number of cache file pairs (json+meta) deleted
     */
    private int invalidateCacheFilesOnDisk(CachedEndpoint endpoint, Path cacheDir) {
        if (!java.nio.file.Files.exists(cacheDir)) {
            return 0;
        }
        ...
        try (var stream = java.nio.file.Files.list(cacheDir)) {
            List<Path> filesToDelete = stream
                    .filter(p -> {
                        ...
                        return fileName.startsWith(cacheKeyPrefix) &&
                                (fileName.endsWith(".json") || fileName.endsWith(".meta") || fileName.endsWith(".json.tmp"));
                    })
                    .collect(Collectors.toList());
            ...
            log.error("Failed to scan cache directory for {}: {}", endpoint, e.getMessage(), e);
        }
     ...
     * Deletes cache files from disk even if not loaded in memory or cache is disabled.
     */
    public void invalidate(CachedEndpoint endpoint, String variant) {
        ...
        if (cache != null) {
            ...
            // Cache not in memory, but may exist on disk - delete files directly
            deleteVariantCacheFiles(endpoint, variant);
            log.info("Cache files deleted for {} variant={} (not in memory)", endpoint, variant);
        }
    }

    /**
     * Delete cache files for a specific variant of an endpoint.
     */
    private void deleteVariantCacheFiles(CachedEndpoint endpoint, String variant) {
        Path cacheDir = Paths.get(cacheDirectory);
        String baseName = endpoint.getCacheKey() + "-" + variant;

        try {
            Files.deleteIfExists(cacheDir.resolve(baseName + ".json"));
            Files.deleteIfExists(cacheDir.resolve(baseName + ".meta"));
            Files.deleteIfExists(cacheDir.resolve(baseName + ".json.tmp"));
        } catch (Exception e) {
            log.warn("Failed to delete cache files for {} variant={}: {}", endpoint, variant, e.getMessage());
        }
    }
