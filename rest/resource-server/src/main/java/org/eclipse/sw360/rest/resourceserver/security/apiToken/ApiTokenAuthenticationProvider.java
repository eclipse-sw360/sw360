...
public class ApiTokenAuthenticationProvider implements AuthenticationProvider {
	...
	private Optional<RestApiToken> getApiTokenFromUser(String tokenHash, User sw360User) {
		return sw360User.getRestApiTokens().stream().filter(t -> t.getToken().equals(tokenHash)).findFirst();
	}
	...
	private Set<GrantedAuthority> getGrantedAuthoritiesFromApiToken(RestApiToken restApiToken) {
		return restApiToken.getAuthorities().stream().map(SimpleGrantedAuthority::new).collect(Collectors.toSet());
	}

	private Set<GrantedAuthority> getGrantedAuthoritiesFromUserAccess(UserAccess userAccess) {
		return Stream.of(userAccess.name().split("_")).map(SimpleGrantedAuthority::new).collect(Collectors.toSet());
	}

	private PreAuthenticatedAuthenticationToken authenticatedApiUser(User user, String credentials,
