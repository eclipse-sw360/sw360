...
public class ReleaseCacheCondition {
    ...
    /**
     * Check if request has no filters applied (sha1, name, luceneSearch, pagination or newClearing).
    ...
    private static boolean hasNoFilters(HttpServletRequest request) {
        return CommonUtils.isNullEmptyOrWhitespace(request.getParameter("sha1"))
                ...
                && CommonUtils.isNullEmptyOrWhitespace(request.getParameter("page"))
                && CommonUtils.isNullEmptyOrWhitespace(request.getParameter("page_entries"))
                && CommonUtils.isNullEmptyOrWhitespace(request.getParameter("sort"))
