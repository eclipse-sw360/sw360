...
public class KeycloakAccessTokenConverter extends DefaultAccessTokenConverter
	...
	/***
	 * Expects a token which has the keycloak format. The token contains a
	 * resource_access claim which is a list of resources and the granted roles on
	 * this resources. The expectation is that the token has resource client roles
	 * (WRITE/READ) of the sw360-REST-API client. INFO: the
	 * SecurityContextHolder.getContext().getAuthentication().getPrincipal(); will
	 * return user_name of the jwt
	 * 
	 * @param tokenMap
	 *            the raw jwt token
	...
	public OAuth2Authentication extractAuthentication(Map<String, ?> tokenMap) {
		...
		if (tokenMap.containsKey(JWT_RESOURCE_ACCESS)) {
			...
			if (resourceAccess.containsKey(resourceServerProperties.getResourceId())) {
				Map<String, Object> clientAccess = (Map<String, Object>) resourceAccess
						.get(resourceServerProperties.getResourceId());
		...
		// TODO: Right now only the userId is present in the session. But it may not
		// exists in the database.
		// implement user creation/mapping here. So new users in keycloak can directly
		// query the restapi without liferay
		// The idea is to create the users on the fly if they do no exist.
		// based on the keycloak token which contains, name, surename and deparment
		// already
		// !!! -> This needs to be done with care.
		// 1. Consider caching to avoid a lot of calls for the user object
		// 2. Consider caching only for as long the token is the same to ensure valid
		// user info
		// User user = new User();
		// user.setId("test2");
