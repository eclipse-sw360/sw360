...
public class Sw360WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers(HttpMethod.OPTIONS).permitAll() // some JS frameworks make HTTP OPTIONS
																				// requests
				.anyRequest().authenticated().and().addFilterBefore(sw360CustomHeaderAuthenticationFilter(),
	...
	protected void configure(AuthenticationManagerBuilder authenticationManagerBuilder) {
		authenticationManagerBuilder.authenticationProvider(sw360LiferayAuthenticationProvider())
