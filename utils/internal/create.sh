...
find $PROJECT_ROOT -name "pom.xml" -exec sh -c '
    if grep -q "<packaging>war</packaging>" {}; then
        ...
        echo "mvn -q --no-transfer-progress dependency:get -Dartifact=$groupId:$artifactId:$version:war -Dtransitive=false -DremoteRepositories=gitlab-maven::::https://code.siemens.com/api/v4/projects/708/packages/maven"
    fi
' \;
